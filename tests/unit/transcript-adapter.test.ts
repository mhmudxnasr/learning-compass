...
test('Visual Lite is source-designed code only with no image or template branch', () => {
  ...
  assert.match(liteVisual, /No raster\/generated assets, image agents, Visual Mind, preset themes/)
...
test('Lite Visual keeps teaching depth while explicitly removing mandatory quality audits', () => {
  assert.match(liteVisual, /complete, source-specific teaching/)
  assert.match(liteVisual, /one canonical semantic HTML article/)
  ...
  assert.match(liteVisualQuality, /source-specific CSS/)
  assert.match(liteVisual, /quality_checks=not_run/)
  assert.match(liteVisual, /No four-pass ritual/)
  assert.match(liteVisual, /Keep atomic HTML\/PDF publication/)
})

test('Lite Visual publishes one integrity-bound atomic pair and verifies its source record', () => {
  ...
  assert.match(uploader, /passing integrity receipt or historical v6 validation receipt/)
