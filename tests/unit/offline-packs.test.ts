...
import { coherentOfflineResources, offlineDataResource, offlinePackVersion, offlinePairResources } from '../../client/src/offlinePacks.ts'
...
test('offline resources require one exact verified HTML/PDF pair', () => {
  ...
  assert.deepEqual(resources.map((item) => item.role), ['html', 'pdf'])
  assert.equal(resources.every((item) => item.pairId === 'lv-pair-1'), true)
  assert.equal(offlinePairResources(
...
test('metadata-only and partial groups cannot masquerade as offline packs', () => {
  ...
  assert.deepEqual(coherentOfflineResources([
  ...
  assert.notEqual(
    ...
    offlinePackVersion(complete.map((resource) => resource.role === 'pdf' ? { ...resource, sizeBytes: Number(resource.sizeBytes || 0) + 1 } : resource)),
  )
  assert.deepEqual(coherentOfflineResources([
    ...complete,
...
test('book chapter projection never combines two paired revisions', () => {
  const chapters = normalizeBookChapters([], [
    { id: 'old-html', created_at: '2026-08-01', metadata: metadata('lv-old', 'html', { chapter_key: 'chapter-1', chapter_number: 1 }) },
    { id: 'old-pdf', created_at: '2026-08-01', metadata: metadata('lv-old', 'pdf', { chapter_key: 'chapter-1', chapter_number: 1 }) },
    { id: 'new-html', created_at: '2026-08-02', metadata: metadata('lv-new', 'html', { chapter_key: 'chapter-1', chapter_number: 1 }) },
    { id: 'new-pdf', created_at: '2026-08-02', metadata: metadata('lv-new', 'pdf', { chapter_key: 'chapter-1', chapter_number: 1 }) },
  ])
...
test('service worker swaps versioned packs only after every response is cached', () => {
  ...
  const save = worker.slice(worker.indexOf('async function saveOfflinePack'), worker.indexOf('async function removeOfflinePack'))
  const remove = worker.slice(worker.indexOf('async function removeOfflinePack'), worker.indexOf('function serializeOfflinePackMutation'))
  ...
  assert.match(save, /try \{ await caches\.delete\(previous\.cacheName\) \} catch/)
  assert.match(save, /await caches\.delete\(cacheName\)/)
  assert.match(worker, /if \(!pairs\.size\) throw new Error\('Offline packs require at least one complete verified HTML\/PDF pair\.'\)/)
  ...
  assert.match(worker, /state = missing\.length \? 'partial' : expectedVersion.*'superseded'/s)
  ...
  assert.match(worker, /Promise\.allSettled\(names\.filter/)
  ...
  assert.ok(remove.indexOf('await writeOfflinePackIndex(nextIndex)') < remove.indexOf('await caches.delete(manifest.cacheName)'))
})
