...
import { DURABLE_UNKNOWN_MUTATION_EXPIRES_AT, mutationReservationDisposition } from '../../src/services/mutation-recovery.ts'
...
test('mutation reservations distinguish safe retries from unknown commit outcomes', () => {
  ...
  for (const status of [301, 307, 308, 500, 502, 503, 504]) assert.equal(mutationReservationDisposition(status), 'hold_unknown')
