...
const signingEnv = {
  ...process.env,
  LITE_VISUAL_RECEIPT_SIGNING_KEY: 'test-lite-visual-receipt-signing-key-2026-08-28',
}
...
test('Visual Lite v6 rejects media, scripts, and preset/template markup before rendering', () => {
  ...
  try {
    ...
    writeFileSync(
      htmlPath,
      '<!doctype html><html lang="ar" dir="rtl"><head><meta name="viewport" content="width=device-width"><meta name="lite-visual-design-intent" content="شرح مصدر محدد بوضوح"><meta name="lite-visual-design-signature" content="إيقاع مستمد من المصدر"><title>اختبار</title><style>@page{size:A4}@media print{body{color:#000}}</style></head><body><main><article data-canonical-content="true"><h1>اختبار</h1><section data-source-scope="scope-01"><img src="figure.png"><script>bad()</script></section></article></main></body></html>',
    )
...
test('Visual Lite v6 validates exact source coverage and emits a hash-bound receipt', (t) => {
  ...
  try {
    const source = Array.from(
      { length: 10 },
      (_, index) => `الفكرة ${index + 1} تشرح السبب والنتيجة والمثال والشرط والحدود بوضوح كامل`,
    ).join(' ')
    ...
    writeFileSync(
      workItemPath,
      JSON.stringify({ recommendation_id: 'rec-1', source_url: 'https://example.com/source', source_title: 'المصدر' }),
    )
        ...
        spans: [
          {
            ...
            word_start: 0,
            word_end: wordCount,
            anchor: 'complete source',
            summary: 'كل أفكار المصدر وأمثلته وشروطه وحدوده',
          },
        ],
      }),
    )
    ...
    const chromeBinary = ['google-chrome', 'chromium', 'chromium-browser'].find(
    ...
    const result = spawnSync(
      'python3',
      [validator, ...baseArgs, '--html', htmlPath, '--pdf', pdfPath, '--receipt-out', receiptPath],
    ...
    writeFileSync(
      hiddenPrintPath,
      html.replace('@media print{', '@media print{[data-exact-source-scope]{display:none}'),
    )
    ...
    writeFileSync(
      lowContrastPath,
      html.replace('</style>', '[data-exact-source-scope]{color:#fff;background:#fff}</style>'),
    )
    ...
    writeFileSync(
      coveredPath,
      html.replace('<body>', '<body><div style="position:fixed;inset:0;background:#111;z-index:9999"></div>'),
    )
