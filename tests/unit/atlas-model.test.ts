...
import { branchConstellations, branchSubtreeIds, createAtlasModel, expandVisibleIds, initialVisibleIds, isSubtreeVisible, isolatedVisibleIds, nodeAncestry, nodeTitle, nodeTypeBadge, rootBranchFor, toggleSubtree, visibleIdsForDepth, visibleIdsForFrontier } from '../../client/src/features/atlas/model.ts'
...
test('atlas hides archived nodes, their descendants, and connected edges', () => {
  ...
  assert.deepEqual(model.nodes.map((node) => node.id), ['cat-tools', 'paused'])
...
test('atlas derives a cluster from category ancestry', () => {
  ...
  assert.deepEqual(model.clusters.get('Tools & Craft')?.map((node) => node.id), ['category', 'branch'])
...
test('nodeAncestry resolves full parent path from root to leaf', () => {
  ...
  assert.deepEqual(path.map((n) => n.id), ['cat', 'b1', 'sub'])
})
