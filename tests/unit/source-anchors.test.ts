...
class AnchorDatabase {
  ...
  source = {
    id: 'rec-1',
    video_title: 'Grounded source',
    video_url: 'https://example.com/essay',
    creator: 'Author',
    content_type: 'article',
    branch_id: 'branch-1',
    branch_label: 'Evidence',
    branch_status: 'active',
    branch_domain: 'thinking',
    branch_domain_verified: 'thinking',
    thread_id: null,
  }
  ...
  prepare(sql: string) {
    const statement: any = {
      ...
      first: async () => {
        ...
        if (sql.includes('FROM recommendations r LEFT JOIN recommendation_meta') && sql.includes('WHERE r.id=?'))
          return statement.args[0] === this.source.id ? this.source : null
        if (sql.includes('SELECT b.id,b.status,b.super_category'))
          return statement.args[0] === 'branch-1'
            ? { id: 'branch-1', status: 'active', super_category: 'thinking', domain_id: 'thinking' }
            : null
        if (sql.includes('FROM source_annotations a')) {
          ...
          return annotation
            ? {
                ...annotation,
                source_title: this.source.video_title,
                source_url: this.source.video_url,
                source_creator: this.source.creator,
                branch_label: this.source.branch_label,
                branch_status: this.source.branch_status,
                branch_domain: this.source.branch_domain,
              }
            : null
        }
        return null
      ...
      run: async () => {
        if (sql.includes('INSERT INTO source_annotations')) {
          if (sql.includes('revision_of_annotation_id')) {
            if (this.failNextRevisionOwnership) {
              this.failNextRevisionOwnership = false
              return { meta: { changes: 0 } }
            }
            const previous = this.annotations.get(String(statement.args[10]))
            if (!previous || previous.status !== 'active' || previous.source_checksum !== statement.args[11])
              return { meta: { changes: 0 } }
            this.annotation = {
              ...previous,
              id: statement.args[0],
              locator_type: statement.args[1],
              selector_json: statement.args[2],
              selector_source_identities_json: statement.args[3],
              quote: statement.args[4],
              context_before: statement.args[5],
              context_after: statement.args[6],
              language: statement.args[7],
              source_checksum: statement.args[8],
              revision_of_annotation_id: statement.args[9],
              status: 'active',
              created_at: '2026-08-31 12:01:00',
              updated_at: '2026-08-31 12:01:00',
            }
          } else {
            if (this.failNextOwnershipInsert) {
              this.failNextOwnershipInsert = false
              return { meta: { changes: 0 } }
            }
            this.annotation = {
              id: statement.args[0],
              recommendation_id: statement.args[12],
              artifact_id: statement.args[1],
              thread_id: statement.args[2],
              branch_id: statement.args[13],
              locator_type: statement.args[3],
              selector_json: statement.args[4],
              selector_source_identities_json: statement.args[5],
              quote: statement.args[6],
              context_before: statement.args[7],
              context_after: statement.args[8],
              language: statement.args[9],
              source_checksum: statement.args[10],
              created_by: statement.args[11],
              status: 'active',
          ...
          if (this.failNextInPlaceUpdate) {
            this.failNextInPlaceUpdate = false
            return { meta: { changes: 0 } }
          }
          if (!annotation || annotation.status !== 'active' || annotation.source_checksum !== statement.args[9])
            return { meta: { changes: 0 } }
          Object.assign(annotation, {
            locator_type: statement.args[0],
            selector_json: statement.args[1],
            selector_source_identities_json: statement.args[2],
            quote: statement.args[3],
            context_before: statement.args[4],
            context_after: statement.args[5],
            language: statement.args[6],
            source_checksum: statement.args[7],
            updated_at: '2026-08-31 12:01:00',
          })
          ...
          if (
            ...
            (statement.args.length > 1 && annotation.source_checksum !== statement.args[1])
          )
            return { meta: { changes: 0 } }
          if (statement.args.length > 2 && !this.annotations.has(String(statement.args[2])))
            return { meta: { changes: 0 } }
...
class AnchorSqliteD1 {
  constructor(privateSqlite: DatabaseSync) {
    this.sqlite = privateSqlite
  }
...
test('source anchor checksum is deterministic and binds the exact evidence fields', async () => {
  const base = {
    ...
    locator_type: 'web',
    selector: { url: 'https://example.com/essay' },
    quote: 'Exact claim',
  ...
  assert.equal(
    await checksum({ ...base, selector: { beta: 2, alpha: 1 } }),
    await checksum({ ...base, selector: { alpha: 1, beta: 2 } }),
  )
...
test('source selection resolution, creation, editing, and archive preserve ownership and search projection', async () => {
  ...
  const resolvedFragment = await app.request(
    'https://app.test/resolve?source_url=https%3A%2F%2Fexample.com%2Fessay%23claim',
    {},
    env,
  )
  ...
  const resolvedHistorical = await app.request(
    'https://app.test/resolve?source_url=https%3A%2F%2Fexample.com%2Fessay%3Futm_source%3Dnew-share%23claim',
    {},
    env,
  )
  ...
  const created = await app.request(
    'https://app.test/',
    {
      ...
      body: JSON.stringify({
        recommendation_id: 'rec-1',
        locator_type: 'web',
        selector: { url: 'https://example.com/essay', text_quote: { exact: 'Exact claim' } },
        quote: 'Exact claim',
        context_before: 'Before',
        context_after: 'After',
        language: 'en',
  ...
  assert.equal(
    createdBody.annotation.source_checksum,
    await checksum({
      recommendation_id: 'rec-1',
      locator_type: 'web',
      selector: createdBody.annotation.selector,
      quote: 'Exact claim',
      context_before: 'Before',
      context_after: 'After',
    }),
  ...
  const edited = await app.request(
    `https://app.test/${createdBody.annotation.id}`,
    {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ quote: 'Corrected exact claim', selector: { locator: '#claim' } }),
    },
    env,
  )
  ...
  const languageOnly = await app.request(
    `https://app.test/${editedBody.annotation.id}`,
    {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  const racedLanguage = await app.request(
    `https://app.test/${editedBody.annotation.id}`,
    {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  const response = await app.request(
    'https://app.test/',
    {
      ...
      body: JSON.stringify({
        recommendation_id: 'rec-1',
        locator_type: 'web',
        selector: { url: 'https://example.com/essay', locator: 'https://example.com/essay#claim' },
        quote: 'Same exact claim',
      }),
    },
    { DB } as any,
  )
...
test('atomic anchor creation accepts UTM and fragment variants through normalized dedup identity', async () => {
  ...
  try {
    const accepted = await app.request(
      'https://app.test/',
      {
        ...
        body: JSON.stringify({
          ...
          locator_type: 'web',
          selector: { url: 'https://example.com/essay?utm_source=android#claim' },
          quote: 'Tracked exact claim',
        }),
      },
      { DB } as any,
    )
    ...
    const stored = sqlite
      .prepare(`SELECT selector_json,selector_source_identities_json,source_checksum FROM source_annotations`)
      .get() as any
    ...
    const rejected = await app.request(
      'https://app.test/',
      {
        ...
        body: JSON.stringify({
          recommendation_id: 'rec',
          locator_type: 'web',
          selector: { url: 'https://example.com/unrelated?utm_source=android#claim' },
          quote: 'Wrong source claim',
        }),
      },
      { DB } as any,
    )
...
test('anchor creation preserves an exact long Android passage and rejects an oversized selector before hashing', async () => {
  ...
  const longResponse = await app.request(
    'https://app.test/',
    {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  const oversizedQuote = await app.request(
    'https://app.test/',
    {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  const oversizedContext = await app.request(
    'https://app.test/',
    {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
test('anchor creation and revision fail closed when canonical ownership changes after validation', async () => {
  ...
  const revised = await app.request(
    `https://app.test/${original.id}`,
    {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ quote: 'Changed claim' }),
    },
    { DB: reviseDB } as any,
  )
  ...
  const oversizedRevision = await app.request(
    `https://app.test/${original.id}`,
    {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ quote: 'q'.repeat(10001) }),
    },
    { DB: reviseDB } as any,
  )
...
test('client surfaces exact anchor search, explicit learning actions, and browser-selection intake', () => {
  ...
  assert.match(search, /annotations:\s*\{\s*label: 'Source anchors'/)
