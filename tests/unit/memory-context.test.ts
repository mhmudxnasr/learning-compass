...
test('memory retrieval tokenizes Arabic and receipts every filtered or truncated candidate', async () => {
  ...
  const memories = [
    { id: 'mem-ar-1', memory_key: 'learning:arabic', memory_kind: 'durable', value_json: JSON.stringify('التعلم الفعال'), confidence: 0.95, source: 'test', status: 'approved', evidence_json: '[]', updated_at: now },
    { id: 'mem-ar-2', memory_key: 'learning:arabic:secondary', memory_kind: 'durable', value_json: JSON.stringify('التعلم الفعال بالتطبيق'), confidence: 0.85, source: 'test', status: 'active', evidence_json: '[]', updated_at: now },
    { id: 'mem-en-1', memory_key: 'unrelated', memory_kind: 'durable', value_json: JSON.stringify('unrelated weather preference'), confidence: 1, source: 'test', status: 'active', evidence_json: '[]', updated_at: now },
  ]
  const assertions = [
    ...Array.from({ length: 14 }, (_, index) => ({ assertion_key: `تعلم.${index}`, category: 'learning', scope: null, value_json: JSON.stringify('التعلم الفعال'), weight: 1, confidence: 1, status: 'active', source_kind: 'test', version: 1, updated_at: now })),
    { assertion_key: 'unrelated.assertion', category: 'other', scope: null, value_json: JSON.stringify('weather'), weight: 1, confidence: 1, status: 'active', source_kind: 'test', version: 1, updated_at: now },
  ]
  let receiptArgs: any[] = []
  const db: any = {
    prepare(sql: string) {
  ...
  assert.equal(context.excluded.filter((item: any) => item.item_type === 'profile_assertion' && item.reason === 'limit_truncated').length, 2)
  assert.ok(context.excluded.some((item: any) => item.assertion_key === 'unrelated.assertion' && item.reason === 'no_query_match'))
