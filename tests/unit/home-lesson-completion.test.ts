...
test('Home completion has deliberate animated, accessible, and mobile-safe states', () => {
  ...
  assert.match(studioCss, /\.continuum-finish-lesson\s*\{[\s\S]*min-height:\s*44px/)
  assert.match(studioCss, /\.continuum-finish-lesson\s*\{\s*width:\s*auto;\s*min-height:\s*44px;/)
