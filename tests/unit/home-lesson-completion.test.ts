...
test('Home completion uses the canonical direct lesson mutation and refreshes in place', () => {
  ...
  assert.match(home, /\/learning\/core\/threads\/\$\{encodeURIComponent\(threadId\)\}\/lessons\/\$\{encodeURIComponent\(lesson\.id\)\}/)
...
test('Home completion has deliberate animated, accessible, and mobile-safe states', () => {
  for (const hook of ['continuum-finish-lesson', 'is-finishing', 'is-finished', 'continuum-turn-error']) assert.match(home, new RegExp(hook))
  ...
  assert.match(studioCss, /\.continuum-finish-lesson\s*\{[\s\S]*min-height:\s*36px/)
  assert.match(studioCss, /\.continuum-finish-lesson\s*\{ width:\s*100%; min-height:\s*44px;/)
