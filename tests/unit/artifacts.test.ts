...
import { inspectArtifactContent, liteVisualReceiptSignature, liteVisualTargetSha256, LITE_VISUAL_ATTESTATION_KEY_ID, LITE_VISUAL_AUDIT_PROVENANCE, LITE_VISUAL_RECEIPT_SCHEMA, LITE_VISUAL_WORKFLOW_CONTRACT, mergeArtifactMultipartMetadata, normalizeQualityAssurance, sha256Hex, validLiteVisualAttestation, validateArtifactIntegrity, validateLiteVisualPair } from '../../src/artifact-metadata.ts'
...
test('atomic Lite Visual pair validation binds the receipt to exact code-only files', async () => {
  ...
  const checks = { source_coverage: true, claim_traceability: true, exact_source_html: true, exact_source_pdf: true, canonical_html: true, code_only: true, rtl: true, accessibility: true, responsive: true, print_a4: true, pdf_parity: true }
  const target = { recommendation_id: 'rec-1', source_url: 'https://source.test/item', source_title: 'Source' }
  const receipt: Record<string, unknown> = { schema_version: LITE_VISUAL_RECEIPT_SCHEMA, workflow_contract: LITE_VISUAL_WORKFLOW_CONTRACT, status: 'passed', source_sha256: source, source_scope_sha256: 'b'.repeat(64), coverage_ledger_sha256: 'c'.repeat(64), html_sha256: await sha256Hex(htmlBytes), pdf_sha256: await sha256Hex(pdfBytes), work_item_sha256: 'd'.repeat(64), source_extraction_sha256: 'e'.repeat(64), target, target_sha256: await liteVisualTargetSha256(target), checks }
  receipt.attestation = { algorithm: 'hmac-sha256', key_id: LITE_VISUAL_ATTESTATION_KEY_ID, signature: await liteVisualReceiptSignature(receipt, receiptSigningKey) }
  const metadata = { pair_id: 'lv-source-v4', recommendation_id: 'rec-1', source_url: target.source_url, source_title: target.source_title, source_checksum: source, generator: 'lite-visual', workflow_contract: LITE_VISUAL_WORKFLOW_CONTRACT, asset_policy: 'code-only', recommended_start: 'html' }
  const valid = await validateLiteVisualPair(metadata, receipt, { name: 'companion.html', type: 'text/html' }, { name: 'companion.pdf', type: 'application/pdf' }, htmlBytes, pdfBytes, receiptSigningKey)
  assert.equal(valid.ok, true, valid.failures.join('\n'))
  const forgedReceipt = { ...receipt, checks: { ...checks, exact_source_html: true }, attestation: { algorithm: 'hmac-sha256', key_id: LITE_VISUAL_ATTESTATION_KEY_ID, signature: '0'.repeat(64) } }
  const forged = await validateLiteVisualPair(metadata, forgedReceipt, { name: 'companion.html', type: 'text/html' }, { name: 'companion.pdf', type: 'application/pdf' }, htmlBytes, pdfBytes, receiptSigningKey)
  assert.equal(forged.ok, false)
  assert.ok(forged.failures.some((failure) => failure.includes('attestation')))
  const tampered = await validateLiteVisualPair(metadata, receipt, { name: 'companion.html', type: 'text/html' }, { name: 'companion.pdf', type: 'application/pdf' }, new TextEncoder().encode(htmlText + 'tampered').buffer, pdfBytes, receiptSigningKey)
  assert.equal(tampered.ok, false)
  assert.ok(tampered.failures.some((failure) => failure.includes('HTML hash')))
  const interactive = await validateLiteVisualPair(metadata, receipt, { name: 'companion.html', type: 'text/html' }, { name: 'companion.pdf', type: 'application/pdf' }, new TextEncoder().encode(htmlText.replace('</article>', '<button>Reveal</button></article>')).buffer, pdfBytes, receiptSigningKey)
...
test('Worker verifies a Python-signed receipt containing an integral float', async () => {
  ...
  const result = spawnSync('python3', ['-c', code], { encoding: 'utf8', env: { ...process.env, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } })
...
class PairDatabase {
  ...
  prepare(sql: string) {
    ...
    const statement: any = {
      ...
      first: async () => {
        if (sql.includes('FROM recommendations')) return { id: 'rec-1', video_url: 'https://source.test/item', video_title: 'Source', source_metadata_json: '{}' }
...
class SqliteD1Statement {
  ...
  execute() { const result = this.database.prepare(this.sql).run(...this.values); return { success: true, meta: { changes: Number(result.changes) } } }
...
class ArtifactDatabase {
  ...
  prepare(sql: string) {
    const statement: any = {
      values: [] as unknown[],
      bind: (...values: unknown[]) => { statement.values = values; return statement },
      first: async () => sql.includes('SELECT * FROM artifacts') || sql.includes('SELECT id,filename,media_type,r2_key FROM artifacts') || sql.includes('SELECT a.*') || sql.includes('SELECT a.id') ? this.row || null : null,
      all: async () => ({ results: [] }),
      run: async () => {
...
async function atomicPairForm() {
  ...
  const target = { recommendation_id: 'rec-1', source_url: 'https://source.test/item', source_title: 'Source' }
  const receipt: Record<string, unknown> = { schema_version: LITE_VISUAL_RECEIPT_SCHEMA, workflow_contract: LITE_VISUAL_WORKFLOW_CONTRACT, status: 'passed', source_sha256: source, source_scope_sha256: 'b'.repeat(64), coverage_ledger_sha256: 'c'.repeat(64), html_sha256: await sha256Hex(htmlBytes), pdf_sha256: await sha256Hex(pdfBytes), work_item_sha256: 'd'.repeat(64), source_extraction_sha256: 'e'.repeat(64), target, target_sha256: await liteVisualTargetSha256(target), checks: { source_coverage: true, claim_traceability: true, exact_source_html: true, exact_source_pdf: true, canonical_html: true, code_only: true, rtl: true, accessibility: true, responsive: true, print_a4: true, pdf_parity: true } }
  receipt.attestation = { algorithm: 'hmac-sha256', key_id: LITE_VISUAL_ATTESTATION_KEY_ID, signature: await liteVisualReceiptSignature(receipt, receiptSigningKey) }
  const fingerprint = await sha256Hex(['source_sha256', 'source_scope_sha256', 'coverage_ledger_sha256', 'html_sha256', 'pdf_sha256'].map((key) => String(receipt[key])).join('\n'))
  form.set('metadata', JSON.stringify({ pair_id: `lv-rec-1-${fingerprint.slice(0, 20)}`, recommendation_id: 'rec-1', source_url: target.source_url, source_title: target.source_title, source_checksum: source, generator: 'lite-visual', workflow_contract: LITE_VISUAL_WORKFLOW_CONTRACT, asset_policy: 'code-only', recommended_start: 'html' }))
...
test('atomic pair route stores both objects before one D1 batch and retains the exact receipt', async () => {
  ...
  const response = await artifactsApp.request('https://compass.test/pairs', { method: 'POST', body: await atomicPairForm() }, {
    ...
    ARTIFACTS: { put: async (key: string, value: ArrayBuffer, options: any) => { puts.push(key); objects.set(key, { size: value.byteLength, customMetadata: options.customMetadata }) }, head: async (key: string) => objects.get(key), delete: async (key: string) => { deletes.push(key) } },
  } as any)
...
test('one guarded activation publishes an exact persisted corpus and completes its immutable job', async () => {
  ...
  const objects = new Map<string, any>()
  const bucket = {
    put: async (key: string, value: ArrayBuffer, options: any) => { objects.set(key, { size: value.byteLength, customMetadata: options.customMetadata }) },
  ...
  for (const role of ['html', 'pdf']) {
    sqlite.prepare('INSERT INTO artifacts(id,filename,media_type,r2_key,size_bytes,metadata_json) VALUES (?,?,?,?,1,?)').run(`old-${role}`, `old.${role}`, role === 'html' ? 'text/html' : 'application/pdf', `old/${role}`, JSON.stringify({ pair_id: 'old-pair', recommendation_id: 'rec-1', role, publication_state: 'ready', validation_status: 'passed', html_sha256: oldHtmlSha, pdf_sha256: oldPdfSha }))
    objects.set(`old/${role}`, { size: 1, customMetadata: { sha256: role === 'html' ? oldHtmlSha : oldPdfSha, pair_id: 'old-pair', role } })
  }
  sqlite.prepare(`INSERT INTO lite_visual_pairs(pair_id,recommendation_id,target_sha256,work_item_sha256,source_extraction_sha256,source_sha256,source_scope_sha256,coverage_ledger_sha256,html_sha256,pdf_sha256,receipt_sha256,html_artifact_id,pdf_artifact_id,html_r2_key,pdf_r2_key,html_size_bytes,pdf_size_bytes,r2_verified,state)
    VALUES ('old-pair','rec-1',?,?,?,?,?,?,?,?,?,'old-html','old-pdf','old/html','old/pdf',1,1,1,'active')`).run(...Array.from({ length: 6 }, (_, index) => String(index + 1).repeat(64)), oldHtmlSha, oldPdfSha, 'c'.repeat(64))
  ...
  const runId = 'run-1'
  const jobId = 'job-1'
  sqlite.prepare(`INSERT INTO agent_jobs(id,job_type,status,payload_json,result_json,recommendation_id,workflow_run_id,workflow_step,updated_at) VALUES (?,'visualise_source','pending',?,'{}','rec-1',?,'resolve_source',datetime('now'))`).run(jobId, JSON.stringify({ recommendation_id: 'rec-1', source_url: 'https://source.test/item', title: 'Source', workflow_contract: LITE_VISUAL_WORKFLOW_CONTRACT, revision_of_pair_id: 'old-pair' }), runId)
  ...
  const replayedAudit = await artifactsApp.request('https://compass.test/corpora', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ...contract, targets: [changedHash] }) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(replayedAudit.status, 409, await replayedAudit.text())
  const corpusResponse = await artifactsApp.request('https://compass.test/corpora', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(contract) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(corpusResponse.status, 201, `${await corpusResponse.clone().text()}\n${JSON.stringify(sqlite.prepare('SELECT * FROM lite_visual_corpus_targets').all())}\n${JSON.stringify(target)}`)
  const corpusId = (await corpusResponse.json() as any).corpus_id

  sqlite.prepare("UPDATE agent_jobs SET status='running',workflow_step='publish_pair',lease_owner='worker-1',lease_expires_at=datetime('now','+5 minutes') WHERE id=?").run(jobId)
  metadata.corpus_id = corpusId; metadata.job_id = jobId; metadata.workflow_run_id = runId; metadata.worker_identity = 'worker-1'; metadata.supersedes_pair_id = 'old-pair'
  form.set('metadata', JSON.stringify(metadata))
  const pairResponse = await artifactsApp.request('https://compass.test/pairs', { method: 'POST', body: form }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(pairResponse.status, 201, await pairResponse.clone().text())
  const pair = await pairResponse.json() as any
  sqlite.prepare("UPDATE agent_jobs SET status='awaiting_activation',lease_owner=NULL,lease_expires_at=NULL,result_json=? WHERE id=?").run(JSON.stringify({ pair_id: pairId, receipt_sha256: receiptSha }), jobId)

  db.beforeBatch = () => sqlite.prepare("UPDATE lite_visual_corpora SET state='aborted',aborted_at=datetime('now') WHERE id=? AND state='staging'").run(corpusId)
  const staleActivation = await artifactsApp.request(`https://compass.test/corpora/${corpusId}/activate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(contract) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(staleActivation.status, 500, await staleActivation.clone().text())
  assert.equal(sqlite.prepare("SELECT json_extract(metadata_json,'$.publication_state') state FROM artifacts WHERE id='old-html'").get().state, 'ready')
  assert.equal(sqlite.prepare("SELECT json_extract(metadata_json,'$.publication_state') state FROM artifacts WHERE id=?").get(pair.html.id).state, 'staged')
  ...
  db.beforeBatch = () => {
    sqlite.prepare("UPDATE lite_visual_corpora SET state='active',activated_at=datetime('now') WHERE id=? AND state='staging'").run(corpusId)
    sqlite.prepare("UPDATE artifacts SET metadata_json=json_set(metadata_json,'$.publication_state','superseded') WHERE json_extract(metadata_json,'$.pair_id')='old-pair'").run()
    sqlite.prepare("UPDATE artifacts SET metadata_json=json_set(metadata_json,'$.publication_state','ready') WHERE json_extract(metadata_json,'$.pair_id')=?").run(pairId)
  ...
  const staleAbort = await artifactsApp.request(`https://compass.test/corpora/${corpusId}/abort`, { method: 'POST' }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(staleAbort.status, 409, await staleAbort.clone().text())
  assert.equal(objects.has(sqlite.prepare('SELECT html_r2_key FROM lite_visual_pairs WHERE pair_id=?').get(pairId).html_r2_key), true)
  ...
  sqlite.prepare("UPDATE artifacts SET metadata_json=json_set(metadata_json,'$.publication_state','ready') WHERE json_extract(metadata_json,'$.pair_id')='old-pair'").run()
  sqlite.prepare("UPDATE artifacts SET metadata_json=json_set(metadata_json,'$.publication_state','staged') WHERE json_extract(metadata_json,'$.pair_id')=?").run(pairId)
  ...
  const activation = await artifactsApp.request(`https://compass.test/corpora/${corpusId}/activate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(contract) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  ...
  assert.equal(sqlite.prepare("SELECT json_extract(metadata_json,'$.publication_state') state FROM artifacts WHERE id='old-html'").get().state, 'superseded')
  assert.equal(sqlite.prepare('SELECT corpus_id FROM lite_visual_active_corpora WHERE thread_id=?').get('thread-1').corpus_id, corpusId)
  assert.equal(sqlite.prepare('SELECT state FROM lite_visual_pairs WHERE pair_id=?').get(pairId).state, 'active')
  assert.ok(pair.html?.id && pair.pdf?.id)

  const corpusRetry = await artifactsApp.request('https://compass.test/corpora', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(contract) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  ...
  const pairRetry = await artifactsApp.request('https://compass.test/pairs', { method: 'POST', body: pairRetryForm }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(pairRetry.status, 200, await pairRetry.clone().text())
  assert.equal((await pairRetry.json() as any).reused, true)

  const rollback = await artifactsApp.request(`https://compass.test/corpora/${corpusId}/rollback`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(contract) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
  assert.equal(rollback.status, 200, await rollback.clone().text())
  assert.equal(sqlite.prepare("SELECT json_extract(metadata_json,'$.publication_state') state FROM artifacts WHERE id='old-html'").get().state, 'ready')
  assert.equal(sqlite.prepare("SELECT json_extract(metadata_json,'$.publication_state') state FROM artifacts WHERE id=?").get(pair.html.id).state, 'superseded')
  assert.equal(sqlite.prepare('SELECT corpus_id FROM lite_visual_active_corpora WHERE thread_id=?').get('thread-1'), undefined)
  assert.equal(sqlite.prepare('SELECT state FROM lite_visual_pairs WHERE pair_id=?').get(pairId).state, 'superseded')
  assert.equal(sqlite.prepare("SELECT status FROM agent_jobs WHERE id='job-1'").get().status, 'completed')
  assert.ok(sqlite.prepare("SELECT json_extract(result_json,'$.rolled_back_at') value FROM agent_jobs WHERE id='job-1'").get().value)
  const rollbackRetry = await artifactsApp.request(`https://compass.test/corpora/${corpusId}/rollback`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(contract) }, { DB: db, ARTIFACTS: bucket, LITE_VISUAL_RECEIPT_SIGNING_KEY: receiptSigningKey } as any)
