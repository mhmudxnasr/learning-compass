  ...
  try {
    const response = await app.request(
      '/rec-1/source-url',
      {
        ...
        body: JSON.stringify({
          source_url: 'https://www.youtube.com/watch?v=3NQJSniI8XM',
          expected_source_url: 'https://al-badr.net/detail/lesson',
        }),
      },
      { DB } as any,
    )
    ...
    assert.deepEqual(DB.batches[1][0].args.slice(0, 2), [
  ...
  try {
    const response = await app.request(
      '/rec-1/source-url',
      {
        ...
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          source_url: 'https://restricted.example/lesson',
          expected_source_url: 'https://al-badr.net/detail/lesson',
        }),
      },
      { DB } as any,
...
class SourceHealthApiDatabase {
  ...
  prepare(sql: string) {
    const statement = {
      ...
      first: async () => {
        if (sql.includes('FROM recommendations WHERE id='))
          return {
            id: 'rec-1',
            video_title: 'Lesson',
            content_type: 'article',
            video_url: 'https://example.com/lesson',
            dedup_key: 'art_example_com_lesson',
            status: 'active',
            deleted_at: null,
            updated_at: '2026-08-01T00:00:00.000Z',
          }
        if (sql.includes('FROM source_health WHERE'))
          return {
            recommendation_id: 'rec-1',
            checked_url: this.healthCheckedUrl,
            status: 'verified',
      ...
      all: async () => {
        if (sql.includes('FROM source_health_attempts'))
          return { results: [{ id: 'source_check_1', purpose: 'current', status: 'verified' }] }
        if (sql.includes('FROM source_url_replacements'))
          return { results: [{ id: 'source_replacement_1', previous_url: 'https://old.example/lesson' }] }
        return { results: [] }
      },
    }
    return statement
...
test('candidate verification and replacement reject a stale observed source URL before network work', async () => {
  ...
  try {
    for (const [path, method] of [
      ['/rec-1/source-url/verify', 'POST'],
      ['/rec-1/source-url', 'PATCH'],
    ] as const) {
      const response = await app.request(
        path,
        {
          ...
          body: JSON.stringify({
            ...
            expected_source_url: 'https://example.com/stale',
          }),
        },
        { DB } as any,
      )
      ...
      assert.deepEqual(await response.json(), {
        error: 'source_url_precondition_failed',
  ...
  prepare(sql: string) {
    const statement = {
      ...
      first: async () => {
        ...
        if (sql.includes('SELECT id,video_url FROM recommendations'))
          return { id: 'book-1', video_url: 'https://books.example/original' }
        if (sql.includes('SELECT r.*'))
          return { id: 'book-1', video_url: 'https://books.example/original', video_title: 'Updated title' }
        return null
      },
    }
    return statement
  ...
  const rejected = await app.request(
    '/books',
    {
      ...
      body: JSON.stringify({
        ...
        branch_id: 'books',
        isbn: '123456789X',
        url: 'https://books.example/replacement',
      }),
    },
...
test('delta sync preserves an existing canonical URL on stable-id conflict', () => {
  ...
  const recommendationUpsert = sync.slice(
    sync.indexOf('INSERT INTO recommendations'),
    sync.indexOf('// Tree nodes delta'),
  )
...
class RecommendationPushUrlDatabase {
  ...
  prepare(sql: string) {
    const statement = {
      ...
      first: async () =>
        sql.includes('SELECT id,video_url FROM recommendations WHERE dedup_key=')
...
test('recommendation push rejects a dedup collision with a different canonical URL before any write', async () => {
  ...
  const response = await app.request(
    '/push',
    {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        video_title: 'Same slug, different source',
        video_url: 'https://example.com/two/lesson',
        content_type: 'article',
      }),
    },
    { DB } as any,
