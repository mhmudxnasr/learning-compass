...
test('NotebookLM learning routes Arabic audio and rich media only when they fit', () => {
  ...
  assert.ok(review.rejected_formats.some((item) => item.format === 'video' && item.reason.includes('no matching concept feature')))
...
test('NotebookLM source receipts distinguish pending, indexed, and failed truth', () => {
  assert.equal(validateNotebookLearningReceipt({
    kind: 'source', recommendation_id: 'rec-1', notebook_id: 'notebook-123', notebook_url: notebookUrl, status: 'pending',
  }).ok, true)
  const indexed = validateNotebookLearningReceipt({
    kind: 'source', recommendation_id: 'rec-1', notebook_id: 'notebook-123', notebook_url: notebookUrl, status: 'indexed',
  })
  ...
  const failed = validateNotebookLearningReceipt({
    kind: 'source', recommendation_id: 'rec-1', notebook_id: 'notebook-123', notebook_url: notebookUrl, status: 'failed',
  })
  assert.equal(failed.ok, false)
  assert.ok(failed.failures.includes('error is required when source status is failed'))
  assert.equal(validateNotebookLearningReceipt({
    kind: 'source', recommendation_id: 'rec-1', notebook_id: 'notebook-123',
    notebook_url: 'https://notebooklm.google.com/notebook/notebook-123', status: 'pending',
  }).ok, true)
})
...
test('NotebookLM artifact receipts cannot claim ready before the learning contract is verified', () => {
  ...
  const pending: NotebookArtifactReceiptInput = {
    ...
    plan_id: 'plan-1', format: 'quiz', status: 'pending', provider_task_id: 'task-1',
    source_grounded: true, custom_prompt_applied: true,
  }
  assert.equal(validateNotebookLearningReceipt(pending, plan).ok, true)

  const incompleteReady = validateNotebookLearningReceipt({
    ...pending, status: 'ready', provider_task_id: undefined, provider_artifact_id: 'artifact-1', question_count: 4,
    hints_before_explanations: false, transfer_question_count: 0,
  }, plan)
  ...
  const ready = validateNotebookLearningReceipt({
    ...pending, status: 'ready', provider_task_id: undefined, provider_artifact_id: 'artifact-1', question_count: 7,
    hints_before_explanations: true, transfer_question_count: 1,
  }, plan)
...
test('NotebookLM receipts reject formats outside their plan and non-Arabic audio', () => {
  ...
  const video = validateNotebookLearningReceipt({
    kind: 'artifact', recommendation_id: 'rec-1', notebook_id: 'notebook-123', notebook_url: notebookUrl,
    plan_id: 'plan-1', format: 'video', status: 'pending', provider_task_id: 'task-1', source_grounded: true, custom_prompt_applied: true,
  }, plan)
  assert.equal(video.ok, false)
  assert.ok(video.failures.includes('format is not selected by the learning output plan'))

  const audioPlan = buildNotebookLearningPlan({ recommendation_id: 'rec-1', purpose: 'review', requested_formats: ['audio'] })
  const audio = validateNotebookLearningReceipt({
    kind: 'artifact', recommendation_id: 'rec-1', notebook_id: 'notebook-123', notebook_url: notebookUrl,
    plan_id: 'plan-2', format: 'audio', status: 'pending', provider_task_id: 'task-2', source_grounded: true, custom_prompt_applied: true, language: 'ar',
  }, audioPlan)
...
test('NotebookLM receipt reducer exposes one reusable source-level status for Thread reads', () => {
  const row = (id: string, intent: string, payload: Record<string, unknown>, createdAt: string) => ({
    id, intent, status_code: 200, verified: 1, receipt_json: JSON.stringify(payload), created_at: createdAt,
  })
  const state = reduceNotebookLearningReceipts([
    row('artifact-new', 'notebooklm_artifact_receipt', { plan_id: 'plan-new', format: 'quiz', status: 'pending' }, '2026-08-20 12:04:00'),
    row('plan-new', 'notebooklm_learning_plan', { source_receipt_id: 'source-indexed', plan: { selected_formats: ['quiz'] } }, '2026-08-20 12:03:00'),
    row('source-indexed', 'notebooklm_source_receipt', { notebook_url: notebookUrl, status: 'indexed' }, '2026-08-20 12:02:00'),
    row('artifact-old', 'notebooklm_artifact_receipt', { plan_id: 'plan-old', format: 'quiz', status: 'ready' }, '2026-08-20 11:00:00'),
  ], notebookUrl)
...
test('NotebookLM receipt reducer hides outputs built from a superseded source index', () => {
  ...
  const state = reduceNotebookLearningReceipts([
    row('source-new', 'notebooklm_source_receipt', { notebook_url: notebookUrl, status: 'indexed' }, '2026-08-20 12:05:00'),
    row('artifact-old', 'notebooklm_artifact_receipt', { plan_id: 'plan-old', format: 'quiz', status: 'ready' }, '2026-08-20 12:04:00'),
    row('plan-old', 'notebooklm_learning_plan', { source_receipt_id: 'source-old', plan: { selected_formats: ['quiz'] } }, '2026-08-20 12:03:00'),
    row('source-old', 'notebooklm_source_receipt', { notebook_url: notebookUrl, status: 'indexed' }, '2026-08-20 12:02:00'),
  ], notebookUrl)
...
test('NotebookLM receipt reducer never carries ready output across a notebook URL change', () => {
  ...
  const state = reduceNotebookLearningReceipts([
    row('artifact-old', 'notebooklm_artifact_receipt', { plan_id: 'plan-old', format: 'quiz', status: 'ready' }, '2026-08-20 12:04:00'),
    row('plan-old', 'notebooklm_learning_plan', { source_receipt_id: 'source-old', plan: { selected_formats: ['quiz'] } }, '2026-08-20 12:03:00'),
    row('source-old', 'notebooklm_source_receipt', { notebook_url: oldNotebookUrl, status: 'indexed' }, '2026-08-20 12:02:00'),
  ], notebookUrl)
...
test('NotebookLM receipt reducer never labels one format with another output status', () => {
  ...
  const state = reduceNotebookLearningReceipts([
    row('artifact-audio', 'notebooklm_artifact_receipt', { plan_id: 'plan-multi', format: 'audio', status: 'ready' }, '2026-08-20 12:04:00'),
    row('plan-multi', 'notebooklm_learning_plan', { source_receipt_id: 'source-indexed', plan: { selected_formats: ['quiz', 'audio'] } }, '2026-08-20 12:03:00'),
    row('source-indexed', 'notebooklm_source_receipt', { notebook_url: notebookUrl, status: 'indexed' }, '2026-08-20 12:02:00'),
  ], notebookUrl)
