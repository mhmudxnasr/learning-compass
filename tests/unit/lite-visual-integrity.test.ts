...
import { LITE_VISUAL_INTEGRITY_SCHEMA, liteVisualReceiptSignature, normalizeQualityAssurance, sha256Hex, validLiteVisualAttestation, validateLiteVisualPair } from '../../src/artifact-metadata.ts'
...
test('direct finish renders once without ledgers, source duplication, editorial passes, or browser/PDF QA; Worker verifies the honest receipt', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'lite-integrity-'))
  try {
    const source = 'This is a complete synthetic source for testing the direct rendering workflow and its file integrity boundary.'
    const html = '<!doctype html><html lang="ar" dir="rtl"><meta charset="utf-8"><title>شرح الفكرة</title><style>@page{size:A4}body{font:18px/1.9 Tahoma}p{color:#eee}</style><body><article data-canonical-content="true"><h1>إزاي نفهم الفكرة؟</h1><p>دي تجربة للملفات، ومش دليل على إن الشرح اتراجع أو إن كل تفاصيل التصميم سليمة.</p></article></body></html>'
    // Low contrast deliberately demonstrates that this receipt does not attest QA.
    writeFileSync(join(dir, 'source.txt'), source)
    writeFileSync(join(dir, 'source-extraction.json'), JSON.stringify({ schema_version: 'lite-visual-source-extraction/v1', status: 'complete', content_sha256: await sha256Hex(source), word_count: 20, method: 'direct-utf8', cache_key: 'c'.repeat(64) }))
    writeFileSync(join(dir, 'companion.html'), html)
    const command = [join(scripts, 'run_workflow.py'), 'finish', '--workdir', dir, '--recommendation-id', 'test-source', '--source-url', 'https://example.test/source', '--source-title', 'Source']
    const finish = () => spawnSync('python3', command, { encoding: 'utf8', env: { ...process.env, LITE_VISUAL_RECEIPT_SIGNING_KEY: key }, timeout: 30_000 })
    const first = finish()
    assert.equal(first.status, 0, first.stderr)
    assert.equal(readFileSync(join(dir, 'companion.html'), 'utf8'), html, 'must not append source text or rewrite the article')
    ...
    const oldServer = spawnSync('python3', ['-c', `import sys;sys.path.insert(0,${JSON.stringify(scripts)})
...
assert calls==['/artifacts/pair-contract'],calls`], { encoding: 'utf8', env: { ...process.env, LITE_VISUAL_RECEIPT_SIGNING_KEY: key } })
    assert.equal(oldServer.status, 0, oldServer.stderr)
    const manifest = join(dir, 'batch.json'), registration = join(dir, 'registration.json')
    const target = { recording_number: 1, recommendation_id: 'test-source', source_url: 'https://example.test/source', source_title: 'Source', workdir: dir, job_id: 'job-test', workflow_run_id: 'run-test', supersedes_pair_id: 'lv-prior-pair' }
    writeFileSync(manifest, JSON.stringify({ thread_id: 'thread-test', targets: [target] }))
    const audit = () => spawnSync('python3', [join(scripts, 'audit_pair_set.py'), manifest, '--out', registration], { encoding: 'utf8', env: { ...process.env, LITE_VISUAL_RECEIPT_SIGNING_KEY: key } })
    const audited = audit()
    ...
    assert.equal(batch.target_set_sha256, await sha256Hex(JSON.stringify([[1, target.recommendation_id, target.source_url, target.source_title, dir]])))
    const fields = ['recording_number', 'recommendation_id', 'chapter_key', 'source_url', 'source_title', 'workdir', 'pair_id', 'target_sha256', 'work_item_sha256', 'source_extraction_sha256', 'source_sha256', 'source_scope_sha256', 'coverage_ledger_sha256', 'html_sha256', 'pdf_sha256', 'receipt_sha256']
    assert.equal(batch.audit_corpus_sha256, await sha256Hex(JSON.stringify(batch.targets.map((t: Record<string, unknown>) => fields.map(field => t[field] ?? '')))))
    ...
    const metadata = { pair_id: 'lv-test-source', recommendation_id: 'test-source', source_checksum: receipt.source_sha256, generator: 'lite-visual', workflow_contract: receipt.workflow_contract, asset_policy: 'code-only', recommended_start: 'html' }
    const check = (candidate: typeof receipt, bytes = htmlBytes) => validateLiteVisualPair(metadata, candidate, { name: 'companion.html', type: 'text/html' }, { name: 'companion.pdf', type: 'application/pdf' }, bytes, pdfBytes, key)
    ...
    for (const mutation of ['false-quality-claim', 'wrong-scope', 'extra-check', 'numeric-check']) {
      ...
      const local = spawnSync('python3', ['-c', `import sys,json;sys.path.insert(0,${JSON.stringify(scripts)});from pair_integrity import receipt_valid;assert not receipt_valid(json.loads(sys.argv[1]))`, JSON.stringify(changed)], { encoding: 'utf8', env: { ...process.env, LITE_VISUAL_RECEIPT_SIGNING_KEY: key } })
      assert.equal(local.status, 0, `${mutation}: ${local.stderr}`)
    }
    const qa = normalizeQualityAssurance({ generator: 'lite-visual', role: 'html', validation_status: 'passed', validation_receipt_schema: receipt.schema_version })
...
test('direct checkpoint evidence reports zero reviewed scopes/claims explicitly while old checkpoints remain strict', () => {
  ...
  const html = { html_sha256: 'a'.repeat(64), coverage_ledger_sha256: 'b'.repeat(64), claim_count: 0, canonical_selector: 'article[data-canonical-content=true]' }
  ...
  assert.deepEqual(validateLiteVisualCheckpointEvidence('publish_pair', { validation_schema: LITE_VISUAL_INTEGRITY_SCHEMA, validation_status: 'passed', receipt_sha256: 'a'.repeat(64) }), [])
})
