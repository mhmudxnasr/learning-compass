  ...
  const DB = {
    prepare(sql: string) {
      ...
      const statement: any = {
        ...
        async first() {
          if (sql.includes('FROM recommendations WHERE id=?') && values[0] === 'rec-1')
            return { id: 'rec-1', video_title: 'Source', notebook_url: notebookUrl }
          return null
        },
        async all() {
          ...
          return {
            results: receipts
              .filter((row) => row.target === values[0])
        ...
        async run() {
          if (sql.includes('INSERT INTO agent_receipts')) {
            ...
            receipts.push({
              ...
              verified: values[6],
              receipt_json: values[7],
              created_at: `2026-08-20 12:00:${String(tick).padStart(2, '0')}`,
            })
          }
...
const post = (path: string, body: unknown, env: any) =>
  app.request(
    `https://example.test${path}`,
    {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-agent-name': 'test-notebooklm' },
...
test('NotebookLM API enforces indexed source, focused route, and pending artifact receipts', async () => {
  ...
  assert.equal(
    (await post('/learning/receipts', { ...base, status: 'indexed', provider_source_id: 'source-1' }, env)).status,
    201,
  )
  ...
  assert.deepEqual(
    state.outputs.map((item: any) => [item.format, item.status]),
    [['quiz', 'pending']],
  )
})
