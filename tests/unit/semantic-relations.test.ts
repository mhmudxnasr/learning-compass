...
test('semantic relation migration adds note ownership, explanations, anchors, and review state', () => {
  ...
  for (const field of ['note_id', 'why', 'source_anchor_id', 'target_anchor_id', 'review_state', 'resolution', 'reviewed_at']) assert.match(migration, new RegExp(field))
})
...
test('notes use owned Units and branch dossiers derive rather than persist bridges', () => {
  ...
  assert.ok(product.indexOf('learning_units WHERE recommendation_id=? AND status') > product.indexOf('learning_units WHERE note_id=?'), 'legacy source fallback must follow the canonical note-owned query')
...
test('relations normalize the counterpart from either direction with branch and domain', () => {
  const row = {
    id: 'relation-1', source_unit_id: 'unit-a', target_unit_id: 'unit-b', relation_type: 'supports', confidence: .8,
    why: 'The source mechanism explains the target outcome.', status: 'active', review_state: 'accepted', created_at: '2026-08-23',
    source_statement: 'Mechanism', source_unit_type: 'claim', source_note_id: 'note-a', source_branch_id: 'branch-a', source_branch_label: 'Systems', source_domain_id: 'domain-a', source_domain: 'Thinking', source_anchor_locator: 'p. 4',
    target_statement: 'Outcome', target_unit_type: 'claim', target_note_id: 'note-b', target_branch_id: 'branch-b', target_branch_label: 'Strategy', target_domain_id: 'domain-b', target_domain: 'Business', target_anchor_locator: 'p. 9',
  }
  ...
  assert.deepEqual(incoming.counterpart.branch, { id: 'branch-a', label: 'Systems', domain_id: 'domain-a', domain: 'Thinking' })
...
test('note deletion detaches owned Units before removing the note', () => {
  const product = read('../../src/api/product.ts')
  const deletion = product.slice(product.indexOf("app.delete('/notes/:id'"), product.indexOf("app.post('/notes/:id/process'"))
