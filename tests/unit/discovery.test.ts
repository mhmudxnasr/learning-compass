...
test('bounded weight adaptation keeps weights within ±20% of baseline and normalized to 1.0', () => {
  const currentWeights = [
    { id: 'frontier_potential', dimension: 'frontier_potential', baseline_weight: 0.30, current_weight: 0.30, evidence_count: 0 },
    { id: 'info_gain', dimension: 'info_gain', baseline_weight: 0.20, current_weight: 0.20, evidence_count: 0 },
    { id: 'personal_pull', dimension: 'personal_pull', baseline_weight: 0.15, current_weight: 0.15, evidence_count: 0 },
    { id: 'real_life_relevance', dimension: 'real_life_relevance', baseline_weight: 0.15, current_weight: 0.15, evidence_count: 0 },
    { id: 'source_quality', dimension: 'source_quality', baseline_weight: 0.15, current_weight: 0.15, evidence_count: 0 },
    { id: 'format_exploration', dimension: 'format_exploration', baseline_weight: 0.05, current_weight: 0.05, evidence_count: 0 },
  ]

  const deltas = {
    frontier_potential: +0.20, // Should be clamped to 0.30 * 1.2 = 0.36
  ...
  assert.ok(frontier.current_weight <= 0.40)
...
test('weak-result withholding triggers when candidate total score is below quality threshold', () => {
  ...
  const passesThreshold = candidate.is_verified && candidate.total_score >= 0.60
...
test('pruning rule requires at least 2 distinct run probes with negative evidence', () => {
  ...
  const negativeRunIds = new Set(
    pastEvidence
      .filter((e) => e.signal_value <= -0.6 && e.run_id)
      .map((e) => e.run_id)
  )
