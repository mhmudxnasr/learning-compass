...
test.before(async () => {
  ...
  const [indexModule, captureModule, hardcoverApiModule, notificationsModule, hardcoverServiceModule, rssModule] = await Promise.all([
...
function statement(sql: string, handlers: {
...
test('Telegram webhook failures persist only redacted operational detail', async () => {
  ...
  console.error = () => {}
  try {
    const response = await (worker.fetch as any)(new Request('https://compass.test/api/telegram', {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-telegram-bot-api-secret-token': 'test-webhook-secret' },
      body: JSON.stringify({ update_id: 701, message: { message_id: 1, chat: { id: 42 }, text: 'https://example.com/source' } }),
    }), {
      DB,
...
test('notification delivery persistence and API responses redact upstream credentials', async () => {
    ...
    prepare(sql: string) {
      return statement(sql, {
        first: () => {
          ...
          if (sql.includes('FROM user_settings')) return { value_json: JSON.stringify({ enabled: true, chat_id: '42' }) }
          return null
        },
        all: () => ({ results: [] }),
        run: (_query, values) => {
  ...
  globalThis.fetch = (async () => { throw new Error(leakedDetail) }) as typeof fetch
  try {
    await deliverScheduledReminders({ DB, TELEGRAM_BOT_TOKEN: 'test-bot-token' } as any)
    const response = await notificationsApp.request('https://compass.test/test', {
      method: 'POST',
...
test('RSS failures redact credential-bearing URLs in D1 and API responses', async () => {
  ...
  globalThis.fetch = (async () => { throw new Error(leakedDetail) }) as typeof fetch
  try {
    await assert.rejects(() => syncFeed(DB as any, {
      id: 'feed-1', feed_url: 'https://example.com/feed?token=query-credential-value', title: 'Feed', site_url: null, etag: null, last_modified: null, branch_id: 'branch-1',
    } as any))
    assertRedacted(persisted)

    const response = await captureApp.request('https://compass.test/feeds', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
test('Hardcover failures redact provider detail in D1 and API responses', async () => {
  ...
  await assert.rejects(() => syncHardcoverLibrary(DB as any, 'test-provider-token', async () => credentialErrorResponse()))
  assertRedacted(persisted[0])

  const originalFetch = globalThis.fetch
  globalThis.fetch = (async () => credentialErrorResponse()) as typeof fetch
  try {
    const response = await hardcoverApp.request('https://compass.test/sync', { method: 'POST' }, { DB, HARDCOVER_API_TOKEN: 'test-provider-token' } as any)
...
test('every audited operational boundary calls the shared redactor', () => {
  ...
  for (const source of [rss, capture, hardcoverService, hardcoverApi]) assert.match(source, /redactSensitiveText\(error,/)
