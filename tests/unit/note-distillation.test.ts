...
import {
  ...
  distillationChecksum,
  promoteHighlightToUnit,
  splitDistillationBlocks,
} from '../../src/services/note-distillation.ts'
...
test('highlight creation rejects stale checksums instead of relocating the source', async () => {
  ...
  const DB = {
    prepare(sql: string) {
      ...
      statement.all = async () =>
        sql.includes('FROM note_sections')
          ? { results: [{ section_key: 'body', label: 'Body', content: 'Exact original text' }] }
          : { results: [] }
      statement.run = async () => {
  ...
  const stale = await createClaimHighlight(DB, 'note-1', {
    section_key: 'body',
    block_index: 0,
    block_checksum: 'old',
    claim_text: 'My claim',
  ...
  const created = await createClaimHighlight(DB, 'note-1', {
    section_key: 'body',
    block_index: 0,
  ...
  const DB = {
    prepare(sql: string) {
      ...
      statement.first = async () => ({
        ...
        section_key: 'body',
        block_index: 2,
        block_checksum: 'abc123',
        source_text: 'Exact original',
        claim_text: 'User claim',
        promoted_unit_id: null,
      })
      return statement
    },
