...
} from '../../src/services/delivery-context.ts'
...
function contextDb(saved: unknown = null) {
  return {
    prepare(sql: string) {
      return {
        first: async () => {
          if (sql.includes("setting_key='delivery_context'"))
            return saved == null ? null : { value_json: JSON.stringify(saved) }
...
test('delivery context uses only explicit request, saved, or default sources', async () => {
  ...
  const saved = await resolveDeliveryContext(
    contextDb({ effort: 'light', language: 'ar', delivery_modes: ['listen'], depth_tier: 'adaptive' }),
  )
  assert.equal(saved.source, 'saved')
  assert.equal(saved.context.language, 'ar')

  const requested = await resolveDeliveryContext(contextDb({ effort: 'light' }), {
    effort: 'deep',
    depth_tier: 'advanced',
  })
...
test('delivery matching is neutral when candidate metadata is absent and never gates', () => {
  const resolved = {
    ...
    adaptive_depth: {
      recommended_tier: 'introductory' as const,
      direct_lessons_completed: 0,
      too_shallow_feedback: 0,
      too_deep_feedback: 0,
      advisory_only: true as const,
      progression_effect: 'none' as const,
    },
  }
  assert.deepEqual(deliveryMatch({}, resolved), { matches: true, score: 0.5, compared_fields: 0, advisory_only: true })
...
test('Queue preserves source order and filters only when matches-only is explicit', async () => {
  const rows = [
    {
      id: 'first',
      source_metadata_json: JSON.stringify({ candidate_metadata: { effort: 'light' } }),
      branch_label: 'A',
    },
    {
      id: 'second',
      source_metadata_json: JSON.stringify({ candidate_metadata: { effort: 'deep' } }),
      branch_label: 'B',
    },
  ]
  ...
  assert.deepEqual(
    (await loadCaptureQueue(DB, 50, delivery)).map((item) => item.id),
    ['first', 'second'],
  )
  assert.deepEqual(
    (await loadCaptureQueue(DB, 50, delivery, true)).map((item) => item.id),
    ['second'],
  )
})

test('perspective diversity uses only verified evidence-backed labels and unknown stays neutral', () => {
  const verified = deriveCandidateFeatures({
    title: 'A',
    perspective: { status: 'verified', viewpoint: 'Institutional', school: 'Ostrom', evidence_indexes: [0] },
  })
  const peer = deriveCandidateFeatures({
    title: 'B',
    perspective: { status: 'verified', viewpoint: 'Institutional', school: 'Ostrom', evidence_indexes: [0] },
  })
