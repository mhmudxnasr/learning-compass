...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    try {
      if ((await fetch('http://127.0.0.1:8792/health/live')).ok) break
    } catch {}
  ...
  await query(`
    INSERT INTO tree_nodes (id,type,label,status,parent_id) VALUES ('compass-domain','category','Compass test domain','active','root');
    INSERT INTO tree_nodes (id,type,label,status,parent_id,super_category) VALUES ('compass-branch','branch','Compass test branch','love','root','compass-domain');
  ...
  `)
  const declined = await request('/compass/pick/pick_decline/feedback', {
    method: 'POST',
    body: JSON.stringify({
      outcome: 'declined',
      score: 3,
      disposition: 'drop',
      reason_tags: ['too_shallow'],
      reflection: 'Too shallow for where I am now.',
      expected: 'A substantive treatment.',
      actual: 'A surface overview.',
      effort: 'light',
      length_minutes: 12,
    }),
  })
  ...
  assert.equal(
    declinedRecord.body.notes[0].sections.find((section) => section.section_key === 'reaction').content,
    'Too shallow for where I am now.',
  )
  ...
  const declinedOutcome = parseJson(
    await query(`SELECT rejection_reason FROM recommendation_outcomes WHERE recommendation_id='rec_decline'`),
  ).results[0]
  assert.equal(declinedOutcome.rejection_reason, 'too_shallow')
  const declinedFeedback = parseJson(
    await query(`SELECT exposure_json FROM compass_feedback WHERE pick_id='pick_decline'`),
  ).results[0]
  ...
  `)
  const completed = await request('/compass/pick/pick_complete/feedback', {
    method: 'POST',
    body: JSON.stringify({ outcome: 'completed', score: 9, reflection: 'Useful, practical, and worth revisiting.' }),
  })
  ...
  `)
  const badFitWithoutReason = await request('/compass/pick/pick_dismiss/feedback', {
    method: 'POST',
  ...
  const dismissed = await request('/compass/pick/pick_dismiss/feedback', {
    method: 'POST',
    body: JSON.stringify({ outcome: 'dismissed', reason_tags: ['not_now'] }),
  })
  ...
  const contextBrief =
    'What it is: a primary-source research note.\n• Covers the method, evidence, and practical implication.\n• Expect a focused technical overview.'
  const compassThread = await request('/learning/core/threads', {
    method: 'POST',
    body: JSON.stringify({
      title: 'Evaluate the Compass mechanism',
      thread_type: 'understand',
      guiding_question: 'Which source best explains the mechanism?',
      definition_of_done: 'Explain the mechanism with anchored evidence.',
      activate: true,
    }),
  })
  ...
  const submitted = await request('/compass/picks', {
    ...
    body: JSON.stringify({
      ...
      candidates: [
        {
          canonical_url: 'https://example.com/?context-brief=winner',
          title: 'Context brief primary research',
          ...
          evidence: [
            {
              claim: 'The original research explains its method, evidence, limitations, and practical implication.',
              source_url: 'https://example.com/?context-brief=winner',
            },
          ],
          editorial_review: {
            verdict: 'recommend',
            why_worth_time: 'It directly explains the mechanism with evidence and limitations.',
            unique_value: 'It connects the mechanism to a practical learning decision.',
            depth: 'substantive',
          },
          context_brief: contextBrief,
        },
        {
          canonical_url: 'https://example.com/?context-brief=alternate-one',
          title: 'Context brief alternate one',
          creator: 'Writer One',
          format: 'article',
          source_class: 'blog',
          branch_id: 'compass-branch',
          evidence: [
            {
              claim: 'This source provides a second explanation of the mechanism and its limits.',
              source_url: 'https://example.com/?context-brief=alternate-one',
            },
          ],
          editorial_review: {
            verdict: 'recommend',
            why_worth_time: 'It offers a useful second explanation of the mechanism.',
            unique_value: 'It provides a contrasting practical example for comparison.',
            depth: 'substantive',
          },
        },
        {
          canonical_url: 'https://example.com/?context-brief=alternate-two',
          title: 'Context brief alternate two',
          creator: 'Writer Two',
          format: 'article',
          source_class: 'blog',
          branch_id: 'compass-branch',
          evidence: [
            {
              claim: 'This source presents a distinct account of the mechanism and supporting evidence.',
              source_url: 'https://example.com/?context-brief=alternate-two',
            },
          ],
          editorial_review: {
            verdict: 'recommend',
            why_worth_time: 'It presents a distinct account worth comparing.',
            unique_value: 'It adds another evidence-backed angle to the thread.',
            depth: 'substantive',
          },
        },
      ],
  ...
  const submittedQueueItem = submittedQueue.body.items.find(
    (item) => item.id === startedSubmitted.body.recommendation_id,
  )
  assert.equal(submittedQueueItem.context_brief, contextBrief)
  assert.equal(submittedQueueItem.branch.id, 'compass-branch')
  await request(`/compass/pick/${submittedPick.body.pick.id}/feedback`, {
    method: 'POST',
    body: JSON.stringify({ outcome: 'declined', reason_tags: ['wrong_topic'] }),
  })
  ...
  assert.equal(
    weak.body.pick.context_brief,
    'What it is: a compact source review.\n• Covers the core argument and supporting evidence.\n• Expect a cautious, practical overview.',
  )
  ...
  assert.equal(
    acceptedWeakRecord.body.item.context_brief,
    'What it is: a compact source review.\n• Covers the core argument and supporting evidence.\n• Expect a cautious, practical overview.',
  )
  const queue = await request('/capture/queue')
  assert.equal(
    queue.body.items.find((item) => item.id === acceptedWeak.body.recommendation_id).context_brief,
    'What it is: a compact source review.\n• Covers the core argument and supporting evidence.\n• Expect a cautious, practical overview.',
  )
  assert.equal((await request('/compass/pick')).body.pick.status, 'started')

  const firstOrdinaryFeedback = await request('/feedback/record', {
    method: 'POST',
    body: JSON.stringify({
  ...
  const secondOrdinaryFeedback = await request('/feedback/record', {
    method: 'POST',
    body: JSON.stringify({
  ...
  const ordinaryEvents = ordinaryFeedbackContext.body.feedback_events.filter(
    (event) => event.recommendation_id === firstOrdinaryFeedback.body.source.id && event.source !== 'compass_pick',
  )
  assert.equal(ordinaryEvents.length, 2)
  assert.equal(
    ordinaryEvents.find((event) => event.structured.completion_state === 'in_progress').feedback,
    'I need to continue this when I have a longer block.',
  )
  assert.deepEqual(ordinaryEvents.find((event) => event.structured.completion_state === 'in_progress').reason_tags, [
    'not_now',
  ])
  assert.equal(
    ordinaryEvents.find((event) => event.structured.completion_state === 'completed').structured.length_minutes,
    31,
  )
