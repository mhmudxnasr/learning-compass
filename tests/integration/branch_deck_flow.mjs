...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    try {
      if ((await fetch('http://127.0.0.1:8794/health/live')).ok) break
    } catch {}
  ...
  const request = async (path, options = {}) => {
    const response = await fetch(`http://127.0.0.1:8794${path}`, {
  ...
  assert.ok(
    deck.body.categories.some((item) => item.id === 'cat-mind'),
    'deck returns the canonical category index',
  )
  assert.ok(
    !deck.body.existing.some((item) => item.id === 'practical-ai'),
    'removed paused AI branch does not remain in the top-level registry',
  )
  assert.ok(
    !deck.body.existing.some((item) => item.status === 'held'),
    'the rebuilt personal registry starts with no paused branches',
  )
  ...
  const pruned = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({ id: BRANCH, action: 'prune', label: 'Deep Work', super_category: 'cat-mind' }),
  })
  ...
  let explored = parseJson(
    await query(`SELECT is_pruned, lifecycle_state FROM branch_exploration WHERE id='${BRANCH}'`),
  ).results[0]
  ...
  let assertion = parseJson(
    await query(`SELECT category, status, confidence FROM profile_assertions WHERE assertion_key='${ASSERTION}'`),
  ).results[0]
  ...
  const appliedProposals = parseJson(
    await query(`SELECT COUNT(*) c FROM feedback_proposals WHERE target_label LIKE '%${BRANCH}%' AND status='applied'`),
  ).results[0]
  ...
  const thread = await request('/learning/core/threads', {
    method: 'POST',
    body: JSON.stringify({
      title: 'Branch deck compass effect',
      thread_type: 'understand',
      guiding_question: 'Which branch should drive the next exploration?',
      definition_of_done: 'A branch decision steers Compass.',
      activate: true,
    }),
  })
  ...
  assert.ok(
    exclusions.some((term) => String(term).includes(BRANCH) || String(term).includes('deep work')),
    'pruned branch must block future recommendations',
  )

  // 4. Priority is one explicit renumbered rank, and love status.
  const priorityOrderBeforePromotion = parseJson(
    await query('SELECT branch_id,rank FROM priorities ORDER BY rank'),
  ).results
  const promoted = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({ id: BRANCH, action: 'priority', label: 'Deep Work', super_category: 'cat-mind' }),
  })
  ...
  assertion = parseJson(await query(`SELECT category FROM profile_assertions WHERE assertion_key='${ASSERTION}'`))
    .results[0]
  assert.equal(assertion.category, 'priority')

  // 5. Undo reverses the side effects, not just the tree row.
  const undone = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({
      id: BRANCH,
      action: 'undo',
      label: 'Deep Work',
      restore_status: 'pruned',
      restore_priority_rank: null,
      restore_action: 'priority',
    }),
  })
  ...
  const prioritiesAfterUndo = parseJson(await query(`SELECT COUNT(*) c FROM priorities WHERE branch_id='${BRANCH}'`))
    .results[0]
  ...
  assert.deepEqual(
    priorityOrderAfterUndo,
    priorityOrderBeforePromotion,
    'undo restores the complete priority order without gaps',
  )
  ...
  assertion = parseJson(await query(`SELECT status FROM profile_assertions WHERE assertion_key='${ASSERTION}'`))
    .results[0]
  ...
  const prunedPriority = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({ id: 'taz', action: 'prune', label: 'Tazkiyah & Character', super_category: 'cat-faith' }),
  })
  ...
  assert.deepEqual(
    compactPriorityOrder.map((item) => item.rank),
    [1, 2, 3, 4],
    'archiving a priority closes its rank gap',
  )
  const restoredPriority = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({
      id: 'taz',
      action: 'undo',
      label: 'Tazkiyah & Character',
      restore_status: 'active',
      restore_priority_rank: 1,
      restore_action: 'prune',
    }),
  })
  ...
  assert.deepEqual(
    restoredPriorityOrder,
    seededPriorityOrder,
    'restoring an archived priority restores its exact rank order',
  )

  // 6. Undo of an add removes the branch entirely (it never existed before).
  await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({
      id: 'r1-test-add',
      action: 'add',
      ...
      parent_id: 'cat-mind',
      description: 'Temp branch for undo test',
      leaves_sample: ['a', 'b'],
    }),
  })
  node = parseJson(await query(`SELECT status FROM tree_nodes WHERE id='r1-test-add'`)).results[0]
  assert.equal(node.status, 'active')
  const signalBeforeUpdate =
    parseJson(await query('SELECT recent_signal FROM profile WHERE id=1')).results[0]?.recent_signal ?? null
  const updated = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({
      id: 'r1-test-add',
      action: 'update',
      label: 'Test Add Updated',
      super_category: 'cat-life',
      parent_id: 'cat-life',
      description: 'Updated personal scope',
      leaves_sample: ['one', 'two'],
      contrast_hook: 'Not a test category.',
    }),
  })
  assert.equal(updated.status, 200, JSON.stringify(updated.body))
  assert.equal(updated.body.affinity_score, null, 'editing metadata must not write a taste signal')
  const updatedNode = parseJson(
    await query(
      `SELECT label,parent_id,status,json_extract(meta_json,'$.description') description FROM tree_nodes WHERE id='r1-test-add'`,
    ),
  ).results[0]
  ...
  const partialUpdate = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({ id: 'r1-test-add', action: 'update', label: 'Test Add Renamed' }),
  })
  assert.equal(partialUpdate.status, 200, JSON.stringify(partialUpdate.body))
  const partialNode = parseJson(
    await query(
      `SELECT label,parent_id,json_extract(meta_json,'$.description') description FROM tree_nodes WHERE id='r1-test-add'`,
    ),
  ).results[0]
  assert.deepEqual(
    partialNode,
    { label: 'Test Add Renamed', parent_id: 'cat-life', description: 'Updated personal scope' },
    'partial edits preserve omitted category and metadata',
  )
  const signalAfterUpdate =
    parseJson(await query('SELECT recent_signal FROM profile WHERE id=1')).results[0]?.recent_signal ?? null
  assert.equal(
    signalAfterUpdate,
    signalBeforeUpdate,
    'editing metadata must not change the recommendation profile signal',
  )
  const undidAdd = await request('/brain/branch-swipe', {
    method: 'POST',
    body: JSON.stringify({
      id: 'r1-test-add',
      action: 'undo',
  ...
  const suggest = await request('/brain/branch-suggest', {
    method: 'POST',
    body: JSON.stringify({ mode: 'surprise', count: 3 }),
  })
  ...
  for (const item of suggest.body.suggestions) {
    ...
    assert.ok(
      Object.prototype.hasOwnProperty.call(item, 'plain_language'),
      'each suggestion carries a plain-language brief',
    )
    assert.ok(
      ['low', 'medium', 'high'].includes(item.evidence_confidence),
      'each suggestion carries bounded evidence confidence',
    )
    assert.ok(Array.isArray(item.overlap_candidates), 'each suggestion carries overlap candidates')
    assert.ok(
      Object.prototype.hasOwnProperty.call(item, 'suggested_next_move'),
      'each suggestion carries a cautious next move',
    )
  ...
  console.log(
    `Branch deck flow passed (deck evidence, prune exclusion, priority renumber, undo reversal, undo-of-add, review-before-commit suggest).`,
  )
} finally {
