...
export async function verifyReadingRefinements({ page: suitePage, baseUrl, requestJson, bookId }) {
  ...
  try {
    ...
    for (const [name, width, height] of [
      ...
      assert.equal(await page.getByRole('button', { name: 'Browse visible list (2)', exact: true }).count(), 1)
      ...
      await page.getByRole('button', { name: 'Map settings', exact: true }).click()
      assert.equal(await page.getByRole('button', { name: 'Browse visible list (6)', exact: true }).count(), 1)
      await page.getByRole('button', { name: 'Close controls', exact: true }).click()
