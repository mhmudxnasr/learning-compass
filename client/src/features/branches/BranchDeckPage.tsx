...
function branchNeedsAttention(branch: DeckBranch): boolean {
  ...
  if (branch.balance_state === 'at-risk' || branch.balance_state === 'uncovered' || branch.balance_state === 'cooling')
    return true
...
export function BranchDeckPage({
  ...
  const [history, setHistory] = useState<
    Array<{
      ...
      action: 'keep' | 'hold' | 'priority' | 'prune' | 'add'
    }>
  >([])
    ...
    async (preferredId?: string) => {
      ...
      try {
        ...
        const balanceById = new Map(
          (Array.isArray(balance.branches) ? balance.branches : []).map((branch: any) => [String(branch.id), branch]),
        )
        ...
        setSelectedId((current) => {
          ...
          return (
            nextBranches.find((branch: DeckBranch) => branch.status !== 'pruned')?.id || nextBranches[0]?.id || null
          )
        })
        if (nextCategories.length)
          setForm((current) => (current.category ? current : { ...current, category: nextCategories[0].id }))
      } catch (err: any) {
  ...
  const counts = useMemo(
    () => ({
      ...
      priorities: branches.filter((branch) => branch.status !== 'pruned' && Number(branch.priority_rank || 0) > 0)
        .length,
  ...
  const filteredBranches = useMemo(
    () =>
      branches.filter((branch) => {
        ...
        if (statusFilter === 'priorities' && (branch.status === 'pruned' || Number(branch.priority_rank || 0) < 1))
          return false
        ...
        if (searchQuery.trim()) {
          ...
          const haystack = [
            branch.label,
            branch.description,
            branch.category_label,
            branch.contrast_hook,
            ...(branch.leaves_sample || []),
          ]
            .join(' ')
            .toLowerCase()
  ...
  useEffect(() => {
    if (loading) return
    if (!selectedId || !filteredBranches.some((branch) => branch.id === selectedId))
      setSelectedId(filteredBranches[0]?.id || null)
  ...
  useEffect(() => {
    if (initialSelectedId && branches.some((branch) => branch.id === initialSelectedId))
  ...
  const changeStatus = async (branch: DeckBranch, action: 'keep' | 'hold' | 'priority' | 'prune') => {
    ...
    try {
      ...
      setHistory((current) => [
        ...current,
        { id: branch.id, label: branch.label, status: branch.status, priority: branch.priority_rank ?? null, action },
      ])
      flash(
        action === 'priority'
          ? `${branch.label} is now your first priority.`
          : action === 'hold'
            ? `${branch.label} is paused.`
            : action === 'prune'
              ? `${branch.label} was archived.`
              : `${branch.label} is active again.`,
      )
    ...
    try {
      await api('/brain/branch-swipe', {
        ...
        body: JSON.stringify({
          ...
          action: 'undo',
          label: last.label,
          restore_status: last.status,
          restore_priority_rank: last.priority,
          restore_action: last.action,
        }),
      })
    ...
    try {
      await api('/brain/branch-swipe', {
        ...
        body: JSON.stringify({
          ...
          super_category: form.category,
          parent_id: form.category,
          description: form.description.trim(),
          leaves_sample: form.topics
            .split(',')
            .map((topic) => topic.trim())
            .filter(Boolean),
          contrast_hook: form.boundary.trim(),
        }),
      })
  ...
  const saveBranch = async (event: Event) => {
    ...
    try {
      await api('/brain/branch-swipe', {
        ...
        body: JSON.stringify({
          id,
          action: dialogMode === 'edit' ? 'update' : 'add',
          label: form.label.trim(),
          super_category: form.category,
          parent_id: form.category,
          description: form.description.trim(),
      ...
      if (!edited)
        setHistory((current) => [
          ...current,
          { id, label: form.label.trim(), status: 'candidate', priority: null, action: 'add' },
        ])
      setDialogMode(null)
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (dialogMode && event.key === 'Escape') {
        setDialogMode(null)
        return
      }
      ...
      if (event.key === 'ArrowDown' && index < filteredBranches.length - 1) {
        event.preventDefault()
        setSelectedId(filteredBranches[index + 1].id)
      }
    ...
    <div class="folio-branch-review branch-manager branch-desk" aria-label="Knowledge branches">
      <header class="folio-surface-head folio-branch-head">
        <div>
          <h1>Branch review</h1>
          <p class="folio-lede">
            Decide what deserves attention, then update branch priority, status, scope, and boundaries without leaving
            the dossier.
          </p>
        </div>
        <div class="folio-branch-head-actions">
          ...
          {history.length > 0 && (
            <button class="button secondary" type="button" onClick={undoLast} disabled={saving}>
              Undo last change
            </button>
          )}
          <button class="button primary folio-primary" type="button" onClick={openCreate}>
            New branch
          </button>
        </div>
      </header>
      ...
      <nav class="folio-branch-metrics-bar" aria-label="Filter branches by status">
        {(
          ...
          <button
            ...
            class={`folio-branch-metric-tab ${value === 'priorities' ? 'tab-priority' : value === 'archived' ? 'tab-pruned' : ''} ${statusFilter === value ? 'is-active' : ''}`}
            ...
            <div class="branch-index-tools">
              <label class="branch-area-filter">
                ...
                <select
                  ...
                  onChange={(event) => setCategoryFilter((event.target as HTMLSelectElement).value)}
                  aria-label="Filter branches by area"
                >
              ...
              <span class="folio-branch-list-meta">
          ...
          <div class="folio-branch-list-scroll">
            {loading ? (
              ...
              <div class="folio-branch-list-empty is-error">
                <p>{error}</p>
                <button class="button secondary" type="button" onClick={() => fetchDeck()}>
                  Retry
                </button>
              </div>
            ) : filteredBranches.length === 0 ? (
              <div class="folio-branch-list-empty">
                <strong>{branches.length ? 'No branches match these filters' : 'Your branch map is empty'}</strong>
                <p>
                  {branches.length
                    ? 'Clear a filter or search term.'
                    : 'Create a broad subject that you want Compass to organize around.'}
                </p>
                {branches.length ? (
                  <button
                ...
                {filteredBranches.map((branch) => {
                  ...
                  return (
                    <li key={branch.id}>
                      <button
                        type="button"
                        class={`folio-branch-item ${selectedId === branch.id ? 'is-selected' : ''}`}
                        onClick={() => {
                      ...
                      >
                        <div class="branch-item-top">
                          <span class="branch-item-category">{branch.category_label || branch.super_category}</span>
                          {badge.label !== 'Active' && (
                        ...
                        <div class="branch-item-footer">
                          <span>
                            {branch.mapped_count || 0} {(branch.mapped_count || 0) === 1 ? 'source' : 'sources'}
                          </span>
                          <span class={`branch-frontier-inline frontier-${branch.frontier_state || 'unknown'}`}>
                            {frontierLabel(branch.frontier_state)}
                          </span>
                        </div>
                      </button>
                    </li>
        ...
        <section class="folio-branch-detail-panel branch-dossier-main" aria-label="Branch details">
          {!selected ? (
            <div class="folio-branch-empty-inspector">
              ...
              <p>Choose a branch to see its scope, topics, and everything filed under it.</p>
            </div>
          ) : (
            <article class="folio-branch-card">
              <div class="branch-card-header">
                <div class="branch-card-tags">
                  <span class="branch-tag-category">{selected.category_label || selected.super_category}</span>
                  <span class={`branch-tag-status ${statusLabel(selected).cls}`}>{statusLabel(selected).label}</span>
                </div>
              </div>
              <div class="branch-title-block">
                <h2 class="branch-card-title">{selected.label}</h2>
                <p class="branch-card-description">
                  {selected.description || 'Add a clear purpose so Compass knows what belongs here.'}
                </p>
              </div>
              <section class="branch-detail-section">
                <span class="section-kicker">Topics inside this branch</span>
                {selected.leaves_sample?.length ? (
                  <div class="branch-leaves-grid">
                    {selected.leaves_sample.map((topic) => (
                      <span class="branch-leaf-pill" key={topic}>
                        {topic}
                      </span>
                    ))}
                  </div>
                ) : (
                  <p class="branch-ledger-empty">No starter topics yet. Edit the branch to add them.</p>
                )}
              </section>
              ...
              <form class="branch-inline-editor branch-detail-section" onSubmit={saveSelected}>
                <div class="section-head">
                  <div>
                    <span class="section-kicker">Branch definition</span>
                    <p class="branch-status-note">
                      Edit the stable identity Compass uses when filing and recommending material.
                    </p>
                  </div>
                  <button
                    class="button primary"
                    type="submit"
                    disabled={saving || !form.label.trim() || !form.category}
                  >
                ...
                <div class="branch-inline-fields">
                  <label class="folio-form-field">
                    ...
                    <input
                      ...
                      maxLength={120}
                      required
                      onInput={(event) => setForm({ ...form, label: (event.target as HTMLInputElement).value })}
                    />
                  </label>
                  <label class="folio-form-field">
                    <span>Area</span>
                    <select
                      value={form.category}
                      required
                      onChange={(event) => setForm({ ...form, category: (event.target as HTMLSelectElement).value })}
                    >
                      {categories.map((category) => (
                        <option value={category.id} key={category.id}>
                          {category.label}
                  ...
                  <label class="folio-form-field field-wide">
                    ...
                    <textarea
                      ...
                      maxLength={1000}
                      onInput={(event) =>
                        setForm({ ...form, description: (event.target as HTMLTextAreaElement).value })
                      }
                    />
                  </label>
                  <label class="folio-form-field field-wide">
                    <span>
                      Starter topics <small>comma-separated</small>
                    </span>
                    <input
                      type="text"
                      value={form.topics}
                      maxLength={800}
                      onInput={(event) => setForm({ ...form, topics: (event.target as HTMLInputElement).value })}
                    />
                  </label>
                  <label class="folio-form-field field-wide">
                    <span>Boundary</span>
                    <input
                      ...
                      value={form.boundary}
                      maxLength={500}
                      onInput={(event) => setForm({ ...form, boundary: (event.target as HTMLInputElement).value })}
                    />
                  </label>
                </div>
              </form>
              ...
              <section class="branch-detail-section branch-ledger" aria-label="Items in this branch">
                ...
                {ledgerLoading ? (
                  <p class="branch-ledger-empty">Loading branch contents…</p>
                ) : !ledger ? (
                  <p class="branch-ledger-empty">Branch contents are unavailable.</p>
                ) : ledger.recommendations.length +
                    ledger.notes.length +
                    ledger.recall_cards.length +
                    ledger.srs_drafts.length +
                    ledger.artifacts.length +
                    ledger.bridges.length ===
                  0 ? (
                  <p class="branch-ledger-empty">
                    Nothing is filed here yet. New sources can use this branch immediately.
                  </p>
                ) : (
                  <div class="branch-ledger-body">
                    {ledger.recommendations.length > 0 && (
                      <div class="branch-ledger-group">
                        <h4>
                          Sources <span>{ledger.recommendations.length}</span>
                        </h4>
                        <ul class="branch-ledger-list">
                          {ledger.recommendations.map((item) => (
                            <li class="branch-ledger-row" key={item.id}>
                              <a
                                class="branch-ledger-title"
                                href={`#/library/source/${encodeURIComponent(String(item.id))}`}
                              >
                                <strong>{item.video_title || 'Untitled source'}</strong>
                                <span class="folio-branch-pill">
                                  {selected.label} · {selected.super_category}
                                </span>
                                <span class="branch-ledger-sub">
                                  {item.creator || 'Independent source'} ·{' '}
                                  {formatStatus(item.learning_state || item.status)}
                                  {item.consumed_date ? ` · ${formatDate(item.consumed_date)}` : ''}
                                </span>
                              </a>
                            </li>
                          ))}
                    ...
                    {ledger.notes.length > 0 && (
                      <div class="branch-ledger-group">
                        <h4>
                          Notes <span>{ledger.notes.length}</span>
                        </h4>
                        <ul class="branch-ledger-list">
                          {ledger.notes.map((note) => (
                            <li class="branch-ledger-row" key={note.id}>
                              <a
                                class="branch-ledger-title"
                                href={`#/learn/note/${encodeURIComponent(String(note.id))}`}
                              >
                    ...
                    {(ledger.recall_cards.length > 0 || ledger.srs_drafts.length > 0) && (
                      <div class="branch-ledger-group">
                        <h4>
                          Recall{' '}
                          <span>
                            {ledger.recall_cards.length} active · {ledger.srs_drafts.length} drafts
                          </span>
                        </h4>
                      </div>
                    )}
                    {ledger.artifacts.length > 0 && (
                      <div class="branch-ledger-group">
                        <h4>
                          Reading companions and files <span>{ledger.artifacts.length}</span>
                        </h4>
                      </div>
                    )}
                    {ledger.bridges.length > 0 && (
                      <div class="branch-ledger-group branch-bridges">
                        <h4>
                          Cross-branch bridges <span>{ledger.bridges.length}</span>
                        </h4>
                        <ul class="branch-ledger-list">
                          {ledger.bridges.map((bridge) => (
                            <li class="branch-ledger-row" key={bridge.id}>
                              <div class="branch-ledger-title">
                                <div class="bridge-endpoints">
                                  <a href={`#/learn/unit/${encodeURIComponent(bridge.source.unit_id)}`}>
                                    {bridge.source.statement}
                                  </a>
                                  <span>{bridge.relation_type.replace(/_/g, ' ')}</span>
                                  <a href={`#/learn/unit/${encodeURIComponent(bridge.target.unit_id)}`}>
                                    {bridge.target.statement}
                                  </a>
                                </div>
                                <div>
                                  <span class="folio-branch-pill">
                                    {bridge.source.branch.label} · {bridge.source.branch.domain}
                                  </span>
                                  <span class="folio-branch-pill">
                                    {bridge.target.branch.label} · {bridge.target.branch.domain}
                                  </span>
                                </div>
                                <small>{bridge.why}</small>
                              </div>
                            </li>
                          ))}
                        </ul>
        ...
        <aside class="branch-dossier-rail" aria-label="Branch signals and decision">
          {!selected ? (
            <div class="folio-branch-empty-inspector">
              ...
              <p>Select a branch to review its signals and status.</p>
            </div>
          ) : (
            <>
              <div class="branch-rail-head">
                <strong>Signals and decision</strong>
              ...
              <section class="branch-rail-decision">
                ...
                <p class="branch-status-note">
                  ...
                  {selected.priority_rank
                    ? `priority ${selected.priority_rank}`
                    : statusLabel(selected).label.toLowerCase()}
                  .
                </p>
                <div class="branch-rail-actions">
                  {selected.status === 'pruned' ? (
                    <button
                      class="decision-btn btn-keep"
                      type="button"
                      disabled={saving}
                      onClick={() => changeStatus(selected, 'keep')}
                    >
                      Restore branch
                    </button>
                  ) : (
                    <>
                      <button
                        class={`decision-btn btn-keep ${selected.status !== 'held' ? 'is-current' : ''}`}
                        type="button"
                        disabled={saving}
                        onClick={() => changeStatus(selected, 'keep')}
                      >
                      ...
                      <button
                        class={`decision-btn btn-priority ${Number(selected.priority_rank || 0) === 1 ? 'is-current' : ''}`}
                        type="button"
                        disabled={saving}
                        onClick={() => changeStatus(selected, 'priority')}
                      >
                        Make first priority
                      </button>
                      <button
                        class={`decision-btn btn-hold ${selected.status === 'held' ? 'is-current' : ''}`}
                        type="button"
                        disabled={saving}
                        onClick={() => changeStatus(selected, 'hold')}
                      >
                        Pause branch
                      </button>
                      <button
                        ...
                        type="button"
                        disabled={saving}
                        onClick={() => changeStatus(selected, 'prune')}
                      >
                        Archive branch
                      </button>
                    </>
                ...
                <p class="branch-status-note">
                  Pausing removes the branch from active attention. Archiving also blocks future recommendations until
                  restored.
                </p>
              </section>
              <section class="branch-rail-signal">
                <span>Attention</span>
                <strong>{Number(selected.attention_share || 0).toFixed(1)}%</strong>
                <div class="branch-rail-meter">
                  <i style={{ width: `${Math.min(100, Number(selected.attention_share || 0) * 6)}%` }} />
                </div>
                <small>
                  {selected.last_consumed_at
                    ? `Last completed ${formatDate(selected.last_consumed_at)}`
                    : 'No completed source in this window'}
                </small>
              </section>
              <section class="branch-rail-signal">
                <span>Filed here</span>
                <strong>{selected.mapped_count || 0} sources</strong>
                <small>
                  {selected.notes_count || 0} notes · {selected.learning_units || 0} units · {selected.srs_total || 0}{' '}
                  recall cards
                </small>
              </section>
              <section
                class={`branch-rail-signal branch-frontier-signal frontier-${selected.frontier_state || 'unknown'}`}
              >
                <span>Knowledge frontier</span>
                <strong>{frontierLabel(selected.frontier_state)}</strong>
                <small>
                  {selected.frontier_reasons?.join(' · ') || 'Not enough linked evidence to assess this branch.'}
                </small>
                <small>
                  {selected.lifetime_consumed_count || 0} lifetime sources · {selected.accepted_units_count || 0}{' '}
                  accepted units · {selected.completed_lessons_count || 0} direct lessons
                </small>
              </section>
              <section class="branch-rail-signal">
                <span>Priority alignment</span>
                <strong>{branchNeedsAttention(selected) ? 'Needs a decision' : 'In rhythm'}</strong>
                <small>
                  {selected.balance_reasons?.[0] ||
                    (selected.priority_rank
                      ? `Priority ${selected.priority_rank} with ${Number(selected.attention_share || 0).toFixed(1)}% recent attention.`
                      : 'No priority mismatch is recorded.')}
                </small>
              </section>
              <a
                class="button secondary branch-rail-atlas"
                href={`#/map/node/${encodeURIComponent(selected.id)}?mode=atlas`}
              >
                Open in Atlas
              </a>
            </>
      ...
      {dialogMode && (
        ...
        >
          <div
            ref={dialogRef}
            class="folio-modal-card branch-editor-dialog"
            role="dialog"
            aria-modal="true"
          ...
          >
            <header class="folio-modal-header">
              <div>
                <p class="folio-object-kicker">{dialogMode === 'edit' ? 'Branch details' : 'Personal map'}</p>
                <h3 id="branch-editor-title">{dialogMode === 'edit' ? 'Edit branch' : 'Create a knowledge branch'}</h3>
              </div>
              <button
                type="button"
            ...
            <form class="folio-modal-form" onSubmit={saveBranch}>
              <label class="folio-form-field">
                ...
                <input
                  ...
                  onInput={(event) => setForm({ ...form, label: (event.target as HTMLInputElement).value })}
                />
                <small>Use a broad subject you expect to revisit. Put narrower ideas in Topics.</small>
              </label>
              <label class="folio-form-field">
                <span>Area</span>
                <select
              ...
              <label class="folio-form-field">
                ...
                <textarea
                  ...
                  rows={4}
                  maxLength={1000}
                  placeholder="What belongs here, and what should studying it help you understand or do?"
              ...
              <label class="folio-form-field">
                ...
                <input
                  ...
                  value={form.topics}
                  maxLength={800}
                  placeholder="feedback loops, system boundaries, leverage points"
                  onInput={(event) => setForm({ ...form, topics: (event.target as HTMLInputElement).value })}
                />
              </label>
              <label class="folio-form-field">
                ...
                <input
                  ...
                  value={form.boundary}
                  maxLength={500}
                  placeholder="What looks related but does not belong in this branch?"
                  onInput={(event) => setForm({ ...form, boundary: (event.target as HTMLInputElement).value })}
                />
              </label>
              <footer class="folio-modal-footer">
                <button class="button secondary" type="button" onClick={() => setDialogMode(null)}>
                  Cancel
                </button>
                <button
                  ...
                  disabled={saving || !form.label.trim() || !form.category}
                >
                  {saving ? 'Saving…' : dialogMode === 'edit' ? 'Save changes' : 'Create branch'}
                </button>
              </footer>
            </form>
          </div>
