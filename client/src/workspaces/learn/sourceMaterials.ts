...
function artifactDetails(artifact: PathArtifact, kind: 'html' | 'pdf'): string[] {
  ...
  const language = String(metadata.language || metadata.lang || '').trim().toLowerCase()
  ...
  if (metadata.workflow_contract === 'lite-visual-linear/v4' && metadata.validation_status === 'passed') details.push('Verified pair')
  return details
}
...
const notebookFormatLabel = (format?: string | null) => {
  const normalized = String(format || '').trim().replaceAll('-', ' ')
...
function notebookMaterialState(source: PathSource) {
  ...
  return {
    ...
    purpose: learning.index_status === 'pending'
...
function sourceMaterialOptions(source: PathSource): SourceMaterialOption[] {
  ...
  if (source.video_url) options.push({
    kind: 'original',
    format: 'Original',
  ...
  if (source.artifacts?.html) options.push({
    kind: 'html',
    format: 'HTML',
  ...
  if (source.artifacts?.pdf) options.push({
    kind: 'pdf',
    format: 'PDF',
...
export function buildSourceMaterialLauncher(source: PathSource): SourceMaterialLauncher | null {
  ...
  const canLead = (option: SourceMaterialOption) => option.kind !== 'notebooklm'
    || option.availability === 'Ready'
    || option.availability === 'Ready to ask'
  const primary = (declared && options.find((option) => option.kind === declared && canLead(option)))
