...
export function OperationalHealthPanel() {
  ...
  if (system.loading || engine.loading) return <section class="operational-health"><Loading label="Checking operational health" /></section>
  const error = system.error || engine.error
  if (error) return <section class="operational-health"><ErrorState message={error} retry={() => { system.reload(); engine.reload() }} /></section>
  ...
  return <section class="operational-health" aria-labelledby="operational-health-title">
    <div class="section-head"><div><span class="eyebrow">Reliability / recovery</span><h2 id="operational-health-title">Operational health</h2></div><span class={healthy ? 'status healthy' : 'status warning'}>{healthy ? 'Healthy' : 'Needs attention'}</span></div>
    <div class="operational-health-metrics">
      ...
      {metric('Integrity defects', Number(health?.integrity?.active_orphans || 0) + Number(health?.integrity?.quarantined_unresolved || 0), !health?.integrity || !health.ok && Number(health?.integrity?.active_orphans || 0) + Number(health?.integrity?.quarantined_unresolved || 0) > 0)}
    ...
    {((health?.blockers?.length || 0) > 0 || failedGates.length > 0) && <div class="operational-health-notes">
      {(health?.blockers || []).map((blocker) => <p key={blocker}><strong>Operations:</strong> {blocker}</p>)}
