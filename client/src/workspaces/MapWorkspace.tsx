...
const mapModes: Array<{ key: MapMode; label: string; description: string; view: MapView }> = [
  ...
  { key: 'review', label: 'Review', description: 'Decide branch status, priority, scope, and attention', view: 'branches' },
]

function MapModeSwitcher({ active, onRouteChange }: { active: MapMode; onRouteChange?: (route: MapWorkspaceRoute) => void }) {
  return <nav class="workspace-mode-switcher workspace-local-nav map-local-nav" aria-label="Map sections">
    {mapModes.map((item) => (
      <a
        key={item.key}
...
export function MapWorkspace({ route, view, onRouteChange }: MapWorkspaceProps) {
  ...
  const activeMode: MapMode = route?.objectType === 'branch' || requestedMode === 'review' || ['branches', 'balance', 'coverage', 'deck'].includes(String(requestedView)) ? 'review' : 'atlas'
      ...
      {activeMode === 'atlas' ? (
        <Suspense fallback={<Loading label="Preparing spatial atlas" />}>
          <AtlasPage
            ...
            onSelect={(nodeId) => onRouteChange?.({ view: 'atlas', mode: 'atlas', objectType: nodeId ? 'node' : undefined, objectId: nodeId || undefined })}
          />
        </Suspense>
      ) : (
        <Suspense fallback={<Loading label="Opening branch review" />}>
          <BranchDeckPage
            ...
            onSelect={(branchId) => onRouteChange?.({ view: 'branches', mode: 'review', objectType: branchId ? 'branch' : undefined, objectId: branchId || undefined })}
          />
        </Suspense>
      )}
    </div>
