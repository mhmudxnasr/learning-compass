...
function PreferencesView() {
  ...
  const copyThemePrompt = async () => {
    ...
    const effectiveCustomFont = normalizeCustomFont(customFont)
    const prompt = `Act as an award-winning visual designer for Learning Compass, a premium adaptive knowledge console. Create a fresh, genuinely distinctive theme from this brief: ${paletteBrief || 'no colors selected yet'}. Every run must invent a different creative direction and independently choose either DAY or NIGHT mode. Explore unusual but coherent combinations: graphite, editorial paper, mineral, ink, astronomical, lacquer, desert, coastal, or another strong art direction. Keep the palette intentional, restrained, and production-ready; no muddy near-duplicates, accidental brown swaps, generic SaaS blue, gradients, or neon unless the concept truly requires it. Preserve WCAG AA contrast (4.5:1 minimum) for ink on shell/surface and make rail, seams, due, danger, and map unmistakable.
