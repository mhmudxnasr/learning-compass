...
type SettingsPayload = {
  ...
  resolved?: {
    appearance?: {
      ...
      font_size?: DisplayPreferences['fontSize']
      reduced_motion?: boolean
      custom_palette?: CustomPalette
      font?: string
      custom_font?: { ui?: string; display?: string; reading?: string; mono?: string }
    ...
    delivery_context?: {
      ...
      language?: 'any' | 'en' | 'ar'
      delivery_modes?: Array<'read' | 'watch' | 'listen' | 'practice'>
      depth_tier?: 'adaptive' | 'introductory' | 'intermediate' | 'advanced'
    }
  }
}
...
const TYPOGRAPHY_CONTROLS: Array<{
  ...
  { key: 'baseSize', label: 'Base size', description: 'Interface and body text scale', step: 1, suffix: 'px' },
  {
    key: 'displayScale',
    label: 'Display scale',
    description: 'Heading size multiplier across the studio',
    step: 0.05,
    suffix: '×',
  },
  ...
  { key: 'headingWeight', label: 'Heading weight', description: 'Title and section authority', step: 50, suffix: '' },
  {
    key: 'lineHeight',
    label: 'Line height',
    description: 'Vertical breathing room and rhythm',
    step: 0.05,
    suffix: '',
  },
  {
    ...
    description: 'Tracking across interface text',
    step: 0.01,
    suffix: 'em',
  },
  {
    key: 'readingMeasure',
    label: 'Reading width',
    description: 'Comfortable maximum line length',
    step: 1,
    suffix: 'ch',
  },
]
...
const profileFields: ProfileField[] = [
  {
    key: 'identity',
    apiKey: 'identity',
    readKey: 'identity_json',
    label: 'Identity & context',
    description: 'Background and learning context.',
    structured: true,
  },
  {
    key: 'mega_priority',
    apiKey: 'mega_priority',
    readKey: 'mega_priority_json',
    label: 'Mega priority',
    description: 'The highest-level focus areas.',
    structured: true,
  },
  {
    key: 'core_filter',
    apiKey: 'core_filter',
    label: 'Core curation filter',
    description: 'Criteria required for new content.',
    structured: false,
  },
  {
    key: 'reaction_style_json',
    apiKey: 'reaction_style_json',
    label: 'Reaction style',
    description: 'How feedback should be interpreted.',
    structured: true,
  },
  {
    key: 'quality_rules_json',
    apiKey: 'quality_rules_json',
    label: 'Quality & verification',
    description: 'Source verification and content boundaries.',
    structured: true,
  },
  {
    key: 'operational_style_json',
    apiKey: 'operational_style_json',
    label: 'Operational style',
    description: 'How Hermes should work with you.',
    structured: true,
  },
  {
    key: 'patterns_summary_json',
    apiKey: 'patterns_summary_json',
    label: 'Pattern summary',
    description: 'Recurring learning patterns.',
    structured: true,
  },
  {
    key: 'recent_signal',
    apiKey: 'recent_signal',
    label: 'Recent signal',
    description: 'The latest approved learning signal.',
    structured: false,
  },
]
  ...
  return (
    <>
      <div class="settings-local-navigation">
            ...
            <a
              ...
              onClick={(event) => {
                ...
                onRouteChange({
                  view: item.view,
                  mode: item.key,
                  focus: item.key === 'personal' ? 'profile' : undefined,
                })
              }}
            >
        ...
        {active === 'personal' && (
          <nav class="workspace-filter-switcher workspace-local-nav" aria-label="Personal settings filters">
            {personalFilters.map((item) => (
              <a
                key={item.key}
                href={routeFor('personal', item.key)}
                class={focus === item.key ? 'active' : ''}
                aria-current={focus === item.key ? 'page' : undefined}
                onClick={(event) => {
                  if (!onRouteChange) return
                  event.preventDefault()
  ...
  if (Array.isArray(parsed))
    return (
      parsed
        ...
        .filter((item) => item !== 'Not recorded')
        .join(' · ') || 'Not recorded'
    )
  return (
    Object.entries(parsed as Record<string, unknown>)
      .map(([key, item]) => `${labelize(key)}: ${readableText(item)}`)
      .join(' · ') || 'Not recorded'
  )
}
  ...
  return (
    <details class="profile-panel profile-learned-panel">
      <summary>
        ...
        <span>{assertions.length} signals</span>
      </summary>
      <div class="profile-panel-content">
        <div class="profile-signal-toolbar">
          <label>
            Search learned signals
            <input
              aria-label="Search learned signals"
              value={query}
              onInput={(event) => setQuery((event.target as HTMLInputElement).value)}
              placeholder="Search preferences, topics, or boundaries"
            />
          </label>
          <label>
            Show
            <select
              aria-label="Filter learned signals"
              value={filter}
              onChange={(event) => setFilter((event.target as HTMLSelectElement).value)}
            >
        ...
        {filtered.length ? (
          <div class="profile-assertion-list">
            {filtered.slice(0, 24).map((assertion: ProfileRecord) => (
              <article key={assertion.id || assertion.assertion_key}>
                <div class="profile-assertion-head">
                  <div>
                    <span class="meta">
                      {labelize(assertion.category || 'profile')} · {labelize(assertion.source_kind || 'recorded')}
                    </span>
                    <strong>{assertionTitle(assertion)}</strong>
                  </div>
                  <span class={`state state-${assertion.status || 'active'}`}>
                ...
                <details class="profile-signal-details">
                  <summary>Advanced details</summary>
                  <p>{labelize(assertion.assertion_key || 'Profile signal')}</p>
                </details>
              </article>
            ))}
          </div>
        ) : (
          <Empty title="No signals match" body="Try a broader search or reset the filter." />
        )}
...
function ProfileRecordList({
  ...
  getDetail?: (item: any) => unknown
}) {
  return (
    <section class="profile-record-section">
      <div class="section-head">
        <h2>{title}</h2>
      ...
      {items.length ? (
        <div class="profile-record-list">
          {items.slice(0, 24).map((item, index) => (
            <article class="profile-record" key={String(item.id || item.label || item.name || index)}>
              <div class="profile-record-title">
                <strong>{getTitle(item)}</strong>
                <small>{getMeta(item)}</small>
              </div>
              <p>
                {readableText(
                  getDetail ? getDetail(item) : item.description || item.reason || item.why || item.notes || '',
                )}
              </p>
            </article>
          ))}
...
function ProfileEditor({ profile, onSaved }: { profile: ProfileRecord; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    profileFields.forEach((field) => {
      ...
      if (field.structured) {
        try {
          payload[field.apiKey] = JSON.parse(draft[field.key])
        } catch {
          payload[field.apiKey] = draft[field.key]
        }
    ...
    try {
      await api('/brain/profile', { method: 'POST', body: JSON.stringify(payload) })
      ...
      {open && (
        <>
            ...
            {profileFields.map((field) => (
              <label key={field.key}>
                ...
                <textarea
                  ...
                  onInput={(event) =>
                    setDraft((current) => ({ ...current, [field.key]: (event.target as HTMLTextAreaElement).value }))
                  }
                />
              </label>
            ))}
          ...
          <div class="row-actions">
            <button type="button" class="button secondary" onClick={() => setDraft(initial)}>
              Reset
            </button>
        ...
        <div class="model-header-main">
          ...
          <div class="model-header-actions">
            ...
            <details class="model-technical">
              ...
              <span>{data.model_version || 'profile_v2'}</span>
            </details>
          </div>
        </div>
      </section>
...
function PreferenceChoice<T extends string>({
  ...
  return (
    <fieldset class="preference-choice">
      <legend>
        <strong>{label}</strong>
        <span>{description}</span>
      </legend>
      <div class="preference-choice-options">
        {options.map((option) => (
          <label class={value === option.value ? 'active' : ''} key={option.value}>
            <input
              type="radio"
              name={name}
              value={option.value}
      ...
      <div class="theme-preview-frame" aria-hidden="true">
        ...
        <div class="theme-preview-content">
          ...
          <article class="theme-preview-card">
            <div>
              <strong>Build a calmer review loop</strong>
              <small>Current Thread · lesson ready</small>
            </div>
            <span class="theme-preview-score">Ready</span>
          </article>
          <div class="theme-preview-grid">
            <div class="theme-preview-alert">
              <strong>Next action</strong>
              <span>Continue the next lesson in your Thread.</span>
            </div>
...
function ThemeSemanticPreview({
  ...
  return (
    <span
      class={`theme-semantic-preview${font ? ' visual-preset-preview' : ''}`}
      style={previewStyle as any}
      aria-hidden="true"
    >
      <span class="theme-semantic-rail">
        <i />
        <i class="active" />
        <i />
      </span>
      <span class="theme-semantic-canvas">
        <span class="theme-semantic-toolbar">
          <i>{font ? 'Today' : ''}</i>
          <b>{font ? 'Add' : ''}</b>
        </span>
        <span class="theme-semantic-ledger">
          <strong>{font ? 'Next lesson' : ''}</strong>
          <em>{font ? 'Current Thread' : ''}</em>
          <small>{font ? 'Ready' : ''}</small>
        </span>
...
function PreferencesView() {
  ...
  const [savedThemes, setSavedThemes] = useState<Record<string, ThemeBundle>>(() => {
    try {
      return typeof localStorage === 'undefined'
        ? {}
        : JSON.parse(localStorage.getItem('taste-map-saved-themes') || '{}')
    } catch {
      return {}
    }
  })
  ...
  useEffect(() => {
    ...
    const currentPalette = rawPalette
      ? {
          ...
          ...rawPalette,
          ink:
            rawPalette.ink && rawPalette.ink !== rawPalette.accent
              ? rawPalette.ink
              : rawPalette.brand || savedPalette.ink,
          map: rawPalette.map || '#337f8c',
        }
      : savedPalette
  ...
  const selectFont = (fontId: string) => {
    ...
    persist('appearance', {
      ...
      custom_palette: customPalette,
      font: fontId,
      custom_font: fontId === 'custom' ? effectiveCustomFont : undefined,
    })
  }
  ...
  const applyVisualPreset = (preset: VisualPreset, paletteOverride?: CustomPalette) => {
    ...
    const nextDisplay = {
      ...
      radius: preset.display.radius,
      fontSize: preset.display.fontSize,
      reducedMotion,
    }
  ...
  const surpriseMe = async () => {
    ...
    try {
      const generated = await api<{ day?: CustomPalette; night?: CustomPalette }>('/ai/theme-variants', {
        method: 'POST',
        body: JSON.stringify({ current: customPalette, mode: themeMode }),
      })
  ...
  const updateTypography = (key: keyof TypographyPreferences, value: number) => {
    ...
    typographySaveTimer.current = window.setTimeout(() => {
      persist('appearance', {
        ...
        custom_palette: customPalette,
        font,
        custom_font: font === 'custom' ? customFont : undefined,
        typography: next,
      })
    }, 280)
  }
  ...
  const switchThemeMode = (mode: 'day' | 'night') => {
    setThemeMode(mode)
    try {
      localStorage.setItem('taste-map-theme-mode', mode)
    } catch {}
  ...
  const generateVariant = async (direction = 1) => {
    setVariantGenerating(true)
    try {
      const generated = await api<{ day?: CustomPalette; night?: CustomPalette; model?: string }>(
        '/ai/theme-variants',
        { method: 'POST', body: JSON.stringify({ current: customPalette, mode: themeMode }) },
      )
      if (generated.day && generated.night) {
        const nextPair = { day: generated.day, night: generated.night }
        setThemePair(nextPair)
      ...
      /* use the instant local variant when Gemini is unavailable */
    }
  ...
  const copyThemeJson = async () => {
    ...
    await navigator.clipboard.writeText(
      JSON.stringify(
        {
          name: 'Learning Compass visual system',
          modes: themePair,
          appearance: {
            font,
            customFont: effectiveCustomFont,
            typography,
            density,
            radius,
            fontSize,
  ...
  const exportTheme = () => {
    ...
    link.href = url
    link.download = 'learning-compass-theme.json'
    link.click()
  ...
  const applyThemeJson = (parsed: any) => {
    ...
    const nextDisplay = {
      density: appearance.density || density,
      radius: appearance.radius || radius,
      fontSize: appearance.fontSize || fontSize,
      reducedMotion: typeof appearance.reducedMotion === 'boolean' ? appearance.reducedMotion : reducedMotion,
    }
    setCustomPalette(nextPalette)
    setTheme('custom')
    applyTheme('custom', nextPalette)
    setFont(nextFont)
    setCustomFont(nextCustomFont)
    setTypography(nextTypography)
    setDensity(nextDisplay.density)
    setRadius(nextDisplay.radius)
    setFontSize(nextDisplay.fontSize)
    setReducedMotion(nextDisplay.reducedMotion)
    applyFont(nextFont, nextFont === 'custom' ? nextCustomFont : undefined)
    applyTypography(nextTypography)
    applyDisplayPreferences(nextDisplay)
    persist('appearance', {
      theme: 'custom',
      ...
      radius: nextDisplay.radius,
      font_size: nextDisplay.fontSize,
      reduced_motion: nextDisplay.reducedMotion,
      custom_palette: nextPalette,
      font: nextFont,
      custom_font: nextFont === 'custom' ? nextCustomFont : undefined,
    ...
    return Boolean(
      ...
      appearance.radius ||
      appearance.fontSize ||
      appearance.reducedMotion !== undefined,
    )
  }
  ...
  const importTheme = async (file?: File) => {
    ...
    try {
      ...
      setStatus(
        fullSystem
          ? 'Visual system imported — colors, fonts, type, and interface settings applied'
          : 'Palette imported — use a full visual-system JSON to include fonts and type',
      )
      window.setTimeout(() => setStatus(''), 1600)
    } catch {
      setStatus('That file is not a valid theme JSON.')
    }
  }
    ...
    const next = {
      ...
      [name]: {
        ...
        appearance: {
          ...
          fontSize,
          reducedMotion,
          responsiveViewport: 'auto' as const,
        },
      },
    }
    setSavedThemes(next)
    try {
      localStorage.setItem('taste-map-saved-themes', JSON.stringify(next))
    } catch {}
  ...
  const loadNamedTheme = (name: string) => {
    ...
    if (appearance.font) {
      ...
      applyFont(appearance.font, appearance.font === 'custom' ? appearance.customFont : undefined)
    }
    const nextCustomFont = normalizeCustomFont(appearance.customFont || customFont)
    ...
    persist('appearance', {
      ...
      custom_font: nextCustomFont,
      typography: appearance.typography || typography,
    })
  ...
  const handleApplyPastedCodes = () => {
    ...
    if (pasted.startsWith('{')) {
      try {
        ...
        setStatus(
          fullSystem
            ? 'Visual system applied — colors, fonts, type, and interface settings updated'
            : 'JSON palette applied — add an appearance object to change fonts and type',
        )
      } catch {
  ...
  const copyThemePrompt = async () => {
    const paletteBrief = Object.entries(customPalette)
      .filter(([, value]) => value)
  ...
  const saveLearning = (value: number) => {
    setRetention(value)
    persist('learning', { retention: value, queue_cap: 5 })
  }
  const saveDelivery = (
    patch: Partial<{
      effort: 'light' | 'moderate' | 'deep'
    ...
    setDeliveryEffort(next.effort)
    setDeliveryLanguage(next.language)
    setDeliveryModes(next.delivery_modes)
    setDepthTier(next.depth_tier)
    persist('delivery_context', next)
  }
  const saveSrs = (next: Partial<{ auto_extract: boolean }>) => {
    const current = { minimum_rating: 7, auto_extract: autoExtract, ...next }
    setAutoExtract(current.auto_extract)
    persist('srs_drafts', current)
  }
  const activePreset = VISUAL_PRESETS.find((preset) => {
    const typeMatches = (Object.keys(preset.typography) as Array<keyof TypographyPreferences>).every(
      (key) => preset.typography[key] === typography[key],
    )
    return (
      preset.theme === theme &&
      preset.font === font &&
  ...
  const activeSwatches = activeThemePreset?.swatches || [
    customPalette.brand,
    customPalette.shell,
  ...
  const themePresetGroups = [
    {
      mode: 'light' as const,
      title: 'Day palettes',
      description: 'Paper-bright systems for daylight and focused reading.',
    },
    {
      mode: 'dark' as const,
      title: 'Night palettes',
      description: 'Low-light systems with luminous type and restrained signals.',
    },
  ]
  ...
  return (
    <div class="settings-page preferences-page">
      <header class="settings-intro preferences-hero">
        <div>
          <span class="settings-active-label">Settings / Preferences</span>
          <h1>Preferences</h1>
          <p>
            Shape how Learning Compass looks, reads, and supports your learning. Every change applies across the whole
            studio.
          </p>
        </div>
        <output class="preferences-save-state" aria-live="polite" data-working={Boolean(status)}>
          {status || 'Saved automatically'}
        </output>
      </header>

      <section class="settings-active-system" aria-label="Active visual system">
        <span class="preferences-active-swatch" aria-hidden="true">
          {activeSwatches.map((color, index) => (
            <i key={`${color}-${index}`} style={{ background: color }} />
          ))}
        </span>
        <div class="preferences-active-copy">
          <span class="settings-active-label">Active workspace</span>
          <strong>{activePreset?.name || 'Custom tuning'}</strong>
          <small>
            {activeThemeName} · {activeFontPreset?.name || (font === 'custom' ? 'Custom font stacks' : font)} ·{' '}
            {effectiveBaseSize}px · {density}
          </small>
        </div>
        <button type="button" class="preferences-reset" onClick={() => applyVisualPreset(VISUAL_PRESETS[0])}>
          Restore focused study
        </button>
      </section>

      <details class="settings-jump-disclosure">
        <summary>Jump to a preference</summary>
        <nav class="settings-jump-nav" aria-label="Preference sections">
          <a href="#visual-presets-heading" onClick={(event) => jumpToPreference(event, 'visual-presets-heading')}>
            Workspace
          </a>
          <a href="#interface-tokens" onClick={(event) => jumpToPreference(event, 'interface-tokens')}>
            Comfort
          </a>
          <a href="#theme-section" onClick={(event) => jumpToPreference(event, 'theme-section')}>
            Theme
          </a>
          ...
          <a href="#atlas-preferences" onClick={(event) => jumpToPreference(event, 'atlas-preferences')}>
            Map
          </a>
        </nav>
      </details>
      ...
      <div class="preferences-layout">
        <aside class="preferences-preview-rail" aria-label="Current appearance preview">
          <ThemeContextPreview />
          <div class="preferences-scope-note">
            <strong>One system, everywhere</strong>
            <p>Your appearance choices apply to Home, Library, Learn, Map, Settings, dialogs, and object views.</p>
          </div>
        </aside>
        <div class="preferences-main">
            ...
            <div class="visual-presets-grid">
              {VISUAL_PRESETS.map((preset) => {
                ...
                return (
                  <button
                    type="button"
                    class={`visual-preset-card${isActive ? ' active' : ''}`}
                    aria-pressed={isActive}
                    key={preset.id}
                    onClick={() => applyVisualPreset(preset)}
                  >
                    {themePreset ? (
                      <ThemeSemanticPreview
                        palette={paletteFromThemePreset(themePreset)}
                    ...
                    <span class="visual-preset-copy">
                      <span class="visual-preset-reference">{preset.inspiration}</span>
                      <strong>{preset.name}</strong>
                      <small>{preset.description}</small>
                      <span class="visual-preset-spec">
                        {fontPreset?.name} · {preset.typography.baseSize}px · {preset.display.density}
                      </span>
                    </span>
                  </button>
                )
          ...
          <section class="preference-panel preference-comfort" id="interface-tokens" aria-labelledby="comfort-title">
            <div class="section-head">
              <div>
                <span class="preference-section-number">02 · Comfort & layout</span>
                <h2 id="comfort-title">Make the workspace easy to use</h2>
                <p class="section-description">
                  These controls change every canvas, panel, row, and action—not only this page.
                </p>
              </div>
            </div>
            <div class="preference-choice-grid">
              ...
              <PreferenceChoice
                name="corner-style"
                label="Corners"
                description="The shape of panels and controls."
                value={radius}
                options={[
              ...
              <PreferenceChoice
                name="interface-size"
                label="Text size"
                description="The interface scale across the studio."
                value={fontSize}
                options={[
              ...
              <PreferenceToggle
                label="Reduce motion"
                description="Remove nonessential transitions and animated movement."
                checked={reducedMotion}
                onChange={(value) => saveDisplay({ reducedMotion: value })}
              />
            </div>
          </section>
          ...
          <details class="theme-section preference-disclosure" id="theme-section">
            <summary>
              <span class="preference-summary-icon" aria-hidden="true">
                {activeSwatches.map((color, index) => (
                  <i key={`${color}-${index}`} style={{ background: color }} />
                ))}
              </span>
              <span>
                <strong>Theme</strong>
                <small>
                  {activeThemeName} · {THEME_PRESETS.length + 1} choices
                </small>
              </span>
              <em>Browse</em>
            </summary>
            <div class="preference-disclosure-body">
              <div class="section-head">
                <div>
                  <span class="preference-section-number">03 · Theme</span>
                  <h2>Choose the atmosphere</h2>
                  <p class="section-description">
                    Pick a complete day or night palette. Your content and learning data stay unchanged.
                  </p>
                </div>
              </div>
              ...
              <div class="theme-preset-groups">
                {themePresetGroups.map((group) => (
                  <section class="theme-preset-group" aria-labelledby={`theme-group-${group.mode}`} key={group.mode}>
                    <header class="theme-preset-group-heading">
                      <div>
                        <h3 id={`theme-group-${group.mode}`}>{group.title}</h3>
                        <p>{group.description}</p>
                      </div>
                      <span>{THEME_PRESETS.filter((preset) => preset.mode === group.mode).length} palettes</span>
                    </header>
                          ...
                          >
                            ...
                            <span class="theme-preset-copy">
                              <span class="theme-preset-title-row">
                                <strong class="theme-preset-title">{preset.name}</strong>
                                {isSelected && <span class="theme-selected-marker">Selected</span>}
                              </span>
                              <small class={`theme-preset-mode mode-${preset.mode}`}>
                ...
                <section class="theme-preset-group theme-custom-choice" aria-labelledby="theme-group-custom">
                  <header class="theme-preset-group-heading">
                    <div>
                      <h3 id="theme-group-custom">Custom visual system</h3>
                      <p>Build a paired day and night system from the semantic roles below.</p>
                    </div>
                    <span>11 color roles</span>
                  </header>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span class="theme-preset-copy">
                      <span class="theme-preset-title-row">
                        <strong class="theme-preset-title">Your visual system</strong>
                        {theme === 'custom' && <span class="theme-selected-marker">Selected</span>}
                      </span>
                      <small class={`theme-preset-mode mode-${customThemeMode}`}>
                        {themeMode === 'night' ? 'Night pair' : 'Day pair'}
                      </small>
                      <span class="theme-preset-desc">
                        Tune every semantic color, explore variants, and exchange the complete system when you need it.
                      </span>
                    </span>
                  </button>
                </section>
                ...
                <div class="custom-palette-panel" aria-label="Custom visual system editor">
                  <div class="custom-palette-header">
                    <div>
                      ...
                      <p>
                        Choose a mode, explore a direction, then tune the semantic roles. Changes preview immediately
                        and save automatically.
                      </p>
                    </div>
                    <span class="theme-palette-mode-status">
                      Editing {themeMode === 'night' ? 'night' : 'day'} palette
                    </span>
                  </div>
                  <div class="theme-workshop-stage">
                    <div class="theme-workshop-stage-copy">
                      <strong>Explore this pair</strong>
                      <span>Generate a related direction or move through your local edit history.</span>
                    </div>
                    <div class="theme-workshop-toolbar" aria-label="Theme workshop controls">
                      <div class="theme-mode-switch" role="group" aria-label="Theme mode">
                        <button
                          type="button"
                          class={themeMode === 'day' ? 'active' : ''}
                          aria-pressed={themeMode === 'day'}
                          onClick={() => switchThemeMode('day')}
                        >
                          Day
                        </button>
                        <button
                          type="button"
                          class={themeMode === 'night' ? 'active' : ''}
                      ...
                      <button
                        ...
                        disabled={variantGenerating}
                      >
                        {variantGenerating ? 'Gemini is designing…' : 'Generate variant'}
                      </button>
                      <button type="button" class="btn-secondary" onClick={surpriseMe} disabled={variantGenerating}>
                        Surprise me
                      </button>
                      <button
                        type="button"
                        class="btn-secondary"
                        onClick={undoPalette}
                        disabled={!paletteHistory.length}
                      >
                        Undo
                      </button>
                      <button
                  ...
                  <div class="theme-color-workspace">
                    <div class="custom-palette-fields" aria-label="Semantic color roles">
                      {CUSTOM_COLOR_GROUPS.map((group) => (
                        <fieldset class="custom-color-group" key={group.name}>
                          <legend>
                            <strong>{group.name}</strong>
                            <span>{group.description}</span>
                          </legend>
                          <div class="custom-color-group-fields">
                            {group.fields.map(({ key, label, fallback, description }) => (
                              <div class="custom-color-item" key={key}>
                                <label for={`color-${key}`}>
                                  <strong>{label}</strong>
                                ...
                                <div class="custom-color-input-group">
                                  <input
                                    ...
                                    value={normalizeColor(customPalette[key] || fallback, fallback)}
                                    onInput={(event) =>
                                      updateCustomColor(key, (event.target as HTMLInputElement).value)
                                    }
                                  />
                                  <input
                                    id={`color-${key}`}
                    ...
                    <aside class="theme-contrast-report" aria-labelledby="theme-contrast-title">
                      <header>
                        <div>
                          <h4 id="theme-contrast-title">Rendered contrast</h4>
                          <p>
                            Checks use the CSS colors the studio actually renders, including automatic foreground
                            correction.
                          </p>
                        </div>
                        <strong class={contrastFailures ? 'has-review' : 'all-pass'}>
                          {contrastFailures ? `${contrastFailures} to review` : 'All pass'}
                        </strong>
                      </header>
                      <div class="theme-contrast-grid">
                        {contrastChecks.map((check) => (
                          <span class={check.passes ? 'contrast-pass' : 'contrast-fail'} key={check.id}>
                            <i
                            ...
                            <span>
                              ...
                              <small>
                                {check.ratio === null
                                  ? 'Invalid color'
                                  : `${check.ratio.toFixed(1)}:1 · ${check.passes ? 'Pass' : 'Review'}`}
                              </small>
                            </span>
                          </span>
                        ))}
                      </div>
                      <p class="theme-contrast-note">
                        Target: 4.5:1 for normal text. Authored colors stay unchanged; derived text tokens may be
                  ...
                  <details class="theme-workshop-advanced">
                    <summary>
                      <span>
                        <strong>Transfer, automation & snapshots</strong>
                        <small>
                          Paste colors, exchange JSON, copy an AI brief, or save this full system for later.
                        </small>
                      </span>
                      <em>Open tools</em>
                    </summary>
                    <div class="theme-workshop-advanced-body">
                      <div class="theme-json-explainer" role="note">
                        ...
                        <span>
                          JSON can carry day/night palettes, font roles, type rhythm, density, corners, text size, and
                          motion across the whole studio.
                        </span>
                        <small>Use web-loadable fonts or include reliable fallbacks for local typefaces.</small>
                      </div>
                      <div
                        class="theme-workshop-toolbar theme-transfer-toolbar"
                        aria-label="Visual system transfer controls"
                      >
                        <button
                          type="button"
                          class="palette-prompt-button"
                          onClick={copyThemePrompt}
                          title="Copy a prompt for another AI to generate the full visual system"
                        >
                          {promptCopied ? 'AI prompt copied' : 'Copy AI prompt'}
                        </button>
                        <button
                          type="button"
                          class="btn-secondary"
                          onClick={copyThemeJson}
                          title="Copy colors, fonts, typography, and interface settings"
                        >
                          Copy JSON
                        </button>
                        <button
                          ...
                          class="btn-secondary"
                          onClick={exportTheme}
                          title="Download the complete visual system"
                        >
                          Export JSON
                        </button>
                        <label
                          class="theme-import-button"
                          title="Apply colors, fonts, typography, and interface settings across the studio"
                        >
                          <span>Import JSON</span>
                          <input
                            type="file"
                            accept="application/json"
                            aria-label="Import visual system JSON"
                            onChange={(event) => importTheme((event.target as HTMLInputElement).files?.[0])}
                          />
                        </label>
                        <button type="button" class="btn-secondary" onClick={saveNamedTheme}>
                          Save snapshot
                        </button>
                        {Object.keys(savedThemes).length > 0 && (
                          <select
                            class="theme-saved-select"
                            aria-label="Saved themes"
                            value=""
                            onChange={(event) => loadNamedTheme((event.target as HTMLSelectElement).value)}
                          >
                      ...
                      <div class="custom-palette-paste-box">
                        <textarea
                          ...
                          value={pasteCodes}
                          onInput={(event) => setPasteCodes((event.target as HTMLTextAreaElement).value)}
                          rows={4}
                        />
                        <div class="custom-palette-actions">
                          ...
                          <button
                            type="button"
                            class="btn-secondary"
                            onClick={() => commitPalette(DEFAULT_CUSTOM_PALETTE)}
                          >
                            Reset palette
                          </button>
                        </div>
                  ...
                  <div class="theme-token-map" aria-label="Theme token usage">
                    <span>
                      <strong>Brand</strong>
                      <small>primary actions · focus</small>
                    </span>
                    <span>
                      <strong>Rail</strong>
                    ...
                    <span>
                      <strong>Seam</strong>
                      <small>borders · dividers</small>
                    </span>
                    <span>
                      <strong>Due / Danger</strong>
                      <small>status feedback</small>
                    </span>
                    <span>
                      <strong>Map</strong>
                      <small>atlas links · branches</small>
                    </span>
                  </div>
                  <p class="theme-palette-save-note" aria-live="polite">
                    {paletteDirty ? 'Saving palette changes…' : 'Palette changes save automatically.'}
                  </p>
                </div>
              )}
            </div>
          ...
          <details class="font-section preference-disclosure" id="font-section">
            <summary>
              <span
                class="preference-summary-type"
                style={{ fontFamily: activeFontPreset?.ui || customFont.ui }}
                aria-hidden="true"
              >
                Aa
              </span>
              <span>
                ...
                <small>
                  {activeFontPreset?.name || (font === 'custom' ? 'Custom font stacks' : font)} · interface, reading,
                  headings, and code
                </small>
              </span>
              <em>Choose</em>
            ...
            <div class="preference-disclosure-body">
              <div class="section-head">
                <div>
                  <span class="preference-section-number">04 · Reading & fonts</span>
                  <h2>Choose the voice of the page</h2>
                  <p class="section-description">
                    Select a coordinated family for interface text, long-form reading, headings, and code metadata.
                  </p>
                </div>
              </div>
              <div class="font-presets-grid" role="group" aria-label="Fonts">
                {FONT_PRESETS.map((f) => (
                  ...
                  >
                    <span class="font-preset-sample" style={{ fontFamily: f.ui }}>
                      Aa
                    </span>
                ...
                >
                  <span class="font-preset-sample" style={{ fontFamily: customFont.ui }}>
                    Aa
                  </span>
                  <span class="font-preset-copy">
              ...
              {font === 'custom' && (
                <div class="custom-font-panel">
                  <div class="custom-font-header">
                    ...
                    <p>
                      These four roles control the whole studio. Web fonts load automatically; local-only faces need a
                      reliable fallback.
                    </p>
                  </div>
                  ...
                  <p class="custom-font-hint">
                    Try: Inter, Plus Jakarta Sans, Newsreader, Literata, JetBrains Mono, Fira Code, IBM Plex Sans
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        persist('appearance', {
                          ...
                          custom_palette: customPalette,
                          font: 'custom',
                          custom_font: DEFAULT_CUSTOM_FONT,
                        })
                      }}
                    >
                      Reset font stacks
          ...
          <details class="typography-controls-section preference-disclosure" id="type-controls">
            <summary>
              <span class="preference-summary-type" aria-hidden="true">
                Tt
              </span>
              <span>
                <strong>Detailed typography</strong>
                <small>
                  {effectiveBaseSize}px body · {typography.lineHeight} line height · {typography.readingMeasure}ch
                  measure
                </small>
              </span>
              <em>Fine-tune</em>
            ...
            <div class="preference-disclosure-body">
              <div class="section-head">
                <div>
                  <span class="preference-section-number">05 · Type tuning</span>
                  <h2>Adjust reading rhythm</h2>
                  <p class="section-description">
                    Fine-tune scale, weight, rhythm, and line length across the entire studio.
                  </p>
                  <p class="type-responsive-note">
                    <strong>{effectiveBaseSize}px effective body size</strong> ·{' '}
                    {viewportBoost
                      ? `wide-screen comfort adds ${viewportBoost}% at ${viewportWidth}px`
                      : 'mobile and standard desktop use your selected size'}{' '}
                    · reading measure stays {typography.readingMeasure}ch
                  </p>
                </div>
                <button type="button" class="btn-secondary" onClick={resetTypography}>
                  Reset type
                </button>
              </div>
              <div class="typography-layout">
                    ...
                    return (
                      <label class="type-range" key={key}>
                        ...
                        <input
                          ...
                          step={step}
                          value={value}
                          onInput={(event) => updateTypography(key, Number((event.target as HTMLInputElement).value))}
                        />
                      </label>
                    )
                  })}
                ...
                <div
                  ...
                  style={{
                    ...
                    fontSize: `calc(${typography.baseSize * typography.displayScale}px * var(--font-viewport-scale, 1))`,
                    lineHeight: typography.lineHeight,
                    maxWidth: `${typography.readingMeasure}ch`,
                  }}
                >
                  ...
                  <p
                    style={{
                      ...
                      fontSize: `calc(${typography.baseSize}px * var(--font-viewport-scale, 1))`,
                      fontWeight: typography.bodyWeight,
                      letterSpacing: `${typography.letterSpacing}em`,
                    }}
                  >
                    A calmer type system makes the next decision easier to see. Adjust weight, rhythm, and measure until
                    the page feels effortless to scan and comfortable to stay in.
                  </p>
                  <p
                    dir="rtl"
                    style={{
                      fontFamily: 'var(--font-reading), "Noto Naskh Arabic", "IBM Plex Sans Arabic", serif',
                      fontSize: `calc(${typography.baseSize * 1.05}px * var(--font-viewport-scale, 1))`,
                      lineHeight: typography.lineHeight,
                      margin: '4px 0 0',
                    }}
                  >
                    العلم النافع يبني العقل ويُهذب السلوك · رحلة التعلّم المستمر والبحث عن الحق
                  </p>
                  <code style={{ fontFamily: 'var(--font-mono)' }}>
                    retain → apply → remember · {effectiveBaseSize}px · {typography.lineHeight} rhythm
                  </code>
                </div>
              </div>
            </div>
          ...
          <section
            class="preference-panel learning-preferences"
            id="learning-preferences"
            aria-labelledby="learning-preferences-title"
          >
            <div class="section-head">
              <div>
                <span class="preference-section-number">06 · Learning behavior</span>
                <h2 id="learning-preferences-title">Choose what happens automatically</h2>
                <p class="section-description">
                  Keep the learning loop deliberate while deciding which routine steps Learning Compass can prepare for
                  you.
                </p>
              </div>
            </div>
            <div class="preference-learning-grid">
              <article class="preference-setting-group">
                <div class="preference-group-heading">
                  <span>Review</span>
                  <h3>Recall & Queue</h3>
                  <p>Set the review target without changing the five-item commitment limit.</p>
                </div>
                <div class="setting-row">
                  ...
                  <select
                    aria-label="Recall retention target"
                    value={retention}
                ...
                <div class="setting-row">
                  <div>
                    ...
                    <span>Used only as your explicit saved delivery context.</span>
                  </div>
                  <select
                    aria-label="Default delivery effort"
                    value={deliveryEffort}
                    onChange={(event) =>
                      saveDelivery({ effort: (event.currentTarget as HTMLSelectElement).value as any })
                    }
                  >
                ...
                <div class="setting-row">
                  <div>
                    ...
                    <span>No language is inferred from activity.</span>
                  </div>
                  <select
                    aria-label="Default delivery language"
                    value={deliveryLanguage}
                    onChange={(event) =>
                      saveDelivery({ language: (event.currentTarget as HTMLSelectElement).value as any })
                    }
                  >
                ...
                <div class="setting-row">
                  <div>
                    <strong>Depth</strong>
                    <span>
                      Adaptive is an advisory derived from direct lesson completions and depth feedback; it never
                      changes progress.
                    </span>
                  </div>
                  <select
                    aria-label="Default depth tier"
                    value={depthTier}
                    onChange={(event) =>
                      saveDelivery({ depth_tier: (event.currentTarget as HTMLSelectElement).value as any })
                    }
                  >
                ...
                <div class="setting-row">
                  <div>
                    ...
                    <span>Leave all unchecked to accept any mode.</span>
                  </div>
                  <span>
                    {(['read', 'watch', 'listen', 'practice'] as const).map((mode) => (
                      <label key={mode}>
                        <input
                          type="checkbox"
                          ...
                          onChange={(event) =>
                            saveDelivery({
                              delivery_modes: (event.currentTarget as HTMLInputElement).checked
                                ? [...deliveryModes, mode]
                                : deliveryModes.filter((item) => item !== mode),
                            })
                          }
                        />{' '}
                        {labelize(mode)}
              ...
              <article class="preference-setting-group">
                <div class="preference-group-heading">
                  ...
                  <p>Choose where the system may prepare context or apply strong evidence.</p>
                </div>
                <PreferenceToggle
                  label="Enrich new captures"
                  description="Let the capture workflow add source metadata before triage."
                  checked={enrichCapture}
                  onChange={(value) => {
                    setEnrichCapture(value)
                    persist('ai_curation', { enrich_capture: value })
                  }}
                />
                <div class="setting-row">
                  <div>
                  ...
                  <select
                    aria-label="Profile learning"
                    value={profileMode}
                    onChange={(event) => {
                      const value = (event.target as HTMLSelectElement).value
                      setProfileMode(value)
                      persist('profile_automation', { mode: value, policy_version: 'profile_v2' })
                    }}
                  >
                ...
                <PreferenceToggle
                  ...
                  checked={autoExtract}
                  onChange={(value) => saveSrs({ auto_extract: value })}
                />
              </article>
          ...
          <details class="preference-disclosure atlas-preferences" id="atlas-preferences">
            <summary>
              <span class="preference-summary-map" aria-hidden="true">
                Map
              </span>
              <span>
                <strong>Map display</strong>
                <small>Labels, links, arrows, and constellation forces</small>
              </span>
              <em>Advanced</em>
            </summary>
            <div class="preference-disclosure-body">
              <div class="section-head">
                <div>
                  <span class="preference-section-number">07 · Map display</span>
                  <h2>Shape the Atlas</h2>
                  <p class="section-description">
                    Tune how the knowledge map reads and moves. These controls affect Map only.
                  </p>
                </div>
              </div>
              <div class="atlas-preference-grid">
                <PreferenceToggle
                  label="Show arrows"
                  description="Draw direction arrows on relationship links."
                  checked={arrows}
                  onChange={(value) => {
                    setArrows(value)
                    saveAtlas({ arrows: value })
                  }}
                />
                <PreferenceToggle
                  label="Animate Map transitions"
                  description="Smooth camera moves, expansions, and constellation drags."
                  checked={atlasAnimate}
                  onChange={(value) => {
                    setAtlasAnimate(value)
                ...
                <label class="type-range">
                  ...
                  <small>Labels fade out as you zoom past this point (log scale).</small>
                  <input
                    type="range"
                    min={-1}
                    max={1}
                    ...
                    onInput={(event) => {
                      ...
                      setTextFade(v)
                      saveAtlas({ text_fade_threshold: v })
                    }}
                  />
                </label>
                <label class="type-range">
                  ...
                  <small>Scale every node on the map.</small>
                  <input
                    type="range"
                    min={0.1}
                    max={3}
                ...
                <label class="type-range">
                  ...
                  <small>Weight of relationship lines.</small>
                  <input
                    type="range"
                    min={0.1}
                    max={6}
                ...
                <label class="type-range">
                  ...
                  <small>Thickness of lines from a branch to its child nodes.</small>
                  <input
                    type="range"
                    min={0.1}
                    max={6}
                    ...
                    onInput={(event) => {
                      const v = Number((event.target as HTMLInputElement).value)
                      setBranchLinkThickness(v)
                      saveAtlas({ branch_link_thickness: v })
                    }}
                  />
                </label>
              </div>
              <div class="section-head atlas-force-heading">
                <h3>Constellation forces</h3>
              ...
              <div class="atlas-preference-grid">
                <label class="type-range">
                  ...
                  <small>Pull of cluster islands toward the canvas center.</small>
                  <input
                    type="range"
                    min={0}
                    max={2}
                ...
                <label class="type-range">
                  ...
                  <small>How strongly nodes push apart to avoid overlap.</small>
                  <input
                    type="range"
                    min={0}
                    max={50}
                ...
                <label class="type-range">
                  ...
                  <small>Length of the links that connect related nodes.</small>
                  <input
                    type="range"
                    min={0}
                    max={3}
              ...
              <div class="row-actions">
                {['failed', 'conflict'].includes(item.state) && (
                  <button
                    ...
                    onClick={() => resolveOfflineMutation(item.id, 'retry').then(refresh)}
                  >
                    Retry
                  </button>
                )}
                <button
                  type="button"
                  class="button secondary"
                  onClick={() => resolveOfflineMutation(item.id, 'discard').then(refresh)}
                >
                  Discard
                </button>
              </div>
  ...
  return (
    <div class="settings-page data-settings-page">
      <section class="settings-intro">
        ...
        <p>
          Manage the records that help Learning Compass understand you, then verify their quality, portability, and
          recovery.
        </p>
      </section>
      ...
      <section class="data-trust-panel" aria-labelledby="data-trust-title">
        <div class="section-head">
          ...
          <span class={`data-trust-state ${qualityTrusted ? 'is-trusted' : 'is-warning'}`}>
            {qualityTrusted ? 'Trusted' : 'Needs attention'}
          </span>
        </div>
        {quality?.checks?.length ? (
          <>
            <div class="data-trust-summary">
              <strong>
                {Number(quality.summary?.passing || 0)} / {Number(quality.summary?.total || quality.checks.length)}
              </strong>
              <span>
                contracts passing across {Number(quality.counts?.active_sources || 0)} active sources,{' '}
                {Number(quality.counts?.learning_events || 0)} learning events, and{' '}
                {Number(quality.counts?.enabled_feeds || 0)} feeds.
              </span>
            </div>
            <div class="data-trust-grid">
              {quality.checks.map((item) => (
                <article
                  class={`data-trust-check ${item.status === 'passing' ? 'is-passing' : 'is-failing'}`}
                  key={item.id}
                >
                  <div>
                  ...
                  <p>{item.message}</p>
                  <small>
                    {item.coverage_percent}% coverage · {item.total} checked
                  </small>
                </article>
              ))}
            </div>
            <p class="settings-help">
              These are explicit completeness, validity, uniqueness, and lineage checks—not an opaque engagement score.
              Last checked {formatDate(quality.checked_at)}.
            </p>
          </>
        ) : (
          <Empty
            title="Data contracts unavailable"
            body="The runtime inventory did not return a data-quality report. Refresh status before relying on this record."
          />
        )}
      </section>
      <section>
        ...
        <div class="setting-row">
          <div>
            ...
            <span>
              Download typed media status, progress, ratings, tags, personal notes, branch context, and recommendation
              history. Notes, Threads, recall, settings, and R2 files are not included.
            </span>
          </div>
          <button
            type="button"
            ...
            onClick={() =>
              triggerExport('/recommendations/export?format=json&limit=5000', 'learning-compass-data.json')
            }
          >
        ...
        <div class="setting-row">
          <div>
            ...
            <span>
              Download a readable ledger with type, status, progress, rating, branch, link, and personal note.
            </span>
          </div>
          <button
            type="button"
      ...
      <section class="recovery-status-panel">
        ...
        {system.data?.recovery?.latest ? (
          <div class="setting-row">
            <div>
              <strong>Daily D1 + R2 snapshot</strong>
              <span>
                Latest disposable restore rehearsal{' '}
                {formatDate(system.data.recovery.latest.restore_rehearsed_at || system.data.recovery.latest.created_at)}{' '}
                · {Number(system.data.recovery.latest.artifact_count || 0)} artifacts verified.
              </span>
            </div>
            <span class={`setting-value ${system.data.recovery.ok ? 'is-active' : ''}`}>
              {system.data.recovery.ok ? 'Current' : 'Stale'}
            </span>
          </div>
        ) : (
          <Empty
            title="No verified full backup"
            body="Run the production backup and restore rehearsal before relying on recovery."
          />
        )}
        <p class="settings-help">
          The operator snapshot includes canonical D1 state and every R2 object, verifies checksums, restores into a
          disposable database, and is retained outside this repository.
        </p>
      </section>
      <section>
        <div class="section-head">
...
function SystemView() {
  const capabilities = useData<{ capabilities?: Capability[]; authentication?: string }>(
    '/agent/capabilities?view=summary',
  )
  const system = useData<SystemPayload>('/agent/system')
  const [query, setQuery] = useState('')
  const [method, setMethod] = useState('ALL')
  if (capabilities.loading || system.loading) return <Loading label="Reading the system inventory" />
  if (capabilities.error || system.error)
    return (
      <ErrorState
        message={capabilities.error || system.error}
        retry={() => {
          capabilities.reload()
          system.reload()
        }}
  ...
  const filtered = operations.filter(
    (item) =>
      ...
      `${item.method} ${item.path} ${item.description}`.toLowerCase().includes(query.trim().toLowerCase()),
  )
  const grouped = filtered.reduce<Record<string, Capability[]>>((result, item) => {
    const area = capabilityArea(item.path)
    result[area] = [...(result[area] || []), item]
    return result
  }, {})
  const writes = operations.filter((item) => item.method !== 'GET').length
  return (
    <div class="system-console settings-system-page">
      <section class="system-hero">
        <div>
          <span class="eyebrow">Settings / System</span>
          <h1>System status and advanced operations</h1>
          <p>
            Check the health of Learning Compass, review Hermes activity, and inspect the allow-listed operations
            available to the product.
          </p>
        </div>
        <div class="system-hero-actions">
          <a href="/agent/openapi.json" target="_blank" rel="noreferrer">
      ...
      <div class="system-summary">
        ...
        <div>
          <strong>{operations.length - writes}</strong>
          <span>Read operations</span>
        </div>
        <div>
          <strong>{writes}</strong>
          <span>Guarded writes</span>
        </div>
        <div>
          <strong>{system.data?.schedule?.length || 0}</strong>
      ...
      <section>
        <div class="section-head">
          ...
          <span>{system.data?.status || 'unknown'}</span>
        </div>
        <div class="system-health-grid">
          <article>
            <i class="healthy" />
            <span>
              <strong>{system.data?.service || 'Learning Compass Worker'}</strong>
              <small>{system.data?.environment || 'Runtime available'}</small>
            </span>
          </article>
          {(system.data?.storage || []).map((item) => (
            <article key={item.name}>
              <i class={/connected|managed|active/i.test(item.status) ? 'healthy' : 'warning'} />
              <span>
                <strong>{item.name}</strong>
      ...
      <div class="system-two-column">
        <section>
          <div class="section-head">
            <h2>Schedules</h2>
            <span>{system.data?.schedule?.length || 0} configured</span>
          </div>
          <div class="schedule-list">
            {(system.data?.schedule || []).length ? (
              system.data!.schedule!.map((item) => (
                <article key={item.id}>
                  <div class="schedule-head">
                    <span class="method-badge method-post">CRON</span>
                    <div>
                  ...
                  <small>
                    Last success {item.last_success ? formatDate(item.last_success) : 'not recorded'} · search{' '}
                    {item.last_search_sync ? formatDate(item.last_search_sync) : 'not recorded'}
                  </small>
                </article>
              ))
            ) : (
              <Empty
                ...
                body="Maintenance remains on-demand until a schedule is explicitly configured."
              />
            )}
          </div>
        </section>
        <section>
          <div class="section-head">
            <h2>On demand only</h2>
            <span>{system.data?.on_demand_only?.length || 0} workflows</span>
          </div>
          <div class="on-demand-list">
            {(system.data?.on_demand_only || []).map((item) => (
              <div key={item}>
                <i />
                <span>{item}</span>
              </div>
      ...
      <section>
        <div class="section-head">
          <h2>Advanced API operations</h2>
          <span>
            {filtered.length} of {operations.length}
          </span>
        </div>
        <div class="api-catalog-head">
          <div class="api-filters">
            <label>
              Search path or capability
              <input
                value={query}
                onInput={(event) => setQuery((event.target as HTMLInputElement).value)}
                placeholder="e.g. profile, export, notes"
              />
            </label>
            <label>
              Method
              <select value={method} onChange={(event) => setMethod((event.target as HTMLSelectElement).value)}>
                <option value="ALL">All methods</option>
                {[...new Set(operations.map((item) => item.method))].sort().map((item) => (
        ...
        {filtered.length ? (
          <div class="api-groups">
            {(Object.entries(grouped) as Array<[string, Capability[]]>).map(([area, items]) => (
              <section key={area}>
                ...
                <div class="api-operation-list">
                  {items.map((item) => (
                    <article key={`${item.method}:${item.path}`}>
                      ...
                      <small>{item.method === 'GET' ? 'Read only' : 'Validated · audit logged'}</small>
                    </article>
                  ))}
                </div>
              </section>
          ...
          <Empty
            ...
            body="Try a broader search or reset the method filter."
            action={
              <button
                type="button"
                class="button secondary"
      ...
      <section class="system-safety">
        <div class="section-head">
          <h2>Safety boundaries</h2>
          <span>{capabilities.data?.authentication || 'Product validation remains active'}</span>
        </div>
        {(system.data?.safety || []).map((item) => (
...
export function SettingsWorkspace({ route, view, onRouteChange, onCapture }: SettingsWorkspaceProps) {
  ...
  const requestedMode =
    route?.mode || routed.mode || query.get('mode') || route?.view || route?.slug || view || routed.view
  const requestedFocus = route?.focus || routed.focus || query.get('focus') || ''
  const activeMode: SettingsMode =
    requestedMode === 'data' ? 'data' : requestedMode === 'system' ? 'system' : 'personal'
  const activeFocus: SettingsFocus =
    requestedFocus === 'preferences' || requestedMode === 'preferences' ? 'preferences' : 'profile'
  const activeView = normalizeView(activeMode === 'personal' ? activeFocus : activeMode)
  return (
    <div class="settings-workspace workspace-surface">
      <SettingsModeSwitcher active={activeMode} focus={activeFocus} onRouteChange={onRouteChange} />
      {activeView === 'profile' && <ProfileView />}
      {activeView === 'preferences' && <PreferencesView />}
      {activeView === 'data' && <DataView onCapture={onCapture} />}
