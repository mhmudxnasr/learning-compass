...
const PRESET_FEEDS = [
  {
    ...
    branch: 'systems-thinking',
    desc: 'Practical AI dev & CLI tools',
  },
  {
    title: 'Latent.Space',
    url: 'https://www.latent.space/feed',
    branch: 'systems-thinking',
    desc: 'AI engineering & tools',
  },
  {
    title: 'Ars Technica Tech Lab',
    url: 'https://feeds.arstechnica.com/arstechnica/technology-lab',
    branch: 'systems-thinking',
    desc: 'Broader tech & deep dives',
  },
  {
    title: 'The Verge AI',
    url: 'https://www.theverge.com/rss/ai-artificial-intelligence/index.xml',
    branch: 'systems-thinking',
    desc: 'AI hardware & product news',
  },
]
...
export function FeedsView({ data, handlers }: { data: LibraryRecord; handlers: LibraryViewHandlers }) {
  ...
  const [confirmAction, setConfirmAction] = useState<{
    type: 'clear' | 'delete-feed' | 'delete-entry'
    id: string
  ...
  const feeds = useMemo(() => (Array.isArray(data.feeds) ? data.feeds : []), [data.feeds])
  const totalEntries = useMemo(
    () => feeds.reduce((sum: number, f: LibraryRecord) => sum + Number(f.entry_count || 0), 0),
    [feeds],
  )
  ...
  useEffect(() => {
    ...
    api<{ feed: LibraryRecord; items: LibraryRecord[]; total: number }>(
      `/capture/feeds/${encodeURIComponent(selectedFeedId || 'all')}/entries?limit=50`,
    )
      .then((res) => {
        if (active) {
  ...
  const selectedFeed = isAllFeeds
    ...
    : feeds.find((f: LibraryRecord) => String(f.id) === selectedFeedId) || {
        id: 'all',
        title: 'All Subscribed Sources',
        feed_url: `${feeds.length} subscribed publications`,
      }
  ...
  const filteredEntries = useMemo(() => {
    return feedEntries.filter((item: LibraryRecord) => {
      const isQueued =
        item.learning_state === 'in_progress' ||
        (item.status === 'active' && !['captured', 'inbox'].includes(String(item.learning_state || 'captured')))
      ...
      if (searchQuery.trim()) {
        ...
        const titleMatch = String(item.video_title || item.title || '')
          .toLowerCase()
          .includes(query)
        const reasonMatch = String(item.why_this || item.reason || '')
          .toLowerCase()
          .includes(query)
        const feedTitleMatch = String(item.feed_title || '')
          .toLowerCase()
          .includes(query)
    ...
    <div class="folio-library-view folio-feeds-view">
      <div class="folio-view-intro">
        <div>
          ...
          <h1>RSS Feeds</h1>
          <p>
            Articles stream directly into your Library as captured sources. Triage them into your Queue or exclude them.
          </p>
        </div>
        <div class="folio-view-intro-actions">
          ...
          <span class="folio-count-readout">
            ...
            <small>
              {feeds.length === 1 ? 'feed' : 'feeds'} · {totalEntries} {totalEntries === 1 ? 'entry' : 'entries'}
            </small>
          </span>
        </div>
      </div>
      ...
      {showSubscribePanel && (
        <section class="folio-shelf folio-feed-subscribe-shelf" aria-labelledby="subscribe-feed-heading">
          <form class="folio-intake-form folio-feed-subscribe-form" onSubmit={submitSubscribe}>
            ...
            <p class="folio-feed-branch-help">
              New source records inherit this reviewed branch. If an article already exists under another reviewed
              branch, its canonical mapping is preserved.
            </p>
            {branchDeck.error && (
              <p class="folio-inline-warning" role="alert">
                Branches could not be loaded. Retry before subscribing.
              </p>
            )}
          </form>
        </section>
      ...
      {!feeds.length && !showSubscribePanel && (
        <div class="folio-feed-empty-state">
          ...
          <p>
            Subscribe to practical RSS or Atom feeds to receive developer, tool, and essay updates for triage into your
            Learning Queue.
          </p>
          ...
          <button type="button" class="folio-button folio-button-primary" onClick={() => setShowSubscribePanel(true)}>
            <Icon name="rss" size={14} />
        ...
        <section class="folio-shelf folio-feed-entries-shelf" aria-labelledby="feed-entries-title">
          <div class="folio-feed-stream-banner">
            <div class="folio-feed-banner-info">
              ...
              <div class="folio-feed-banner-meta">
                <span class="folio-feed-meta-url" title={selectedFeed.feed_url}>
              ...
              {!isAllFeeds && selectedFeed.branch_label && (
                <a
                  class="folio-badge folio-badge-branch folio-feed-branch-pill"
                  href={`#/map/branch/${encodeURIComponent(String(selectedFeed.branch_id))}`}
                >
            ...
            <div class="folio-feed-banner-actions">
              ...
              {handlers.onClearFeedEntries &&
                feedEntries.length > 0 &&
                (confirmAction?.type === 'clear' &&
                confirmAction.id === (isAllFeeds ? 'all' : String(selectedFeed.id)) ? (
                  <div class="folio-feed-confirm-box">
                    <span class="folio-feed-confirm-text">
                      Clear all {entriesTotal} articles{isAllFeeds ? ' across all feeds' : ''}?
                    </span>
                    <button
                      type="button"
                      class="folio-feed-btn folio-feed-btn-danger"
                  ...
                  <button
                    ...
                    class="folio-feed-btn folio-feed-btn-clear"
                    onClick={() =>
                      setConfirmAction({ type: 'clear', id: isAllFeeds ? 'all' : String(selectedFeed.id) })
                    }
                    disabled={handlers.busyId === 'clear-feed-entries'}
                    title={
                      isAllFeeds
                        ? 'Clear all imported articles across all feeds'
                        : 'Clear imported articles for this feed'
                    }
                  >
                    <Icon name="trash" size={13} />
          ...
          {loadingEntries ? (
            ...
            <div class="folio-record-list" aria-label={`Articles from ${selectedFeed.title || selectedFeed.feed_url}`}>
              {filteredEntries.map((item: LibraryRecord) => {
                ...
                const isQueued =
                  item.learning_state === 'in_progress' ||
                  (item.status === 'active' &&
                    !['captured', 'inbox'].includes(String(item.learning_state || 'captured')))
                const isConsumed = item.status === 'consumed'
                const isExcluded = item.status === 'rejected' || item.learning_state === 'excluded'
                const isInbox = !isQueued && !isConsumed && !isExcluded
                ...
                return (
                  <article class="folio-record" key={item.id}>
                    <div class="folio-record-main">
                      <RecordMeta>
                        RSS · {item.feed_title || selectedFeed.title || 'Feed'} ·{' '}
                        {formatDate(item.published_at || item.created_at || item.feed_imported_at)}
                      </RecordMeta>
                      ...
                      {item.branch_label && (
                        <a
                          class="folio-badge folio-badge-branch folio-feed-branch-pill"
                          href={`#/map/branch/${encodeURIComponent(String(item.branch_id))}`}
                        >
                          <span class="badge-format">Branch</span>
                      ...
                      <div class="folio-row-actions">
                          ...
                          <>
                            <button
                              ...
                              onClick={() => {
                                ...
                                setFeedEntries((prev) =>
                                  prev.map((e) =>
                                    e.id === item.id ? { ...e, learning_state: 'in_progress', status: 'active' } : e,
                                  ),
                                )
                              }}
                              disabled={handlers.busyId === item.id}
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setFeedEntries((prev) =>
                                  prev.map((e) =>
                                    e.id === item.id ? { ...e, learning_state: 'excluded', status: 'rejected' } : e,
                                  ),
                                )
                              }}
                              disabled={handlers.busyId === item.id}
                        ...
                        {isQueued && (
                          <a
                            class="folio-button folio-button-primary"
                            href="#/library?mode=queue"
                            title="View in Learning Queue"
                          >
                            In Queue
                          </a>
                        )}
                        {isConsumed && (
                          <span class="folio-feed-status-pill is-consumed">
                            Consumed{' '}
                            {item.user_rating && item.user_rating !== 'unset' ? `(${item.user_rating}/10)` : ''}
                          </span>
                        )}
                        {isExcluded && (
                          <>
                            <span class="folio-feed-status-pill is-excluded">Excluded</span>
                            <button
                              type="button"
                              class="folio-button"
                              onClick={() => {
                        ...
                        <a class="folio-button" href={objectHref('source', String(item.id))}>
                          Record
                        </a>
                      ...
                      {isInbox && (
                        <small class="folio-action-note">
                          Exclude is an administrative archive action. It does not teach Compass that the source was a
                          bad fit.
                        </small>
                      )}
            ...
            <div class="folio-feed-empty-state">
              ...
              <p>
                No articles matched your current status filter ("{statusFilter}") or search query "{searchQuery}".
              </p>
              <button
                type="button"
                class="folio-button"
      ...
      {filtered.length ? (
        <div class="folio-files-ledger" aria-label="Source artifacts">
          {filtered.map((group) => {
            ...
            const primaryHref = htmlFile
              ? artifactLink(htmlFile)
              : pdfFile
                ? artifactLink(pdfFile)
                : artifactLink(primary)
...
function BranchContextBadges({ item }: { item: LibraryRecord }) {
  ...
  return (
    <div class="folio-queue-badges" aria-label="Branch context">
      ...
      {item.note ? (
        <a class="folio-badge folio-badge-note" href="#/learn?mode=practice&focus=notes" title="Open field notes">
          Note taken: {item.note.title}
        </a>
      ) : null}
      {item.recall?.count > 0 ? (
        <a class="folio-badge folio-badge-recall" href="#/learn?mode=practice&focus=recall" title="Open recall deck">
          {item.recall.count} approved {item.recall.count === 1 ? 'card' : 'cards'}
          {item.recall.due > 0 ? ` · ${item.recall.due} due today` : ''}
        </a>
      ) : null}
...
function SourceObject({
  ...
  const companionHtml =
    artifacts.find((artifact: LibraryRecord) => String(artifact.id) === String(companions.html?.id)) ||
    companions.html ||
    null
  const companionPdf =
    artifacts.find((artifact: LibraryRecord) => String(artifact.id) === String(companions.pdf?.id)) ||
    companions.pdf ||
    null
  const offlineResources = sourceOfflineResources(item, companionHtml, companionPdf)
  return (
    <div class="folio-object-sections">
      <section class="folio-object-section">
        ...
        <p class="folio-record-note">Opening this source is passive. Start a tracked learning session from Queue.</p>
      </section>
      {sourceLink(item) && (
        <SourceHealthControl
          sourceId={String(item.id)}
          ...
          companionHref={
            offlineResources.length
              ? companions.html?.id
                ...
                : `/artifacts/${encodeURIComponent(String(companions.pdf.id))}`
              : null
          }
      ...
      {branch && (
        <section class="folio-object-section">
          ...
          <div class="folio-row-actions">
            <a class="folio-button" href={`#/map/branch/${encodeURIComponent(String(branch.id))}`}>
              Open branch dossier
            </a>
          </div>
          <p class="folio-record-note">
            {branch.status === 'pruned'
              ? 'This branch is pruned — review the mapping before starting.'
              : branch.status && branch.status !== 'unverified'
                ? `Branch status: ${branch.status.replace(/_/g, ' ')}.`
                : 'Branch match is not verified yet.'}
          </p>
        </section>
      )}
      {(companions.html || companions.pdf) && (
        <section class="folio-object-section">
          <div class="folio-section-heading">
            <h2>Reading companions</h2>
            <span>
              {[(companions.html && 'HTML') || null, (companions.pdf && 'PDF') || null].filter(Boolean).join(' + ')}
            </span>
          </div>
          <div class="folio-row-actions">
            {companions.html && (
            ...
            {companions.pdf && (
              <a
                ...
                rel="noreferrer"
              >
                Download A4 PDF{companions.pdf.size_bytes ? ` · ${formatBytes(companions.pdf.size_bytes)}` : ''}
              </a>
            )}
          </div>
          <OfflinePackControl
            ...
            resources={offlineResources}
          />
          <p class="folio-record-note">Canonical Arabic reading companion rendered from one verified body.</p>
        </section>
      )}
      <section class="folio-object-section">
        <div class="folio-section-heading">
          <h2>Recall cards</h2>
          <span>
            {recall.count} approved{recall.due > 0 ? ` · ${recall.due} active due` : ''}
          </span>
        </div>
        {(record.srs?.cards || []).length ? (
          <ul class="folio-recall-list">
            {(record.srs.cards || []).map((card: LibraryRecord) => (
              <li key={card.id}>
                <strong>{card.question}</strong>
                <span>
                  Topic: {card.topic || 'General'} · {recallScheduleLabel(card)} · {card.repetitions} reps
                </span>
              </li>
            ))}
          </ul>
        ...
        {recall.count === 0 && drafts.length === 0 && (
          <div class="folio-row-actions">
      ...
      {thread && (
        <section class="folio-object-section">
          ...
          <a class="folio-linked-object" href={`#/learn/thread/${encodeURIComponent(String(thread.id))}`}>
            ...
            <span>
              {thread.role || 'Attached source'} · {formatStatus(thread.status)}
            </span>
          </a>
      ...
      <section class="folio-object-section">
        ...
        {artifacts.length ? (
          artifacts.map((file: LibraryRecord) => (
            <a class="folio-linked-object" href={artifactLink(file)} target="_blank" rel="noreferrer" key={file.id}>
              <strong>{file.filename || fileKind(file)}</strong>
              <span>{fileKind(file)} · passive open</span>
            </a>
          ))
        ) : (
      ...
      {notes.map((note: LibraryRecord) => (
        <section class="folio-object-section" key={note.id}>
          <div class="folio-section-heading">
            <h2>{note.kind === 'reflection' ? 'Reflection' : 'Extracted note'}</h2>
            <span>{formatStatus(note.status || 'draft')}</span>
          </div>
          {(note.sections || []).map((section: LibraryRecord) => (
            <div class="folio-bilingual-block" dir={section.direction || 'auto'} key={section.section_key}>
    ...
    <div class="folio-object-sections">
      <section class="folio-object-section">
        ...
        <dl class="folio-property-list">
          ...
          <div>
            <dt>Source</dt>
            <dd>{metadata.source_title || metadata.source_url || 'Not linked'}</dd>
          </div>
          <div>
            <dt>Created</dt>
...
function BookObject({
  ...
  return (
    <div class="book-overview-fold book-dossier">
      ...
      {progress.total > 0 && (
        <section class="book-overview-progress" aria-label="Reading progress">
          <div>
            <span>
      ...
      {nextChapter ? (
        <section class="book-overview-next" aria-labelledby="book-next-chapter-title">
          <div>
        ...
        <section class="book-overview-next is-empty">
          <div>
      ...
      <nav class="book-dossier-index" aria-label="Book hub sections">
        {(
          ...
          <button type="button" key={section} onClick={() => jump(`book-${section}`)}>
            {label}
          </button>
        ))}
      </nav>
      ...
      <div class="book-dossier-layout">
        <main class="book-dossier-main">
          ...
          <section id="book-study" class="book-dossier-section">
            ...
            {notes.length ? (
              <details class="book-dossier-notes">
                <summary>
        ...
        <aside class="book-dossier-aside" aria-label="Book learning context">
          <section id="book-recall" class="book-dossier-side-section">
            ...
            {cards.length ? (
              <ul class="folio-recall-list">
                {cards.slice(0, 6).map((card: LibraryRecord) => (
                  <li key={card.id}>
          ...
          <section id="book-history" class="book-dossier-side-section">
            ...
            {sessions.length ? (
              ...
              <p class="folio-record-note">
                No tracked sessions yet. Start from Queue when this book earns active attention.
              </p>
            )}
          </section>
          ...
          <section id="book-files" class="book-dossier-side-section">
            ...
            {artifacts.length ? (
              <div class="book-file-list">
                {artifacts.map((artifact: LibraryRecord) => {
                  const artifactMetadata = parseMetadata(artifact.metadata_json)
                  return (
                    <a
                      class="folio-linked-object"
                      ...
                      key={artifact.id}
                    >
                      <strong>{artifact.filename || fileKind(artifact)}</strong>
                      <span>
                        {artifactMetadata.chapter_title || artifactMetadata.role || fileKind(artifact)}
