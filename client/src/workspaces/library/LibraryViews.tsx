  ...
  return (
    <div class={`folio-library-view folio-queue-view folio-queue-view-${viewMode}`}>
      ...
      {items.length ? (
        ...
        <ViewEmpty title="Queue is clear" body="A source appears here only after a deliberate commitment." />
      )}
      {handlers.notice && (
...
export function FeedsView({ data, handlers }: { data: LibraryRecord; handlers: LibraryViewHandlers }) {
  ...
  const totalEntries = feeds.reduce((sum: number, feed: LibraryRecord) => sum + Number(feed.entry_count || 0), 0)
            ...
            <section class="folio-shelf folio-feeds-shelf" aria-labelledby="feeds-shelf-title">
              ...
              <div class="folio-feeds-list" role="list">
                {feeds.map((feed: LibraryRecord) => {
                  ...
                  return (
                    <article
                      class={`folio-record folio-feed-record${isSelected ? ' is-selected' : ''}`}
                      key={feed.id}
                      role="listitem"
                    >
                      <div class="folio-record-main">
                        <RecordMeta>
                          {feed.entry_count || 0} {(feed.entry_count || 0) === 1 ? 'entry' : 'entries'} · Last checked{' '}
                          {feed.last_checked_at ? formatDate(feed.last_checked_at) : 'never'}
                        </RecordMeta>
                        ...
                        {feed.branch_label && (
                          <a
                            ...
                            href={`#/map/branch/${encodeURIComponent(String(feed.branch_id))}`}
                          >
                            <span class="badge-format">Default branch</span>
                            <span>{feed.branch_label}</span>
                          </a>
                        )}
                          ...
                          {handlers.onSyncFeed && (
                            ...
                            >
                              <Icon
                                name="sync"
                                size={14}
                                class={handlers.busyId === `sync:${feed.id}` ? 'folio-icon-spin' : ''}
                              />
                  ...
                  <article class="folio-record" key={item.id}>
                    <div class="folio-record-main">
                      ...
                      <div class="folio-row-actions">
                        ...
                        {isConsumed && <span class="folio-record-note">Consumed</span>}
                        {isExcluded && <span class="folio-record-note">Excluded</span>}
                        {href && (
    ...
    <div class="folio-object-sections">
      ...
      <section class="folio-object-section">
        <div class="folio-section-heading">
          <h2>Active recall</h2>
          <span>
            {recall.count} approved{recall.due > 0 ? ` · ${recall.due} due` : ''}
          </span>
        </div>
          ...
          <ul class="folio-recall-list">
            {(record.srs.cards || []).map((card: LibraryRecord) => (
              <li key={card.id}>
                ...
                <span>
                  Topic: {card.topic || 'General'} · Due {formatDate(card.due_at)} · {card.repetitions} reps
                </span>
              </li>
            ))}
          </ul>
    ...
    try {
      await api('/annotations', {
        ...
        body: JSON.stringify({
          ...
          selector: locator.trim() ? { locator: locator.trim() } : {},
          quote: quote.trim(),
          created_by: 'user',
        }),
      })
      setQuote('')
      setLocator('')
      setNotice('Passage saved to the evidence ledger. Hermes can now ground a proposal in it.')
      await load()
    } catch (error: any) {
          ...
          {annotations.slice(0, 8).map((annotation) => (
            <article key={annotation.id}>
              ...
              <small>
                ...
                {annotation.selector?.locator || 'Locator not recorded'} · {formatDate(annotation.created_at)}
              </small>
              <div class="folio-row-actions">
                <a
                  class="folio-button"
        ...
        <aside class="book-dossier-aside" aria-label="Book learning context">
          <section id="book-recall" class="book-dossier-side-section">
            <div class="book-dossier-section-head">
              <h2>Recall</h2>
              <span>
                {recall.count} approved{recall.due ? ` · ${recall.due} due` : ''}
              </span>
            </div>
            {cards.length ? (
              <ul class="folio-recall-list">
                {cards.slice(0, 6).map((card: LibraryRecord) => (
                  <li key={card.id}>
                    ...
                    <span>
                      {card.topic || 'General'} · Due {formatDate(card.due_at)}
                    </span>
                  </li>
                ))}
              </ul>
