...
export function bookReadingState(book: BookRecord): PersonalBookState {
  const explicit = String(book.reading_state || '').trim().toLowerCase()
...
export function bookChapters(book: BookRecord) {
  ...
  return chapters.flatMap((chapter) => {
    const number = [chapter.position, chapter.number, chapter.chapter_number]
      .map(Number)
...
export function bookProgress(book: BookRecord) {
  ...
  if (canonical && Number.isFinite(Number(canonical.total))) {
    ...
    return { completed, finished: completed, total, percent: total ? Number(canonical.percent ?? Math.round((completed / total) * 100)) : 0 }
  }
...
export function chapterActionCopy(book: BookRecord, chapter = bookNextChapter(book)) {
  ...
  if (chapter.completed || chapter.completed_at || bookReadingState(book) === 'finished' || (progress.total > 0 && progress.completed === progress.total)) return `Review${suffix}`
