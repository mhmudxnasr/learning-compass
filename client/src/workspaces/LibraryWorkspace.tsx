...
export function LibraryWorkspace({ route, embedded = false, onInspect, onSelect, onNavigate }: LibraryWorkspaceProps) {
  ...
  useEffect(() => {
    if (objectType !== 'source' || data?.item?.content_type !== 'book' || String(data.item.id) !== activeRoute.objectId) return
  ...
  const modeSwitcher = embedded || objectType ? null : <LibraryModeSwitcher activeView={view} objectType={objectType} onNavigate={onNavigate} />
