...
import { buildSourceMaterialLauncher, type SourceMaterialKind, type SourceMaterialOption } from './learn/sourceMaterials'
...
function MaterialDock({ source, label = 'Available lesson materials' }: { source?: PathSource | null; label?: string }) {
  const launcher = source ? buildSourceMaterialLauncher(source) : null
  if (!launcher) return null
  const materials = [launcher.primary, ...launcher.alternatives]
  return <div class="continuum-material-dock" aria-label={label}>
    {materials.map((option) => <MaterialLink key={`${option.kind}-${option.href}`} option={option}/>)}
  </div>
}
...
export function HomeWorkspace({ onCapture, onInspect, onNavigate }: HomeWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    const remainsCurrent = threads.some((thread) => thread.current_stage?.lessons?.[0]?.id === lessonCompletion.lessonId)
  ...
  const htmlFile = activeSource?.artifacts?.html || files.find((file: LibraryRecord) => String(file.role || '').toLowerCase() === 'html' || /html/i.test(String(file.media_type || file.filename || '')))
  const pdfFile = activeSource?.artifacts?.pdf || files.find((file: LibraryRecord) => String(file.role || '').toLowerCase() === 'pdf' || /pdf/i.test(String(file.media_type || file.filename || '')))
  ...
  const finishLesson = async (threadId: string, lesson: ThreadLesson) => {
    ...
    try {
      ...
      setLessonCompletionError({ lessonId: lesson.id, message: reason instanceof Error ? reason.message : 'Lesson completion failed.' })
    }
  }

  return <div class="folio-home-workspace continuum-home">
    <header class="folio-home-header continuum-home-header">
      ...
      <div class="continuum-home-status" aria-label="Today’s study state">
        <span><i class="continuum-live-dot"/>{readyLessons || 0} {readyLessons === 1 ? 'turn' : 'turns'} ready</span>
      ...
      <main class="folio-home-main continuum-home-main">
        {resurfacingItem && <section class="folio-home-resurfacing" aria-labelledby="home-resurfacing-title">
          ...
          <article class="folio-resurfacing-card">
            <div class="folio-resurfacing-copy">
              ...
              <h3><a class="item-title-link" href={itemHref(resurfacingItem)}>{resurfacingItem.title}</a></h3>
        ...
        <section class="folio-home-threads continuum-turns" aria-labelledby="home-threads-title">
          <div class="folio-section-heading folio-home-threads-heading">
            ...
            <a class="folio-heading-link" href={routeHref('learn', 'paths')} title="Browse Threads" aria-label="Browse Threads"><Icon name="learn" size={19}/></a>
          </div>
          {threads.length ? <div class="folio-home-thread-list" role="list">
            {threads.map((thread, index) => {
              ...
              const completionPhase = lesson && lessonCompletion?.lessonId === lesson.id ? lessonCompletion.phase : null
              const completionError = lesson && lessonCompletionError?.lessonId === lesson.id ? lessonCompletionError.message : ''
              const canFinish = Boolean(lesson && stage?.status === 'in_progress' && lessonReadiness(lesson) !== 'needs_material')
              return <article key={`${thread.id}:${lesson?.id || 'path'}`} class={`folio-home-thread-lesson continuum-turn${lesson?.status === 'in_progress' ? ' is-active' : ''}${completionPhase === 'saving' ? ' is-finishing' : ''}${completionPhase === 'finished' ? ' is-finished' : ''}`} role="listitem" aria-busy={completionPhase === 'saving' || undefined}>
                <span class="continuum-turn-number" aria-hidden="true">{String(index + 1).padStart(2, '0')}</span>
                ...
                {(primarySource || canFinish || completionError) && <div class="continuum-turn-tools">
                  ...
                  >
                    <span class="continuum-finish-mark" aria-hidden="true"><Icon name="check" size={13}/></span>
                    <span>{completionPhase === 'finished' ? 'Completed' : completionPhase === 'saving' ? 'Saving…' : 'Complete'}</span>
                  </button>}
                  {completionError && <p class="continuum-turn-error" role="alert">{completionError}</p>}
                </div>}
              </article>
            })}
        ...
        <section class="folio-home-feeds continuum-quiet-section" aria-labelledby="home-feeds-title">
          <div class="folio-section-heading">
            ...
            <a class="folio-heading-link" href={routeHref('library', 'triage', 'feeds')} title="Open Feeds" aria-label="Open Feeds"><Icon name="rss" size={19}/></a>
          </div>
          {feeds.length ? <div class="folio-home-feed-list" role="list" aria-label="Subscribed RSS feeds">
            {feeds.map((feed: LibraryRecord) => <a class="folio-home-feed-item" href={routeHref('library', 'triage', 'feeds')} role="listitem" key={feed.id}>
              <span class="folio-home-feed-copy"><strong>{feed.title || feed.feed_url}</strong><small>{feed.entry_count || 0} {Number(feed.entry_count || 0) === 1 ? 'entry' : 'entries'}</small></span>
              <Icon name="chevron" size={15}/>
            </a>)}
          </div> : <p>No feed subscriptions yet. Add a publication to keep incoming reading in one deliberate stream.</p>}
        </section>
      </main>
          ...
          <div class="folio-home-focus-copy">
            <div class="folio-section-heading">
              <div>
                ...
                <h2 id="home-focus-title">{activeSource ? <a class="item-title-link" href={itemHref(activeSource)}>{sourceTitle(activeSource)}</a> : 'No active source'}</h2>
              </div>
              {activeSource && <span class="folio-status-mark">{activeSource.learning_state === 'in_progress' ? 'In progress' : 'Queued'}</span>}
            </div>
            {activeSource ? <>
              <p class="folio-record-meta">{sourceCreator(activeSource)} · {sourceFormat(activeSource)}{activeSource.estimated_minutes ? ` · ~${activeSource.estimated_minutes} min` : ''}</p>
              <p class="folio-home-rationale">{activeSource.context_brief || activeSource.why_this || 'The most immediate useful commitment on your shelf.'}</p>
              <MaterialDock source={activeMaterialSource} label="Current source materials"/>
              {otherFiles.length > 0 && <div class="continuum-extra-files" aria-label="Other source files">
                {otherFiles.map((file: LibraryRecord) => <a href={`/artifacts/${file.id}`} target="_blank" rel="noreferrer" key={file.id} title={file.filename || sourceFileLabel(file)}><Icon name="file" size={14}/><span>{sourceFileLabel(file)}</span></a>)}
              </div>}
              <div class="folio-home-actions-bar">
                <a class="folio-button" href={itemHref(activeSource)}>Open item</a>
              ...
              <a class="continuum-all-files" href={itemHref(activeSource, 'files')}>Item files</a>
            </> : <Empty
              title="The working shelf is clear"
              body="Save a source, then commit it to Queue when it earns your attention."
              action={<button type="button" class="folio-button folio-button-primary" onClick={() => onCapture ? onCapture() : navigate(routeHref('library', 'catalog', 'all'), onNavigate)}>Save a source</button>}
            />}
          </div>
        </section>
        ...
        <section class="folio-home-queue continuum-queue" aria-labelledby="home-queue-title">
          <div class="folio-section-heading">
            <div>
              ...
              <h2 id="home-queue-title">{items.length ? `${items.length} ${items.length === 1 ? 'item' : 'items'}` : 'Queue is empty'}</h2>
            </div>
            <a class="folio-heading-link" href={routeHref('library', 'triage', 'queue')} title="Open Queue" aria-label="Open Queue"><Icon name="queue" size={19}/></a>
          </div>
          {items.length ? <div class="folio-home-queue-list" role="list">
            {items.map((item: LibraryRecord, index: number) => {
              const isSelected = activeSource && String(activeSource.id) === String(item.id)
              return <div key={item.id} role="listitem" class={`folio-home-queue-item${isSelected ? ' is-active' : ''}`}>
                <span class="continuum-queue-index">{String(index + 1).padStart(2, '0')}</span>
                <a class="folio-home-queue-item-title item-title-link" href={itemHref(item)}>{sourceTitle(item)}</a>
                <button type="button" class="item-focus-button" onClick={() => setSelectedSourceId(String(item.id))} aria-label={`Focus ${sourceTitle(item)}`} aria-pressed={Boolean(isSelected)}>{isSelected ? 'Active' : 'Focus'}</button>
              </div>
            })}
          </div> : <p class="folio-record-note">Add a source only when it has earned your attention.</p>}
        </section>
      </aside>
    </div>
  </div>
