...
import { objectHref, sourceCreator, sourceLink, sourceTitle, type LibraryRecord } from './library/types'
...
function targetHref(target: string | undefined, recommendationId?: string) {
  if (recommendationId) return objectHref('source', recommendationId)
  if (target === 'learn.recall') return '#/learn/recall'
  if (target === 'curate.queue') return '#/library'
  if (target === 'curate.inbox') return '#/library/inbox'
  if (target === 'map.atlas') return '#/map'
  if (target === 'insights.taste') return '#/settings/profile'
  return '#/library'
}
