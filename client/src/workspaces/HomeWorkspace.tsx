...
import { sourceCreator, sourceTitle, type LibraryRecord } from './library/types'
...
export function HomeWorkspace({ onCapture, onInspect, onNavigate }: HomeWorkspaceProps) {
  ...
  const resurfacingSource = resurfacingData?.item

  useEffect(() => {
    setResurfacingItem(resurfacingSource || null)
  }, [resurfacingSource])
  useEffect(() => {
    const item = resurfacingSource
  ...
  const htmlFile = files.find(
    (f: LibraryRecord) =>
      String(f.role || '').toLowerCase() === 'html' || /html/i.test(String(f.media_type || f.filename || '')),
  )
  const pdfFile = files.find(
    (f: LibraryRecord) =>
      String(f.role || '').toLowerCase() === 'pdf' || /pdf/i.test(String(f.media_type || f.filename || '')),
  )
  ...
  return (
    <div class="folio-home-workspace">
      <header class="folio-home-header">
        <div>
          <p class="folio-kicker">Working ledger</p>
          <h1>Today</h1>
        </div>
        <p class="folio-home-intro">
          Resume the work already in motion. Everything else stays quiet until you ask for it.
        </p>
      </header>
      ...
      <section class="folio-home-focus" aria-labelledby="home-focus-title">
        <div class="folio-home-focus-copy">
          ...
          {activeSource ? (
            <>
              <p class="folio-record-meta">
                {sourceCreator(activeSource)} · {activeSource.content_type || 'Source'}
              ...
              <p class="folio-home-rationale">
                {activeSource.context_brief ||
                  ...
                  'This source is next because it is the most immediate useful commitment on your shelf.'}
              </p>
              <div class="folio-home-actions-bar">
                ...
                <div class="folio-quick-links" aria-label="Source links and companions">
                  ...
                  {pdfFile && (
                    <a
                      class="folio-quick-link"
                      href={`/artifacts/${pdfFile.id}`}
                      target="_blank"
                      rel="noreferrer"
                      title="Open PDF companion"
                    >
                  ...
                  {notebookUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      title="Open Google NotebookLM"
                    >
                      <Icon name="spark" size={14} />
                      <span>NBLM</span>
                    </a>
                  )}
                  <a class="folio-quick-link folio-quick-link-all" href={routeHref('library', 'assets', 'files')}>
                    All files
                  </a>
                </div>
              </div>
            ...
            <Empty
              ...
              body="Save a source, open its record, and commit it to Queue when it earns your attention."
              action={
                <button
                  type="button"
                  class="folio-button folio-button-primary"
      ...
      <div class="folio-home-sequence">
        <section class="folio-home-threads" aria-labelledby="home-threads-title">
          ...
          {threads.length ? (
            <div class="folio-home-thread-list">
              {threads.map((thread) => {
                ...
                return (
                  <a
                    key={thread.id}
                    class={`folio-home-thread-lesson${lesson?.status === 'in_progress' ? ' is-active' : ''}`}
                    href={href}
                    title={`Open ${thread.title}: ${title}`}
                    role="listitem"
                  >
          ...
          <div class="folio-section-heading">
            <div>
              ...
              <h2 id="home-queue-title">
                {items.length ? `${items.length} ${items.length === 1 ? 'item' : 'items'} in queue` : 'Queue is empty'}
              </h2>
            </div>
            <a
              class="folio-heading-link"
