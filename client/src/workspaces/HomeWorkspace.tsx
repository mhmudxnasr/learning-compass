...
export function HomeWorkspace({ onCapture, onInspect, onNavigate }: HomeWorkspaceProps) {
  ...
  return <div class="folio-home-workspace">
    <header class="folio-home-header">
      <h1>Home</h1>
      <button type="button" class="folio-button folio-button-primary" onClick={() => onCapture ? onCapture() : navigate(routeHref('library', 'triage', 'queue'), onNavigate)}>
    ...
    <section class="folio-home-focus" aria-labelledby="home-focus-title">
      <div class="folio-home-focus-copy">
        ...
        </> : <Empty title="The working shelf is clear" body="Capture or curate one source when you are ready to make a deliberate commitment." action={<button type="button" class="folio-button folio-button-primary" onClick={() => onCapture ? onCapture() : navigate(routeHref('library', 'triage', 'queue'), onNavigate)}>Capture a source</button>}/>}
      </div>
    </section>
    ...
    <section class="folio-home-capture-signal" aria-label="Capture signal"><div><p class="folio-kicker">Capture signal</p><h2>Feeds & Subscriptions</h2><p>Manage RSS feeds and imported articles directly in Triage.</p></div><div class="folio-row-actions"><button type="button" class="folio-button folio-button-primary" onClick={() => onCapture ? onCapture() : navigate(routeHref('library', 'triage', 'queue'), onNavigate)}><Icon name="capture" size={16}/>Capture</button><a class="folio-button" href={routeHref('library', 'triage', 'feeds')}>Open Feeds</a></div></section>
  </div>
}
