...
export function LearnWorkspace({ route }: LearnWorkspaceProps = {}) {
  ...
  const content =
    incomingAnchorError === 'selection_too_large' ? (
      <Empty
        title="Selection is too long to anchor"
        body={`The selected passage has ${Number.isFinite(incomingAnchorLength) ? incomingAnchorLength.toLocaleString() : 'more than 10,000'} characters. Source anchors preserve exact passages and accept at most 10,000, so nothing was truncated or saved.`}
        action={
          <a class="button secondary" href="#/learn?mode=practice&focus=notes">
            Return to Notes
          </a>
        }
      />
    ) : shareIntakeId ? (
      <ShareAnchorLearningAction shareIntakeId={shareIntakeId} />
    ) : hasAnchorAction ? (
      <SourceAnchorLearningAction
        annotationId={annotationId}
        incomingUrl={incomingAnchorUrl}
        incomingQuote={incomingAnchorQuote}
        incomingTitle={incomingAnchorTitle}
      />
...
function annotationLocator(annotation: SourceAnnotation) {
  return String(
    annotation.selector?.locator || annotation.selector?.url || annotation.source_url || 'Exact source passage',
  )
}
...
function ShareAnchorLearningAction({ shareIntakeId }: { shareIntakeId: string }) {
  ...
  const reloadIntake = intakeState.reload
  ...
  useEffect(() => {
    ...
    api(`/api/share-intakes/${encodeURIComponent(intake.id)}/consume`, {
      method: 'POST',
      body: JSON.stringify({ annotation_id: recoverableAnnotationId }),
    })
      .then(() => reloadIntake())
      .catch(() => undefined)
  }, [intake, completionKind, recoverableAnnotationId, reloadIntake])
  ...
  if (!intake || completionKind !== 'anchor')
    return (
      <Empty
        title="Shared passage unavailable"
        body="This share receipt is missing or does not contain an anchored passage."
        action={
          <a class="button secondary" href="#/learn?mode=practice&focus=notes">
            Return to Notes
          </a>
  ...
  if (intake.status === 'consumed' && !recoverableAnnotationId)
    return (
      <Empty
        title="Shared passage completed"
        body="Its saved source anchor is no longer available."
        action={
          <a class="button secondary" href="#/learn?mode=practice&focus=notes">
            Return to Notes
          </a>
...
function SourceAnchorLearningAction({
  annotationId,
  incomingUrl,
  incomingQuote,
  incomingTitle,
  ...
  const annotationState = useData<{ annotation: SourceAnnotation }>(
    effectiveId ? `/annotations/${encodeURIComponent(effectiveId)}` : undefined,
  )
  const resolution = useData<{
    found: boolean
    source?: {
      id: string
      title?: string
  ...
  }>(!effectiveId && incomingUrl ? `/annotations/resolve?source_url=${encodeURIComponent(incomingUrl)}` : undefined)
  ...
  const saveIncomingAnchor = async () => {
    ...
    try {
      ...
      const payload = await api<{ annotation: SourceAnnotation }>('/annotations', {
        ...
        body: JSON.stringify({
          ...
          selector: {
            locator: incomingUrl,
            url: incomingUrl,
            text_quote: { exact: incomingQuote.trim() },
            ...(shareIntakeId ? { share_intake_id: shareIntakeId } : {}),
          },
      ...
      if (shareIntakeId) {
        try {
          await api(`/api/share-intakes/${encodeURIComponent(shareIntakeId)}/consume`, {
            method: 'POST',
            body: JSON.stringify({ annotation_id: payload.annotation.id }),
          })
        } catch {
          intakePending = true
        }
      ...
      setNotice(error?.message || 'The source anchor could not be saved.')
    } finally {
      setWorking(false)
    }
  }
  ...
  if (!effectiveId) {
    ...
    if (!resolution.data?.found || !resolution.data.source?.branch_verified) {
      ...
      return (
        <article class="learn-object-detail source-anchor-action">
          ...
          <p>
            {needsBranch
              ? 'Capture the same URL under a reviewed, non-pruned branch before anchoring the passage.'
              : 'Capture the source under a reviewed branch first.'}{' '}
            This tab keeps the selected passage while Capture opens separately.
          </p>
          <div class="folio-row-actions">
            <a class="folio-button folio-button-primary" href={captureHref} target="_blank" rel="noreferrer">
              {needsBranch ? 'Review source branch' : 'Capture source'}
            </a>
            <button type="button" class="folio-button" onClick={resolution.reload}>
              Check again
            </button>
          </div>
        </article>
    ...
    return (
      <article class="learn-object-detail source-anchor-action">
        ...
        <dl>
          ...
          {resolution.data.source?.branch_label && (
            <div>
              <dt>Branch</dt>
              <dd>{resolution.data.source.branch_label}</dd>
            </div>
          )}
        </dl>
        <p>
          Saving preserves this exact passage and locator. It does not create a note, Learning Unit, or recall card.
        </p>
        ...
        <div class="folio-row-actions">
          <button
            ...
            onClick={saveIncomingAnchor}
            disabled={working}
          >
            {working ? 'Saving…' : 'Save source anchor'}
          </button>
          <a class="folio-button" href={`#/library/source/${encodeURIComponent(resolution.data.source?.id || '')}`}>
            Open source record
          </a>
        </div>
      </article>
  ...
  if (annotationState.error && !annotationState.data)
    return <ErrorState message={annotationState.error} retry={annotationState.reload} />
  const annotation = annotationState.data?.annotation
  if (!annotation || annotation.status === 'archived')
    return (
      <Empty
        title="Source anchor unavailable"
        body="This anchor was archived or no longer exists."
        action={
          <a class="button secondary" href="#/learn?mode=practice&focus=notes">
            Return to Notes
          </a>
  ...
  return (
    <article class="learn-object-detail source-anchor-action">
      ...
      <dl>
        <div>
          <dt>Locator</dt>
          <dd>{annotationLocator(annotation)}</dd>
        </div>
        {annotation.branch_label && (
          <div>
            <dt>Branch</dt>
            <dd>{annotation.branch_label}</dd>
          </div>
        )}
        {annotation.source_checksum && (
          <div>
            <dt>Checksum</dt>
            <dd>{annotation.source_checksum.slice(0, 16)}…</dd>
          </div>
        )}
      </dl>
      ...
      <div class="folio-row-actions" role="group" aria-label="Use source anchor">
        <button
          type="button"
          class={`folio-button${action === 'note' ? ' folio-button-primary' : ''}`}
          aria-pressed={action === 'note'}
          onClick={() => {
            setAction('note')
            setNotice('')
          }}
        ...
        <button
          type="button"
          class={`folio-button${action === 'unit' ? ' folio-button-primary' : ''}`}
          aria-pressed={action === 'unit'}
          onClick={() => {
            setAction('unit')
            setNotice('')
          }}
        ...
        <button
          type="button"
          class={`folio-button${action === 'card' ? ' folio-button-primary' : ''}`}
          aria-pressed={action === 'card'}
          onClick={() => {
            setAction('card')
            setNotice('')
          }}
        ...
        <a
          class="folio-button"
          href={`#/library/source/${encodeURIComponent(annotation.recommendation_id)}?annotation=${encodeURIComponent(annotation.id)}`}
        >
          Return to source
        </a>
      </div>
...
function AnchoredNoteForm({
  ...
  const [title, setTitle] = useState(
    annotation.source_title ? `Note on ${annotation.source_title}` : 'Source-anchored note',
  )
    ...
    try {
      const result = await api<{ id: string }>('/notes', {
        ...
        body: JSON.stringify({
          ...
          kind: 'note',
          status: 'draft',
          recommendation_id: annotation.recommendation_id,
          branch_id: annotation.branch_id || undefined,
          thread_id: annotation.thread_id || undefined,
          source_url: annotation.source_url || undefined,
          provenance: [
            {
              annotation_id: annotation.id,
              reason: 'Learner explicitly created this note from the source anchor.',
              confidence: 1,
            },
          ],
          sections: [
      ...
      onNotice(error?.message || 'The note could not be created.')
    } finally {
      setSaving(false)
    }
  }
  return (
    <form class="source-annotation-form" onSubmit={submit}>
      ...
      <label>
        ...
        <textarea
          ...
          maxLength={12000}
          dir="auto"
          onInput={(event) => setLearnerNote((event.currentTarget as HTMLTextAreaElement).value)}
          placeholder="Explain, question, or apply the passage in your own words…"
          required
        />
      </label>
      <button
        type="submit"
        class="folio-button folio-button-primary"
        disabled={saving || !title.trim() || !learnerNote.trim()}
      >
    ...
    try {
      const result = await api<{ id: string }>('/learning/core/units', {
        ...
        body: JSON.stringify({
          ...
          user_synthesis: synthesis.trim() || undefined,
          stance: 'uncertain',
          confidence: 0.5,
          status: 'draft',
          created_by: 'user',
          ...
          anchors: [
            {
              annotation_id: annotation.id,
              artifact_id: annotation.artifact_id || undefined,
              anchor_type: unitAnchorType(annotation.locator_type),
              locator: annotationLocator(annotation),
              excerpt: annotation.quote,
              checksum: annotation.source_checksum || undefined,
            },
          ],
        }),
      })
      ...
      onNotice(error?.message || 'The Learning Unit could not be created.')
    } finally {
      setSaving(false)
    }
  }
  return (
    <form class="source-annotation-form" onSubmit={submit}>
      ...
      <div class="source-annotation-fields">
        <label>
          Type
          <select value={unitType} onChange={(event) => setUnitType((event.currentTarget as HTMLSelectElement).value)}>
      ...
      <label>
        ...
        <textarea
          ...
          maxLength={12000}
          dir="auto"
          onInput={(event) => setStatement((event.currentTarget as HTMLTextAreaElement).value)}
          placeholder="State the idea precisely in your own words…"
          required
        />
      </label>
      <label>
        Your synthesis <small>optional</small>
        <textarea
          value={synthesis}
          maxLength={12000}
          dir="auto"
          onInput={(event) => setSynthesis((event.currentTarget as HTMLTextAreaElement).value)}
        />
      </label>
      <button type="submit" class="folio-button folio-button-primary" disabled={saving || !statement.trim()}>
        {saving ? 'Creating…' : 'Create draft Unit'}
      </button>
    </form>
  )
}

function AnchoredCardForm({
  annotation,
  onNotice,
}: {
  annotation: SourceAnnotation
    ...
    try {
      const result = await api<{ card_id: string }>('/learning/srs/create', {
        ...
        body: JSON.stringify({
          ...
          answer: answer.trim(),
          topic: annotation.source_title || 'source anchor',
          branch: annotation.branch_id || undefined,
          source_anchor: annotationLocator(annotation),
        }),
      })
      window.location.hash = `/learn/card/${encodeURIComponent(result.card_id)}`
    } catch (error: any) {
      onNotice(error?.message || 'The recall card could not be created.')
    } finally {
      setSaving(false)
    }
  }
  return (
    <form class="source-annotation-form" onSubmit={submit}>
      ...
      <p>
        The question and answer must be learner-authored Arabic. The exact source anchor remains attached as provenance.
      </p>
      <label>
        Question
        <textarea
      ...
      <label>
        ...
        <textarea
          ...
          dir="rtl"
          lang="ar"
          onInput={(event) => setAnswer((event.currentTarget as HTMLTextAreaElement).value)}
          required
        />
      </label>
      <button
      ...
      >
        {saving ? 'Creating…' : 'Create recall card'}
      </button>
    </form>
  )
}
