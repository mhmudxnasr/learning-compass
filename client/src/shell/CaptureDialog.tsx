...
export function CaptureDialog({
  ...
  const branchDeck = useData<{
    existing?: Array<{ id: string; label: string; category_label?: string; status?: string }>
  }>(open ? '/brain/branch-deck' : undefined)
  const branchOptions = (branchDeck.data?.existing || []).filter(
  ...
  const chooseKind = (next: CaptureKind) => {
    ...
    window.requestAnimationFrame(() =>
      document.getElementById(next === 'source' ? 'capture-source-input' : 'capture-personal-title')?.focus(),
    )
  }
  ...
  const reset = () => {
    ...
    setTitle('')
    setCreator('')
    setPersonalState('planned')
    setUrl('')
    setReleaseYear('')
    setDurationMinutes('')
    setProgressCurrent('')
    setProgressTotal('')
    setProgressUnit('items')
    setRating('')
    setTags('')
    setPersonalNote('')
      ...
      if (kind === 'source') {
        ...
        if (shareIntakeId) {
          try {
            await api(`/api/share-intakes/${encodeURIComponent(shareIntakeId)}/consume`, {
              method: 'POST',
              body: JSON.stringify({ recommendation_id: result.id }),
            })
          } catch {
            intakePending = true
          }
        ...
        setStatus(
          intakePending
            ...
            : result?.duplicate
              ? 'Source already exists. Existing record preserved.'
              : 'Source saved.',
        )
      } else {
        await api('/capture/personal', {
          ...
          body: JSON.stringify({
            ...
            progress_current: progressCurrent || undefined,
            progress_total: progressTotal || undefined,
  ...
  return (
    ...
    >
      <section
        ref={dialogRef}
        class="dialog capture-dialog capture-dialog-advanced"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header class="capture-dialog-head">
          <div class="capture-dialog-heading">
            ...
            <h2 id="capture-title">Add to Learning Compass</h2>
          </div>
          <button
            type="button"
        ...
        <p class="capture-dialog-lede">
          Save a source, or log something you read, watched, listened to, studied, or played. You can refine every field
          later.
        </p>
        ...
        <form class="capture-dialog-form" onSubmit={submit}>
            ...
            <div class="capture-kind-grid">
              {captureKinds.map((item) => (
                <button
                  key={item.key}
                  type="button"
                  class={kind === item.key ? 'capture-kind-option is-active' : 'capture-kind-option'}
            ...
            <>
              <div class="capture-input-group">
                ...
                <textarea
                  ...
                  placeholder="Paste a URL, title, or note…"
                  rows={3}
                />
              </div>
                ...
                {file ? (
                  ...
                  <label for="capture-file-input" class="capture-file-label">
                    ...
                    <div class="capture-file-label-text">
                      <strong>Attach a local file</strong>
                      <small>PDF, EPUB, HTML, audio, or text</small>
                    </div>
                  </label>
                )}
              </div>
            ...
            <>
              <div class="capture-personal-core">
                <div class="capture-input-group capture-title-field">
                  <label for="capture-personal-title" class="capture-label">
                    <span>Title</span>
                  </label>
                  <input
                    id="capture-personal-title"
                ...
                <div class="capture-input-group">
                  <label for="capture-personal-creator" class="capture-label">
                    <span>{creatorLabel(kind)}</span>
                  </label>
                  <input
                    id="capture-personal-creator"
                    class="capture-control"
                    value={creator}
                    onInput={(event) => setCreator((event.currentTarget as HTMLInputElement).value)}
                    placeholder={kind === 'book' ? 'Required for books' : 'Optional'}
                    required={kind === 'book'}
                  />
                </div>
                <div class="capture-input-group">
                  <label for="capture-personal-state" class="capture-label">
                    <span>Status</span>
                  </label>
                  <select
                    id="capture-personal-state"
                    class="capture-select"
                    value={personalState}
                    onChange={(event) =>
                      setPersonalState((event.currentTarget as HTMLSelectElement).value as PersonalState)
                    }
                  >
              ...
              <details class="capture-more-fields">
                <summary>
                  More details <span>Progress, rating, tags, and notes</span>
                </summary>
                <div class="capture-more-fields-body">
                  <div class="capture-fields-grid">
                    <label class="capture-input-group" for="capture-personal-url">
                      <span class="capture-label">Link</span>
                      <input
                        id="capture-personal-url"
                        class="capture-control"
                        type="url"
                        value={url}
                    ...
                    <label class="capture-input-group" for="capture-personal-year">
                      <span class="capture-label">Release year</span>
                      <input
                        id="capture-personal-year"
                        class="capture-control"
                        type="number"
                        min="1800"
                        max={new Date().getFullYear() + 5}
                        value={releaseYear}
                        onInput={(event) => setReleaseYear((event.currentTarget as HTMLInputElement).value)}
                        placeholder="Optional"
                      />
                    </label>
                    <label class="capture-input-group" for="capture-personal-duration">
                      <span class="capture-label">Duration (minutes)</span>
                      <input
                        id="capture-personal-duration"
                        class="capture-control"
                        type="number"
                        min="0"
                    ...
                    <label class="capture-input-group" for="capture-personal-rating">
                      <span class="capture-label">Rating (0–10)</span>
                      <input
                        id="capture-personal-rating"
                        class="capture-control"
                        type="number"
                        min="0"
                        max="10"
                        step="0.5"
                        value={rating}
                        onInput={(event) => setRating((event.currentTarget as HTMLInputElement).value)}
                        placeholder="Optional"
                      />
                    </label>
                  </div>
                  <div class="capture-progress-row">
                    <label class="capture-input-group" for="capture-progress-current">
                      <span class="capture-label">Progress</span>
                      <input
                        id="capture-progress-current"
                        class="capture-control"
                        type="number"
                        min="0"
                        value={progressCurrent}
                        onInput={(event) => setProgressCurrent((event.currentTarget as HTMLInputElement).value)}
                        placeholder="Current"
                      />
                    </label>
                    <span aria-hidden="true">of</span>
                    <label class="capture-input-group" for="capture-progress-total">
                      <span class="capture-label">Total</span>
                      <input
                        id="capture-progress-total"
                        class="capture-control"
                        type="number"
                        min="1"
                        value={progressTotal}
                        onInput={(event) => setProgressTotal((event.currentTarget as HTMLInputElement).value)}
                        placeholder="Total"
                      />
                    </label>
                    <label class="capture-input-group" for="capture-progress-unit">
                      <span class="capture-label">Unit</span>
                      <input
                        id="capture-progress-unit"
                        class="capture-control"
                        value={progressUnit}
                        onInput={(event) => setProgressUnit((event.currentTarget as HTMLInputElement).value)}
                      />
                    </label>
                  </div>
                  <label class="capture-input-group" for="capture-personal-tags">
                    <span class="capture-label">Tags</span>
                    <input
                      id="capture-personal-tags"
                      class="capture-control"
                      value={tags}
                      onInput={(event) => setTags((event.currentTarget as HTMLInputElement).value)}
                      placeholder="psychology, documentary, favorite"
                    />
                    <small class="capture-field-help">Separate tags with commas.</small>
                  </label>
                  <label class="capture-input-group" for="capture-personal-note">
                    <span class="capture-label">Personal note</span>
                    <textarea
                      id="capture-personal-note"
                      class="capture-textarea capture-personal-note"
                      rows={2}
                      value={personalNote}
                      onInput={(event) => setPersonalNote((event.currentTarget as HTMLTextAreaElement).value)}
                      placeholder="What mattered, what you want to remember, or why you stopped…"
                    />
                  </label>
                </div>
              </details>
          ...
          <div class="capture-input-group capture-branch-field">
            <label for="capture-branch-input" class="capture-label">
              <span>Knowledge branch</span>
            </label>
            <select
              id="capture-branch-input"
              class="capture-select"
              value={branchId}
            ...
            <small class="capture-field-help">
              Required so this record and the preferences learned from it keep one durable context.
            </small>
            {branchDeck.error && (
              <span class="capture-field-error" role="alert">
                Branches could not be loaded. Close and retry before saving.
              </span>
            )}
          </div>
          ...
          <footer class="capture-dialog-footer">
            ...
            <button
              ...
              disabled={(kind === 'source' ? !canSaveSource : !canSavePersonal) || saving || branchDeck.loading}
              type="submit"
            >
              {saving
                ? 'Saving…'
                : kind === 'source'
                  ? 'Save source'
                  : `Save ${captureKinds.find((item) => item.key === kind)?.label || 'item'}`}
            </button>
          </footer>
        </form>
      </section>
