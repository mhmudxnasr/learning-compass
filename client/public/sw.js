const CACHE = 'learning-compass-shell-v52'
...
self.addEventListener('activate', (event) => event.waitUntil(Promise.all([self.clients.claim(), cleanupOrphanOfflinePackCaches(), caches.keys().then((keys) => Promise.all(keys.filter((key) => ![CACHE, DATA_CACHE, ARTIFACT_CACHE, OFFLINE_PACK_INDEX_CACHE].includes(key) && !key.startsWith(OFFLINE_PACK_PREFIX)).map((key) => caches.delete(key))))])))
...
async function cleanupOrphanOfflinePackCaches(index) {
  try {
    ...
    const active = new Set(Object.values(manifests).map((manifest) => manifest?.cacheName).filter(Boolean))
    const names = await caches.keys()
    await Promise.allSettled(names.filter((name) => name.startsWith(OFFLINE_PACK_PREFIX) && !active.has(name)).map((name) => caches.delete(name)))
  } catch { /* orphan cleanup never invalidates an indexed ready pack */ }
}

async function writeOfflinePackIndex(index) {
  const cache = await caches.open(OFFLINE_PACK_INDEX_CACHE)
  await cache.put(OFFLINE_PACK_INDEX_URL, new Response(JSON.stringify(index), { headers: { 'content-type': 'application/json', 'cache-control': 'no-store' } }))
}
...
function allowedOfflineResource(resource) {
  ...
  let url
  try { url = new URL(String(resource.url || ''), self.location.origin) } catch { return null }
  ...
  return { ...resource, ...(resource.role === 'data' ? { snapshot } : {}), url: `${url.pathname}${url.search}`, sizeBytes }
}
...
function validateOfflinePack(pack) {
  ...
  if (!id || !version || !rawResources.length || rawResources.length > 200) throw new Error('A bounded, versioned offline pack is required.')
  const resources = rawResources.map(allowedOfflineResource)
  if (resources.some((resource) => !resource)) throw new Error('Offline packs may contain only exact same-origin companions and supported metadata snapshots.')
  ...
  return { id, title: String(pack.title || 'Offline pack').trim().slice(0, 500), scope: String(pack.scope || 'source'), version, resources: unique }
}
...
async function offlinePackStatus(packId, expectedVersion) {
  ...
  if (!manifest) return { supported: true, state: 'not-downloaded', stored: false, packId: id, expectedVersion, ...estimate }
  ...
  const state = missing.length ? 'partial' : expectedVersion && manifest.version !== expectedVersion ? 'superseded' : 'ready'
...
async function saveOfflinePack(pack) {
  ...
  const sequence = Object.values(index).reduce((highest, manifest) => Math.max(highest, Number(manifest.sequence || 0)), 0) + 1
  ...
  try {
    for (const resource of validated.resources) {
      const response = resource.role === 'data'
        ? new Response(JSON.stringify(resource.snapshot), { status: 200, headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' } })
      ...
      if (resource.role === 'html' && !contentType.includes('html')) throw new Error('The HTML companion did not return HTML.')
      if (resource.role === 'pdf' && !contentType.includes('pdf')) throw new Error('The PDF companion did not return PDF.')
      if (resource.role !== 'data') {
        ...
        if (serverArtifactId !== resource.artifactId
          ...
          throw new Error('The companion pair is no longer the verified current version. Refresh the page before saving it offline.')
        }
      }
      const copy = response.clone()
      const measuredBytes = Number((await copy.blob()).size || 0)
      if (measuredBytes !== resource.sizeBytes) throw new Error(`The exact byte size changed for ${resource.role}. Refresh the page before saving it offline.`)
    ...
    if (stagedMissing.length) throw new Error('The browser evicted part of the staged offline pack before it could be committed. Free storage and refresh.')
    const estimate = await storageEstimate()
    const manifestResources = validated.resources.map(({ snapshot, ...resource }) => resource)
    const manifest = { id: validated.id, title: validated.title, scope: validated.scope, version: validated.version, cacheName, resources: manifestResources, resourceCount: validated.resources.length, sizeBytes: totalBytes, savedAt: new Date().toISOString(), sequence }
    await cache.put(`/__learning-compass-offline-pack__/${encodeURIComponent(validated.id)}`, new Response(JSON.stringify(manifest), { headers: { 'content-type': 'application/json' } }))
    for (const resource of validated.resources) if (!(await cache.match(resource.url))) throw new Error('The browser evicted part of the staged offline pack before it could be committed. Free storage and refresh.')
    ...
    if (previous?.cacheName && previous.cacheName !== cacheName) {
      try { await caches.delete(previous.cacheName) } catch { /* harmless orphan; activation cleanup can reclaim it later */ }
    }
    ...
    try { await caches.delete(cacheName) } catch { /* a failed cleanup never hides the original failure */ }
    ...
    return { supported: true, state: storageFull ? 'storage-full' : 'error', stored: Boolean(previous?.cacheName), packId: validated.id, error: message, ...(await storageEstimate()) }
  }
}
...
async function removeOfflinePack(packId) {
  ...
  if (manifest?.cacheName) {
    try { await caches.delete(manifest.cacheName) } catch { /* activation cleanup reclaims the orphan */ }
  }
...
function serializeOfflinePackMutation(task) {
  ...
  offlinePackMutation = result.then(() => undefined, () => undefined)
  return result
}
...
self.addEventListener('message', (event) => {
  ...
  const task = action === 'offline-pack:save'
  ...
  event.waitUntil(task.then(reply).catch((error) => reply({ supported: true, state: 'error', error: error instanceof Error ? error.message : 'Offline-pack action failed.' })))
})
...
self.addEventListener('fetch', (event) => {
  ...
  const offlinePackData = /^\/capture\/[^/]+\/record$/.test(url.pathname) || /^\/learning\/core\/threads\/[^/]+\/path$/.test(url.pathname)
  if (offlinePackData) {
    event.respondWith(fetch(event.request).catch(() => matchLatestOfflinePackResource(event.request).then((cached) => cached || Promise.reject(new Error('Offline pack is not available')))))
    return
  }
