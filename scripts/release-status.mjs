...
import {
  REQUIRED_READ_HEADROOM,
  REQUIRED_WRITE_HEADROOM,
...
const [live, readiness, budget, deploymentsText, d1Text, commitSha, branch, status] = await Promise.all([
  ...
  capture('npx', ['wrangler', 'd1', 'execute', database, '--remote', '--config', config, '--json', '--command', releaseSql]),
...
const snapshot = buildReleaseSnapshot({
  ...
  readiness,
  budget,
  localMigrations: readdirSync(resolve(repoRoot, 'migrations')).filter((name) => /^\d+_.*\.sql$/.test(name)).sort(),
