...
export function buildReleaseSnapshot({
  ...
  if (!Array.isArray(d1Results) || d1Results.length !== 4) throw new Error('Remote release-status query returned an unexpected result set')

  const deployment = [...deployments].sort((left, right) => String(left.created_on).localeCompare(String(right.created_on))).at(-1)
  ...
  const applied = rows(d1Results[0], 'migrations').map((row) => ({ name: String(row.name), applied_at: row.applied_at || null }))
  const schemaRows = rows(d1Results[1], 'release schema').map((row) => ({ name: String(row.name), type: String(row.type) }))
  ...
  const blockers = [
    ...
    ...(writes.budget <= 0 || writes.headroom < requiredWriteHeadroom ? ['write_budget_missing_required_headroom'] : []),
    ...(!backup || backup.status !== 'verified' || !backup.restore_rehearsed_at ? ['verified_restore_receipt_unavailable'] : []),
  ]
