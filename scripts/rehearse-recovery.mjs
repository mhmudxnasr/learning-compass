...
if (!manifestPath)
  throw new Error('Usage: node scripts/rehearse-recovery.mjs --manifest /path/to/snapshot.json [--no-record]')

const hash = (buffer) => createHash('sha256').update(buffer).digest('hex')
const manifestBytes = readFileSync(manifestPath)
const manifest = JSON.parse(manifestBytes.toString('utf8'))
if (manifest.format !== 'learning-compass-full-recovery-v2')
...
try {
  execFileSync('sqlite3', [databasePath, `.read ${JSON.stringify(sqlPath)}`], {
    stdio: 'pipe',
  ...
  const foreignKeyRows = JSON.parse(
    execFileSync('sqlite3', ['-json', databasePath, 'PRAGMA foreign_key_check;'], { encoding: 'utf8' }) || '[]',
  )
  if (foreignKeyRows.length)
    throw new Error(`Restored SQLite foreign-key check failed: ${JSON.stringify(foreignKeyRows.slice(0, 10))}`)
  const counts =
    JSON.parse(
      execFileSync(
        'sqlite3',
    ...
    (SELECT COUNT(*) FROM srs_cards) recall_cards;`,
        ],
        { encoding: 'utf8' },
      ) || '[]',
    )[0] || {}
  if (Number(counts.artifacts || 0) !== Number(manifest.artifacts.count || 0))
    throw new Error(
      `Restored artifact count ${counts.artifacts || 0} does not match snapshot ${manifest.artifacts.count || 0}`,
    )
  ...
  if (artifactBytes !== Number(manifest.artifacts.bytes || 0))
    throw new Error('R2 backup byte total does not match the snapshot manifest')
