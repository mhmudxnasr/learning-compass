...
const load = async () => {
  const picks =
    await query(`SELECT p.id,p.strategy,p.status,p.engine_version,p.shadow_json,p.rationale_json,p.recommendation_id,o.outcome_status,o.learning_value,o.actual_score,o.rejection_reason
  ...
  return picks.map((row) => {
    ...
    return {
      ...
      v1:
        v1raw && hasReadModel
          ? {
              score: Number(v1raw.score || 0),
              confidence: Number(v1raw.confidence || 0),
              abstained: Boolean(v1raw.abstention_reason),
            }
          : null,
      v2:
        v2raw && hasReadModel
          ? {
              score: Number(v2raw.score || 0),
              confidence: Number(v2raw.confidence || 0),
              abstained: Boolean(v2raw.abstention_reason),
            }
          : null,
    }
  })
...
const main = async () => {
  ...
  const engineStats = (engine) => {
    const scoredOutcomes = outcomes.filter(
      (item) => item.has_read_model && !item[engine].abstained && item[engine].score > 0,
    )
    const consumedScores = consumed
      .filter((item) => item.has_read_model && !item[engine].abstained)
      .map((item) => item[engine].score)
    ...
    return {
      ...
      discrimination_gap:
        mean(consumedScores) != null && mean(rejectedScores) != null
          ? Math.round((mean(consumedScores) - mean(rejectedScores)) * 1000) / 1000
          : null,
  ...
  if (AS_JSON) {
    ...
    console.log(
      `  ${report.picks_total} picks · ${report.picks_with_read_model} with v1/v2 read-model · ${report.consumed} consumed · ${report.rejected} rejected (${report.rejected_with_reason} with reason)`,
    )
    console.log(
      `  coverage: ${report.coverage.consumed_with_shadow} consumed + ${report.coverage.rejected_with_shadow} rejected with shadow scores`,
    )
    console.log(`\n  ${'metric'.padEnd(30)} ${'v1 (serving)'.padEnd(16)} ${'v2 (shadow)'}`)
    console.log(
      `  ${'consumed avg score'.padEnd(30)} ${fmt(report.v1.consumed_avg_score).padEnd(16)} ${fmt(report.v2.consumed_avg_score)}`,
    )
    console.log(
      `  ${'rejected avg score'.padEnd(30)} ${fmt(report.v1.rejected_avg_score).padEnd(16)} ${fmt(report.v2.rejected_avg_score)}`,
    )
    console.log(
      `  ${'discrimination gap (Δ)'.padEnd(30)} ${fmt(report.v1.discrimination_gap).padEnd(16)} ${fmt(report.v2.discrimination_gap)}`,
    )
    console.log(`  ${'abstained picks'.padEnd(30)} ${String(report.v1.abstained).padEnd(16)} ${report.v2.abstained}`)
    console.log(
      `  ${'withheld from consumed'.padEnd(30)} ${String(report.v1.withheld_from_consumed).padEnd(16)} ${report.v2.withheld_from_consumed}`,
    )
    console.log(`\n  verdict: ${report.verdict}`)
    if (report.verdict === 'insufficient_shadow_coverage') {
      console.log(
        `  (${MIN_COVERAGE} resolved-with-shadow needed to trust a discrimination verdict; current data covers most picks before shadow recording existed.)`,
      )
    }
  }
}
