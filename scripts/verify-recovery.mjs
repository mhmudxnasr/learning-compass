...
if (!/CREATE TABLE|INSERT INTO|BEGIN TRANSACTION/i.test(sql.toString('utf8')))
  throw new Error('SQL export does not contain a recognizable D1 payload.')

const declaredMigrations = new Map((manifest.migrations || []).map((entry) => [entry.path, entry.sha256]))
const currentMigrations = readdirSync('migrations')
  .filter((name) => /^\d+_.*\.sql$/.test(name))
  .sort()
if (declaredMigrations.size !== currentMigrations.length)
  throw new Error('Recovery manifest migration inventory does not match the current release.')
for (const name of currentMigrations) {
  ...
  if (declaredMigrations.get(path) !== currentHash)
    throw new Error(`Recovery manifest migration hash mismatch: ${path}`)
}
  ...
  JSON.stringify(
    {
      ...
      sql_sha256: hash,
      migration_count: currentMigrations.length,
      r2_included: Boolean(manifest.r2?.included),
    },
    null,
    2,
  ),
