...
const outputRoot = resolve(
  value('--output-dir') || process.env.LEARNING_COMPASS_BACKUP_DIR || '/home/mahmud/backups/learning-compass',
)
...
execFileSync('/usr/bin/node', ['scripts/export-recovery.mjs', '--remote', '--output', sqlPath], {
  cwd: process.cwd(),
  stdio: 'inherit',
})
...
const inventoryOutput = execFileSync(
  ...
  [
    ...
    '--json',
    '--command',
    'SELECT id,filename,r2_key,size_bytes FROM artifacts ORDER BY id;',
  ],
  { encoding: 'utf8', maxBuffer: 20 * 1024 * 1024 },
)
const inventoryResult = JSON.parse(inventoryOutput)
const inventory = inventoryResult?.[0]?.results || []
if (inventory.some((item) => !item.r2_key))
  throw new Error('Every canonical artifact must have an R2 key before a full backup can pass')
...
const downloadArtifact = async (artifact, index) => {
  ...
  objects[index] = {
    ...
    size_bytes: size,
    sha256,
    file: relative(snapshotDir, objectPath),
  }
  completed += 1
  if (completed % 25 === 0 || completed === inventory.length)
    console.log(`Verified ${completed}/${inventory.length} remote R2 objects`)
}
const configuredConcurrency = Number(process.env.LEARNING_COMPASS_BACKUP_CONCURRENCY || 18)
const workerCount = Math.min(
  Math.max(1, Number.isFinite(configuredConcurrency) ? Math.floor(configuredConcurrency) : 18),
  24,
  inventory.length,
)
await Promise.all(
...
const manifest = {
  ...
  d1: {
    ...
    manifest: basename(d1ManifestPath),
    sha256: d1Manifest.sql_sha256,
    bytes: d1Manifest.sql_bytes,
  },
...
execFileSync(
  '/usr/bin/node',
  ['scripts/rehearse-recovery.mjs', '--manifest', manifestPath, ...(record ? [] : ['--no-record'])],
  ...
  JSON.stringify(
    {
      ...
      artifact_count: objects.length,
      artifact_bytes: artifactBytes,
      retained_snapshots: Math.min(snapshots.length, retain),
    },
    null,
    2,
  ),
