...
const requireBudgetHeadroom = async (phase) => {
  ...
  const providerLimitsMatch =
    Number(body?.reads?.cloudflare_limit) === CLOUDFLARE_FREE_READ_LIMIT &&
  ...
  if (!response.ok || body?.policy?.ok !== true || !policy.ok || !providerLimitsMatch) {
    throw new Error(
      `${phase} production budget policy failed (${response.status}): ${JSON.stringify({
        reads: body?.reads,
        writes: body?.writes,
        policy: body?.policy,
        expected: policy,
