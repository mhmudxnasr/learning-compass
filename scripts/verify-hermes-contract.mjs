...
const normalizeDurableEntry = (entry) => entry
  .normalize('NFKC')
  .toLocaleLowerCase('en')
  .replace(/\s+/gu, ' ')
  .replace(/[.!?؟،؛:…]+$/gu, '')
  .trim()
const durableEntryTokens = (entry) => new Set(
  normalizeDurableEntry(entry).match(/[\p{L}\p{N}]+(?:[+#/]+[\p{L}\p{N}]*)*/gu) || [],
)
...
if (!runtimeBudgets?.default_telegram || !runtimeBudgets?.compass_telegram || !runtimeBudgets?.memory_chars || !runtimeBudgets?.loaded_skill_bytes || !runtimeBudgets?.read_slo_ms) {
...
for (const [name, file] of Object.entries(runtimeBudgets.prompt_contracts || {})) {
  if (typeof file !== 'string' || !existsSync(join(root, file))) throw new Error(`Hermes prompt contract path is invalid: ${name}`)
  ...
  if (promptContract.schema !== 'hermes-prompt-budget/v1' || promptContract.platform !== 'telegram') throw new Error(`Hermes prompt contract schema drift: ${name}`)
  for (const field of ['max_system_prompt_bytes', 'max_tool_schema_bytes', 'max_fixed_payload_bytes', 'max_skills_index_bytes']) {
    if (promptContract[field] !== expected[field]) throw new Error(`Hermes prompt budget drift: ${name}.${field}`)
  }
  if (!Array.isArray(promptContract.expected_tools) || promptContract.expected_tools.length !== expected.exact_tools) throw new Error(`Hermes prompt tool inventory drift: ${name}`)
  if (!promptContract.require_memory_readable || !promptContract.require_memory_within_limits || !promptContract.require_memory_tail_loaded || !promptContract.require_no_duplicate_memory_entries) {
    throw new Error(`Hermes prompt memory gate drift: ${name}`)
  }
}
if (runtimeBudgets.read_slo_ms.filtered_capabilities_p95 !== 1000 || runtimeBudgets.read_slo_ms.briefing_p95 !== 1000) throw new Error('Hermes production read SLO drift')
...
if (turnLedgerContract?.owner !== 'learning-compass-site-operator/scripts/site_request.py'
...
const requiredPolicy = [
  ...
  ['personal_library', 'typed books and media use canonical branch-verified records with editable personal state and append-only lineage; personal status never enters Queue or advances learning'],
  ...
  ['branch_domain', 'every captured, recommended, or queued item requires a verified non-pruned branch plus persisted super_category/domain and a visible branch pill on every rendering surface; synthetic rounds are retired'],
]
...
if (authentication?.learning_compass_api_token_required !== false
...
for (const retired of ["from './auth'", "app.post('/auth/session'", 'authenticatePrivateRequest', 'createBrowserSession', 'privateApiRequired', 'configuredApiToken']) {
  if (workerEntrySource.includes(retired)) throw new Error(`Retired private-auth plumbing remains in Worker entry: ${retired}`)
}
const browserClientSource = read('client/src/auth.ts')
for (const retired of ['TASTE_MAP_API_TOKEN', 'LEARNING_COMPASS_API_TOKEN', 'x-api-token', '/auth/session', '__Host-learning_compass_session']) {
  if (browserClientSource.includes(retired)) throw new Error(`Retired private-auth plumbing remains in browser client: ${retired}`)
}
for (const [file, required] of [
  ['docs/API.md', 'Learning Compass API reads and writes do not require an API token or browser session. `POST /auth/session` is retired.'],
...
if (existsSync(localSkillsRoot)) {
  const siteClientPath = join(localSkillsRoot, 'workflow', 'learning-compass-site-operator', 'scripts', 'site_request.py')
  ...
  if (!/^browser:\n(?:[ \t]+.*\n)*?[ \t]+allow_private_urls:\s*false\s*$/m.test(configText) || !/^security:\n(?:[ \t]+.*\n)*?[ \t]+allow_private_urls:\s*false\s*$/m.test(configText)) {
    throw new Error('Default Hermes profile must deny private/internal URLs by default')
  }
  if (!/^WEBHOOK_ENABLED=false\s*$/m.test(envText)) throw new Error('Default Hermes environment overrides the disabled webhook listener')
  ...
  const routerSkillBytes = Buffer.byteLength(readFileSync(join(localSkillsRoot, 'workflow', 'learning-compass-operating-system', 'SKILL.md'), 'utf8'))
  const operatorSkillBytes = Buffer.byteLength(readFileSync(join(localSkillsRoot, 'workflow', 'learning-compass-site-operator', 'SKILL.md'), 'utf8'))
  if (routerSkillBytes > runtimeBudgets.loaded_skill_bytes['learning-compass-operating-system']) throw new Error(`Hermes router skill budget exceeded: ${routerSkillBytes}`)
  if (operatorSkillBytes > runtimeBudgets.loaded_skill_bytes['learning-compass-site-operator']) throw new Error(`Hermes site-operator skill budget exceeded: ${operatorSkillBytes}`)
  if (routerSkillBytes + operatorSkillBytes > runtimeBudgets.loaded_skill_bytes.ordinary_router_plus_operator) throw new Error(`Hermes ordinary loaded-skill budget exceeded: ${routerSkillBytes + operatorSkillBytes}`)
  ...
  if (!existsSync(join(profileRoot, '.no-bundled-skills'))) throw new Error('Compass profile must opt out of bundled skill seeding')
  ...
  if (!/^browser:\n(?:[ \t]+.*\n)*?[ \t]+allow_private_urls:\s*false\s*$/m.test(profileConfigText) || !/^security:\n(?:[ \t]+.*\n)*?[ \t]+allow_private_urls:\s*false\s*$/m.test(profileConfigText)) {
    throw new Error('Compass Hermes profile must deny private/internal URLs by default')
  }
  if (!/^WEBHOOK_ENABLED=false\s*$/m.test(profileEnvText)) throw new Error('Compass Hermes environment overrides the disabled webhook listener')
  const profileDisabledBlock = profileConfigText.match(/^skills:\n((?:[ \t]+.*\n?)*)/m)?.[1] || ''
  const profileDisabledSkills = new Set([...profileDisabledBlock.matchAll(/^\s+-\s+([^#\s]+)\s*$/gm)].map((match) => match[1]))
  const profileDisabledInline = profileDisabledBlock.match(/disabled:\s*['"]?(\[[^\n]+\])['"]?/)?.[1]
  if (profileDisabledInline) {
    try { for (const name of JSON.parse(profileDisabledInline)) profileDisabledSkills.add(String(name)) } catch {}
  }
  ...
  for (const skill of activeSkills) {
    ...
    if (profileDisabledSkills.has(skill.name)) throw new Error(`Canonical Compass profile skill is disabled: ${skill.name}`)
    ...
    for (const file of canonicalFiles) {
      ...
      if (/\.(?:md|py|js|mjs|sh|json|ya?ml)$/.test(file)) {
        ...
        for (const forbidden of [/\bInbox\b/i, /learning[- ]evidence/i, /learner evidence/i, /Thread evidence/i, /proof (?:work|action|requirement|counter)/i, /ready_to_verify/i, /evidence_pending/i, /\/learning\/core\/evidence/i, /2026-08-18/, /notebooklm-mcp/i, /compatible MCP session/i, /optional recall-draft workflow/i, /silently producing an unlinked item/i, /Private mode is opt-in through/i, /Writes require `x-api-token` when `API_TOKEN` is configured/i, /Incremental FTS5/i, /\/knowledge\/blind-spots/i]) {
  ...
  const profileInstalledSkillNames = walkSkillFiles(profileSkillsRoot)
    ...
    .map((file) => readFileSync(file, 'utf8').match(/^name:\s*([^\s]+)\s*$/m)?.[1])
    .filter(Boolean)
  const unownedProfileSkills = profileInstalledSkillNames.filter((name) => !activeNames.has(name) && !profileDisabledSkills.has(name))
  if (unownedProfileSkills.length) throw new Error(`Unowned Compass profile skills remain enabled: ${unownedProfileSkills.join(', ')}`)
  ...
  if (!existsSync(profileUser) || readFileSync(rootUser, 'utf8') !== readFileSync(profileUser, 'utf8')) throw new Error('Compass profile user memory drift')
  ...
  if (readFileSync(rootSoul, 'utf8') !== read('docs/learning-compass-hermes-soul.md')) throw new Error('Checked-in Hermes SOUL source drift')

  for (const [label, file] of [['MEMORY.md', rootMemory], ['USER.md', rootUser]]) {
    const text = readFileSync(file, 'utf8').trim()
    const chars = Array.from(text).length
    const limit = runtimeBudgets.memory_chars[label]
    if (!Number.isInteger(limit) || chars > limit) throw new Error(`${label} character budget exceeded: ${chars}/${limit}`)
  ...
  for (const [label, text] of [['default', configText], ['compass', profileConfigText]]) {
    ...
    if ([...text.matchAll(/^\s+webhook:\n\s+enabled:\s*false\s*$/gm)].length < 2) throw new Error(`${label} Hermes profile must keep the unused webhook listeners disabled`)
  }
}
...
for (const route of ['GET /compass/context', 'POST /compass/semantic/index', 'POST /compass/pick/:id/candidates', 'POST /capture/:id/branch-map', 'GET /capture/personal', 'POST /capture/personal', 'PATCH /capture/personal/:id', 'POST /assistant/interpret', 'GET /hardcover', 'POST /hardcover/sync', 'POST /hardcover/import']) {
