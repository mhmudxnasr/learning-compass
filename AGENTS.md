...
- The authoritative Riyadh corpus is the ordered 294-recording manifest with target-set SHA-256 `1de52aac5305c071c027650b8494971d7ccbd83a1bfe497b8fbcb067eeae6be4`. A smaller release is allowed only after an explicit user instruction to publish early, and only as an exact ordered prefix manifest hash-bound to that full manifest. Every included target must pass signed v6 validation and its own aggregate audit, the prefix activates atomically, omitted lessons retain their current pairs, and the release must never be reported as full-corpus completion.
