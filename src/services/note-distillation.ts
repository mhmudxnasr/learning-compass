...
async function loadBlocks(db: D1Database, noteId: string): Promise<DistillationBlock[]> {
  const sections = await db
    .prepare(`SELECT section_key,label,content FROM note_sections WHERE note_id=? ORDER BY position`)
    .bind(noteId)
    .all<any>()
  const pending = (sections.results || []).flatMap((section: any) =>
    splitDistillationBlocks(section.content).map(async (text, blockIndex) => ({
...
export async function createClaimHighlight(db: D1Database, noteId: string, input: any) {
  ...
  if (!sectionKey || !Number.isInteger(blockIndex) || blockIndex < 0 || !suppliedChecksum || !claimText.trim())
    return { error: 'section_key, block_index, block_checksum, and claim_text required', status: 400 as const }
  const block = (await loadBlocks(db, noteId)).find(
    (candidate) => candidate.section_key === sectionKey && candidate.block_index === blockIndex,
  )
  if (!block) return { error: 'source block not found', status: 404 as const }
  if (block.checksum !== suppliedChecksum)
    return { error: 'source block changed; reload before highlighting', status: 409 as const }
  const id = makeId('highlight')
  await db
    .prepare(
      `INSERT INTO note_claim_highlights (id,note_id,section_key,block_index,block_checksum,source_text,claim_text) VALUES (?,?,?,?,?,?,?)`,
    )
    .bind(id, noteId, sectionKey, blockIndex, block.checksum, block.text, claimText)
    .run()
...
export async function appendSynthesisRevision(db: D1Database, noteId: string, input: any) {
  ...
  const latest = await db
    .prepare(`SELECT COALESCE(MAX(revision),0) revision FROM note_synthesis_revisions WHERE note_id=?`)
    .bind(noteId)
    .first<any>()
  const revision = Number(latest?.revision || 0) + 1
  const id = makeId('synthesis')
  await db
    .prepare(`INSERT INTO note_synthesis_revisions (id,note_id,revision,synthesis_text) VALUES (?,?,?,?)`)
    .bind(id, noteId, revision, synthesisText)
    .run()
...
export async function promoteHighlightToUnit(db: D1Database, noteId: string, highlightId: string) {
  const highlight = await db
    .prepare(
      `SELECT h.*,n.recommendation_id FROM note_claim_highlights h JOIN notes n ON n.id=h.note_id WHERE h.id=? AND h.note_id=?`,
    )
    .bind(highlightId, noteId)
    .first<any>()
  ...
  if (!highlight.recommendation_id)
    return { error: 'a source-backed note is required for Unit promotion', status: 409 as const }
  ...
  await db.batch([
    db
      .prepare(
        `INSERT OR IGNORE INTO learning_units (id,unit_type,statement,stance,confidence,recommendation_id,source_revision_checksum,created_by,status,note_id) VALUES (?,'claim',?,'uncertain',0.5,?,?,'user','accepted',?)`,
      )
      .bind(unitId, highlight.claim_text, highlight.recommendation_id, highlight.block_checksum, noteId),
    db
      .prepare(
        `INSERT OR IGNORE INTO unit_anchors (id,unit_id,recommendation_id,anchor_type,locator,excerpt,checksum) VALUES (?,?,?,'section',?,?,?)`,
      )
      .bind(anchorId, unitId, highlight.recommendation_id, locator, highlight.source_text, highlight.block_checksum),
    db
      .prepare(
        `INSERT INTO learning_unit_revisions (unit_id,actor_type,next_json,reason) SELECT ?,'user',?,'promoted_note_highlight' WHERE NOT EXISTS (SELECT 1 FROM learning_unit_revisions WHERE unit_id=? AND reason='promoted_note_highlight')`,
      )
      .bind(
        unitId,
        JSON.stringify({ type: 'claim', statement: highlight.claim_text, note_id: noteId, anchor: locator }),
        unitId,
      ),
    db
      .prepare(
        `UPDATE note_claim_highlights SET promoted_unit_id=?,promoted_at=datetime('now') WHERE id=? AND promoted_unit_id IS NULL`,
      )
      .bind(unitId, highlightId),
  ])
