export type LearningScopeRef =
  { kind: 'thread'; id: string } | { kind: 'level'; id: string } | { kind: 'lesson'; id: string }
...
export class LearningScopeError extends Error {
  constructor(
    public code: 'invalid_scope' | 'scope_not_found' | 'scope_integrity_error',
    message: string,
  ) {
    super(message)
  }
...
export async function resolveLearningScope(db: D1Database, ref: LearningScopeRef): Promise<ResolvedLearningScope> {
  const id = String(ref?.id || '')
    .trim()
    .slice(0, 120)
  if (!id || !['thread', 'level', 'lesson'].includes(ref?.kind))
    throw new LearningScopeError('invalid_scope', 'A Thread, Level, or Lesson scope is required.')
  if (ref.kind === 'thread') {
  ...
  if (ref.kind === 'lesson') {
    const lesson = await db
      .prepare(`SELECT l.id,l.title,l.thread_id,l.stage_id FROM thread_lessons l WHERE l.id=?`)
      .bind(id)
      .first<any>()
    ...
    return {
      ...
      id: lesson.id,
      title: lesson.title,
      threadId: lesson.thread_id,
      levelId: lesson.stage_id,
      lessonId: lesson.id,
    }
  }
  const level = await db
    .prepare(
      `SELECT s.id,s.title,s.thread_id,t.title AS thread_title FROM learning_path_stages s JOIN learning_threads t ON t.id=s.thread_id WHERE s.id=?`,
    )
    .bind(id)
    .first<any>()
...
const parseMetadata = (row: any) => {
  ...
  try {
    ...
    /* keep malformed legacy metadata isolated */
  }
...
export async function loadThreadLearningMaterials(db: D1Database, threadId: string): Promise<ThreadLearningMaterials> {
  ...
  const levelsResult = await db
    .prepare(`SELECT id,title FROM learning_path_stages WHERE thread_id=? ORDER BY position`)
    .bind(thread.threadId)
    .all<any>()
  const levels = levelsResult.results || []
  const lessonsResult = await db
    .prepare(`SELECT id,title,stage_id FROM thread_lessons WHERE thread_id=? ORDER BY stage_id,position`)
    .bind(thread.threadId)
    .all<any>()
  ...
  const [notesResult, filesResult, cardsResult, draftsResult] = await Promise.all([
    ...
    `,
      ...
      .bind(thread.threadId, thread.threadId, thread.threadId)
      .all<any>(),
    db
      .prepare(
        `
  ...
  if (notes.length) {
    const sections = await db
      .prepare(
        `SELECT note_id,section_key,label,content,direction,position FROM note_sections WHERE note_id IN (${notes.map(() => '?').join(',')}) ORDER BY note_id,position`,
      )
      .bind(...notes.map((note: any) => note.id))
      .all<any>()
    for (const section of sections.results || [])
      sectionsByNote.set(section.note_id, [...(sectionsByNote.get(section.note_id) || []), section])
  }
  ...
  const output: ThreadLearningMaterials = {
    thread: emptyMaterials(),
    levels: new Map(levels.map((level: any) => [String(level.id), emptyMaterials()])),
    lessons: new Map(lessons.map((lesson: any) => [String(lesson.id), emptyMaterials()])),
  }
  const ownerFor = (stageId: unknown, lessonId: unknown): ScopeOwner =>
    lessonId
  ...
  const add = (kind: keyof ScopedLearningMaterials, row: any) => {
    ...
    if (stageId && !levelIds.has(stageId))
      throw new LearningScopeError('scope_integrity_error', `${kind} belongs to a Level outside this Thread.`)
    if (lessonId && !lessonIds.has(lessonId))
      throw new LearningScopeError('scope_integrity_error', `${kind} belongs to a Lesson outside this Thread.`)
