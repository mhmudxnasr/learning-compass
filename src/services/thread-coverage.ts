...
export async function loadThreadCoverageAnchors(db: D1Database): Promise<ThreadCoverageAnchor[]> {
  const rows = await db.prepare(`
    ...
    FROM learning_path_stages s JOIN learning_threads t ON t.id=s.thread_id WHERE t.status!='abandoned' AND t.superseded_at IS NULL
    UNION ALL
    ...
    FROM thread_lessons l JOIN learning_threads t ON t.id=l.thread_id WHERE t.status!='abandoned' AND t.superseded_at IS NULL
    UNION ALL
