...
const has = (input: PersonalLibraryInput, key: keyof PersonalLibraryInput) => Object.prototype.hasOwnProperty.call(input, key)
...
export function normalizePersonalLibraryInput(input: PersonalLibraryInput, fallback: Partial<NormalizedPersonalLibraryInput> = {}): ValidationResult {
  ...
  const progressCurrent = nullableNumber(has(input, 'progress_current') ? input.progress_current : fallback.progress_current)
  ...
  if (rawUrl && !/^https?:\/\/[^\s<>"']+$/i.test(rawUrl)) return { ok: false, error: 'url must be an http or https address' }
  const maximumYear = new Date().getUTCFullYear() + 5
  if (releaseYear !== null && (!Number.isInteger(releaseYear) || releaseYear < 1800 || releaseYear > maximumYear)) return { ok: false, error: `release_year must be between 1800 and ${maximumYear}` }
  if (duration !== null && (!Number.isInteger(duration) || duration < 0 || duration > 1_000_000)) return { ok: false, error: 'duration_minutes must be a non-negative integer' }
  if (progressCurrent !== null && (progressCurrent < 0 || progressCurrent > 1_000_000)) return { ok: false, error: 'progress_current must be between 0 and 1000000' }
  if (progressTotal !== null && (progressTotal <= 0 || progressTotal > 1_000_000)) return { ok: false, error: 'progress_total must be between 1 and 1000000' }
  if (progressCurrent !== null && progressTotal !== null && progressCurrent > progressTotal) return { ok: false, error: 'progress_current cannot exceed progress_total' }
  if (rating !== null && (rating < 0 || rating > 10)) return { ok: false, error: 'rating must be between 0 and 10' }

  const normalizedRating = normalizeRating(rating)
  const progressUnit = boundedText(has(input, 'progress_unit') ? input.progress_unit : fallback.progress_unit, 40) || defaultProgressUnit(itemType)
  return {
    ok: true,
    value: {
      title,
...
function keyPart(value: string) {
  return value.normalize('NFKD').replace(/\p{M}/gu, '').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '_').replace(/^_+|_+$/g, '').slice(0, 72) || 'untitled'
}

export function personalLibraryDedupKey(item: Pick<NormalizedPersonalLibraryInput, 'url' | 'title' | 'creator' | 'item_type'>) {
...
async function verifiedBranch(DB: D1Database, branchId: string) {
  return DB.prepare(`SELECT n.id,n.label,n.status
    FROM tree_nodes n
    WHERE n.id=? AND n.type='branch'
      ...
      AND lower(COALESCE(n.status,''))!='pruned'`).bind(branchId).first<{ id: string; label: string; status: string }>()
}
...
function projectPersonalItem(row: any) {
  return {
    ...
    branch: row.branch_id ? { id: row.branch_id, label: row.branch_label || row.branch_id, status: row.branch_status || 'active' } : null,
    ...
    progress_current: row.projected_progress_current == null ? (row.progress_current == null ? null : Number(row.progress_current)) : Number(row.projected_progress_current),
    progress_total: row.projected_progress_total == null ? (row.progress_total == null ? null : Number(row.progress_total)) : Number(row.projected_progress_total),
...
export async function loadPersonalLibraryItem(DB: D1Database, recommendationId: string) {
  const row = await DB.prepare(`${itemProjectionSql} WHERE p.recommendation_id=? AND r.deleted_at IS NULL AND COALESCE(r.status,'')!='deleted'`).bind(recommendationId).first<any>()
  return row ? projectPersonalItem(row) : null
}

export async function loadPersonalLibrary(DB: D1Database, filters: { q?: string; item_type?: string; state?: string; limit?: number; offset?: number } = {}) {
  ...
  const [rows, filteredCount, totals, byType, byState, byBranch, activity] = await Promise.all([
    DB.prepare(`${itemProjectionSql}${where} ORDER BY p.updated_at DESC,p.created_at DESC LIMIT ? OFFSET ?`).bind(...bindings, limit, offset).all<any>(),
    DB.prepare(`SELECT COUNT(*) count FROM personal_library_items p JOIN recommendations r ON r.id=p.recommendation_id${where}`).bind(...bindings).first<{ count: number }>(),
    DB.prepare(`SELECT COUNT(*) total,SUM(CASE WHEN r.user_score IS NOT NULL THEN 1 ELSE 0 END) rated,
      ...
      WHERE r.deleted_at IS NULL AND COALESCE(r.status,'')!='deleted'`).first<any>(),
    DB.prepare(`SELECT p.item_type key,COUNT(*) count FROM personal_library_items p JOIN recommendations r ON r.id=p.recommendation_id
      WHERE r.deleted_at IS NULL AND COALESCE(r.status,'')!='deleted' GROUP BY p.item_type ORDER BY count DESC,p.item_type`).all<any>(),
    DB.prepare(`SELECT p.state key,COUNT(*) count FROM personal_library_items p JOIN recommendations r ON r.id=p.recommendation_id
      WHERE r.deleted_at IS NULL AND COALESCE(r.status,'')!='deleted' GROUP BY p.state ORDER BY count DESC,p.state`).all<any>(),
    DB.prepare(`SELECT m.branch_id id,COALESCE(n.label,m.branch_id,'Unmapped') label,COUNT(*) count
      FROM personal_library_items p JOIN recommendations r ON r.id=p.recommendation_id
      ...
      GROUP BY m.branch_id,COALESCE(n.label,m.branch_id,'Unmapped') ORDER BY count DESC,label LIMIT 8`).all<any>(),
    DB.prepare(`SELECT substr(p.updated_at,1,7) month,COUNT(*) count FROM personal_library_items p JOIN recommendations r ON r.id=p.recommendation_id
      WHERE r.deleted_at IS NULL AND COALESCE(r.status,'')!='deleted' AND p.updated_at>=date('now','start of month','-5 months')
      GROUP BY substr(p.updated_at,1,7) ORDER BY month`).all<any>(),
  ])
  ...
  return {
    ...
    summary: {
      ...
      by_branch: (byBranch.results || []).map((item: any) => ({ id: item.id || null, label: String(item.label), count: Number(item.count || 0) })),
      activity: (activity.results || []).map((item: any) => ({ month: String(item.month), count: Number(item.count || 0) })),
    },
  }
}
...
export async function createPersonalLibraryItem(DB: D1Database, input: PersonalLibraryInput) {
  ...
  const duplicate = await DB.prepare(`SELECT r.id,m.branch_id,p.recommendation_id personal_id
    ...
    WHERE r.dedup_key=? AND r.deleted_at IS NULL AND COALESCE(r.status,'')!='deleted'`).bind(dedupKey).first<any>()
  if (duplicate) {
    if (duplicate.branch_id && duplicate.branch_id !== item.branch_id) return { ok: false as const, status: 409, error: 'branch_mapping_conflict', recommendation_id: duplicate.id }
    return { ok: false as const, status: 409, error: duplicate.personal_id ? 'personal_item_already_exists' : 'canonical_source_already_exists', recommendation_id: duplicate.id }
  }
  ...
  const metadata = {
    ...
    ...(item.item_type === 'book' ? { book_reading_state: bookStateFromPersonalState(item.state), book_primary: 0 } : {}),
  }
  ...
  await DB.batch([
    DB.prepare(`INSERT INTO recommendations
      ...
      .bind(id, item.title, item.creator || null, item.item_type, url, branch.label, item.rating_label, item.rating, dedupKey),
    DB.prepare(`INSERT INTO recommendation_meta
    ...
    DB.prepare(`INSERT INTO personal_library_items
      ...
      .bind(id, item.item_type, item.state, item.release_year, item.duration_minutes, item.progress_current, item.progress_total, item.progress_unit, JSON.stringify(item.tags), item.personal_note || null, startedAt, completedAt),
    DB.prepare(`INSERT INTO learning_events
      ...
      .bind(eventId, eventId, id, JSON.stringify({ item_type: item.item_type, state: item.state, branch_id: branch.id })),
  ])
  try {
    await DB.prepare("INSERT OR REPLACE INTO search_idx(source,ref_id,text) VALUES ('rec',?,?)")
      .bind(id, [item.title, item.creator, item.item_type, item.tags.join(' ')].filter(Boolean).join(' ')).run()
  } catch { /* search maintenance will repair the best-effort projection */ }
...
export async function updatePersonalLibraryItem(DB: D1Database, recommendationId: string, input: PersonalLibraryInput) {
  ...
  if (has(input, 'item_type') && String(input.item_type || '') !== existing.item_type) {
    return { ok: false as const, status: 409, error: 'item_type is fixed after creation; create a corrected record instead' }
  }
  const storedProfile = await DB.prepare(`SELECT progress_current,progress_total FROM personal_library_items WHERE recommendation_id=?`).bind(recommendationId).first<{ progress_current: number | null; progress_total: number | null }>()
  const normalized = normalizePersonalLibraryInput(input, {
  ...
  await DB.batch([
    ...
    DB.prepare(`UPDATE personal_library_items SET state=?,release_year=?,duration_minutes=?,progress_current=?,progress_total=?,progress_unit=?,tags_json=?,personal_note=?,started_at=?,completed_at=?,updated_at=datetime('now') WHERE recommendation_id=?`)
      .bind(item.state, item.release_year, item.duration_minutes, item.progress_current, item.progress_total, item.progress_unit, JSON.stringify(item.tags), item.personal_note || null, startedAt, completedAt, recommendationId),
    DB.prepare(`INSERT INTO learning_events
      (id,idempotency_key,event_type,actor_type,evidence_weight,recommendation_id,occurred_at,payload_json)
      VALUES (?,?,'personal_library_updated','user',0,?,datetime('now'),?)`)
      .bind(eventId, eventId, recommendationId, JSON.stringify({ before: { state: existing.state, branch_id: existing.branch_id, rating: existing.rating }, after: { state: item.state, branch_id: branch.id, rating: item.rating } })),
  ])
  try {
    await DB.prepare("INSERT OR REPLACE INTO search_idx(source,ref_id,text) VALUES ('rec',?,?)")
      .bind(recommendationId, [item.title, item.creator, item.item_type, item.tags.join(' ')].filter(Boolean).join(' ')).run()
  } catch { /* search maintenance will repair the best-effort projection */ }
