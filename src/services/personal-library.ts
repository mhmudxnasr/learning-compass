...
export async function updatePersonalLibraryItem(DB: D1Database, recommendationId: string, input: PersonalLibraryInput) {
  ...
  if (
    String(stored?.video_url || '') !== url &&
    normalizeUrlForDedup(String(stored?.video_url || '')) !== normalizeUrlForDedup(url)
  ) {
    return {
      ok: false as const,
      status: 409,
      ...
      message:
        'Canonical link changes require a verified guarded source replacement; the other personal fields were not changed.',
  ...
  await DB.batch([
    DB.prepare(
      `UPDATE recommendations SET video_title=?,creator=?,branch=?,user_rating=?,user_score=?,dedup_key=?,updated_at=datetime('now') WHERE id=?`,
