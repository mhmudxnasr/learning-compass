...
export async function loadCaptureQueue(
  ...
  const artifactRows = recommendationIds.length
    ? await DB.prepare(
        `SELECT id,filename,media_type,size_bytes,created_at,metadata_json FROM artifacts
  ...
  const mappedItems = (rows.results || []).map((row: any) => {
    ...
    const sourceHealth =
      sourceHealthMatchesCurrentUrl(row.source_health_checked_url, row.video_url) && row.source_health_status
        ? {
