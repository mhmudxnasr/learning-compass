...
export async function embedTexts(env: Pick<Bindings, 'AI'>, values: unknown[]): Promise<number[][] | null> {
  ...
  try {
    ...
    return vectors.length === text.length &&
      vectors.every((vector: unknown) => Array.isArray(vector) && vector.length === 768)
      ? vectors.map((vector: number[]) => vector.map(Number))
      : null
  } catch {
    return null
  }
...
export async function indexSemanticDocuments(
  env: Pick<Bindings, 'AI' | 'COMPASS_VECTORS' | 'DB'>,
  documents: Array<{
    id: string
    kind: 'recommendation' | 'thread' | 'note' | 'unit' | 'annotation'
    sourceId: string
    text: unknown
    language?: string
  }>,
) {
  if (!env.AI || !env.COMPASS_VECTORS)
    return { enabled: false, indexed: 0, skipped: documents.length, reason: 'semantic_bindings_unavailable' }
  const prepared = await Promise.all(
    documents.map(async (document) => ({
      ...document,
      text: normalizedText(document.text),
      hash: await hashText(normalizedText(document.text)),
    })),
  )
  ...
  for (const document of prepared) {
    ...
    const existing = await env.DB.prepare(
      `SELECT content_hash,status FROM semantic_documents WHERE document_kind=? AND source_id=?`,
    )
      .bind(document.kind, document.sourceId)
      .first<any>()
  ...
  const vectors = await embedTexts(
    env,
    changed.map((document) => document.text),
  )
  if (!vectors) return { enabled: true, indexed: 0, skipped: prepared.length, reason: 'embedding_unavailable' }
  try {
    await env.COMPASS_VECTORS.upsert(
      changed.map((document, index) => ({
        id: document.id,
        values: vectors[index],
        namespace: 'learning-compass',
        metadata: { kind: document.kind, source_id: document.sourceId, language: document.language || 'und' },
      })),
    )
    await env.DB.batch(
      changed.map((document) =>
        env.DB.prepare(
          `INSERT INTO semantic_documents(id,document_kind,source_id,content_hash,model,status,error,indexed_at,updated_at)
      VALUES (?,?,?,?,?,'indexed',NULL,datetime('now'),datetime('now'))
      ON CONFLICT(document_kind,source_id) DO UPDATE SET id=excluded.id,content_hash=excluded.content_hash,model=excluded.model,status='indexed',error=NULL,indexed_at=datetime('now'),updated_at=datetime('now')`,
        ).bind(document.id, document.kind, document.sourceId, document.hash, SEMANTIC_MODEL),
      ),
    )
    ...
    await env.DB.batch(
      ...
      ON CONFLICT(document_kind,source_id) DO UPDATE SET content_hash=excluded.content_hash,model=excluded.model,status='failed',error=excluded.error,updated_at=datetime('now')`,
        ).bind(document.id, document.kind, document.sourceId, document.hash, SEMANTIC_MODEL, message),
      ),
    )
...
export async function semanticSourceMatches(env: Pick<Bindings, 'AI' | 'COMPASS_VECTORS'>, query: unknown, topK = 20) {
  ...
  try {
    const response = await env.COMPASS_VECTORS.query(vectors[0], {
    ...
    return {
      enabled: true,
      matches: (response?.matches || []).map((match: any) => ({
        id: String(match.id),
    ...
    return { enabled: true, matches: [] as Array<{ id: string; score: number }> }
  }
}
