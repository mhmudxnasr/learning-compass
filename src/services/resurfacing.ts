...
function shapeItem(row: any, day: string, event?: any) {
  const overdueDays = Math.max(0, Math.floor((Date.parse(`${day}T00:00:00Z`) - Date.parse(`${row.due_at}T00:00:00Z`)) / 86400000))
  return {
...
async function todayPresentation(DB: D1Database, day: string) {
  return DB.prepare('SELECT id,recommendation_id,cairo_day,action FROM resurfacing_presentations WHERE cairo_day=? ORDER BY presented_at,id LIMIT 1').bind(day).first<any>()
}

async function eligibleItem(DB: D1Database, recommendationId: string, day: string) {
  return DB.prepare(`${itemQuery} AND r.id=? ORDER BY rs.due_at,rs.id LIMIT 1`).bind(day, day, day, recommendationId).first<any>()
}
...
export async function getDailyResurfacing(DB: D1Database, options: { limit?: number; now?: Date } = {}) {
  ...
  const rows = await DB.prepare(`${itemQuery} ORDER BY rs.due_at,rs.id,r.id LIMIT ?`).bind(day, day, day, limit).all<any>()
  const ranked = (rows.results || []).map((row) => shapeItem(row, day)).sort((a, b) => b.score - a.score || String(a.recommendation_id).localeCompare(String(b.recommendation_id)))
...
export async function setResurfacingPreference(DB: D1Database, recommendationId: string, starred: boolean) {
  const source = await DB.prepare("SELECT id FROM recommendations WHERE id=? AND status='consumed' AND deleted_at IS NULL").bind(recommendationId).first()
  if (!source) return null
  await DB.prepare(`INSERT INTO resurfacing_preferences(recommendation_id,starred,updated_at) VALUES (?,?,datetime('now'))
    ON CONFLICT(recommendation_id) DO UPDATE SET starred=excluded.starred,updated_at=datetime('now')`).bind(recommendationId, starred ? 1 : 0).run()
...
export async function createResurfacingPresentation(DB: D1Database, recommendationId: string, now = new Date()) {
  ...
  await DB.prepare(`INSERT OR IGNORE INTO resurfacing_presentations(id,recommendation_id,cairo_day,presented_at) VALUES (?,?,?,?)`)
    .bind(id, recommendationId, day, now.toISOString()).run()
...
export async function actOnResurfacing(DB: D1Database, eventId: string, action: ResurfacingAction) {
  const event = await DB.prepare('SELECT id,recommendation_id,cairo_day,action FROM resurfacing_presentations WHERE id=?').bind(eventId).first<any>()
  if (!event) return null
  if (event.action) return { id: event.id, recommendation_id: event.recommendation_id, action: event.action, reused: true }
  const due = await DB.prepare('SELECT id FROM resurfacing WHERE recommendation_id=? AND resolved_at IS NULL AND due_at<=date(?,\'start of day\') ORDER BY due_at,id LIMIT 1')
    .bind(event.recommendation_id, event.cairo_day).first<any>()
  const statements: D1PreparedStatement[] = [
    DB.prepare("UPDATE resurfacing_presentations SET action=?,acted_at=datetime('now') WHERE id=? AND action IS NULL").bind(action, eventId),
  ]
  if (due) statements.push(action === 'snooze'
...
export async function scheduleResurfacing(DB: D1Database, recommendationId: string) {
  const existing = await DB.prepare('SELECT id FROM resurfacing WHERE recommendation_id=? LIMIT 1').bind(recommendationId).first()
  if (existing) return false
  await DB.batch([
    DB.prepare("INSERT INTO resurfacing(recommendation_id,stage,due_at,notes) VALUES (?,'30d',date('now','+30 days'),'auto-scheduled on consume')").bind(recommendationId),
    DB.prepare("INSERT INTO resurfacing(recommendation_id,stage,due_at,notes) VALUES (?,'90d',date('now','+90 days'),'auto-scheduled on consume')").bind(recommendationId),
    DB.prepare("INSERT INTO resurfacing(recommendation_id,stage,due_at,notes) VALUES (?,'180d',date('now','+180 days'),'auto-scheduled on consume')").bind(recommendationId),
  ])
  return true
}

export async function backfillResurfacing(DB: D1Database) {
  const rows = await DB.prepare(`SELECT r.id FROM recommendations r
    JOIN recommendation_meta m ON m.recommendation_id=r.id
    JOIN tree_nodes b ON b.id=m.branch_id AND b.type IN ('branch','leaf') AND lower(COALESCE(b.status,''))!='pruned'
    JOIN tree_nodes d ON d.id=b.super_category AND d.type='category' AND lower(COALESCE(d.status,''))!='pruned'
    ...
    ORDER BY r.id LIMIT 100`).all<{ id: string }>()
  let created = 0
