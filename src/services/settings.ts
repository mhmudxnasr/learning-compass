...
export type TasteMapSettings = {
  appearance: {
    ...
    font?: string
    custom_font?: Record<string, string>
...
const color = (value: unknown, fallback?: string) =>
  typeof value === 'string' && /^(#[0-9a-f]{3,8}|rgba?\\([^)]{3,32}\\))$/i.test(value.trim()) ? value.trim() : fallback
const fontStack = (value: unknown, fallback?: string) =>
  typeof value === 'string' && value.length <= 240 && /^[a-zA-Z0-9 ,.'"()_-]+$/.test(value) ? value.trim() : fallback
...
export function normalizeSettings(input: unknown): TasteMapSettings {
  ...
  const custom_palette = Object.fromEntries(
    (
      ['brand', 'shell', 'highlight', 'accent', 'ink', 'surface', 'rail', 'seam', 'due', 'danger', 'map'] as const
    ).flatMap((key) => {
  ...
  const custom_font = Object.fromEntries(
    (['ui', 'display', 'reading', 'mono'] as const).flatMap((key) => {
  ...
  const typography = Object.fromEntries(
    (
      [
        ...
        ['lineHeight', 1.15, 2.3],
        ['letterSpacing', -0.04, 0.1],
        ['displayScale', 0.8, 1.5],
        ['readingMeasure', 45, 90],
      ] as const
    ).flatMap(([key, min, max]) => {
      const value = typographySource[key]
      return typeof value === 'number' && Number.isFinite(value) ? [[key, Math.min(max, Math.max(min, value))]] : []
    }),
  )
  const requestedTheme =
    typeof appearance.theme === 'string' && /^[a-z0-9_-]{1,40}$/.test(appearance.theme)
      ? appearance.theme
  ...
  return {
    appearance: {
      ...
      density: oneOf(
        appearance.density,
        ['comfortable', 'balanced', 'compact'] as const,
      ...
      font:
        typeof appearance.font === 'string' && /^[a-z0-9_-]{1,40}$/.test(appearance.font)
          ? appearance.font
          : defaultSettings.appearance.font,
      ...(Object.keys(custom_font).length ? { custom_font } : {}),
      ...(Object.keys(typography).length ? { typography } : {}),
    },
    learning: {
      retention:
        appearance && [85, 90, 95].includes(Number((source.learning as any)?.retention))
          ? (Number((source.learning as any).retention) as 85 | 90 | 95)
          : defaultSettings.learning.retention,
      queue_cap: 5,
    },
    srs_drafts: {
      enabled: bool((source.srs_drafts as any)?.enabled, defaultSettings.srs_drafts.enabled),
      minimum_rating: 7,
      auto_extract: bool((source.srs_drafts as any)?.auto_extract, defaultSettings.srs_drafts.auto_extract),
    },
    ai_curation: {
      enrich_capture: bool((source.ai_curation as any)?.enrich_capture, defaultSettings.ai_curation.enrich_capture),
    },
    profile_proposals: {
      review_required: bool(
        (source.profile_proposals as any)?.review_required,
        defaultSettings.profile_proposals.review_required,
      ),
    },
    profile_automation: {
      mode: oneOf(
        (source.profile_automation as any)?.mode,
        ['automatic', 'manual'] as const,
        defaultSettings.profile_automation.mode,
      ),
      policy_version: 'profile_v2',
    },
    recommendation_engine: {
      mode: oneOf(
        (source.recommendation_engine as any)?.mode,
        ['shadow', 'v2'] as const,
        defaultSettings.recommendation_engine.mode,
      ),
      engine_version: 'v2',
      objective_version: 'learning_value_v2',
    },
    ...
    atlas: {
      ...
      text_fade_threshold: clamp(
        (source.atlas as any)?.text_fade_threshold,
        -1,
        1,
        defaultSettings.atlas.text_fade_threshold,
      ),
      ...
      branch_link_thickness: clamp(
        (source.atlas as any)?.branch_link_thickness,
        0.1,
        6,
        defaultSettings.atlas.branch_link_thickness,
      ),
...
export async function loadSettings(DB: D1Database): Promise<TasteMapSettings> {
  const rows = await DB.prepare('SELECT setting_key,value_json FROM user_settings').all<{
    setting_key: keyof TasteMapSettings
    value_json: string
  }>()
  const input: Record<string, unknown> = {}
  for (const row of rows.results || []) {
    try {
      input[row.setting_key] = JSON.parse(row.value_json)
    } catch {}
  }
