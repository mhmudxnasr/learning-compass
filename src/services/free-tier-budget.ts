...
export function getFreeTierBudgetPolicy(
  readBudget = DAILY_READ_BUDGET,
  writeBudget = DAILY_WRITE_BUDGET,
) {
  const reads = {
    budget: readBudget,
  ...
  const blockers = [
    ...(readBudget <= 0 || reads.headroom < reads.required_headroom ? ['read_budget_missing_required_headroom'] : []),
    ...(writeBudget <= 0 || writes.headroom < writes.required_headroom ? ['write_budget_missing_required_headroom'] : []),
  ]
