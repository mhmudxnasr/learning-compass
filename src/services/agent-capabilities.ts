...
const BODY_SCHEMAS: Record<string, JsonSchema> = {
  ...
  'PATCH /recommendations/:id/source-url': objectSchema({ source_url: { type: 'string', format: 'uri' } }, [
    'source_url',
  ]),
  ...
  'POST /learning/core/threads/:id/lessons/:lessonId/sources': objectSchema(
    ...
    ['recommendation_id', 'role', 'branch_id'],
  ),
  ...
  'POST /learning/srs/review': objectSchema(
    { card_id: { type: 'string', minLength: 1 }, grade: { type: 'integer', minimum: 0, maximum: 5 } },
    ['card_id', 'grade'],
  ),
...
const PRECONDITION_OVERRIDES: Record<string, string[]> = {
  ...
  'POST /learning/srs/review': [
    ...
    'Confirm the exact card state before recording the review.',
  ],
}
