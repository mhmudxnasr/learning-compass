...
const PERSONAL_LIBRARY_PROPERTIES: Record<string, JsonSchema> = {
  ...
  state: {
    ...
    description:
      'Canonical personal lifecycle field. Use completed for watched, read, or finished; do not send personal_state.',
  },
...
const COMPASS_EVIDENCE_SCHEMA = objectSchema(
  {
    ...
    source_url: {
      ...
      format: 'uri',
      pattern: '^https?://',
      description: 'Direct public HTTP(S) source for this claim.',
    },
    anchor: {
      type: 'string',
      minLength: 1,
      maxLength: 1000,
      description: 'Preferred precise section, timestamp, page, or quotation anchor.',
    },
  },
  ['claim', 'source_url'],
)
...
const COMPASS_CANDIDATE_SCHEMA = objectSchema(
  {
    ...
    delivery_modes: {
      type: 'array',
      items: { type: 'string', enum: ['read', 'watch', 'listen', 'practice'] },
      minItems: 1,
      maxItems: 4,
    },
...
const RECALL_MUTATION_PRECONDITION_PROPERTIES: Record<string, JsonSchema> = {
  expected_content_revision: {
    type: 'integer',
    minimum: 1,
    description: 'Exact content_revision from the latest card read.',
  },
  expected_scheduler_revision: {
    type: 'integer',
    minimum: 1,
    description: 'Exact scheduler_revision from the latest card read.',
  },
  expected_status_revision: {
    type: 'integer',
    minimum: 1,
    description: 'Exact status_revision from the latest card read.',
  },
  expected_repair_status: {
    type: 'string',
    enum: ['active', 'paused', 'retired'],
    description: 'Exact repair_status from the latest card read.',
  },
}
const RECALL_MUTATION_PRECONDITION_FIELDS = [
  'expected_content_revision',
  'expected_scheduler_revision',
  'expected_status_revision',
  'expected_repair_status',
...
const BODY_SCHEMAS: Record<string, JsonSchema> = {
  'POST /capture': objectSchema(
    {
      ...
      branch_reason: { type: 'string', maxLength: 500 },
    },
    ['source', 'branch_id'],
  ),
  ...
  'POST /assistant/interpret': objectSchema(
    {
      message: { type: 'string', minLength: 1, maxLength: 12000 },
      mode: { type: 'string', enum: ['log', 'questions', 'mixed'] },
    },
    ['message'],
  ),
  'POST /hardcover/sync': objectSchema({}, []),
  'POST /hardcover/import': objectSchema(
    {
      branch_id: { type: 'string', minLength: 1, maxLength: 160 },
      book_ids: { type: 'array', items: { type: 'integer', minimum: 1 }, maxItems: 500 },
    },
    ['branch_id'],
  ),
  'POST /capture/feeds': objectSchema(
    {
      url: { type: 'string', format: 'uri' },
      branch_id: { type: 'string', minLength: 1 },
  ...
  'POST /capture/:id/triage': objectSchema(
    {
      action: {
        ...
        description:
          'queue creates a commitment, dequeue neutrally returns an active Queue item to captured, and exclude is an administrative exclusion.',
      },
      ...
      reason: {
        ...
        minLength: 1,
        maxLength: 120,
        description: 'Administrative exclusion reason; used only with action=exclude.',
      },
    },
    ['action'],
  ),
  'POST /sessions/start': objectSchema(
    {
    ...
    ['recommendation_id', 'thread_id', 'target_kind'],
  ),
    ...
    ...objectSchema(
      {
        recommendation_id: {
          type: 'string',
          minLength: 1,
          description: 'Exact canonical source ID; do not send source_id.',
        },
        source_url: { type: 'string', format: 'uri' },
        title: {
          type: 'string',
          minLength: 1,
          description: 'Exact source title fallback when no canonical ID or URL is available.',
        },
        thread_id: { type: 'string' },
        branch_id: { type: 'string', minLength: 1, maxLength: 160 },
        feedback: {
          ...
          minLength: 1,
          maxLength: 10000,
          description: "The user's exact feedback preserved verbatim; never summarize or omit it.",
        },
        score: {
          type: 'number',
          minimum: 0,
          maximum: 10,
          description:
            'Canonical numeric rating field. Include it whenever the user supplied a rating; do not send rating.',
        },
        ...
        effort: { type: 'string', enum: ['light', 'moderate', 'deep'] },
        length_minutes: { type: 'integer', minimum: 0, maximum: 100000 },
      },
      ['feedback'],
    ),
    anyOf: [{ required: ['recommendation_id'] }, { required: ['source_url'] }, { required: ['title'] }],
  },
  ...
  'POST /notes/:id/distillation/highlights': objectSchema(
    {
      section_key: { type: 'string', minLength: 1, maxLength: 120 },
      block_index: { type: 'integer', minimum: 0 },
      block_checksum: { type: 'string', minLength: 1, maxLength: 160 },
      claim_text: { type: 'string', minLength: 1, maxLength: 4000 },
    },
    ['section_key', 'block_index', 'block_checksum', 'claim_text'],
  ),
  'POST /notes/:id/distillation/syntheses': objectSchema(
    { synthesis_text: { type: 'string', minLength: 1, maxLength: 4000 } },
    ['synthesis_text'],
  ),
  'POST /learning/core/units/:id/relations': objectSchema(
    {
      target_unit_id: { type: 'string', minLength: 1 },
      relation_type: {
        type: 'string',
        enum: ['supports', 'contradicts', 'qualifies', 'example_of', 'depends_on', 'applies_to'],
      },
      confidence: { type: 'number', minimum: 0, maximum: 1 },
      why: { type: 'string', minLength: 1, maxLength: 4000 },
      source_anchor_id: { type: 'string', minLength: 1 },
      target_anchor_id: { type: 'string', minLength: 1 },
    },
    ['target_unit_id', 'relation_type', 'why', 'source_anchor_id'],
  ),
  'PATCH /learning/core/contradictions/:id': objectSchema(
    {
      review_state: { type: 'string', enum: ['accepted', 'resolved', 'dismissed'] },
  ...
  'POST /brain/resurfacing/presentations': objectSchema(
    { recommendation_id: { type: 'string', minLength: 1, maxLength: 100 } },
    ['recommendation_id'],
  ),
  'POST /brain/resurfacing/:eventId/action': objectSchema(
    { action: { type: 'string', enum: ['reviewed', 'snooze', 'dismissed'] } },
    ['action'],
  ),
  'POST /recommendations/map': objectSchema(
    {
  ...
  'PATCH /recommendations/:id/source-url': objectSchema(
    {
      source_url: { type: 'string', format: 'uri' },
      expected_source_url: { type: 'string', minLength: 1, maxLength: 2048 },
    },
    ['source_url', 'expected_source_url'],
  ),
  'POST /recommendations/:id/source-health/check': objectSchema({
    expected_source_url: { type: 'string', format: 'uri' },
  }),
  'POST /recommendations/:id/source-url/verify': objectSchema(
    {
      source_url: { type: 'string', format: 'uri' },
      expected_source_url: { type: 'string', minLength: 1, maxLength: 2048 },
    },
  ...
  'POST /learning/core/threads/:id/stages/:stageId/lessons': objectSchema(
    {
  ...
  'POST /learning/core/threads/:id/sources': objectSchema(
    {
      recommendation_id: { type: 'string', minLength: 1 },
      role: { type: 'string', enum: ['primary', 'supporting', 'counterevidence', 'reference'] },
      expected_contribution: { type: 'string', minLength: 1, maxLength: 1000, pattern: '\\S' },
      position: { type: 'integer', minimum: 0 },
      branch_id: {
        type: 'string',
        minLength: 1,
        description: 'Optional optimistic precondition; persisted Library ownership remains authoritative.',
      },
    },
    ['recommendation_id', 'expected_contribution'],
  ),
  'PATCH /learning/core/threads/:id/sources/:sourceId': objectSchema({
    role: { type: 'string', enum: ['primary', 'supporting', 'counterevidence', 'reference'] },
    expected_contribution: { type: 'string', minLength: 1, maxLength: 1000, pattern: '\\S' },
    position: { type: 'integer', minimum: 0 },
  }),
  'POST /learning/core/threads/:id/stages/:stageId/sources': objectSchema(
    {
      recommendation_id: { type: 'string', minLength: 1 },
      role: { type: 'string', enum: ['primary', 'case', 'challenge', 'reference', 'optional'] },
  ...
  'PATCH /learning/core/threads/:id/stages/:stageId/sources/:sourceId': objectSchema({
    role: { type: 'string', enum: ['primary', 'case', 'challenge', 'reference', 'optional'] },
    required: { type: 'boolean' },
    expected_contribution: { type: 'string', minLength: 1, maxLength: 1000, pattern: '\\S' },
    position: { type: 'integer', minimum: 0 },
  }),
  'POST /learning/core/threads/:id/lessons/:lessonId/sources': objectSchema(
    {
      recommendation_id: { type: 'string', minLength: 1 },
      role: { type: 'string', enum: ['primary', 'case', 'challenge', 'reference', 'optional'] },
      expected_contribution: { type: 'string', minLength: 1, maxLength: 1000, pattern: '\\S' },
      expected_source_url: {
        type: 'string',
        format: 'uri',
        description: 'Optional canonical URL precondition. Required when attaching a reviewed Find material result.',
      },
      position: { type: 'integer', minimum: 0 },
      branch_id: {
        type: 'string',
  ...
  'PATCH /learning/core/threads/:id/lessons/:lessonId/sources/:sourceId': objectSchema({
    role: { type: 'string', enum: ['primary', 'case', 'challenge', 'reference', 'optional'] },
    expected_contribution: { type: 'string', minLength: 1, maxLength: 1000, pattern: '\\S' },
    position: { type: 'integer', minimum: 0 },
  }),
  'POST /learning/core/threads/:id/lessons/:lessonId/material-request': objectSchema({
    idempotency_key: { type: 'string', minLength: 1, maxLength: 160 },
  }),
  'POST /learning/core/canon/domains': objectSchema(
    {
      title: { type: 'string', minLength: 1 },
      slug: { type: 'string' },
      kind: { type: 'string', enum: ['family', 'domain'] },
    ...
    ['title', 'branch_id', 'boundary'],
  ),
  'PATCH /learning/core/canon/domains/:id': objectSchema({
    title: { type: 'string' },
    boundary: { type: 'string' },
    orientation: { type: 'string' },
    branch_id: { type: 'string' },
    curation_status: { type: 'string', enum: ['unmapped', 'curating', 'complete'] },
    validation_state: { type: 'string', enum: ['untested', 'exploring', 'field_tested'] },
    sort_order: { type: 'integer' },
  }),
  'PUT /learning/core/canon/domains/:id/entries/:role': objectSchema(
    {
      title: { type: 'string', minLength: 1 },
      author: { type: 'string', minLength: 1 },
      canonical_url: { type: 'string', format: 'uri' },
      isbn: { type: 'string' },
      why_slot: { type: 'string', minLength: 1 },
      beginner_case: { type: 'string', minLength: 1 },
      expert_case: { type: 'string', minLength: 1 },
      unique_contribution: { type: 'string', minLength: 1 },
      limitations: { type: 'string', minLength: 1 },
      difficulty: { type: 'string', minLength: 1 },
      rejected_alternative: { type: 'string', minLength: 1 },
      rejection_reason: { type: 'string', minLength: 1 },
      evidence: { type: 'array', minItems: 1 },
      recommendation_id: { type: 'string' },
      editorial_status: { type: 'string', enum: ['draft', 'reviewed', 'approved'] },
      validation_state: { type: 'string', enum: ['untested', 'exploring', 'field_tested'] },
  ...
  'POST /learning/srs/review': objectSchema(
    {
      card_id: { type: 'string', minLength: 1 },
      grade: { type: 'integer', minimum: 0, maximum: 5 },
      ...RECALL_MUTATION_PRECONDITION_PROPERTIES,
    },
    ['card_id', 'grade', ...RECALL_MUTATION_PRECONDITION_FIELDS],
  ),
  'PUT /srs/drafts/:id': objectSchema({
    question: { type: 'string', minLength: 1, description: 'Recall question written primarily in Arabic.' },
    answer: { type: 'string', minLength: 1, description: 'Recall answer written primarily in Arabic.' },
    topic: { type: 'string' },
    branch: { type: 'string' },
    card_type: { type: 'string' },
    source_anchor: { type: 'string' },
  }),
  'POST /learning/srs/create': objectSchema(
    {
      thread_id: { type: 'string' },
      stage_id: { type: 'string' },
      lesson_id: { type: 'string' },
      ...
      source_anchor: { type: 'string', maxLength: 1000 },
      question: { type: 'string', minLength: 1, description: 'Recall question written primarily in Arabic.' },
      answer: { type: 'string', minLength: 1, description: 'Recall answer written primarily in Arabic.' },
      topic: { type: 'string' },
      branch: { type: 'string' },
  ...
  'PUT /learning/srs/cards/:id': objectSchema(
    {
      question: { type: 'string', minLength: 1 },
      answer: { type: 'string', minLength: 1 },
      change_kind: { type: 'string', enum: ['wording', 'semantic'] },
      reason: { type: 'string', maxLength: 1000 },
      ...RECALL_MUTATION_PRECONDITION_PROPERTIES,
    },
    ['question', 'answer', 'change_kind', ...RECALL_MUTATION_PRECONDITION_FIELDS],
  ),
  'POST /learning/srs/cards/:id/status': objectSchema(
    {
      status: { type: 'string', enum: ['active', 'paused', 'retired'] },
      reason: { type: 'string', maxLength: 1000 },
      ...RECALL_MUTATION_PRECONDITION_PROPERTIES,
    },
    ['status', ...RECALL_MUTATION_PRECONDITION_FIELDS],
  ),
  'POST /learning/srs/cards/:id/reset': objectSchema(
    {
      confirm: { const: true },
      reason: { type: 'string', maxLength: 1000 },
      ...RECALL_MUTATION_PRECONDITION_PROPERTIES,
    },
    ['confirm', ...RECALL_MUTATION_PRECONDITION_FIELDS],
  ),
  ...
  'POST /agent/jobs/reconcile': objectSchema({
    apply: {
      type: 'boolean',
      description: 'False performs a dry reconciliation report; true applies only the conservative declared repairs.',
    },
  }),
  'POST /agent/jobs/:id/claim': objectSchema(
    {
      worker: {
        type: 'string',
        minLength: 1,
        maxLength: 120,
        description: 'Stable lease owner reused for heartbeat, checkpoint, completion, and failure.',
      },
    },
    ['worker'],
  ),
  'POST /agent/jobs/:id/checkpoint': objectSchema(
    {
      ...
      step: {
        ...
        minLength: 1,
        maxLength: 120,
        description: 'The next workflow step declared by the exact job payload.',
      },
      evidence: { type: 'object', description: 'Step-specific deterministic evidence required by the job contract.' },
    },
    ['worker', 'step', 'evidence'],
  ),
  'POST /agent/jobs/:id/complete': objectSchema(
    {
      worker: {
        type: 'string',
        minLength: 1,
        maxLength: 120,
        description: 'Must match the active lease owner; all other fields are job-output-contract specific.',
      },
    },
    ['worker'],
  ),
  'POST /agent/jobs/:id/fail': objectSchema(
    {
      worker: { type: 'string', minLength: 1, maxLength: 120 },
  ...
  'POST /agent/jobs/:id/heartbeat': objectSchema({ worker: { type: 'string', minLength: 1, maxLength: 120 } }, [
    'worker',
  ]),
  'PATCH /learning/core/threads/:id/projects/:projectId': objectSchema(
    { status: { type: 'string', enum: ['not_started', 'in_progress', 'completed', 'deferred'] } },
    ['status'],
  ),
  'PATCH /learning/core/threads/:id/stages/:stageId': objectSchema({
    position: { type: 'integer', minimum: 0 },
    title: { type: 'string' },
    objective: { type: 'string' },
    description: { type: 'string' },
    stage_type: { type: 'string', enum: ['orientation', 'curriculum', 'application', 'advanced'] },
  ...
  'POST /analytics/hermes/improvements': objectSchema(
    {
...
const VERIFICATION_OVERRIDES: Record<string, string | null> = {
  ...
  'POST /learning/core/threads/:id/lessons/:lessonId/material-request':
...
const PRECONDITION_OVERRIDES: Record<string, string[]> = {
  ...
  'DELETE /recommendations/:id/permanent': [
    'Target must be archived/non-active.',
    'Explicit irreversible confirmation is required.',
  ],
  'DELETE /notes/:id': [
    'Read and resolve the exact note before irreversible deletion.',
    'Explicit destructive intent is required.',
  ],
  ...
  'POST /learning/srs/review': [
    'The learner must supply or confirm the recall grade.',
    'Read the exact card and forward its complete content/scheduler/status revision token and repair status.',
  ],
  'PUT /learning/srs/cards/:id': [
    'Read the exact card and forward its complete content/scheduler/status revision token and repair status.',
  ],
  'POST /learning/srs/cards/:id/status': [
    'Read the exact card and forward its complete content/scheduler/status revision token and repair status.',
  ],
  'POST /learning/srs/cards/:id/reset': [
    'Read the exact card, forward its complete content/scheduler/status revision token and repair status, and require explicit learner confirmation before resetting its schedule.',
  ],
  'PATCH /recommendations/:id/source-url': [
    'Read the exact current source URL and send it as expected_source_url.',
    'Verify the candidate URL directly and reject any deduplication collision before replacement.',
  ],
  'POST /learning/core/threads/:id/stages/:stageId/sources': [
    'Search the existing Library first.',
    'Use persisted non-pruned branch and domain ownership; client branch_id is only an optimistic precondition.',
  ],
  'POST /learning/core/threads/:id/lessons/:lessonId/sources': [
    'Search the existing Library first.',
    'Use persisted non-pruned branch and domain ownership; client branch_id is only an optimistic precondition.',
  ],
  'POST /learning/core/threads/:id/lessons/:lessonId/material-request': [
    'Confirm the exact lesson is in the current Level, incomplete, and has neither authored content nor an attached source.',
    'This explicit request may abstain and cannot attach, Queue, start, or advance anything.',
  ],
}
...
const capabilityPathSource = (path: string, capture = false) =>
  path
    .split('/')
    .map((segment) =>
...
export function resolveCapabilityReadbacks(
  key: string,
  ...
  try {
    concretePathname = new URL(concretePath, 'https://agent.invalid').pathname
  } catch {
    /* use the bounded path fallback */
  }
  ...
  return values.map((value) =>
    template.replace(new RegExp(`:${placeholder}(?![A-Za-z0-9_])`), encodeURIComponent(String(value))),
  )
}
...
const deriveRisk = (method: AgentMethod, path: string) => {
  ...
  if (
    /permanent|\/learning\/core\/threads\/:id$|\/notes\/:id$|engine\/activate|recalibrate|repair|notifications\/test/.test(
      path,
    )
  )
    return 'high'
...
export function buildCapabilityCatalog(
  capabilities: readonly CapabilityTuple[],
  filters: { domain?: string; intent?: string; method?: string; q?: string } = {},
) {
  ...
  return capabilities
    .map(([method, path, description]) => {
      ...
      const requiredFields = Array.isArray((requestBodySchema as any)?.required)
        ? (requestBodySchema as any).required
        : []
      return {
        method,
        path,
        ...
        preconditions:
          PRECONDITION_OVERRIDES[key] || (method === 'GET' ? [] : ['Read the exact target before mutation.']),
    ...
    .filter((item) => {
      ...
      return (
        !q || `${item.method} ${item.path} ${item.description} ${item.domain} ${item.intent}`.toLowerCase().includes(q)
      )
    })
}
...
export function buildAgentOpenApi(origin: string, capabilities: readonly CapabilityTuple[]) {
  ...
  for (const capability of catalog) {
    ...
    if (capability.method !== 'GET')
      parameters.push({
        ...
        required: false,
        description:
          'Stable retry key for direct product mutations. Generic /agent/request uses body idempotency_key instead.',
    ...
    const operation: Record<string, unknown> = {
      ...
      responses: {
        '200': {
          ...
          content: { 'application/json': { schema: capability.response_schema } },
        },
    ...
    if (capability.request_body_schema)
  ...
  paths['/agent/request'] = {
    post: {
      ...
      requestBody: {
        required: true,
        content: { 'application/json': { schema: { $ref: '#/components/schemas/AgentRequest' } } },
      },
      responses: {
        '200': {
          description: 'Dry-run or verified operation receipt',
          content: { 'application/json': { schema: { $ref: '#/components/schemas/AgentReceipt' } } },
        },
        '400': { $ref: '#/components/responses/ValidationError' },
        '403': { $ref: '#/components/responses/Forbidden' },
        '409': { $ref: '#/components/responses/Conflict' },
  ...
  return {
    ...
    components: {
      schemas: {
        AgentAssertion: objectSchema(
          { path: { type: 'string', pattern: '^/' }, field: { type: 'string', minLength: 1 }, equals: {} },
          ['path', 'field', 'equals'],
        ),
        AgentRequest: BODY_SCHEMAS['POST /agent/request'],
        AgentReceipt: objectSchema(
          {
            intent: { type: 'string' },
            target: { type: 'string' },
            before: {},
          ...
          ['intent', 'target', 'before', 'mutation_or_job', 'after', 'evidence', 'blocker'],
        ),
      },
      responses: {
        ...
        Conflict: {
          description: 'Product gate or optimistic precondition failed',
          content: { 'application/json': { schema: errorSchema } },
        },
