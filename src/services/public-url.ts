...
export function validatePublicHttpUrl(value: unknown) {
  ...
  const invalidIpv6 = host.includes(':') && (host === '::1' || host === '::' || isIpv4MappedIpv6(host) || /^(?:fc|fd|fe8|fe9|fea|feb)/i.test(host) || host.startsWith('2001:db8:'))
...
export function validatePushEndpoint(value: unknown) {
  ...
  if (!isTrustedWebPushHost(url.hostname.toLowerCase()) || (url.port && url.port !== '443')) throw new Error('push_endpoint_origin_not_allowed')
  return publicUrl
}
