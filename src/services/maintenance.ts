...
async function rebuildSearch(DB: D1Database) {
  ...
  await DB.batch([
    ...
    DB.prepare(
      "INSERT INTO search_idx(source,ref_id,text) SELECT 'annotation',id,TRIM(COALESCE(quote,'') || ' ' || COALESCE(context_before,'') || ' ' || COALESCE(context_after,'') || ' ' || COALESCE(language,'')) FROM source_annotations WHERE status='active'",
    ),
