...
const locatorFor = (annotation: any, selector: Record<string, unknown>) => clean(
  selector.locator || selector.page || selector.timestamp || selector.css_selector || selector.url || annotation.source_url,
  1000,
) || `quote:${clean(annotation.source_checksum, 160)}`

const anchorTypeFor = (locatorType: string) => locatorType === 'video'
  ? 'timestamp'
  : locatorType === 'pdf'
    ? 'page'
    : locatorType === 'web'
...
async function sourceBelongsToThread(DB: D1Database, recommendationId: string, threadId: string) {
  return DB.prepare(`SELECT t.id FROM learning_threads t
    ...
    .bind(threadId, recommendationId, recommendationId, recommendationId).first<any>()
}
...
export async function loadSourceAnnotationEvidence(
  ...
  const row = await DB.prepare(`SELECT a.id,a.recommendation_id,a.artifact_id,a.thread_id,a.branch_id,a.locator_type,a.selector_json,a.quote,
    ...
    WHERE a.id=? AND a.status='active'`).bind(id).first<any>()
  if (!row) throw new SourceAnnotationEvidenceError('annotation_not_found', 'The source anchor is unavailable, archived, or no longer has valid source ownership.', 404)
  if (!row.branch_id || row.branch_id !== row.canonical_branch_id) {
    throw new SourceAnnotationEvidenceError('annotation_branch_stale', 'The source anchor no longer matches the source’s canonical branch. Review the anchor before deriving from it.')
  }
  ...
  if (expectedRecommendationId && row.recommendation_id !== expectedRecommendationId) {
    throw new SourceAnnotationEvidenceError('annotation_source_conflict', 'The source anchor belongs to a different source.')
  }
  if (expectedBranchId && row.branch_id !== expectedBranchId) {
    throw new SourceAnnotationEvidenceError('annotation_branch_conflict', 'The source anchor belongs to a different branch.')
  }
  if (row.artifact_id) {
    const artifact = await DB.prepare(`SELECT id FROM artifacts WHERE id=? AND json_extract(metadata_json,'$.recommendation_id')=?`)
      .bind(row.artifact_id, row.recommendation_id).first<any>()
    if (!artifact) throw new SourceAnnotationEvidenceError('annotation_artifact_stale', 'The source anchor’s artifact no longer belongs to its source.')
  }
  const threadId = expectedThreadId || clean(row.thread_id, 120)
  if (expectedThreadId && row.thread_id && row.thread_id !== expectedThreadId) {
    throw new SourceAnnotationEvidenceError('annotation_scope_conflict', 'The source anchor belongs to a different Learning Thread.')
  }
  if (threadId && !(await sourceBelongsToThread(DB, row.recommendation_id, threadId))) {
    throw new SourceAnnotationEvidenceError('annotation_scope_conflict', 'The source anchor’s source is not placed in this Learning Thread.')
  }
  const selector = parseSelector(row.selector_json)
  const checksum = clean(row.source_checksum, 160)
  if (!checksum) throw new SourceAnnotationEvidenceError('annotation_checksum_missing', 'The source anchor has no durable evidence checksum.')
  return {
    id: row.id,
