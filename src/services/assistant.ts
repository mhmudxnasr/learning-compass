...
function normalizeItem(value: any) {
  ...
  return {
    ...
    creator: text(value?.creator, 300),
    item_type: itemType,
    state,
    url: text(value?.url, 2048),
    release_year: value?.release_year == null ? null : Number(value.release_year),
    ...
    tags: Array.isArray(value?.tags)
      ? [...new Set(value.tags.map((tag: unknown) => text(tag, 60)).filter(Boolean))].slice(0, 20)
      : [],
...
export async function interpretAssistantMessage(
  env: Pick<Bindings, 'OPENCODE_ZEN_API_KEY'>,
  message: string,
  mode: AssistantMode = 'mixed',
) {
  const cleanMessage = text(message, 12000)
  if (!cleanMessage)
    return {
      available: true,
      reply: 'اكتبلي حاجة استهلكتها أو حاجة عايزني أسألك عنها.',
      items: [],
      profile_signals: [],
      questions: [],
      mode,
  ...
  if (!result)
    return {
      available: false,
      reply: 'المساعد مش متاح دلوقتي. اكتبها في النموذج العادي وأنا هسجلها، أو جرّب تاني بعد شوية.',
      items: [],
      profile_signals: [],
      questions: [],
      mode,
  ...
  if (!parsed || typeof parsed !== 'object')
    return {
      available: false,
      reply: 'فهمت إن فيه حاجة عايز تسجلها، بس محتاج تكتبها بشكل أوضح.',
      items: [],
      profile_signals: [],
      questions: [],
      mode,
  ...
  const profileSignals = Array.isArray(parsed.profile_signals)
    ? parsed.profile_signals
        .map((signal: any) => ({
          key: text(signal?.key, 100),
          category: text(signal?.category || 'taste', 80),
          value: text(signal?.value, 1000),
          confidence: Math.max(0, Math.min(1, Number(signal?.confidence || 0))),
        }))
        .filter((signal: any) => signal.key && signal.value && signal.confidence >= 0.65)
        .slice(0, 8)
    : []
  const questions = Array.isArray(parsed.questions)
    ? parsed.questions
        .map((question: unknown) => text(question, 500))
        .filter(Boolean)
        .slice(0, 2)
    : []
  return {
    available: true,
    model: result.model,
    reply: text(parsed.reply, 1200) || 'تمام، فهمت عليك.',
    items,
