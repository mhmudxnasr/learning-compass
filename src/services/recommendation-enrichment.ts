  ...
  for (let start = 0; start < ids.length; start += CHUNK_SIZE) {
    ...
    const [notes, recall, artifacts] = await Promise.all([
      DB.prepare(
        ...
        .all<any>(),
      DB.prepare(
        `SELECT recommendation_id,COUNT(*) recall_count,SUM(CASE WHEN due_at IS NOT NULL AND due_at<=date('now') THEN 1 ELSE 0 END) due_count FROM srs_cards WHERE recommendation_id IN (${placeholders}) GROUP BY recommendation_id`,
      )
        .bind(...chunk)
        .all<any>(),
      DB.prepare(
