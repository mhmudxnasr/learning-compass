...
export async function loadIntegrityHealth(DB: D1Database) {
  const [open, metadata, sessions, notes, reviews] = await Promise.all([
    ...
    DB.prepare(
      `SELECT COUNT(*) count FROM recommendation_meta m LEFT JOIN recommendations r ON r.id=m.recommendation_id WHERE r.id IS NULL AND NOT EXISTS (SELECT 1 FROM integrity_quarantine q WHERE q.entity_type='recommendation_meta' AND q.entity_id=m.recommendation_id AND q.reason='missing_recommendation')`,
    ).first<any>(),
    DB.prepare(
      `SELECT COUNT(*) count FROM learning_sessions s LEFT JOIN recommendations r ON r.id=s.recommendation_id WHERE s.recommendation_id IS NOT NULL AND r.id IS NULL AND NOT EXISTS (SELECT 1 FROM integrity_quarantine q WHERE q.entity_type='learning_session' AND q.entity_id=s.id AND q.reason='missing_recommendation')`,
    ).first<any>(),
    DB.prepare(
      `SELECT COUNT(*) count FROM notes n LEFT JOIN recommendations r ON r.id=n.recommendation_id WHERE n.recommendation_id IS NOT NULL AND r.id IS NULL AND NOT EXISTS (SELECT 1 FROM integrity_quarantine q WHERE q.entity_type='note' AND q.entity_id=n.id AND q.reason='missing_recommendation')`,
    ).first<any>(),
    DB.prepare(
      `SELECT COUNT(*) count FROM srs_review_events e LEFT JOIN srs_cards x ON x.id=e.card_id WHERE x.id IS NULL AND NOT EXISTS (SELECT 1 FROM integrity_quarantine q WHERE q.entity_type='srs_review_event' AND q.entity_id=CAST(e.id AS TEXT) AND q.reason='missing_card')`,
    ).first<any>(),
  ])
  const details = {
  ...
  return {
    ok: activeOrphans === 0 && unresolved === 0,
...
export async function loadJobHealth(DB: D1Database) {
  const [counts, stale, oldest, failures, delayed, deadLetters, overdueRetry] = await Promise.all([
    ...
    DB.prepare(
      "SELECT COUNT(*) count FROM agent_jobs WHERE status='failed' AND datetime(updated_at)>=datetime('now','-24 hours')",
    ).first<any>(),
...
export async function loadMaintenanceHealth(DB: D1Database, now = Date.now()) {
  const rows = await DB.prepare(
    "SELECT key,value FROM kv_store WHERE key IN ('maintenance_last_run','maintenance_last_success','maintenance_last_receipt','fts_last_sync')",
  ...
  let receipt: unknown
  try {
    receipt = values.maintenance_last_receipt ? JSON.parse(values.maintenance_last_receipt) : null
  } catch {
    receipt = null
  }
  return {
...
export async function loadRecoveryHealth(DB: D1Database, now = Date.now()) {
  const sourceCount = await DB.prepare('SELECT COUNT(*) count FROM recommendations WHERE deleted_at IS NULL').first<{
    count: number
  }>()
  let latest: any
  try {
    latest = await DB.prepare(
      `SELECT id,status,storage_target,d1_sha256,d1_bytes,artifact_count,artifact_bytes,created_at,restore_rehearsed_at,error
      FROM recovery_backups WHERE status='verified' ORDER BY created_at DESC LIMIT 1`,
    ).first<any>()
  } catch {
    latest = null
  }
...
export async function loadOperationalHealth(env: Bindings, now = Date.now()) {
  ...
  try {
    ...
    const blockers = [
      ...
      ...(!integrity.ok
        ? [`${integrity.active_orphans} active and ${integrity.quarantined_unresolved} unresolved integrity records`]
        : []),
      ...(!jobs.ok
        ? [
            `${jobs.failed_last_24h} recent failures, ${jobs.overdue_retries} overdue retries, ${jobs.stale_running} stale leases, ${jobs.dead_letters} dead letters`,
          ]
        : []),
      ...
      ...(Object.entries(release.bindings).filter(([, configured]) => !configured).length
        ? [
            `Release bindings unavailable: ${Object.entries(release.bindings)
