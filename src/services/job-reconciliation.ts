...
export async function reconcileVisualJobs(env: Bindings, apply = false) {
  const jobs = await env.DB.prepare(`SELECT id,status,payload_json,result_json,created_at FROM agent_jobs
    WHERE job_type='visualise_source' AND status IN ('pending','retry') ORDER BY created_at,id`).all<any>()
  const actions: ReconciliationAction[] = []
  for (const job of jobs.results || []) {
    const payload = parse(job.payload_json)
    const recommendationId = String(payload.recommendation_id || '').trim() || null
    if (!recommendationId) {
      actions.push({ job_id: job.id, recommendation_id: null, action: 'cancel', reason: 'Job has no canonical source target.' })
      continue
    }
    const recommendation = await env.DB.prepare('SELECT id FROM recommendations WHERE id=? AND deleted_at IS NULL').bind(recommendationId).first<{ id: string }>()
    if (!recommendation) {
  ...
  if (apply) {
    for (const action of actions) {
      ...
      const result = action.action === 'complete'
        ? { pair_id: action.pair_id, html_artifact_id: action.html_artifact_id, pdf_artifact_id: action.pdf_artifact_id, validation_status: 'passed', reconciled: true, reconciled_at: new Date().toISOString() }
        : { reconciled: true, cancelled_reason: action.reason, reconciled_at: new Date().toISOString() }
      await env.DB.batch([
        action.action === 'complete'
          ? env.DB.prepare("UPDATE agent_jobs SET status='completed',workflow_step='verify_record',result_json=?,error=NULL,lease_owner=NULL,lease_expires_at=NULL,updated_at=datetime('now') WHERE id=? AND status IN ('pending','retry')").bind(JSON.stringify(result), action.job_id)
          : env.DB.prepare("UPDATE agent_jobs SET status='cancelled',result_json=?,error=?,lease_owner=NULL,lease_expires_at=NULL,updated_at=datetime('now') WHERE id=? AND status IN ('pending','retry')").bind(JSON.stringify(result), action.reason.slice(0, 1000), action.job_id),
