...
export function validateLiteVisualCheckpointEvidence(step: string, evidence: Record<string, unknown> = {}) {
  ...
  if (step === 'author_html') {
    ...
    for (const field of ['word_count', 'span_count']) if (!Number.isInteger(Number(evidence[field])) || Number(evidence[field]) < 1) failures.push(`${field} must be a positive integer`)
  }
  if (step === 'render_pdf') {
    ...
    if (!Number.isInteger(evidence.claim_count) || Number(evidence.claim_count) < 1) failures.push('claim_count must be a positive integer')
