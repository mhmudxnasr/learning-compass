...
export const LITE_VISUAL_STAGES = [
  'resolve_source',
  'extract_source',
  'map_coverage',
  'author_html',
...
export function resolveLiteVisualResume(payload: Record<string, unknown>, workflowStep: unknown) {
  const isCurrent =
    ...
    Number(payload.workflow_version) === LITE_VISUAL_WORKFLOW_VERSION
  const requestedStep = String(workflowStep || '')
  return {
    is_current: isCurrent,
    resume_from:
      isCurrent && LITE_VISUAL_STAGES.includes(requestedStep as (typeof LITE_VISUAL_STAGES)[number])
        ? requestedStep
        : 'resolve_source',
  }
}
...
export function validateLiteVisualCheckpointEvidence(step: string, evidence: Record<string, unknown> = {}) {
  ...
  for (const field of LITE_VISUAL_CHECKPOINT_REQUIREMENTS[step] || [])
    if (!present(evidence[field])) failures.push(`${field} is required`)
  if (step === 'map_coverage') {
    if (evidence.schema_version !== LITE_VISUAL_SOURCE_EXTRACTION_SCHEMA)
      failures.push(`schema_version must be ${LITE_VISUAL_SOURCE_EXTRACTION_SCHEMA}`)
    if (evidence.status !== 'complete') failures.push('status must be complete')
    if (!SHA256_RE.test(String(evidence.content_sha256 || '')))
      failures.push('content_sha256 must be a full lowercase SHA-256')
    if (!SHA256_RE.test(String(evidence.cache_key || ''))) failures.push('cache_key must be a full lowercase SHA-256')
    if (!Number.isInteger(Number(evidence.word_count)) || Number(evidence.word_count) < 1)
      failures.push('word_count must be a positive integer')
  ...
  if (step === 'author_html') {
    ...
    for (const field of ['word_count', 'span_count']) {
      ...
      if (!Number.isInteger(Number(evidence[field])) || Number(evidence[field]) < minimum)
        failures.push(`${field} must be an integer of at least ${minimum}`)
    }
  }
  if (step === 'render_pdf') {
    ...
    if (!Number.isInteger(evidence.claim_count) || Number(evidence.claim_count) < minimumClaims)
      failures.push(`claim_count must be an integer of at least ${minimumClaims}`)
  ...
  if (step === 'publish_pair') {
    if (!['lite-visual-validation/v6', 'lite-visual-integrity/v1'].includes(String(evidence.validation_schema)))
      failures.push('validation_schema must be lite-visual-validation/v6 or lite-visual-integrity/v1')
  ...
  if (step === 'verify_record') {
    if (!/^lv-[A-Za-z0-9._-]+$/.test(String(evidence.pair_id || '')))
      failures.push('pair_id must be a valid lv-* identifier')
