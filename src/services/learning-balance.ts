...
export function knowledgeFrontier(input: { consumed: number; units: number; lessons: number; latestRecall: number | null }): { state: FrontierState; reasons: string[] } {
  const { consumed, units, lessons, latestRecall } = input
  if (!consumed && !units && !lessons) return { state: 'unexplored', reasons: ['No lifetime source, accepted unit, or completed lesson is linked here'] }
  if ((latestRecall != null && latestRecall < 0.6) || (consumed <= 1 && !units && !lessons)) {
    return { state: 'weak', reasons: [latestRecall != null && latestRecall < 0.6 ? `Latest recall was ${Math.round(latestRecall * 100)}%` : 'Only one consumed source is linked, without accepted units or completed lessons'] }
  }
  if (consumed >= 8 && units >= 5 && lessons >= 3 && latestRecall != null && latestRecall >= 0.8) {
    return { state: 'saturated', reasons: [`${consumed} sources, ${units} accepted units, and ${lessons} completed lessons`, `Latest recall was ${Math.round(latestRecall * 100)}%`] }
  }
  if (consumed >= 3 && (units >= 2 || lessons >= 1) && (latestRecall == null || latestRecall >= 0.6)) {
    return { state: 'deeper-study-ready', reasons: [`Foundation established through ${consumed} sources and ${units + lessons} consolidated learning signals`, latestRecall == null ? 'No recall result yet' : `Latest recall was ${Math.round(latestRecall * 100)}%`] }
  }
  return { state: 'developing', reasons: [`Building depth with ${consumed} sources, ${units} accepted units, and ${lessons} completed lessons`, latestRecall == null ? 'No recall result yet' : `Latest recall was ${Math.round(latestRecall * 100)}%`] }
}
...
export async function buildLearningBalance(DB: Db, windowDays = 90) {
  ...
  const [allRawNodes, priorities, recommendations, notes, cards, reviews, acceptedUnits, completedLessons, explorations] = await Promise.all([
    ...
    safeAll(DB.prepare(`SELECT DISTINCT l.id,m.branch_id FROM thread_lessons l
  ...
  const isPruned = (node: any) => {
    ...
    while (current && !seen.has(String(current.id))) {
      ...
      if (String(current.status || '').toLowerCase() === 'pruned' || explicitlyPruned.has(String(current.id))) return true
  ...
  const stats = new Map<string, { consumed: number; lifetimeConsumed: number; last: string | null; notes: number; srs: number; due: number; grades: number[]; latestGrade: number | null; latestReview: string | null; units: number; lessons: number }>()
  const getStats = (id: string) => {
    if (!stats.has(id)) stats.set(id, { consumed: 0, lifetimeConsumed: 0, last: null, notes: 0, srs: 0, due: 0, grades: [], latestGrade: null, latestReview: null, units: 0, lessons: 0 })
    return stats.get(id)!
  }
  ...
  for (const recommendation of recommendations) {
    ...
    if (node) addToPath(node, (value) => { value.lifetimeConsumed += 1 })
  ...
  for (const review of reviews) {
    ...
    if (!current || String(review.reviewed_at || '') > current.reviewedAt) latestReviewByCard.set(String(review.card_id), { grade: Number(review.grade), reviewedAt: String(review.reviewed_at || '') })
  }
  ...
  for (const unit of acceptedUnits) {
    ...
    if (node) addToPath(node, (value) => { value.units += 1 })
  }
  ...
  const nodes: BalanceNode[] = rawNodes
    ...
    .map((node: any) => {
      ...
      const branchStats = stats.get(id) || { consumed: 0, lifetimeConsumed: 0, last: null, notes: 0, srs: 0, due: 0, grades: [], latestGrade: null, latestReview: null, units: 0, lessons: 0 }
      ...
      return {
        ...
        state, reasons,
        frontier_state: frontier.state, frontier_reasons: frontier.reasons,
        lifetime_consumed_count: branchStats.lifetimeConsumed, accepted_units_count: branchStats.units,
        completed_lessons_count: branchStats.lessons, latest_recall: latestRecall == null ? null : Math.round(latestRecall * 100) / 100,
      }
    })
