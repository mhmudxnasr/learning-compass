...
const positiveRevision = (value: unknown) =>
  Number.isInteger(Number(value)) && Number(value) >= 1 ? Number(value) : null
const strictPositiveRevision = (value: unknown) =>
...
export function recallRepairReason(
  ...
  if (status === 'retired') {
    return {
      code: 'retired_by_learner',
      lapse_count: lapseCount,
      threshold,
      message: 'Retired by you. It no longer appears in due review.',
    }
  }
