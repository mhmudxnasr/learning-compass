...
app.get('/sw.js', async (c) => {
  ...
  assetUrl.searchParams.set('release', 'shell-v53-data-v5-packs-v1')
...
app.post('/api/share-intakes/:id/resolve', async (c) => {
  try {
    ...
    if (error instanceof ShareIntakeError)
      return c.json({ error: error.code, message: error.message }, error.status as 400 | 404 | 409)
    throw error
  }
})
...
app.post('/api/share-target', async (c) => {
  try {
    ...
    const candidate = sourceUrl || text || ''
    if (
      !candidate ||
      candidate.length > 10000 ||
      (title?.length || 0) > 500 ||
      (text?.length || 0) > 10000 ||
      (url?.length || 0) > 2048
    ) {
    ...
    const query = new URLSearchParams({
      action: kind === 'review' ? 'review-share' : 'capture',
      share_intake: intake.id,
    })
