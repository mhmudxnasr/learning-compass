...
async function verifiedBranch(db: D1Database, id: string) {
  return db
    .prepare(
      `SELECT id,label,status,type FROM tree_nodes WHERE id=? AND COALESCE(status,'')!='pruned' AND type IN ('root','category','branch','leaf')`,
    )
    .bind(id)
    .first<any>()
}
...
async function loadDomain(db: D1Database, idOrSlug: string) {
  return db
    .prepare(
      `SELECT d.*,n.label branch_label,n.status branch_status,
...
function entryInput(body: any) {
  ...
  for (const field of dossierFields)
    entry[field] = clean(body?.[field], field === 'title' || field === 'author' ? 500 : 6000)
...
function validateEntry(entry: Record<string, any>) {
  ...
  if (entry.editorial_status === 'approved' && !entry.canonical_url && !entry.isbn)
    return 'approved entries require a canonical URL or ISBN'
  if (entry.editorial_status === 'approved' && entry.evidence.length === 0)
    return 'approved entries require selection evidence'
  return ''
}
  ...
  try {
    ...
    if (!atlas)
      return c.json({
        ...
        families: [],
        domains: [],
        counts: { domains: 0, unmapped: 0, curating: 0, complete: 0, field_tested: 0 },
      })
    ...
    if (query) {
      filters.push(
        `(LOWER(d.title) LIKE ? OR LOWER(d.boundary) LIKE ? OR EXISTS (SELECT 1 FROM canon_entries e WHERE e.domain_id=d.id AND (LOWER(e.title) LIKE ? OR LOWER(e.author) LIKE ?)))`,
      )
      params.push(...Array(4).fill(`%${query}%`))
    }
    if (curationStates.has(status)) {
      filters.push(`d.curation_status=?`)
      params.push(status)
    }
    if (validationStates.has(validation)) {
      filters.push(`d.validation_state=?`)
      params.push(validation)
    }
    if (family) {
      filters.push(`(d.parent_id=? OR d.id=?)`)
    ...
    const rows = await c.env.DB.prepare(
      ...
      ORDER BY CASE d.kind WHEN 'family' THEN 0 ELSE 1 END,COALESCE(p.sort_order,d.sort_order),d.sort_order,d.title`,
    )
      .bind(...params)
      .all<any>()
    ...
    const counts = await c.env.DB.prepare(
      ...
      FROM canon_domains WHERE atlas_id=? AND kind='domain'`,
    )
      .bind(atlas.id)
      .first<any>()
    return c.json({
      atlas,
      families: all.filter((row: any) => row.kind === 'family'),
      domains: all.filter((row: any) => row.kind === 'domain'),
      counts: Object.fromEntries(Object.entries(counts || {}).map(([key, value]) => [key, Number(value || 0)])),
    })
  } catch (error) {
    return c.json(safeError('Canon atlas failed')(error), 500)
  }
})
...
app.get('/domains/:id', async (c) => {
  try {
    ...
    const [entries, revisions] = await Promise.all([
      c.env.DB.prepare(
        `SELECT e.*,r.status recommendation_status,r.video_url recommendation_url,
        ...
        WHERE e.domain_id=? ORDER BY CASE e.role WHEN 'foundation' THEN 0 WHEN 'representative' THEN 1 ELSE 2 END`,
      )
        .bind(domain.id)
        .all<any>(),
      c.env.DB.prepare(
        `SELECT id,entry_id,role,previous_json,replacement_reason,replaced_at FROM canon_entry_revisions WHERE domain_id=? ORDER BY replaced_at DESC LIMIT 50`,
      )
        .bind(domain.id)
        .all<any>(),
    ])
    return c.json({
      ...
      revisions: (revisions.results || []).map((row: any) => ({
        ...row,
        previous: JSON.parse(row.previous_json),
        previous_json: undefined,
      })),
    })
  } catch (error) {
    return c.json(safeError('Canon domain failed')(error), 500)
  }
})
...
app.get('/entries/:id', async (c) => {
  try {
    const entry = await c.env.DB.prepare(
      `SELECT e.*,d.title domain_title,d.slug domain_slug,d.branch_id,n.label branch_label,n.status branch_status,r.status recommendation_status
    ...
    return c.json(safeError('Canon entry failed')(error), 500)
  }
})
...
app.post('/domains', async (c) => {
  try {
    ...
    if (!atlas || !title || !slug || !branchId || !boundary)
      return c.json({ error: 'atlas, title, slug, branch_id, and boundary are required' }, 400)
    if (!(await verifiedBranch(c.env.DB, branchId)))
      return c.json({ error: 'verified non-pruned branch required' }, 409)
    ...
    if (parentId) {
      const parent = await c.env.DB.prepare(`SELECT id FROM canon_domains WHERE id=? AND atlas_id=? AND kind='family'`)
        .bind(parentId, atlas.id)
        .first()
      if (!parent) return c.json({ error: 'Canon family not found' }, 404)
    }
    const id = clean(body.id, 120) || makeId('canon_domain')
    await c.env.DB.prepare(
      `INSERT INTO canon_domains (id,atlas_id,slug,kind,parent_id,branch_id,title,boundary,orientation,curation_status,validation_state,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    )
      .bind(
        id,
        atlas.id,
        ...
        title,
        boundary,
        clean(body.orientation, 6000) || null,
        body.curation_status === 'curating' ? 'curating' : 'unmapped',
        validationStates.has(body.validation_state) ? body.validation_state : 'untested',
...
app.patch('/domains/:id', async (c) => {
  try {
    ...
    await c.env.DB.prepare(
      `UPDATE canon_domains SET title=?,boundary=?,orientation=?,branch_id=?,curation_status=?,validation_state=?,sort_order=?,updated_at=datetime('now') WHERE id=?`,
    )
      .bind(
        title,
        boundary,
        body.orientation === undefined ? domain.orientation : clean(body.orientation, 6000) || null,
        branchId,
        curation,
        validation,
        body.sort_order === undefined ? domain.sort_order : Number(body.sort_order || 0),
        domain.id,
      )
      .run()
    return c.json({ ok: true, id: domain.id, curation_status: curation, validation_state: validation })
  } catch (error: any) {
    if (/complete Canon domains require/.test(error?.message || ''))
      return c.json({ error: 'A complete Canon domain requires three distinct approved dossiers.' }, 409)
...
app.put('/domains/:id/entries/:role', async (c) => {
  try {
    ...
    if (domain.curation_status === 'complete')
      return c.json({ error: 'Reopen the Canon domain before replacing an approved entry.' }, 409)
    const body = await c.req.json<any>().catch(() => ({}))
    const entry = entryInput(body)
    const problem = validateEntry(entry)
    if (problem) return c.json({ error: problem }, 400)
    if (entry.recommendation_id) {
      const source = await c.env.DB.prepare(`SELECT id,status FROM recommendations WHERE id=? AND deleted_at IS NULL`)
        .bind(entry.recommendation_id)
        .first<any>()
      if (!source) return c.json({ error: 'linked source record not found' }, 404)
      if (source.status === 'consumed')
        return c.json({ error: 'Consumed books are ineligible for a new Canon selection.' }, 409)
    }
    const excluded = await c.env.DB.prepare(
      `SELECT
      EXISTS (SELECT 1 FROM mastered m WHERE m.kind='book' AND LOWER(m.label)=LOWER(?)) consumed,
      EXISTS (SELECT 1 FROM blacklist b WHERE LOWER(COALESCE(b.work,b.name))=LOWER(?) OR (LOWER(b.name)=LOWER(?) AND COALESCE(b.work,'')='')) blacklisted`,
    )
      .bind(entry.title, entry.title, entry.author)
      .first<any>()
    ...
    const existing = await c.env.DB.prepare(`SELECT * FROM canon_entries WHERE domain_id=? AND role=?`)
      .bind(domain.id, role)
      .first<any>()
    const replacementReason = clean(body.replacement_reason, 2000)
    if (existing && (existing.title !== entry.title || existing.author !== entry.author) && !replacementReason)
      return c.json({ error: 'replacement_reason is required when changing the selected book' }, 400)
    const id = existing?.id || clean(body.id, 120) || makeId('canon_entry')
    const statements: D1PreparedStatement[] = []
    if (existing && (existing.title !== entry.title || existing.author !== entry.author))
      statements.push(
        c.env.DB.prepare(
          `INSERT INTO canon_entry_revisions (entry_id,domain_id,role,previous_json,replacement_reason) VALUES (?,?,?,?,?)`,
        ).bind(existing.id, domain.id, role, JSON.stringify(existing), replacementReason),
      )
    statements.push(
      c.env.DB.prepare(
        `INSERT INTO canon_entries (id,domain_id,role,title,author,canonical_url,isbn,why_slot,beginner_case,expert_case,unique_contribution,limitations,difficulty,rejected_alternative,rejection_reason,evidence_json,recommendation_id,editorial_status,validation_state)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(domain_id,role) DO UPDATE SET title=excluded.title,author=excluded.author,canonical_url=excluded.canonical_url,isbn=excluded.isbn,why_slot=excluded.why_slot,beginner_case=excluded.beginner_case,expert_case=excluded.expert_case,unique_contribution=excluded.unique_contribution,limitations=excluded.limitations,difficulty=excluded.difficulty,rejected_alternative=excluded.rejected_alternative,rejection_reason=excluded.rejection_reason,evidence_json=excluded.evidence_json,recommendation_id=excluded.recommendation_id,editorial_status=excluded.editorial_status,validation_state=excluded.validation_state,updated_at=datetime('now')`,
      ).bind(
        id,
        domain.id,
        role,
        ...
        entry.beginner_case,
        entry.expert_case,
        entry.unique_contribution,
        entry.limitations,
        entry.difficulty,
        entry.rejected_alternative,
    ...
    if (domain.curation_status === 'unmapped')
      statements.push(
        c.env.DB.prepare(
          `UPDATE canon_domains SET curation_status='curating',updated_at=datetime('now') WHERE id=?`,
        ).bind(domain.id),
      )
    ...
    if (/UNIQUE/.test(error?.message || ''))
      return c.json({ error: 'A Canon domain cannot select the same book twice.' }, 409)
...
app.post('/entries/:id/capture', async (c) => {
  try {
    const entry = await c.env.DB.prepare(
      `SELECT e.*,d.branch_id,n.id branch_exists,n.status branch_status,
      ...
      FROM canon_entries e JOIN canon_domains d ON d.id=e.domain_id LEFT JOIN tree_nodes n ON n.id=d.branch_id WHERE e.id=?`,
    )
      .bind(c.req.param('id'))
      .first<any>()
    if (!entry) return c.json({ error: 'Canon entry not found' }, 404)
    if (!entry.branch_exists || entry.branch_status === 'pruned')
      return c.json({ error: 'Canon domain needs a verified non-pruned branch before capture' }, 409)
    if (entry.consumed || entry.blacklisted)
      return c.json({ error: 'Consumed or blacklisted books cannot be captured from Canon.' }, 409)
    if (entry.recommendation_id)
      return c.json({ ok: true, id: entry.recommendation_id, duplicate: true, state: 'existing' })
    ...
    if (result.status === 'consumed' || result.status === 'rejected')
      return c.json(
        { error: 'This book already exists as consumed or rejected and cannot be captured from Canon.' },
        409,
      )
    await c.env.DB.batch([
      c.env.DB.prepare(
        `UPDATE recommendations SET content_type='book',creator=COALESCE(NULLIF(creator,''),?),updated_at=datetime('now') WHERE id=?`,
      ).bind(entry.author, result.id),
      c.env.DB.prepare(
        `UPDATE recommendation_meta SET branch_id=?,updated_at=datetime('now') WHERE recommendation_id=?`,
      ).bind(entry.branch_id, result.id),
      c.env.DB.prepare(`UPDATE canon_entries SET recommendation_id=?,updated_at=datetime('now') WHERE id=?`).bind(
        result.id,
        entry.id,
      ),
    ])
    return c.json(
      { ok: true, ...result, state: result.duplicate ? 'existing' : 'captured', branch_id: entry.branch_id },
      result.duplicate ? 200 : 201,
    )
  } catch (error) {
    return c.json(safeError('Canon capture failed')(error), 500)
  }
})
...
app.post('/domains/:id/thread', async (c) => {
  try {
    ...
    if (!domain.branch_label || domain.branch_status === 'pruned')
      return c.json({ error: 'Canon field needs a verified non-pruned branch before creating a Thread.' }, 409)
    const readiness = await c.env.DB.prepare(
      `SELECT COUNT(*) total,
      ...
      FROM canon_entries WHERE domain_id=?`,
    )
      .bind(domain.id)
      .first<any>()
    if (
      domain.curation_status !== 'complete' ||
      Number(readiness?.total || 0) !== 3 ||
      Number(readiness?.approved || 0) !== 3 ||
      Number(readiness?.roles || 0) !== 3
    ) {
      return c.json({ error: 'Finish the approved three-book Canon path before creating a Thread.' }, 409)
    }
    if (Number(readiness?.captured || 0) < 1)
      return c.json({ error: 'Capture at least one Canon book before creating a Thread.' }, 409)
    const existing = await c.env.DB.prepare(
      `SELECT id FROM learning_threads WHERE superseded_at IS NULL AND status NOT IN ('verified','abandoned') AND guiding_question=? LIMIT 1`,
    )
      .bind(`What is the strongest working understanding of ${domain.title}, and how can I test it?`)
      .first<any>()
    if (existing) return c.json({ ok: true, id: existing.id, duplicate: true })
    const id = makeId('thread')
    await c.env.DB.batch([
      c.env.DB.prepare(
        `INSERT INTO learning_threads (id,title,thread_type,guiding_question,why_now,definition_of_done,status,priority) VALUES (?,?, 'understand', ?, ?, ?, 'draft', 0)`,
      ).bind(
        id,
        `${domain.title} — Guided Exploration`,
        `What is the strongest working understanding of ${domain.title}, and how can I test it?`,
      ...
      c.env.DB.prepare(
        ...
        FROM canon_entries e WHERE e.domain_id=? AND e.recommendation_id IS NOT NULL`,
      ).bind(id, domain.id),
    ])
    return c.json({ ok: true, id, duplicate: false }, 201)
  } catch (error) {
    return c.json(safeError('Canon Thread creation failed')(error), 500)
  }
})
