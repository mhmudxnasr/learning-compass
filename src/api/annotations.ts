...
async function withDerivations(DB: Bindings['DB'], row: any) {
  ...
  const [units, notes, drafts] = await Promise.all([
    DB.prepare(
      ...
      .all<any>(),
    DB.prepare(
      `SELECT id,title,kind,status,provenance_json FROM notes WHERE provenance_json LIKE ? ORDER BY updated_at DESC LIMIT 50`,
    )
      .bind(`%${row.id}%`)
      .all<any>(),
    DB.prepare(
      `SELECT id,question,answer,status,provenance_json FROM srs_drafts WHERE provenance_json LIKE ? ORDER BY updated_at DESC LIMIT 50`,
    )
      .bind(`%${row.id}%`)
      .all<any>(),
  ])
...
app.get('/', async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT * FROM source_annotations WHERE ${clauses.join(' AND ')} ORDER BY created_at DESC LIMIT ?`,
  )
    .bind(...binds, limit)
    .all<any>()
  return c.json({
...
app.post('/', async (c) => {
  ...
  const [source, artifact, thread, branch] = await Promise.all([
    c.env.DB.prepare(`SELECT id,video_url FROM recommendations WHERE id=? AND deleted_at IS NULL`)
      .bind(recommendationId)
      .first<any>(),
    body.artifact_id
      ? c.env.DB.prepare(`SELECT id FROM artifacts WHERE id=?`).bind(clean(body.artifact_id, 120)).first<any>()
      : Promise.resolve(null),
    body.thread_id
      ? c.env.DB.prepare(`SELECT id FROM learning_threads WHERE id=? AND superseded_at IS NULL`)
          .bind(clean(body.thread_id, 120))
          .first<any>()
      : Promise.resolve(null),
    body.branch_id
      ? c.env.DB.prepare(`SELECT id,status FROM tree_nodes WHERE id=? AND type IN ('branch','category','leaf')`)
          .bind(clean(body.branch_id, 120))
          .first<any>()
      : Promise.resolve(null),
  ])
  ...
  await c.env.DB.prepare(
    ...
    .bind(
      id,
      recommendationId,
      body.artifact_id ? clean(body.artifact_id, 120) : null,
      body.thread_id ? clean(body.thread_id, 120) : null,
      body.branch_id ? clean(body.branch_id, 120) : null,
      locatorType,
      JSON.stringify(metadata).slice(0, 12000),
      quote,
      clean(body.context_before, 2000) || null,
      clean(body.context_after, 2000) || null,
      clean(body.language, 40) || null,
      clean(body.source_checksum, 160) || null,
