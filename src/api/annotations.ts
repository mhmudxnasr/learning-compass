...
const selectorSourceUrls = (selector: Record<string, unknown>) =>
  Array.from(new Set([selector.url, selector.locator].filter(isValidUrl)))
const selectorSourceIdentities = (selector: Record<string, unknown>) =>
  Array.from(new Set(selectorSourceUrls(selector).map(normalizeSourceUrlIdentity)))
...
export async function annotationEvidenceChecksum(input: {
  recommendation_id: string
  locator_type: string
  selector: Record<string, unknown>
  quote: string
...
async function syncSearchProjection(DB: Bindings['DB'], id: string, status: 'active' | 'archived', text = '') {
  try {
    if (status === 'archived')
      await DB.prepare("DELETE FROM search_idx WHERE source='annotation' AND ref_id=?").bind(id).run()
    else
      await DB.prepare("INSERT OR REPLACE INTO search_idx(source,ref_id,text) VALUES ('annotation',?,?)")
        .bind(id, text)
        .run()
  } catch {
    /* Maintenance rebuilds the portable projection on older local databases. */
  }
}
...
async function resolveSourceByUrl(DB: Bindings['DB'], rawUrl: string) {
  ...
  return DB.prepare(
    `SELECT r.id,r.video_title,r.video_url,r.creator,r.content_type,m.branch_id,n.label branch_label,n.status branch_status,n.super_category branch_domain,
...
async function withDerivations(DB: Bindings['DB'], row: any) {
  ...
  const [units, notes, drafts] = await Promise.all([
    DB.prepare(
      `SELECT u.id,u.unit_type,u.statement,u.status,u.confidence
      FROM unit_anchors a JOIN learning_units u ON u.id=a.unit_id
      WHERE a.annotation_id=? ORDER BY u.updated_at DESC`,
    )
      .bind(row.id)
      .all<any>(),
    DB.prepare(
      `SELECT n.id,n.title,n.kind,n.status,n.provenance_json FROM notes n
      ...
      .all<any>(),
    DB.prepare(
      `SELECT d.id,d.question,d.answer,d.status,d.provenance_json FROM srs_drafts d
  ...
  return {
    ...
    derivations: {
      units: units.results || [],
      notes: (notes.results || []).map((item: any) => ({
        ...item,
        provenance: parseJson(item.provenance_json, []),
        provenance_json: undefined,
      })),
      recall_drafts: (drafts.results || []).map((item: any) => ({
        ...item,
        provenance: parseJson(item.provenance_json, []),
        provenance_json: undefined,
      })),
...
app.get('/', async (c) => {
  ...
  if (recommendationId) {
    clauses.push('recommendation_id=?')
  ...
  const rows = await c.env.DB.prepare(
    `${annotationSelect} WHERE ${clauses.map((clause) => `a.${clause}`).join(' AND ')} ORDER BY a.created_at DESC LIMIT ?`,
  )
    .bind(...binds, limit)
    .all<any>()
  return c.json({
    annotations: await Promise.all((rows.results || []).map((row: any) => withDerivations(c.env.DB, row))),
  })
})
...
app.post('/', async (c) => {
  ...
  if (quote.length > QUOTE_MAX_LENGTH)
    return c.json(
      { error: 'quote_too_large', message: `quote must contain at most ${QUOTE_MAX_LENGTH} characters` },
      400,
    )
    ...
    return c.json(
      {
        ...
        message: `each context window must contain at most ${CONTEXT_MAX_LENGTH} characters`,
      },
      400,
    )
  }
  if (!recommendationId || !locatorTypes.has(locatorType) || !quote)
    return c.json({ error: 'recommendation_id, locator_type, and quote are required' }, 400)
  const metadata =
    body.selector && typeof body.selector === 'object' && !Array.isArray(body.selector) ? body.selector : {}
  const selectorJson = serializeSelector(metadata)
  if (!selectorJson)
    return c.json(
      {
        error: 'selector_too_large',
        message: `selector must serialize to at most ${SELECTOR_JSON_MAX_LENGTH} characters`,
      },
      400,
    )
  ...
  const [source, artifact] = await Promise.all([
    c.env.DB.prepare(
      `SELECT r.id,r.video_url,m.branch_id,
      COALESCE(
      ...
      .first<any>(),
    artifactId
      ? c.env.DB.prepare(`SELECT id FROM artifacts WHERE id=? AND json_extract(metadata_json,'$.recommendation_id')=?`)
          .bind(artifactId, recommendationId)
          .first<any>()
  ...
  if (selectorUrls.length && !(await selectorUrlsBelongToRecommendation(c.env.DB, recommendationId, selectorUrls))) {
    return c.json(
      {
        error: 'annotation_source_url_mismatch',
        message: 'The selector URL does not belong to this source or its verified replacement lineage.',
      },
      409,
    )
  }
  ...
  if (!branchId)
    return c.json(
      { error: 'source_branch_required', message: 'Map the source to a reviewed branch before anchoring evidence.' },
      409,
    )
  if (body.branch_id && clean(body.branch_id, 120) !== branchId)
    return c.json({ error: 'annotation branch does not match source ownership' }, 409)
  const [thread, branch] = await Promise.all([
    ...
    branchId
      ? c.env.DB.prepare(
          `SELECT b.id,b.status,b.super_category,d.id domain_id FROM tree_nodes b
      JOIN tree_nodes d ON d.id=b.super_category AND d.type='category' AND lower(COALESCE(d.status,''))!='pruned'
      WHERE b.id=? AND b.type IN ('branch','leaf') AND lower(COALESCE(b.status,''))!='pruned'`,
        )
          .bind(branchId)
          .first<any>()
      : Promise.resolve(null),
  ...
  const checksum = await annotationEvidenceChecksum({
  ...
  const inserted = await c.env.DB.prepare(
    ...
    .bind(
      ...
      locatorType,
      selectorJson,
      selectorIdentitiesJson,
      quote,
      contextBefore,
      contextAfter,
      language,
      checksum,
      ['agent', 'system'].includes(body.created_by) ? body.created_by : 'user',
      recommendationId,
      branchId,
      artifactId,
      artifactId,
  ...
  if (!inserted.meta.changes)
    return c.json(
      {
        error: 'annotation_ownership_conflict',
        message:
          'The source, branch, Thread, artifact, or selector URL changed before this anchor could be saved. Reload and try again.',
      },
      409,
    )
  await syncSearchProjection(
    c.env.DB,
    id,
    'active',
    [quote, contextBefore, contextAfter, language].filter(Boolean).join(' '),
  )
...
app.patch('/:id', async (c) => {
  const current = await c.env.DB.prepare(`${mutableAnnotationSelect} WHERE a.id=? AND a.status='active'`)
    .bind(c.req.param('id'))
    .first<any>()
  ...
  if (suppliedQuote !== null && suppliedQuote.length > QUOTE_MAX_LENGTH)
    return c.json(
      { error: 'quote_too_large', message: `quote must contain at most ${QUOTE_MAX_LENGTH} characters` },
      400,
    )
  if (
    (suppliedContextBefore !== null && suppliedContextBefore.length > CONTEXT_MAX_LENGTH) ||
  ...
  if (!locatorTypes.has(locatorType) || !quote)
    return c.json({ error: 'valid locator_type and quote are required' }, 400)
  const selector =
    body.selector === undefined
      ? parseJson(current.selector_json)
      : body.selector && typeof body.selector === 'object' && !Array.isArray(body.selector)
        ? body.selector
        : null
  ...
  if (
    selectorUrls.length &&
    !(await selectorUrlsBelongToRecommendation(c.env.DB, current.recommendation_id, selectorUrls))
  ) {
    return c.json(
      {
        error: 'annotation_source_url_mismatch',
  ...
  if (checksum !== current.source_checksum) {
    ...
    const [revisionInsert, previousArchive] = await c.env.DB.batch([
      ...
      c.env.DB.prepare(
        `UPDATE source_annotations SET status='archived',updated_at=datetime('now')
    ...
    if (!revisionInsert.meta.changes || !previousArchive.meta.changes) {
      return c.json(
        {
          error: 'annotation_revision_conflict',
          message: 'This anchor changed while it was being revised. Reload the source dossier and try again.',
        },
        409,
      )
    }
    ...
    return c.json({
      ok: true,
      revised: true,
      previous_annotation_id: current.id,
      annotation: await withDerivations(c.env.DB, row),
    })
  }
  const updated = await c.env.DB.prepare(
    `UPDATE source_annotations SET locator_type=?,selector_json=?,selector_source_identities_json=?,quote=?,context_before=?,context_after=?,language=?,source_checksum=?,updated_at=datetime('now')
    WHERE id=? AND status='active' AND source_checksum=?
      AND EXISTS (
  ...
  if (!updated.meta.changes)
    return c.json(
      {
        error: 'annotation_revision_conflict',
        message: 'This anchor changed while it was being updated. Reload the source dossier and try again.',
      },
      409,
    )
  await syncSearchProjection(
  ...
  const row = await c.env.DB.prepare(`${annotationSelect} WHERE a.id=? AND a.status='active'`)
    .bind(current.id)
    .first<any>()
...
app.post('/:id/archive', async (c) => {
  const result = await c.env.DB.prepare(
    `UPDATE source_annotations SET status='archived',updated_at=datetime('now') WHERE id=? AND status='active'`,
  )
    .bind(c.req.param('id'))
    .run()
