...
app.post('/:id/triage', async (c) => {
  ...
  const item = await c.env.DB.prepare(
    `SELECT r.id,r.video_url,r.video_title,r.content_type,r.status,r.deleted_at,m.learning_state,m.branch_id,n.id branch_exists,n.label branch_label,n.status branch_status
  ...
  const result = await c.env.DB.prepare(
    ...
    .bind(c.req.param('id'), body.override_queue_cap === true ? 1 : 0)
    .run()
  if (!result.meta.changes) return c.json({ error: 'queue_full', ...queueDecision(5, false) }, 409)
  if (thread) {
    await c.env.DB.prepare(
      `INSERT INTO thread_sources (thread_id,recommendation_id,role,status) VALUES (?,?,'supporting','active') ON CONFLICT(thread_id,recommendation_id) DO UPDATE SET status='active',updated_at=datetime('now')`,
    )
      .bind(thread.id, c.req.param('id'))
      .run()
  }
...
app.get('/:id/record', async (c) => {
  ...
  const [
    ...
    c.env.DB.prepare(
      ...
      .all<any>(),
    c.env.DB.prepare(
      `SELECT id,question,answer,topic,due_at,repetitions,interval_days,ease_factor,unit_id,thread_id,scheduler_version FROM srs_cards WHERE recommendation_id=? ORDER BY due_at`,
    )
      .bind(recommendationId)
      .all<any>(),
