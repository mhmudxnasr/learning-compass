...
import {
  LITE_VISUAL_CHECKPOINT_REQUIREMENTS,
  LITE_VISUAL_SOURCE_EXTRACTION_SCHEMA,
  LITE_VISUAL_STAGES,
  LITE_VISUAL_WORKFLOW_CONTRACT,
  LITE_VISUAL_WORKFLOW_VERSION,
  resolveLiteVisualResume,
} from '../services/lite-visual-workflow'
import {
  createPersonalLibraryItem,
  loadPersonalLibrary,
  loadPersonalLibraryItem,
  PERSONAL_ITEM_STATES,
  PERSONAL_ITEM_TYPES,
  updatePersonalLibraryItem,
  type PersonalItemState,
  type PersonalItemType,
  type PersonalLibraryInput,
...
app.post('/', async (c) => {
  ...
  try {
    const body = await c.req.json<{
      ...
      override_queue_cap?: boolean
    }>()
    const source = String(body.source || '').trim()
    if (!source) return c.json({ error: 'source required' }, 400)

    const artifact = body.artifact_id
      ? await DB.prepare(`SELECT id,filename,media_type,r2_key FROM artifacts WHERE id=?`)
    ...
    const branch = branchId
      ? await DB.prepare(
          "SELECT n.id,n.label,n.status,n.super_category FROM tree_nodes n WHERE n.id=? AND n.type='branch' AND (n.parent_id='root' OR EXISTS (SELECT 1 FROM tree_nodes p WHERE p.id=n.parent_id AND p.type='category'))",
        )
          .bind(branchId)
          .first<any>()
      : null
    if (branchId && !branch) return c.json({ error: 'branch not found' }, 404)
    if (String(branch?.status || '').toLowerCase() === 'pruned')
      return c.json({ error: 'cannot capture to a pruned branch', branch_id: branch.id }, 409)
      ...
      ...(branch
        ? {
            branch: {
              ...
              source:
                String(c.req.header('x-agent-name') || 'user_capture')
                  .trim()
                  .slice(0, 100) || 'user_capture',
            },
          }
        : {}),
    })
    if ('branchConflict' in result)
      return c.json(
        { error: 'branch_mapping_conflict', recommendation_id: result.id, existing_branch_id: result.branchConflict },
        409,
      )
    return c.json(
      {
    ...
    return c.json(safeError('Capture failed')(error), 500)
  }
})
...
app.get('/personal', async (c) => {
  ...
  if (itemType && !PERSONAL_ITEM_TYPES.includes(itemType as PersonalItemType))
    return c.json({ error: 'unsupported item_type' }, 400)
  if (state && !PERSONAL_ITEM_STATES.includes(state as PersonalItemState))
    return c.json({ error: 'unsupported state' }, 400)
  try {
    return c.json(
      await loadPersonalLibrary(c.env.DB, {
        q: c.req.query('q'),
        item_type: itemType,
        state,
...
app.get('/', async (c) => {
  const rows = await c.env.DB.prepare(
    `SELECT r.*,m.learning_state,m.branch_id,m.tags_json,m.source_metadata_json,
    ...
    ORDER BY CASE WHEN json_extract(m.source_metadata_json,'$.resurface_at') IS NOT NULL AND datetime(json_extract(m.source_metadata_json,'$.resurface_at'))<=datetime('now') THEN 0 ELSE 1 END, r.created_at DESC LIMIT 200`,
  ).all()
  return c.json({ items: (rows.results || []).map(withoutLegacyRound) })
})

app.get('/queue', async (c) => {
  const delivery = await resolveDeliveryContext(
    c.env.DB,
    deliveryContextFromQuery((key) => c.req.query(key)),
  )
  ...
  return c.json({
    ...
    receipt: {
      order: 'unchanged',
      filtering: matchesOnly ? 'explicit_matches_only' : 'none',
      all_items_visible: !matchesOnly,
    },
  })
})
...
app.get('/feeds', async (c) => {
  const rows = await c.env.DB.prepare(
    `SELECT fs.*,n.label branch_label,n.status branch_status,COUNT(fe.guid) entry_count
...
app.get('/feeds/:id/entries', async (c) => {
  ...
  if (feedId === 'all') {
    const [rows, count] = await Promise.all([
      c.env.DB.prepare(
        `SELECT r.*,m.learning_state,m.branch_id,n.label branch_label,n.status branch_status,fe.guid,fe.published_at,fe.created_at feed_imported_at,fs.title feed_title,fs.id feed_id
    ...
    return c.json({
      feed: { id: 'all', title: 'All Subscribed Sources', feed_url: 'All feeds' },
      items: (rows.results || []).map(withoutLegacyRound),
      total: count?.count || 0,
      limit,
      offset,
    })
  ...
  const feed = await c.env.DB.prepare(
    `SELECT fs.id,fs.title,fs.feed_url,fs.site_url,fs.last_checked_at,fs.branch_id,n.label branch_label,n.status branch_status
    FROM feed_sources fs LEFT JOIN tree_nodes n ON n.id=fs.branch_id WHERE fs.id=?`,
  )
    .bind(feedId)
    .first<any>()
...
app.post('/feeds', async (c) => {
  try {
    ...
    const branch = await c.env.DB.prepare(
      "SELECT n.id,n.label,n.status FROM tree_nodes n WHERE n.id=? AND n.type='branch' AND (n.parent_id='root' OR EXISTS (SELECT 1 FROM tree_nodes p WHERE p.id=n.parent_id AND p.type='category'))",
    )
      .bind(branchId)
      .first<any>()
    if (!branch) return c.json({ error: 'branch not found' }, 404)
    if (String(branch.status || '').toLowerCase() === 'pruned')
      return c.json({ error: 'cannot subscribe to a pruned branch', branch_id: branch.id }, 409)
    const limit = feedImportLimit(body.limit)
    if (limit === null) return c.json({ error: 'limit must be a number from 1 to 20' }, 400)
    return c.json(
      { ok: true, ...(await addFeed(c.env.DB, body.url, { id: branch.id, label: branch.label }, limit)) },
      201,
    )
  } catch (error) {
...
app.post('/feeds/:id/sync', async (c) => {
  const feed = await c.env.DB.prepare(
    `SELECT id,feed_url,title,site_url,etag,last_modified,branch_id FROM feed_sources WHERE id=?`,
  )
    .bind(c.req.param('id'))
    .first<any>()
  if (!feed) return c.json({ error: 'feed not found' }, 404)
  ...
  if (limit === null) return c.json({ error: 'limit must be a number from 1 to 20' }, 400)
  try {
    return c.json({ ok: true, ...(await syncFeed(c.env.DB, feed, limit)) })
  } catch (error) {
...
app.delete('/feeds/:id/entries/:recId', async (c) => {
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare(
      `UPDATE recommendations SET status='deleted',deleted_at=datetime('now'),updated_at=datetime('now') WHERE id=?`,
    ).bind(recId),
    c.env.DB.prepare(
      `UPDATE recommendation_meta SET learning_state='excluded',updated_at=datetime('now') WHERE recommendation_id=?`,
    ).bind(recId),
  ])
...
app.delete('/feeds/:id/entries', async (c) => {
  ...
  if (feedId === 'all') {
    await c.env.DB.batch([
      c.env.DB.prepare(
        `UPDATE recommendations SET status='deleted',deleted_at=datetime('now'),updated_at=datetime('now') WHERE id IN (SELECT recommendation_id FROM feed_entries)`,
      ),
      c.env.DB.prepare(
        `UPDATE recommendation_meta SET learning_state='excluded',updated_at=datetime('now') WHERE recommendation_id IN (SELECT recommendation_id FROM feed_entries)`,
      ),
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      `UPDATE recommendations SET status='deleted',deleted_at=datetime('now'),updated_at=datetime('now') WHERE id IN (SELECT recommendation_id FROM feed_entries WHERE feed_id=?)`,
    ).bind(feedId),
    c.env.DB.prepare(
      `UPDATE recommendation_meta SET learning_state='excluded',updated_at=datetime('now') WHERE recommendation_id IN (SELECT recommendation_id FROM feed_entries WHERE feed_id=?)`,
    ).bind(feedId),
...
app.post('/:id/triage', async (c) => {
  const body: {
    action?: 'queue' | 'exclude' | 'dequeue'
    thread_id?: string
    override_queue_cap?: boolean
    reason?: string
  } = await c.req.json().catch(() => ({}))
  const item = await c.env.DB.prepare(
    `SELECT r.id,r.video_url,r.video_title,r.content_type,r.why_this,r.status,r.deleted_at,m.learning_state,m.branch_id,n.id branch_exists,n.label branch_label,n.status branch_status
    FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id
    LEFT JOIN tree_nodes n ON n.id=m.branch_id WHERE r.id=?`,
  )
    .bind(c.req.param('id'))
    .first<any>()
  ...
  if (body.action === 'exclude') {
    const exclusionReason =
      String(body.reason || 'capture_exclusion')
        .trim()
        .slice(0, 120) || 'capture_exclusion'
    await c.env.DB.batch([
      c.env.DB.prepare(`UPDATE recommendations SET status='rejected',updated_at=datetime('now') WHERE id=?`).bind(
        c.req.param('id'),
      ),
      c.env.DB.prepare(
        `UPDATE recommendation_meta SET learning_state='excluded',updated_at=datetime('now') WHERE recommendation_id=?`,
      ...
      c.env.DB.prepare(
        `INSERT INTO recommendation_outcomes (id,recommendation_id,creator,format,branch_id,outcome_status,rejection_reason,evaluated_at)
        ...
        ON CONFLICT(recommendation_id) DO UPDATE SET outcome_status='rejected',rejection_reason=COALESCE(excluded.rejection_reason,recommendation_outcomes.rejection_reason),evaluated_at=datetime('now')`,
      ).bind(`outcome_${c.req.param('id')}`, exclusionReason, c.req.param('id')),
    ])
  ...
  if (body.action === 'dequeue') {
    ...
    const result = await c.env.DB.prepare(
      `UPDATE recommendation_meta SET learning_state='captured',updated_at=datetime('now')
      WHERE recommendation_id=? AND learning_state IN ('queued','in_progress')
        AND EXISTS (SELECT 1 FROM recommendations r WHERE r.id=recommendation_meta.recommendation_id AND r.status='active' AND r.deleted_at IS NULL)`,
    )
      .bind(c.req.param('id'))
      .run()
  ...
  if (item.content_type === 'book') {
    return c.json(
      {
        error: 'books_separate_from_queue',
        message: 'Books are tracked separately in the Books room and cannot be added to Queue.',
      },
      400,
    )
  }
  if (!item.branch_id || !item.branch_exists) {
    return c.json(
      {
        error: 'branch_mapping_required',
        message: 'Map this source to a verified knowledge branch before adding it to Queue.',
      },
      409,
    )
  }
  ...
  const expectedContribution =
    String(item.why_this || '')
      .trim()
      .slice(0, 1000) ||
    `Queued as supporting material for this Thread: ${String(item.video_title || item.id)
      .trim()
      .slice(0, 900)}.`
  const threadEligibility = thread
    ? `AND EXISTS (
    ...
    : ''
  const queueStatement = c.env.DB.prepare(
    `INSERT INTO recommendation_meta (recommendation_id,learning_state,updated_at)
    ...
    WHERE (?=1 OR (SELECT COUNT(*) FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.status='active' AND COALESCE(r.content_type, '') != 'book' AND COALESCE(m.learning_state,'queued') IN ('queued','in_progress'))<5)
      ${threadEligibility}
    ON CONFLICT(recommendation_id) DO UPDATE SET learning_state='queued',updated_at=datetime('now')`,
  ).bind(c.req.param('id'), body.override_queue_cap === true ? 1 : 0, ...(thread ? [c.req.param('id'), thread.id] : []))
  const committed = thread
    ? await c.env.DB.batch([
        ...
        ).bind(expectedContribution, c.req.param('id'), thread.id),
      ])
    : [await queueStatement.run()]
  if (!committed[0]?.meta.changes)
    return c.json(
      {
        error: 'queue_full_or_source_ownership_changed',
        message: 'Queue capacity, source ownership, or the selected Thread changed. Reload and try again.',
        ...queueDecision(5, false),
      },
      409,
    )
  if (thread && committed[1]?.meta.changes !== 1)
    return c.json(
      {
        error: 'thread_source_attachment_conflict',
        message:
          'The Queue commitment was saved, but the Thread placement could not be verified. Reload the source before retrying.',
      },
      409,
    )
...
app.post('/:id/branch-map', async (c) => {
  try {
    const body = await c.req
      .json<{ branch_id?: string; confidence?: string; reason?: string }>()
      .catch(() => ({}) as any)
    const confidence = String(body.confidence || '').toLowerCase()
    if (confidence !== 'high')
      return c.json({ error: 'only high-confidence mappings may be applied automatically' }, 422)
    const item = await c.env.DB.prepare(
      "SELECT r.id,m.learning_state FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.id=? AND r.status IN ('active','consumed','rejected')",
    )
      .bind(c.req.param('id'))
      .first<any>()
    if (!item) return c.json({ error: 'recommendation not found' }, 404)
    if (
      !['captured', 'queued', 'in_progress', 'completed', 'excluded'].includes(
        String(item.learning_state || 'captured'),
      )
    )
      return c.json({ error: 'item is not a mappable source' }, 409)
    const branch = await c.env.DB.prepare(
      "SELECT id,label,status FROM tree_nodes WHERE id=? AND type IN ('root','category','branch','leaf')",
    )
      .bind(String(body.branch_id || ''))
      .first<any>()
    if (!branch) return c.json({ error: 'branch not found' }, 404)
    if (String(branch.status || '').toLowerCase() === 'pruned')
      return c.json({ error: 'cannot map to a pruned branch', branch_id: branch.id }, 409)
    const mappingSource =
      String(c.req.header('x-agent-name') || 'user_review')
        .trim()
        .slice(0, 100) || 'user_review'
    await c.env.DB.prepare(
      `INSERT INTO recommendation_meta (recommendation_id,branch_id,source_metadata_json,updated_at) VALUES (?,?,json_object('branch_mapping_confidence',?,'branch_mapping_reason',?,'branch_mapping_source',?),datetime('now')) ON CONFLICT(recommendation_id) DO UPDATE SET branch_id=excluded.branch_id, source_metadata_json=json_patch(COALESCE(recommendation_meta.source_metadata_json,'{}'),excluded.source_metadata_json), updated_at=datetime('now')`,
    )
      .bind(item.id, branch.id, confidence, String(body.reason || '').slice(0, 500), mappingSource)
      .run()
    return c.json({
      ok: true,
    ...
    return c.json(safeError('Queue branch mapping failed')(err), 500)
  }
})
...
app.post('/:id/visualise', async (c) => {
  ...
  const item = await c.env.DB.prepare(
    `SELECT r.id,r.video_url,r.video_title,r.creator,r.content_type,m.source_metadata_json
    FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id
    WHERE r.id=? AND r.status IN ('active','consumed') AND r.deleted_at IS NULL`,
  )
    .bind(c.req.param('id'))
    .first<any>()
  if (!item) return c.json({ error: 'not found' }, 404)
  ...
  try {
    sourceMetadata = JSON.parse(item.source_metadata_json || '{}')
  } catch {}
  ...
  const artifactRows = await c.env.DB.prepare(
    `SELECT id,filename,media_type,size_bytes,metadata_json,created_at FROM artifacts WHERE json_extract(metadata_json,'$.recommendation_id')=? ORDER BY created_at DESC`,
  )
    .bind(item.id)
    .all<any>()
    ...
    if (metadata.workflow_contract === 'lite-visual-linear/v4' && metadata.publication_state === 'ready') {
      ...
      if (!forceRevision)
        return c.json({
          ...
          recommendation_id: item.id,
          pair_id: pairId || null,
          companion: { primary: { role: 'html', id: ready.html.id }, secondary: { role: 'pdf', id: ready.pdf.id } },
        })
      if (!pairId || body.supersedes_pair_id !== pairId)
        return c.json({ error: 'ready_pair_revision_precondition_failed', pair_id: pairId || null }, 409)
    }
  }
  if (forceRevision && (!currentPairId || body.supersedes_pair_id !== currentPairId))
    return c.json({ error: 'ready_pair_revision_precondition_failed', pair_id: currentPairId }, 409)
  ...
  const jobPayload = {
    ...
    source_extraction: {
      schema: LITE_VISUAL_SOURCE_EXTRACTION_SCHEMA,
      command: '/home/mahmud/.hermes/skills/lite-visual/scripts/extract_source.py',
      output: 'source.txt',
      manifest: 'source-extraction.json',
    ...
    ...(item.content_type === 'book'
      ? {
          ...
          chapter_artifact_contract: {
            metadata: ['chapter_key', 'chapter_title', 'chapter_number', 'pair_id', 'role'],
          },
        }
      : {}),
  }
  const existing = forceRevision
    ? await c.env.DB.prepare(
        `SELECT id,status,workflow_step,workflow_run_id,payload_json FROM agent_jobs
        WHERE job_type='visualise_source' AND recommendation_id=? AND status IN ('pending','retry','running','awaiting_activation')
          ...
          AND instr(idempotency_key,?)=1 ORDER BY created_at DESC LIMIT 1`,
      )
        .bind(item.id, currentPairId, LITE_VISUAL_WORKFLOW_CONTRACT, revisionIdempotencyPrefix)
        .first<{
          id: string
          status: string
    ...
    : await c.env.DB.prepare(
        `SELECT id,status,workflow_step,workflow_run_id,payload_json FROM agent_jobs WHERE idempotency_key=?`,
      )
        .bind(idempotencyKey)
        .first<{
          id: string
  ...
  if (existing && ['pending', 'retry', 'running', 'awaiting_activation'].includes(existing.status)) {
    const status =
      existing.status === 'running'
        ? 'working'
        : existing.status === 'awaiting_activation'
          ? 'awaiting_activation'
          : 'queued'
    return c.json(
      {
        ...
        workflow_step: existing.workflow_step || 'resolve_source',
        job_id: existing.id,
        recommendation_id: item.id,
      },
      202,
  ...
  if (!forceRevision && existing) {
    ...
    try {
      previousPayload = JSON.parse(existing.payload_json || '{}')
    } catch {}
    ...
    await c.env.DB.prepare(
      `UPDATE agent_jobs SET status='retry',attempts=0,error=NULL,result_json=json_object('resume_from',?),lease_owner=NULL,lease_expires_at=NULL,workflow_step=?,payload_json=?,recommendation_id=?,trigger_kind='explicit_user_action',workflow_run_id=?,updated_at=datetime('now') WHERE id=?`,
    )
      .bind(resume.resume_from, resume.resume_from, JSON.stringify(jobPayload), item.id, nextWorkflowRunId, existing.id)
      .run()
    return c.json(
      {
        ok: true,
        ...
        upgraded_workflow: !resume.is_current,
        job_id: existing.id,
        recommendation_id: item.id,
      },
      202,
  ...
  await c.env.DB.prepare(
    `INSERT INTO agent_jobs (id,job_type,payload_json,idempotency_key,recommendation_id,trigger_kind,workflow_run_id,workflow_step) VALUES (?,'visualise_source',?,?,?,'explicit_user_action',?,'resolve_source')`,
  )
    .bind(
      jobId,
      JSON.stringify({
...
app.get('/:id/record', async (c) => {
  ...
  const item = await c.env.DB.prepare(
    `SELECT r.*,m.learning_state,m.branch_id,m.tags_json,m.source_metadata_json,m.progress_percent,m.estimated_minutes,
    ...
    FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id LEFT JOIN tree_nodes n ON n.id=m.branch_id WHERE r.id=?`,
  )
  ...
  const [
    ...
    c.env.DB.prepare(
      ...
      .all<any>(),
    c.env.DB.prepare(
      `SELECT n.id,n.recommendation_id,n.title,n.kind,n.status,n.revision,n.source_url,n.source_artifact_id,n.provenance_json,n.updated_at
      FROM notes n WHERE n.recommendation_id=? ORDER BY n.updated_at DESC`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT s.note_id,s.section_key,s.label,s.content,s.direction,s.position FROM note_sections s JOIN notes n ON n.id=s.note_id WHERE n.recommendation_id=? ORDER BY s.note_id,s.position`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT a.id,a.filename,a.media_type,a.r2_key,a.size_bytes,a.metadata_json,a.created_at,r.notebook_url
      ...
      .all<any>(),
    c.env.DB.prepare(
      `SELECT id,question,answer,topic,status,unit_id,thread_id,provenance_json,created_at FROM srs_drafts WHERE recommendation_id=? ORDER BY created_at DESC`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT id,question,answer,topic,due_at,repetitions,interval_days,ease_factor,unit_id,thread_id,scheduler_version,repair_status,paused_at,retired_at FROM srs_cards WHERE recommendation_id=? ORDER BY due_at`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(`SELECT * FROM recommendation_outcomes WHERE recommendation_id=?`)
      .bind(recommendationId)
      .first<any>(),
    c.env.DB.prepare(
      `SELECT id,memory_key,memory_kind,value_json,confidence,source,status,evidence_json,updated_at FROM hermes_memory WHERE evidence_json LIKE ? ORDER BY updated_at DESC`,
    )
      .bind(`%${recommendationId}%`)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT id,status,change_type,target_label,current_json,proposed_json,evidence,reasoning,confidence,created_at,applied_at FROM feedback_proposals WHERE recommendation_id=? ORDER BY created_at DESC`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT id,job_type,status,error,created_at,updated_at,result_json FROM agent_jobs WHERE json_extract(payload_json,'$.recommendation_id')=? ORDER BY created_at DESC`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT t.id,t.title,t.thread_type,t.guiding_question,t.definition_of_done,t.status,ts.role,ts.expected_contribution FROM thread_sources ts JOIN learning_threads t ON t.id=ts.thread_id WHERE ts.recommendation_id=? AND ts.status!='removed' ORDER BY CASE t.status WHEN 'active' THEN 0 ELSE 1 END,t.updated_at DESC`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(`SELECT * FROM learning_units WHERE recommendation_id=? ORDER BY updated_at DESC`)
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT a.* FROM unit_anchors a JOIN learning_units u ON u.id=a.unit_id WHERE u.recommendation_id=? ORDER BY a.created_at`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT * FROM source_annotations WHERE recommendation_id=? AND status='active' ORDER BY created_at DESC LIMIT 200`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT ur.* FROM unit_relations ur JOIN learning_units u ON u.id=ur.source_unit_id WHERE u.recommendation_id=? ORDER BY ur.created_at`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(`SELECT * FROM consolidation_runs WHERE recommendation_id=? ORDER BY requested_at DESC LIMIT 1`)
      .bind(recommendationId)
      .first<any>(),
    c.env.DB.prepare(
      `SELECT * FROM source_learning_dispositions WHERE recommendation_id=? ORDER BY updated_at DESC LIMIT 1`,
    )
      .bind(recommendationId)
      .first<any>(),
    c.env.DB.prepare(
      `SELECT cf.id,cf.pick_id,cf.outcome,cf.score,cf.reason_tags_json,cf.reflection,cf.exposure_json,cf.structured_json,cf.disposition,cf.created_at,p.strategy lane,p.thread_id
      FROM compass_feedback cf LEFT JOIN compass_picks p ON p.id=cf.pick_id
      WHERE cf.recommendation_id=? ORDER BY cf.created_at DESC`,
    )
      .bind(recommendationId)
      .all<any>(),
    c.env.DB.prepare(
      `SELECT e.id entry_id,e.role,d.id domain_id,d.slug domain_slug,d.title domain_title,d.boundary domain_boundary
      FROM canon_entries e JOIN canon_domains d ON d.id=e.domain_id
      WHERE e.recommendation_id=?
      ORDER BY d.sort_order,d.title,CASE e.role WHEN 'foundation' THEN 0 WHEN 'representative' THEN 1 ELSE 2 END`,
    )
      .bind(recommendationId)
      .all<any>(),
  ...
  for (const section of sections.results || [])
    noteSections.set(section.note_id, [...(noteSections.get(section.note_id) || []), section])
  ...
  const noteRows = (notes.results || []).map((note: any) => ({
    ...note,
    provenance: parseList(note.provenance_json),
    provenance_json: undefined,
    sections: noteSections.get(note.id) || [],
  }))
  ...
  const memoryInfluences = (memories.results || []).map((row: any) => {
    let evidence: any[] = []
    try {
    ...
    return {
      ...
      evidence: evidence.filter((item) => item.recommendation_id === recommendationId),
  ...
  for (const relation of relations.results || [])
    relationsByUnit.set(relation.source_unit_id, [...(relationsByUnit.get(relation.source_unit_id) || []), relation])
  const annotationRows = (annotations.results || []).map((annotation: any) => ({
    ...annotation,
    selector: parseJson(annotation.selector_json) || {},
  ...
  const unitRows = (units.results || []).map((unit: any) => ({
    ...unit,
    anchors: anchorsByUnit.get(unit.id) || [],
    relations: relationsByUnit.get(unit.id) || [],
  }))
  const consolidationSteps = consolidation
    ? await c.env.DB.prepare(`SELECT * FROM consolidation_steps WHERE run_id=? ORDER BY position`)
        .bind(consolidation.id)
        .all<any>()
    : { results: [] }
  ...
  const htmlArtifact =
    selectedCompanions.html ||
    directLegacyArtifacts.find(
      (artifact: any) =>
        /html/.test(String(artifact.media_type || '')) ||
    ...
    null
  const pdfArtifact =
    selectedCompanions.pdf ||
    directLegacyArtifacts.find(
      (artifact: any) =>
        /pdf/.test(String(artifact.media_type || '')) ||
        String(artifact.filename || '')
          .toLowerCase()
          .endsWith('.pdf'),
    ) ||
  ...
  const dueCount = cardRows.filter(
    (card: any) => card.repair_status === 'active' && card.due_at && String(card.due_at) <= today,
  ).length
  const branchVerified =
    Boolean(item.verified_branch_id) && String(item.verified_branch_status || '').toLowerCase() !== 'pruned'
  ...
  const branchInfo = branchLabel
    ? {
        id: branchId,
        label: branchLabel,
        status: branchVerified
          ? String(item.verified_branch_status || '')
              .trim()
              .toLowerCase()
          : null,
        super_category: branchVerified ? item.verified_branch_domain || null : null,
        verified: branchVerified,
        linkable: branchVerified,
      }
    : null
  const companions = {
    html: htmlArtifact
      ? { id: htmlArtifact.id, filename: htmlArtifact.filename, size_bytes: htmlArtifact.size_bytes }
      : null,
    pdf: pdfArtifact
      ? { id: pdfArtifact.id, filename: pdfArtifact.filename, size_bytes: pdfArtifact.size_bytes }
      : null,
  }
  const companionMetadata = htmlArtifact ? parseJson(htmlArtifact.metadata_json) || {} : {}
  const companion =
    htmlArtifact && pdfArtifact
      ? {
          status: 'ready',
          pair_id: companionMetadata.pair_id || null,
          primary: { role: 'html', ...companions.html },
          secondary: { role: 'pdf', ...companions.pdf },
        }
      : { status: 'not_ready', pair_id: null, primary: null, secondary: null }

  const bookChapters =
    item.content_type === 'book'
      ? await c.env.DB.prepare(
          `SELECT chapter_key,chapter_title,position,completed_at FROM book_visual_chapters WHERE recommendation_id=? ORDER BY position,chapter_key`,
        )
          .bind(recommendationId)
          .all<any>()
      : { results: [] }
  const bookProjection =
    item.content_type === 'book' ? projectBook(item, bookChapters.results || [], artifactsRows) : null
  if (bookProjection) {
    const primary = await c.env.DB.prepare(
      `SELECT r.id FROM recommendations r JOIN recommendation_meta m ON m.recommendation_id=r.id
  ...
  return c.json({
    item: {
      ...
      branch_status: branchInfo?.status || null,
    ...
    ...(bookProjection
      ? {
          reading_state: bookProjection.reading_state,
          queue_state: bookProjection.queue_state,
          progress: bookProjection.progress,
          next_chapter: bookProjection.next_chapter,
        }
      : {}),
    canon_memberships: canonMembershipRows.results || [],
    srs: {
      drafts: (drafts.results || []).map((draft: any) => ({
        ...draft,
        provenance: parseList(draft.provenance_json),
        provenance_json: undefined,
      })),
      cards: cardRows,
      recall_summary: { count: cardCount, due: dueCount },
    },
    outcome: outcome || null,
    memory_influences: memoryInfluences,
    proposals: (proposals.results || []).map((proposal: any) => ({
      ...proposal,
      current: parseJson(proposal.current_json),
      proposed: parseJson(proposal.proposed_json),
