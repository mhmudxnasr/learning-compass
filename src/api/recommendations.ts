...
app.post('/books', async (c) => {
  ...
  try {
    ...
    const existing = await c.env.DB.prepare('SELECT id FROM recommendations WHERE dedup_key=?')
      .bind(dedupKey)
      .first<{ id: string }>()
...
app.patch('/:id/source-url', async (c) => {
  ...
  const target = await c.env.DB.prepare(
    `SELECT id,video_title,content_type,video_url,dedup_key,status,deleted_at FROM recommendations WHERE id=?`,
  )
    .bind(id)
    .first<any>()
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE recommendations SET video_url=?,dedup_key=?,updated_at=datetime('now') WHERE id=?`).bind(
      sourceUrl,
      dedupKey,
      id,
    ),
