...
export const bookVisibilityPredicate = (alias = 'r') =>
  `(${alias}.status IS NULL OR ${alias}.status!='deleted') AND ${alias}.deleted_at IS NULL`
...
app.get('/active', async (c) => {
  ...
  try {
    ...
    const recommendations = (res.results || []).map((row: any) => {
      const { round: _legacyRound, ...item } = row
      return item
    })
...
app.get('/list', async (c) => {
  ...
  if (status) {
    ...
    where.push(
      "(recommendations.status IS NULL OR recommendations.status != 'deleted') AND recommendations.deleted_at IS NULL",
    )
  }
  if (contentType) {
  ...
  if (q) {
    where.push(
      '(recommendations.video_title LIKE ? OR recommendations.creator LIKE ? OR recommendations.why_this LIKE ?)',
    )
    const like = `%${q}%`
...
app.get('/books', async (c) => {
  ...
  const [artifacts, jobs, canonMembershipRows, threadRows, chapterResult] = await Promise.all([
    ...
    ids.length
      ? c.env.DB.prepare(
          `SELECT id,job_type,status,error,payload_json,updated_at FROM agent_jobs WHERE job_type IN ('visualise_source','extract_notes') AND json_extract(payload_json,'$.recommendation_id') IN (${placeholders}) ORDER BY updated_at DESC,id DESC`,
        )
          .bind(...ids)
          .all<any>()
      : Promise.resolve({ results: [] }),
    ids.length
      ? c.env.DB.prepare(
          `SELECT e.recommendation_id,e.id entry_id,e.role,d.id domain_id,d.slug domain_slug,d.title domain_title,d.boundary domain_boundary
    ...
    ids.length
      ? c.env.DB.prepare(
          `SELECT ts.recommendation_id,t.id,t.title,t.thread_type,t.status,ts.role,ts.expected_contribution
      ...
      ORDER BY CASE t.status WHEN 'active' THEN 0 WHEN 'draft' THEN 1 ELSE 2 END,t.updated_at DESC,t.title`,
        )
          .bind(...ids)
          .all<any>()
      : Promise.resolve({ results: [] }),
    ids.length
      ? c.env.DB.prepare(
          `SELECT recommendation_id,chapter_key,chapter_title,position,completed_at FROM book_visual_chapters WHERE recommendation_id IN (${placeholders}) ORDER BY position,chapter_key`,
        )
          .bind(...ids)
          .all<any>()
      : Promise.resolve({ results: [] }),
  ...
  for (const book of books)
    projections.set(
      String(book.id),
      projectBook(book, chaptersByBook.get(String(book.id)) || [], artifactsByBook.get(String(book.id)) || []),
    )
  ...
  for (const row of jobs.results || []) {
    ...
    if (row.job_type === 'extract_notes') {
      ...
      if (chapter)
        chapter.extraction = { id: row.id, status: row.status, error: row.error || null, updated_at: row.updated_at }
    }
  }
  return c.json({
...
app.post('/books/:id/reading-state', async (c) => {
  ...
  const body = await c.req
    .json<{ state?: string; primary?: boolean }>()
    .catch(() => ({}) as { state?: string; primary?: boolean })
  const readingState = String(body.state || '')
    .trim()
    .toLowerCase()
  if (!['saved', 'reading', 'finished'].includes(readingState))
    return c.json({ error: 'state must be saved, reading, or finished' }, 400)
  const personalState = personalStateFromBookState(readingState)
  if (body.primary === true && readingState !== 'reading')
    return c.json({ error: 'the primary book must be in reading state' }, 400)
  const book = await c.env.DB.prepare(
    `SELECT r.id,m.learning_state FROM recommendations r JOIN recommendation_meta m ON m.recommendation_id=r.id JOIN tree_nodes n ON n.id=m.branch_id WHERE r.id=? AND r.content_type='book' AND ${bookVisibilityPredicate('r')} AND lower(COALESCE(n.status,''))!='pruned'`,
  )
    .bind(recommendationId)
    .first<any>()
  if (!book) return c.json({ error: 'book not found' }, 404)

  const writeTarget = (includePrimary: boolean) =>
    c.env.DB.prepare(`INSERT INTO recommendation_meta (recommendation_id,learning_state,source_metadata_json,updated_at)
  ...
  const syncPersonalEvent = () =>
    ...
    VALUES (?,?,'personal_library_updated','user',0,?,datetime('now'),?)`,
      ...
      personalEventId,
      recommendationId,
      JSON.stringify({ state: personalState, book_reading_state: readingState, source: 'books_reading_state' }),
    )

  if (body.primary === true) {
    await c.env.DB.batch([
      c.env.DB
        .prepare(`UPDATE recommendation_meta SET source_metadata_json=json_patch(COALESCE(source_metadata_json,'{}'),json_object('book_primary',0)),updated_at=datetime('now')
  ...
  return c.json({
    ...
    queue_state: book.learning_state || 'captured',
  })
})
...
app.post('/books/:id/chapters/:chapterKey/complete', async (c) => {
  ...
  const exists = await c.env.DB.prepare(
    `SELECT 1 FROM recommendations r JOIN recommendation_meta m ON m.recommendation_id=r.id JOIN tree_nodes n ON n.id=m.branch_id WHERE r.id=? AND r.content_type='book' AND ${bookVisibilityPredicate('r')} AND lower(COALESCE(n.status,''))!='pruned'`,
  )
    .bind(recommendationId)
    .first()
  if (!exists) return c.json({ error: 'book not found' }, 404)
  const chapter = await c.env.DB.prepare(
    `SELECT chapter_key FROM book_visual_chapters WHERE recommendation_id=? AND chapter_key=?`,
  )
  ...
  if (!chapter) {
    ...
    const last = artifact
      ? await c.env.DB.prepare(
          `SELECT COALESCE(MAX(position),0) position FROM book_visual_chapters WHERE recommendation_id=?`,
        )
          .bind(recommendationId)
    ...
    await c.env.DB.prepare(
      `INSERT INTO book_visual_chapters (recommendation_id,chapter_key,chapter_title,position,updated_at)
      VALUES (?,?,?,?,datetime('now'))
      ...
      .bind(recommendationId, derived.key, derived.title, derived.position)
      .run()
  }
  const body: { completed?: boolean } = await c.req
    .json<{ completed?: boolean }>()
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      `UPDATE book_visual_chapters SET completed_at=?,updated_at=datetime('now') WHERE recommendation_id=? AND chapter_key=?`,
    ).bind(completed ? new Date().toISOString() : null, recommendationId, chapterKey),
    c.env.DB.prepare(`UPDATE personal_library_items SET updated_at=datetime('now') WHERE recommendation_id=?`).bind(
      recommendationId,
    ),
    c.env.DB.prepare(
      `INSERT INTO learning_events
      ...
      personalEventId,
      recommendationId,
      JSON.stringify({ chapter_key: chapterKey, completed, source: 'books_chapter_progress' }),
    ),
  ])
...
app.post('/books/:id/chapters', async (c) => {
  const recommendationId = c.req.param('id')
  const book = await c.env.DB.prepare(
    `SELECT r.id FROM recommendations r JOIN recommendation_meta m ON m.recommendation_id=r.id JOIN tree_nodes n ON n.id=m.branch_id WHERE r.id=? AND r.content_type='book' AND ${bookVisibilityPredicate('r')} AND lower(COALESCE(n.status,''))!='pruned'`,
  )
    .bind(recommendationId)
    .first()
  if (!book) return c.json({ error: 'book not found' }, 404)
  const body = await c.req
    .json<{ chapters?: Array<{ key?: string; title?: string; number?: number; completed?: boolean }> }>()
    .catch(() => ({}) as { chapters?: Array<{ key?: string; title?: string; number?: number; completed?: boolean }> })
  ...
  const normalized = chapters.map((chapter, index) => ({
    key: String(chapter.key || `chapter-${index + 1}`)
      .trim()
      .slice(0, 120),
  ...
  if (normalized.some((chapter) => !chapter.key || !chapter.title))
    return c.json({ error: 'each chapter needs a key and title' }, 400)
  if (normalized.some((chapter) => chapter.key.toLowerCase() === 'book' && chapter.number === 0))
    return c.json({ error: 'the legacy whole-book Chapter 0 placeholder is not a valid chapter' }, 400)
  try {
    await c.env.DB.batch(
    ...
    `,
          ...
          chapter.title,
          chapter.number,
          chapter.completed ? new Date().toISOString() : null,
        ),
      ),
    )
    const saved = await c.env.DB.prepare(
      `SELECT chapter_key key,chapter_title title,position,completed_at FROM book_visual_chapters WHERE recommendation_id=? ORDER BY position,chapter_key`,
    )
      .bind(recommendationId)
      .all()
...
app.post('/books', async (c) => {
  ...
  try {
    const branch = await c.env.DB.prepare(
      `SELECT id,label,status FROM tree_nodes WHERE id=? AND type='branch' AND status!='pruned'`,
    )
      .bind(branchId)
      .first<{ id: string; label: string; status: string }>()
    if (!branch) return c.json({ error: 'valid non-pruned branch_id required' }, 400)
    const existing = await c.env.DB.prepare('SELECT id,video_url FROM recommendations WHERE dedup_key=?')
      .bind(dedupKey)
      ...
      return c.json(
        {
          ...
          message:
            'This ISBN already belongs to a canonical book. Verify and replace its URL through /recommendations/:id/source-url instead of rewriting it during book upsert.',
    ...
    await c.env.DB.batch([
      ...
      c.env.DB.prepare(
        ...
        JSON.stringify({
          isbn: isbn || null,
          source: 'bookshelf',
          ...(existing ? {} : { book_reading_state: 'saved' }),
        }),
      ),
      c.env.DB.prepare(
        `INSERT INTO personal_library_items
      ...
      c.env.DB.prepare(
        ...
        `personal-library-book:${recommendationId}`,
        `personal-library-book:${recommendationId}`,
        existing ? 'personal_library_linked' : 'personal_library_created',
        recommendationId,
        JSON.stringify({ item_type: 'book', state: 'planned', branch_id: branch.id, source: 'bookshelf' }),
      ),
    ])
    const saved = await c.env.DB.prepare(
      `SELECT r.*,m.learning_state,m.branch_id,n.label branch_label,n.status branch_status FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id LEFT JOIN tree_nodes n ON n.id=m.branch_id WHERE r.id=?`,
    )
      .bind(recommendationId)
      .first<any>()
    ...
    return c.json(safeError('Book save failed')(error), 500)
  }
})
...
app.post('/push', async (c) => {
  ...
  try {
        ...
        return c.json(
          {
            error: 'source_url_replacement_required',
            message:
              'This canonical recommendation already has a different source URL. Verify and replace it through the guarded source URL endpoint.',
            recommendation_id: existing.id,
            current_source_url: existing.video_url,
            replacement_endpoint: `/recommendations/${existing.id}/source-url`,
          },
    ...
    for (const dedupKey of dedupKeys) {
      const row = await DB.prepare(`SELECT id FROM recommendations WHERE dedup_key=?`)
        .bind(dedupKey)
        .first<{ id: string }>()
      if (row)
        await DB.prepare(
          ...
          .bind(row.id, JSON.stringify({ imported: true }))
          .run()
    }
    ...
    for (const item of items) {
      if (!item.video_title || !item.video_url) continue
      const searchDedup = deriveDedupKey({
        video_url: normalizeUrlForDedup(item.video_url),
        video_title: item.video_title,
        content_type: item.content_type,
      })
      const searchRow = await DB.prepare(
        `SELECT id,video_title,creator,why_this FROM recommendations WHERE dedup_key=?`,
      )
        .bind(searchDedup)
        .first<any>()
      if (searchRow) {
        try {
          ...
          await DB.prepare("INSERT OR REPLACE INTO search_idx(source, ref_id, text) VALUES ('rec', ?, ?)")
            .bind(searchRow.id, searchText)
            .run()
        } catch {
          /* Search projection is best-effort; maintenance will rebuild it. */
        }
      }
    }
  } catch (err) {
...
app.post('/action', async (c) => {
  ...
  if (body.status === 'consumed') {
    ...
    const currentStatus =
      ids.length === 1 ? await DB.prepare('SELECT status FROM recommendations WHERE id = ?').bind(ids[0]).first() : null
  ...
  const consumedDate = body.status === 'consumed' ? body.consumed_date || new Date().toISOString().split('T')[0] : null
    ...
    if (body.status === 'consumed') {
      for (const id of ids) {
        try {
          ...
          console.warn('resurface sched failed', safeErrorMessage(e))
        }
        try {
          await detectContradiction(DB, id)
        } catch (e) {
          console.warn('contradiction detect failed', safeErrorMessage(e))
        }
        try {
          const item = await DB.prepare(
            `SELECT r.creator,r.content_type,m.branch_id FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.id=?`,
          )
            .bind(id)
            .first<any>()
          await DB.prepare(
            `INSERT INTO recommendation_outcomes (id,recommendation_id,creator,format,branch_id,actual_score,outcome_status,consumed_at,evaluated_at) VALUES (?,?,?,?,?,?, 'consumed', ?, datetime('now')) ON CONFLICT(recommendation_id) DO UPDATE SET actual_score=COALESCE(excluded.actual_score,recommendation_outcomes.actual_score),outcome_status='consumed',consumed_at=excluded.consumed_at,evaluated_at=datetime('now')`,
          )
            .bind(
              `outcome_${id}`,
              id,
          ...
          console.warn('quality ledger failed', safeErrorMessage(e))
        }
    ...
    if (body.status === 'rejected') {
      for (const id of ids) {
        ...
        await DB.prepare(
          ...
          .bind(`outcome_${id}`, rejectionReason, id)
          .run()
        const classified = classifyRecommendationFeedback(body.feedback_kind === 'not_now' ? 'dismissed' : 'declined', [
          rejectionReason,
        ])
...
const loadSourceHealthTarget = (DB: D1Database, id: string) =>
  DB.prepare(
...
app.get('/:id/source-health', async (c) => {
  ...
  try {
    ...
    if (!target || target.status === 'deleted' || target.deleted_at)
...
app.post('/:id/source-health/check', async (c) => {
  ...
  try {
    ...
    if (!target || target.status === 'deleted' || target.deleted_at)
      return c.json({ error: 'recommendation_not_found' }, 404)
    if (
      body.expected_source_url &&
      normalizeUrlForDedup(body.expected_source_url) !== normalizeUrlForDedup(target.video_url)
    ) {
...
app.post('/:id/source-url/verify', async (c) => {
  ...
  if (!body || !isValidUrl(body.source_url) || !isNonEmptyStr(body.expected_source_url, 2048))
    return c.json({ error: 'valid source_url and expected_source_url required' }, 400)
  try {
    const target = await loadSourceHealthTarget(c.env.DB, id)
    if (!target || target.status === 'deleted' || target.deleted_at)
      return c.json({ error: 'recommendation_not_found' }, 404)
...
app.patch('/:id/source-url', async (c) => {
  ...
  try {
    ...
    const collision = await c.env.DB.prepare(
      `SELECT id FROM recommendations WHERE dedup_key=? AND id!=? AND deleted_at IS NULL`,
    )
      .bind(dedupKey, id)
      .first<{ id: string }>()
    ...
    await c.env.DB.batch([
      c.env.DB.prepare(
        `UPDATE recommendations SET video_url=?,dedup_key=?,updated_at=?
        WHERE id=? AND video_url=? AND dedup_key=?`,
      ).bind(sourceUrl, dedupKey, changedAt, id, target.video_url, target.dedup_key),
      c.env.DB.prepare(
        `INSERT INTO source_url_replacements
        ...
        VALUES (?,(SELECT CASE WHEN video_url=? AND dedup_key=? AND updated_at=? THEN id ELSE NULL END FROM recommendations WHERE id=?),?,?,?,?,?,'verified',?,?,?)`,
      ).bind(
        replacementId,
        sourceUrl,
        dedupKey,
        ...
        verification.final_url ?? null,
        changedAt,
      ),
      c.env.DB.prepare(
        `INSERT INTO recommendation_meta (recommendation_id,source_metadata_json,updated_at) VALUES (?,?,datetime('now'))
        ...
        id,
        JSON.stringify({
          preferred_source_url: sourceUrl,
          archive_source_url: target.video_url,
          source_url_updated_at: changedAt,
        }),
        sourceUrl,
        target.video_url,
        changedAt,
      ...
      c.env.DB.prepare(
        ...
        ON CONFLICT(recommendation_id) DO UPDATE SET
          ...
          http_status=excluded.http_status,final_url=excluded.final_url,error_code=excluded.error_code,updated_at=datetime('now')`,
      ).bind(
        id,
        verification.checked_url,
        verification.status,
...
app.patch('/content-types', async (c) => {
  ...
  try {
    ...
    if (unavailable.length)
      return c.json({ error: 'recommendation_not_available', ids: unavailable.map((row: any) => row.id) }, 409)
    ...
    if (nonYoutube.length)
      return c.json({ error: 'video_type_requires_youtube_url', ids: nonYoutube.map((row: any) => row.id) }, 409)

    await c.env.DB.batch(
      ids.map((id) =>
        c.env.DB.prepare(`UPDATE recommendations SET content_type='video',updated_at=datetime('now') WHERE id=?`).bind(
          id,
        ),
      ),
    )
...
app.post('/map', async (c) => {
  ...
  try {
    const branch = await DB.prepare(
      "SELECT id,label,type FROM tree_nodes WHERE id=? AND type IN ('root','category','branch','leaf')",
    )
      .bind(body.branch_id)
      .first<any>()
    if (!branch) return c.json({ error: 'map branch not found' }, 404)
    const sources = await DB.prepare(
      `SELECT id,status FROM recommendations WHERE id IN (${ids.map(() => '?').join(',')})`,
    )
      .bind(...ids)
      .all<any>()
    ...
    if (incomplete.length)
      return c.json(
        { error: 'only completed sources can be mapped', ids: incomplete.map((source: any) => source.id) },
        409,
      )

    const statements = ids.flatMap((id) => [
      DB.prepare(
        `INSERT INTO recommendation_meta (recommendation_id, learning_state, source_metadata_json) VALUES (?, 'completed', '{}') ON CONFLICT(recommendation_id) DO UPDATE SET learning_state='completed', updated_at=datetime('now')`,
      ).bind(id),
      DB.prepare(
        `UPDATE recommendation_meta SET branch_id=?, updated_at=datetime('now') WHERE recommendation_id=?`,
      ).bind(body.branch_id, id),
    ...
    await DB.prepare(`INSERT INTO update_log (kind,summary,details_json) VALUES ('tree_change',?,?)`)
      .bind(
        `Mapped ${ids.length} completed source${ids.length === 1 ? '' : 's'} to ${branch.label || body.branch_id}`,
        JSON.stringify({ recommendation_ids: ids, branch_id: body.branch_id, source: 'hermes' }),
      )
      .run()
    const mapped = await DB.prepare(
      `SELECT r.id,r.video_title,m.branch_id FROM recommendations r JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.id IN (${ids.map(() => '?').join(',')})`,
    )
      .bind(...ids)
      .all<any>()
    return c.json({ ok: true, count: ids.length, branch, sources: mapped.results || [] })
  } catch (err) {
    return c.json(safeError('Map source failed')(err), 500)
  }
})
...
app.post('/delete', async (c) => {
  ...
  try {
    if (body.undo) {
      ...
      await DB.batch([
        DB.prepare(
          "INSERT OR REPLACE INTO undo_queue (id, table_name, row_id, snapshot_json, expires_at) VALUES (?, 'recommendations', ?, ?, datetime('now', '+30 seconds'))",
        ...
        DB.prepare(
          "UPDATE recommendation_meta SET source_metadata_json=json_patch(COALESCE(source_metadata_json,'{}'),json_object('book_primary',0)),updated_at=datetime('now') WHERE recommendation_id=?",
        ).bind(body.id),
      ])
    } else {
      await DB.batch([
...
app.delete('/:id/permanent', async (c) => {
  ...
  try {
    const recommendation = await DB.prepare('SELECT id,status FROM recommendations WHERE id=?')
      .bind(recommendationId)
      .first<{ id: string; status: string }>()
    if (!recommendation) return c.json({ error: 'not found' }, 404)
    if (recommendation.status === 'active')
      return c.json({ error: 'active sources must be archived before permanent deletion' }, 409)
    const liteVisualPair = await DB.prepare(
      'SELECT pair_id,corpus_id,state FROM lite_visual_pairs WHERE recommendation_id=? LIMIT 1',
    )
      .bind(recommendationId)
      .first<any>()
    if (liteVisualPair)
      return c.json(
        {
          error: 'lite_visual_pair_history_is_immutable',
          pair_id: liteVisualPair.pair_id,
          corpus_id: liteVisualPair.corpus_id,
          state: liteVisualPair.state,
        },
        409,
      )
    const legacyLiteVisualPair = await DB.prepare(
      `SELECT json_extract(metadata_json,'$.pair_id') pair_id,json_extract(metadata_json,'$.corpus_id') corpus_id,json_extract(metadata_json,'$.publication_state') state
      FROM artifacts WHERE json_extract(metadata_json,'$.recommendation_id')=? AND json_extract(metadata_json,'$.generator')='lite-visual'
        AND json_extract(metadata_json,'$.pair_id') IS NOT NULL LIMIT 1`,
    )
      .bind(recommendationId)
      .first<any>()
    if (legacyLiteVisualPair)
      return c.json(
        {
          error: 'lite_visual_pair_history_is_immutable',
          pair_id: legacyLiteVisualPair.pair_id,
          corpus_id: legacyLiteVisualPair.corpus_id || null,
          state: legacyLiteVisualPair.state || 'legacy',
        },
        409,
      )

    const artifacts = await DB.prepare(
      `SELECT id,r2_key FROM artifacts WHERE json_extract(metadata_json,'$.recommendation_id')=?`,
    )
      .bind(recommendationId)
      .all<{ id: string; r2_key: string | null }>()

    const unitIds = await DB.prepare('SELECT id FROM learning_units WHERE recommendation_id=?')
      .bind(recommendationId)
      .all<{ id: string }>()
    ...
    const statements: D1PreparedStatement[] = [
      ...
      DB.prepare('DELETE FROM note_sections WHERE note_id IN (SELECT id FROM notes WHERE recommendation_id=?)').bind(
        recommendationId,
      ),
      DB.prepare('DELETE FROM notes WHERE recommendation_id=?').bind(recommendationId),
      DB.prepare(
        'DELETE FROM srs_review_events WHERE card_id IN (SELECT id FROM srs_cards WHERE recommendation_id=?)',
      ).bind(recommendationId),
      ...
      DB.prepare(
        'DELETE FROM consolidation_steps WHERE run_id IN (SELECT id FROM consolidation_runs WHERE recommendation_id=?)',
      ).bind(recommendationId),
      ...
      DB.prepare("DELETE FROM semantic_documents WHERE document_kind='recommendation' AND source_id=?").bind(
        recommendationId,
      ),
      ...
      DB.prepare("DELETE FROM artifacts WHERE json_extract(metadata_json,'$.recommendation_id')=?").bind(
        recommendationId,
      ),
    ...
    if (unitPlaceholders) {
      statements.unshift(
        ...
        DB.prepare(
          `DELETE FROM unit_relations WHERE source_unit_id IN (${unitPlaceholders}) OR target_unit_id IN (${unitPlaceholders})`,
        ).bind(...unitIdList, ...unitIdList),
    ...
    if (ARTIFACTS) {
      for (const artifact of artifacts.results || [])
        if (artifact.r2_key) await ARTIFACTS.delete(artifact.r2_key).catch(() => {})
    }
...
app.post('/undo', async (c) => {
  ...
  try {
    ...
    if (row.table_name === 'recommendations') {
      ...
      await DB.prepare(`UPDATE recommendations SET status=?,deleted_at=?,updated_at=datetime('now') WHERE id=?`)
        .bind(snap.status, snap.deleted_at || null, snap.id)
        .run()
    }
...
async function detectContradiction(DB: any, recId: string) {
  const me = await DB.prepare(
    'SELECT id, dedup_key, user_rating, user_review, video_title FROM recommendations WHERE id = ?',
  )
    .bind(recId)
    .first()
  ...
  const opposite =
    me.user_rating === 'love' || me.user_rating === 'like'
      ? ['meh', 'dislike']
...
app.get('/export', async (c) => {
  ...
  try {
    const result = await DB.prepare(
      `SELECT r.*,m.branch_id,m.tags_json,n.label branch_label,
    ...
    const items = (result.results || []).map((row: any) => {
      const {
        ...
        personal_item_type,
        personal_state,
        personal_release_year,
        personal_duration_minutes,
        personal_progress_current,
        personal_progress_total,
        personal_progress_unit,
        personal_tags_json,
        personal_note,
        personal_started_at,
        personal_completed_at,
        personal_updated_at,
        ...item
      } = row
      let tags: string[] = []
      try {
        tags = JSON.parse(personal_tags_json || row.tags_json || '[]')
      } catch {}
      return {
        ...item,
    ...
    if (format === 'md') {
      const cell = (value: unknown) =>
        String(value ?? '')
          ...
          .replace(/[\r\n]+/g, ' ')
      const header =
        '| Title | Creator | Type | Personal state | Progress | Rating | Branch | URL | Personal note |\n| --- | --- | --- | --- | --- | --- | --- | --- | --- |'
      const rows = items
        .map(
          (i: any) =>
      ...
      return new Response(header + '\n' + rows, {
        headers: {
          ...
          'Content-Disposition': 'attachment; filename="learning-compass-export.md"',
        },
      })
    }
