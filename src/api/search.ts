...
app.get('/evidence', async (c) => {
  ...
  try {
    ...
    const derivations = await c.env.DB.prepare(`SELECT ua.annotation_id,lu.id,lu.unit_type,lu.statement,lu.user_synthesis,lu.status,lu.recommendation_id
      FROM unit_anchors ua JOIN learning_units lu ON lu.id=ua.unit_id
      WHERE ua.annotation_id IN (${ids.map(() => '?').join(',')}) ORDER BY lu.updated_at DESC`).bind(...ids).all<any>()
    ...
    return c.json({ q, total: annotations.length, results: annotations.map((row: any) => ({ ...row, derivations: byAnnotation.get(String(row.id)) || [] })) })
  } catch (error) {
...
app.get('/', async (c) => {
  ...
  try {
    ...
    const like = `%${q}%`
    const [recs, nodes, vault, patterns, threads, units, notes, artifacts, assertions, memories, annotations] = await Promise.all([
