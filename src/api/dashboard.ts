...
app.get('/briefing', async (c) => {
  ...
  try {
    ...
    const [
      ...
      DB.prepare(
        `SELECT r.id,r.video_title,r.creator,r.content_type,r.video_url,r.why_this,r.context_brief,r.notebook_url,r.created_at,
         m.learning_state,m.priority_rank,m.progress_percent,m.estimated_minutes,m.started_at,m.last_opened_at,
         m.branch_id,COALESCE(n.label,r.branch) branch_label,COALESCE(n.status,'love') branch_status,
        ...
        ORDER BY CASE WHEN m.learning_state='in_progress' THEN 0 ELSE 1 END,COALESCE(m.priority_rank,999),r.created_at DESC LIMIT 50`,
      ).all<any>(),
      DB.prepare(`SELECT COUNT(*) count FROM srs_cards WHERE due_at<=date('now')`).first<any>(),
      DB.prepare(
        `SELECT COUNT(*) count FROM recommendations r JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.status='active' AND COALESCE(r.content_type, '') != 'book' AND m.learning_state='captured'`,
      ).first<any>(),
      ...
      DB.prepare(
        ...
        FROM learning_activity_ledger WHERE activity_date>=${weekStart}`,
      ).first<any>(),
      DB.prepare(
        `SELECT COALESCE(MAX(completed),0) count FROM (
        SELECT COUNT(*) completed FROM learning_activity_ledger WHERE event_type='completion'
        GROUP BY strftime('%Y-%W',activity_date)
      )`,
      ).first<any>(),
      DB.prepare(
        `SELECT id,video_title,content_type,creator,user_score,consumed_date AS created_at FROM recommendations WHERE status='consumed' ORDER BY consumed_date DESC LIMIT 6`,
      ).all<any>(),
      DB.prepare(`SELECT summary FROM update_log ORDER BY ts DESC LIMIT 1`).first<any>(),
      DB.prepare(
        `SELECT content_type,COUNT(*) count,ROUND(AVG(user_score),1) average FROM recommendations WHERE status='consumed' AND user_score IS NOT NULL GROUP BY content_type HAVING COUNT(*)>=2 ORDER BY average DESC,count DESC LIMIT 1`,
      ).first<any>(),
      DB.prepare(
        `SELECT date FROM (SELECT date(occurred_at,'+3 hours') date FROM learning_events WHERE evidence_weight>0 UNION SELECT activity_date date FROM learning_activity_ledger WHERE event_type IN ('feedback_recorded','recall_reviewed')) ORDER BY date DESC`,
      ).all<any>(),
      DB.prepare(
    ...
    const activeThreadsResult = await DB.prepare(
      `SELECT t.*,
    ...
    if (threadIds.length) {
      ...
      const stages = await DB.prepare(
        `SELECT * FROM learning_path_stages WHERE thread_id IN (${threadMatches}) ORDER BY thread_id,position`,
      )
        .bind(...threadIds)
        .all<any>()
      const currentStages = activeThreads.map((thread: any) => {
        ...
        return (
          threadStages.find((stage: any) => ['available', 'in_progress'].includes(stage.status)) ||
      ...
      const lessons = stageIds.length
        ? await DB.prepare(
            `SELECT * FROM thread_lessons WHERE stage_id IN (${stageIds.map(() => '?').join(',')}) ORDER BY stage_id,position`,
          )
            .bind(...stageIds)
            .all<any>()
        : { results: [] as any[] }
      ...
      if (visibleLessonIds.length) {
        const lessonSources = await DB.prepare(
          `SELECT ls.*,r.creator,r.content_type FROM thread_lesson_sources ls JOIN recommendations r ON r.id=ls.recommendation_id WHERE ls.lesson_id IN (${visibleLessonIds.map(() => '?').join(',')}) ORDER BY ls.lesson_id,ls.position`,
        )
          .bind(...visibleLessonIds)
          .all<any>()
        homeThreads = homeThreads.map((thread: any) => ({
          ...
          current_stage: thread.current_stage
            ? {
                ...
                lessons: thread.current_stage.lessons.map((lesson: any) => ({
                  ...lesson,
                  sources: (lessonSources.results || []).filter((source: any) => source.lesson_id === lesson.id),
                })),
              }
            : null,
        }))
    ...
    const openConsolidations = await DB.prepare(
      `SELECT cr.id,cr.recommendation_id,cr.state,cr.failure_reason,r.video_title FROM consolidation_runs cr JOIN recommendations r ON r.id=cr.recommendation_id WHERE cr.state NOT IN ('closed','waived') ORDER BY cr.requested_at LIMIT 10`,
    ).all<any>()
    const verifiedOutcomes = await DB.prepare(
    ...
    if (activeItems.length) {
      ...
      const rows = await DB.prepare(
        `SELECT id,filename,media_type,created_at,
        ...
        .bind(...activeItems.map((item: any) => item.id))
        .all<any>()
    ...
    const today =
      (await DB.prepare(
        `SELECT date('now','+3 hours') AS date, ROUND((julianday(date('now','+3 hours','+1 day')) - julianday(datetime('now','+3 hours'))) * 86400) AS seconds_remaining`,
      ).first<any>()) || {}
    const todayDate = String(today.date)
    const activeDates = new Set((activityDates.results || []).map((row: any) => String(row.date)))
    const yesterday = String(
      (await DB.prepare(`SELECT date('now','+3 hours','-1 day') AS date`).first<any>())?.date || '',
    )
    ...
    const insight = topFormat
      ...
      : latestSignal?.summary
        ? {
            title: 'Your map just moved',
            body: latestSignal.summary,
            evidence: 'Latest approved signal',
            target: 'map.atlas',
          }
        : {
            title: 'Your pattern is still forming',
            body: 'Complete and rate two sources to reveal your strongest learning format.',
            evidence: 'Needs two rated completions',
            target: 'curate.queue',
          }
    ...
    return c.json({
      ...
      momentum: {
        completed,
        notes: Number(momentum?.notes || 0),
        reviews: Number(momentum?.reviews || 0),
        streak: currentStreak,
        streak_days: streakDays.results || [],
        personal_best: personalBest,
        longest_streak: longestStreak,
        today_secured: activeDates.has(todayDate),
        last_activity_date: [...activeDates][0] || null,
        current_date: todayDate,
        seconds_remaining: Number(today.seconds_remaining || 0),
    ...
    return c.json(safeError('Momentum failed')(error), 500)
  }
})

app.get('/layout', async (c) => {
  const rows = await c.env.DB.prepare(
    `SELECT module_key,position,pinned,visible FROM dashboard_layout ORDER BY position`,
  ).all()
  return c.json({ modules: rows.results || [] })
})
app.put('/layout', async (c) => {
  const body = await c.req.json<{
    modules: Array<{ key: string; position: number; pinned?: boolean; visible?: boolean }>
  }>()
  await c.env.DB.batch(
    (body.modules || []).map((item) =>
      c.env.DB.prepare(
        `INSERT INTO dashboard_layout (module_key,position,pinned,visible) VALUES (?,?,?,?) ON CONFLICT(module_key) DO UPDATE SET position=excluded.position,pinned=excluded.pinned,visible=excluded.visible,updated_at=datetime('now')`,
      ).bind(item.key, item.position, item.pinned ? 1 : 0, item.visible === false ? 0 : 1),
    ),
  )
