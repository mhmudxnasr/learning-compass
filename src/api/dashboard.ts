  ...
  try {
    ...
    if (threadIds.length) {
      ...
      if (visibleLessonIds.length) {
        ...
        const sourceIds = [
          ...new Set(sourceRows.map((source: any) => String(source.recommendation_id)).filter(Boolean)),
        ]
          ...
          current_stage: thread.current_stage
            ? {
                ...
                lessons: thread.current_stage.lessons.map((lesson: any) => ({
                  ...lesson,
                  sources: sourceWithMaterials.filter((source: any) => source.lesson_id === lesson.id),
                })),
              }
            : null,
        }))
