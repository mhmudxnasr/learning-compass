  ...
  try {
    const data = await cached(`analytics.recs.${days}`, 120000, async () => {
      const [consumed, rejected, ratings, lanes, formats, timeToConsume, resurfacing, exploration] = await Promise.all([
        ...
        // Lane balance
        DB.prepare(
          `SELECT p.strategy, COUNT(*) c FROM compass_picks p JOIN recommendations r ON r.id=p.recommendation_id WHERE r.status='consumed' AND r.consumed_date>=? GROUP BY p.strategy`,
        )
          .bind(since)
        ...
        // Exploration picks
        DB.prepare(
          `SELECT COUNT(CASE WHEN p.strategy IN ('bridge','challenge') THEN 1 END) exploration, COUNT(*) total FROM compass_picks p JOIN recommendations r ON r.id=p.recommendation_id WHERE r.status='consumed' AND r.consumed_date>=?`,
        )
          .bind(since)
