  ...
  try {
    const data = await cached(`analytics.recs.${days}`, 120000, async () => {
      const [consumed, rejected, ratings, lanes, formats, timeToConsume, resurfacing, exploration] = await Promise.all([
        // Accept rate
        DB.prepare(`SELECT COUNT(*) c FROM recommendations WHERE status='consumed' AND consumed_date>=?`)
          .bind(since)
          .first<{ c: number }>(),
        DB.prepare(`SELECT COUNT(*) c FROM recommendations WHERE status='rejected' AND updated_at>=?`)
          .bind(since)
          .first<{ c: number }>(),
        // Rating distribution
        DB.prepare(
          `SELECT user_rating, COUNT(*) c FROM recommendations WHERE status='consumed' AND consumed_date>=? AND user_rating NOT IN ('unset','') GROUP BY user_rating`,
        )
          .bind(since)
          .all<{ user_rating: string; c: number }>(),
        // Lane balance
        DB.prepare(
          `SELECT p.strategy, COUNT(*) c FROM compass_picks p JOIN recommendations r ON r.id=p.recommendation_id WHERE p.workflow_scope='general' AND r.status='consumed' AND r.consumed_date>=? GROUP BY p.strategy`,
        )
          .bind(since)
          .all<{ strategy: string; c: number }>(),
        // Format diversity (Shannon entropy)
        DB.prepare(
          `SELECT content_type, COUNT(*) c FROM recommendations WHERE status='consumed' AND consumed_date>=? AND content_type IS NOT NULL GROUP BY content_type`,
        )
          .bind(since)
          .all<{ content_type: string; c: number }>(),
        // Average time to consume
        DB.prepare(
          `SELECT AVG(julianday(consumed_date) - julianday(COALESCE(activated_at, created_at))) avg_days FROM recommendations WHERE status='consumed' AND consumed_date>=? AND consumed_date IS NOT NULL`,
        )
          .bind(since)
          .first<{ avg_days: number | null }>(),
        // Resurfacing recall rate
        DB.prepare(
          `SELECT COUNT(CASE WHEN resolved_at IS NOT NULL THEN 1 END) resolved, COUNT(*) total FROM resurfacing WHERE due_at>=? AND due_at<=date('now')`,
        )
          .bind(since)
          .first<{ resolved: number; total: number }>(),
        // Exploration picks
        DB.prepare(
          `SELECT COUNT(CASE WHEN p.strategy IN ('bridge','challenge') THEN 1 END) exploration, COUNT(*) total FROM compass_picks p JOIN recommendations r ON r.id=p.recommendation_id WHERE p.workflow_scope='general' AND r.status='consumed' AND r.consumed_date>=?`,
        )
          .bind(since)
          .first<{ exploration: number; total: number }>(),
      ])
      ...
      for (const [rating, count] of Object.entries(ratingDist)) {
        if (ratingValues[rating] !== undefined) {
          ratingSum += ratingValues[rating] * count
          ratingCount += count
        }
      }
      ...
      for (const row of lanes.results || [])
        laneBalance[row.strategy] = laneTotal > 0 ? Math.round((row.c / laneTotal) * 1000) / 1000 : 0
      ...
      // Resurfacing recall
      const resurfacingRecall =
        (resurfacing?.total || 0) > 0
          ? Math.round(((resurfacing?.resolved || 0) / resurfacing!.total) * 1000) / 1000
          : null

      // Exploration rate
      const explorationRate =
        (exploration?.total || 0) > 0
          ? Math.round(((exploration?.exploration || 0) / exploration!.total) * 1000) / 1000
          : null
...
app.post('/snapshot', async (c) => {
  const { DB } = c.env
  try {
    const metricsRes = await fetch(new URL('/analytics/recommendations?days=30', c.req.url).toString())
    const metrics = await metricsRes.json<any>()
    const id = `snap_${Date.now()}_${crypto.randomUUID().slice(0, 6)}`
    const today = new Date().toISOString().split('T')[0]
    await DB.prepare(
      `INSERT INTO analytics_snapshots (id,snapshot_date,period_days,accept_rate,avg_rating,rating_distribution_json,lane_balance_json,format_diversity,avg_time_to_consume_days,resurfacing_recall_rate,total_consumed,total_rejected,exploration_rate) VALUES (?,?,30,?,?,?,?,?,?,?,?,?,?)`,
    )
      .bind(
        id,
        today,
        metrics.accept_rate,
        metrics.avg_rating,
        JSON.stringify(metrics.rating_distribution),
        JSON.stringify(metrics.lane_balance),
        metrics.format_diversity_entropy,
        metrics.avg_time_to_consume_days,
        metrics.resurfacing_recall_rate,
...
app.get('/snapshots', async (c) => {
  ...
  try {
    const rows = await DB.prepare('SELECT * FROM analytics_snapshots ORDER BY snapshot_date DESC LIMIT ?')
      .bind(limit)
      .all<any>()
...
app.get('/session', async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      suggestion: fatigued.length
        ? `Consider non-${fatigued.join('/')} formats`
        : saturated.length
          ? `Consider topics outside ${saturated.join('/')}`
          : null,
    })
  } catch (err) {
...
app.get('/compass/full-context', async (c) => {
  ...
  try {
    const [active, consumed, taste, mastered, blacklist, session, profile, threads] = await Promise.all([
      DB.prepare(
        `SELECT id,video_title,content_type,creator,status,created_at FROM recommendations WHERE status='active' ORDER BY created_at DESC`,
      ).all<any>(),
      DB.prepare(
        `SELECT id,video_title,content_type,creator,user_rating,consumed_date FROM recommendations WHERE status='consumed' ORDER BY consumed_date DESC LIMIT 50`,
      ).all<any>(),
      ...
      DB.prepare(
        `SELECT r.id,r.video_title title,r.content_type format,r.creator,r.consumed_date,m.branch_id topic FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.status='consumed' AND r.consumed_date IS NOT NULL ORDER BY r.consumed_date DESC LIMIT 10`,
      ).all<any>(),
      DB.prepare(
        `SELECT assertion_key,category,value_json,confidence,status FROM profile_assertions WHERE status='active' ORDER BY confidence DESC`,
      ).all<any>(),
      DB.prepare(
        `SELECT id,title,guiding_question,status,priority FROM learning_threads WHERE superseded_at IS NULL AND status NOT IN ('verified','abandoned') ORDER BY priority DESC,updated_at DESC`,
      ).all<any>(),
    ])
    return c.json({
