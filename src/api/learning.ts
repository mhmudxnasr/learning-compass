...
app.post('/srs/review', async (c) => {
  ...
  try {
    ...
    `,
      ...
      .run()
    await DB.prepare(
      `INSERT INTO srs_review_events (card_id,grade,previous_state_json,next_state_json) VALUES (?,?,?,?)`,
    )
      .bind(card_id, grade, JSON.stringify(card), JSON.stringify(next))
      .run()
    if (card.unit_id || card.thread_id) {
      ...
      await recordLearningEvent(DB, {
        ...
        actorType: 'user',
        evidenceWeight: 1,
        idempotencyKey: `core-recall:${card_id}:${Date.now()}`,
...
app.post('/srs/create', async (c) => {
  const { DB } = c.env
  try {
    const { recommendation_id, note_id, thread_id, stage_id, lesson_id, question, answer, topic, branch } =
      await c.req.json<{
