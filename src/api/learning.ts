...
const cleanRecallText = (value: unknown, max: number) => String(value || '').trim().slice(0, max)
...
function recallSchedulerSnapshot(card: any) {
  return {
    ease_factor: Number(card.ease_factor ?? 5), difficulty: Number(card.difficulty ?? 5), stability: Number(card.stability ?? 1),
    interval_days: Number(card.interval_days ?? 1), repetitions: Number(card.repetitions || 0), lapses: Number(card.lapses || 0),
    learning_steps: Number(card.learning_steps || 0), scheduled_days: Number(card.scheduled_days || 0), fsrs_state: Number(card.fsrs_state || 0),
    scheduler_version: card.scheduler_version || FSRS_SCHEDULER_VERSION, due_at: card.due_at || null, last_reviewed_at: card.last_reviewed_at || null,
    repair_lapses_acknowledged: Number(card.repair_lapses_acknowledged || 0),
  }
}

function recallRepairEvent(DB: Bindings['DB'], cardId: string, mutationId: string, action: 'edit' | 'pause' | 'resume' | 'retire' | 'restore' | 'reset', options: { changeKind?: 'wording' | 'semantic'; beforeContent?: any; afterContent?: any; beforeScheduler?: any; afterScheduler?: any; reason?: string } = {}) {
  return DB.prepare(`INSERT INTO srs_card_repair_events
    ...
    SELECT ?,?,?,?,?,?,?,?,? WHERE EXISTS (
      ...
      `card_repair_${crypto.randomUUID()}`, cardId, action, options.changeKind || null,
      options.beforeContent ? JSON.stringify(options.beforeContent) : null, options.afterContent ? JSON.stringify(options.afterContent) : null,
      options.beforeScheduler ? JSON.stringify(options.beforeScheduler) : null, options.afterScheduler ? JSON.stringify(options.afterScheduler) : null,
...
function recallCasWhere(expected: RecallMutationState) {
  return [expected.content_revision, expected.scheduler_revision, expected.status_revision, expected.repair_status] as const
}

async function recallConflictResponse(c: any, cardId: string) {
  const current: any = await c.env.DB.prepare(`SELECT content_revision,scheduler_revision,status_revision,repair_status FROM srs_cards WHERE id=?`).bind(cardId).first()
  return c.json({
...
const recallPreconditionMessage = 'Send expected_content_revision, expected_scheduler_revision, expected_status_revision, and expected_repair_status from the latest card read.'
...
app.get('/srs/repair', async (c) => {
  ...
  try {
    const rows = await DB.prepare(`
      ...
      LIMIT 200
    `).bind(RECALL_REPAIR_LAPSE_THRESHOLD).all<any>()
    ...
    const [reviews, repairEvents, comparisonPool] = await Promise.all([
      DB.prepare(`SELECT id,card_id,grade,previous_state_json,next_state_json,reviewed_at
        FROM srs_review_events WHERE card_id IN (${placeholders}) ORDER BY reviewed_at DESC,id DESC`).bind(...ids).all<any>(),
      DB.prepare(`SELECT id,card_id,action,change_kind,previous_content_json,next_content_json,previous_scheduler_json,next_scheduler_json,reason,created_at
        FROM srs_card_repair_events WHERE card_id IN (${placeholders}) ORDER BY created_at DESC`).bind(...ids).all<any>(),
      DB.prepare(`SELECT id,question,answer,topic,branch,unit_id,thread_id,stage_id,lesson_id
        FROM srs_cards WHERE repair_status!='retired' ORDER BY last_reviewed_at DESC LIMIT 500`).all<any>(),
    ])
    ...
    for (const event of reviews.results || []) {
      ...
      if (list.length < 25) list.push({
        id: (event as any).id, grade: Number((event as any).grade), reviewed_at: (event as any).reviewed_at,
        previous_due: previous.due_at || null, next_due: next.dueAt || next.due_at || null,
        previous_lapses: Number(previous.lapses || 0), next_lapses: Number(next.lapses || 0),
      })
    ...
    for (const event of repairEvents.results || []) {
      ...
      if (list.length < 25) list.push({
        id: (event as any).id, action: (event as any).action, change_kind: (event as any).change_kind,
        reason: (event as any).reason, created_at: (event as any).created_at,
        previous_content: parseRecallJson((event as any).previous_content_json), next_content: parseRecallJson((event as any).next_content_json),
        previous_scheduler: parseRecallJson((event as any).previous_scheduler_json), next_scheduler: parseRecallJson((event as any).next_scheduler_json),
      })
    ...
    const enriched = cards.map((card: any) => {
      ...
      const comparisons = pool.filter((candidate: any) => candidate.id !== card.id && (
      ...
      )).slice(0, 3).map((candidate: any) => ({
        id: candidate.id, question: candidate.question, answer: candidate.answer,
        reason: card.unit_id && candidate.unit_id === card.unit_id ? 'Same retained idea' : card.branch && candidate.branch === card.branch ? 'Same branch' : 'Same topic',
      }))
      return {
        ...card,
...
app.post('/srs/review', async (c) => {
  ...
  try {
    ...
    if ((card.repair_status || 'active') !== 'active') {
      return c.json({ error: 'card_not_active', message: `This card is ${card.repair_status} and cannot be reviewed until it is active.` }, 409)
    }
    ...
    const nextState = { ...next, content_revision: expected.content_revision, scheduler_revision: expected.scheduler_revision + 1, status_revision: expected.status_revision, repair_status: expected.repair_status }
    ...
    `).bind(next.difficulty, next.difficulty, next.stability, next.intervalDays, next.repetitions, next.lapses, next.learningSteps, next.scheduledDays, next.fsrsState, next.schedulerVersion, next.dueAt, mutationId, card_id, ...recallCasWhere(expected)),
      DB.prepare(`INSERT INTO srs_review_events (card_id,grade,previous_state_json,next_state_json)
    ...
    if (card.unit_id || card.thread_id) {
      ...
      await recordLearningEvent(DB, { eventType: 'recall_attempted', actorType: 'user', evidenceWeight: 1, idempotencyKey: `core-recall:${card_id}:${mutationId}`, threadId: card.thread_id || null, recommendationId: card.recommendation_id || null, unitId: card.unit_id || null, payload: { grade, result } })
    }

    return c.json({ ok: true, next_due: next.dueAt, interval_days: next.intervalDays, ease_factor: next.difficulty, scheduler: FSRS_SCHEDULER_VERSION, state: { ...expected, scheduler_revision: expected.scheduler_revision + 1 } })
  } catch (err) {
...
app.put('/srs/cards/:id', async (c) => {
  ...
  try {
    const body = await c.req.json<any>().catch(() => ({}))
    const changeKind = body.change_kind === 'wording' || body.change_kind === 'semantic' ? body.change_kind as 'wording' | 'semantic' : null
    if (!changeKind) return c.json({ error: 'change_kind_required', message: 'Choose wording or semantic so scheduling behavior is explicit.' }, 400)
    const expected = parseRecallMutationPrecondition(body)
    if (!expected) return c.json({ error: 'recall_precondition_required', message: recallPreconditionMessage }, 400)
    ...
    if ((card.repair_status || 'active') === 'retired') return c.json({ error: 'retired_card', message: 'Restore the card before editing it.' }, 409)
    ...
    if (question === card.question && answer === card.answer) return c.json({ error: 'no_change', message: 'The question and answer are unchanged.' }, 400)
    ...
    const afterScheduler = changeKind === 'semantic'
    ...
    const update = changeKind === 'semantic'
      ? DB.prepare(`UPDATE srs_cards SET question=?,answer=?,content_revision=?,content_updated_at=datetime('now'),
          ...
          scheduler_revision=scheduler_revision+1,last_recall_mutation_id=?
          WHERE id=? AND content_revision=? AND scheduler_revision=? AND status_revision=? AND repair_status=?`).bind(
            question, answer, nextRevision, afterScheduler.ease_factor, afterScheduler.difficulty, afterScheduler.stability,
            afterScheduler.interval_days, afterScheduler.repetitions, afterScheduler.lapses, afterScheduler.learning_steps,
            afterScheduler.scheduled_days, afterScheduler.fsrs_state, afterScheduler.scheduler_version, afterScheduler.due_at,
            mutationId, card.id, ...recallCasWhere(expected),
          )
      : DB.prepare(`UPDATE srs_cards SET question=?,answer=?,content_revision=?,content_updated_at=datetime('now'),repair_lapses_acknowledged=COALESCE(lapses,0),last_recall_mutation_id=?
          WHERE id=? AND content_revision=? AND scheduler_revision=? AND status_revision=? AND repair_status=?`)
    ...
    const results = await DB.batch([
      ...
      recallRepairEvent(DB, card.id, mutationId, 'edit', {
        changeKind, beforeContent, afterContent, beforeScheduler, afterScheduler,
        reason: body.reason || (changeKind === 'semantic' ? 'Learner marked the meaning as changed; FSRS state reset.' : 'Learner repaired wording; FSRS state preserved.'),
      }),
    ])
    if (Number(results[0]?.meta?.changes || 0) !== 1) return recallConflictResponse(c, card.id)
    return c.json({
      ok: true, card_id: card.id, change_kind: changeKind, content_revision: nextRevision,
      scheduling: changeKind === 'semantic' ? 'reset' : 'preserved', next_due: afterScheduler.due_at,
      review_history_preserved: true,
      state: { ...expected, content_revision: nextRevision, scheduler_revision: expected.scheduler_revision + (changeKind === 'semantic' ? 1 : 0) },
    })
  } catch (err) {
...
app.post('/srs/cards/:id/status', async (c) => {
  ...
  try {
    ...
    if (previousStatus === nextStatus) {
      ...
      const result = await DB.prepare(`UPDATE srs_cards SET last_recall_mutation_id=?
        WHERE id=? AND content_revision=? AND scheduler_revision=? AND status_revision=? AND repair_status=?`)
        .bind(mutationId, card.id, ...recallCasWhere(expected)).run()
    ...
    const action = nextStatus === 'paused' ? 'pause' : nextStatus === 'retired' ? 'retire' : previousStatus === 'retired' ? 'restore' : 'resume'
    const mutationId = `recall_status_${crypto.randomUUID()}`
    const results = await DB.batch([
      DB.prepare(`UPDATE srs_cards SET repair_status=?,paused_at=CASE WHEN ?='paused' THEN datetime('now') ELSE NULL END,retired_at=CASE WHEN ?='retired' THEN datetime('now') ELSE NULL END,
      ...
      recallRepairEvent(DB, card.id, mutationId, action, {
        beforeContent: recallContentSnapshot(card), afterContent: recallContentSnapshot(card),
        beforeScheduler: recallSchedulerSnapshot(card), afterScheduler: recallSchedulerSnapshot(card),
        reason: body.reason || `Learner changed recall status from ${previousStatus} to ${nextStatus}.`,
      }),
    ])
    if (Number(results[0]?.meta?.changes || 0) !== 1) return recallConflictResponse(c, card.id)
    return c.json({ ok: true, changed: true, card_id: card.id, status: nextStatus, scheduling: 'preserved', review_history_preserved: true, state: { ...expected, repair_status: nextStatus, status_revision: expected.status_revision + 1 } })
  } catch (err) {
...
app.post('/srs/cards/:id/reset', async (c) => {
  ...
  try {
    const body = await c.req.json<any>().catch(() => ({}))
    if (body.confirm !== true) return c.json({ error: 'reset_confirmation_required', message: 'Send confirm: true to reset FSRS scheduling.' }, 400)
    const expected = parseRecallMutationPrecondition(body)
    if (!expected) return c.json({ error: 'recall_precondition_required', message: recallPreconditionMessage }, 400)
    const card = await DB.prepare(`SELECT * FROM srs_cards WHERE id=?`).bind(c.req.param('id')).first<any>()
    if (!card) return c.json({ error: 'card not found' }, 404)
    if (!recallMutationStateMatches(card, expected)) return recallConflictResponse(c, c.req.param('id'))
    if ((card.repair_status || 'active') === 'retired') return c.json({ error: 'retired_card', message: 'Restore the card before resetting it.' }, 409)
    const beforeScheduler = recallSchedulerSnapshot(card)
    ...
    const results = await DB.batch([
      DB.prepare(`UPDATE srs_cards SET ease_factor=?,difficulty=?,stability=?,interval_days=?,repetitions=?,lapses=?,learning_steps=?,scheduled_days=?,fsrs_state=?,scheduler_version=?,due_at=?,last_reviewed_at=NULL,repair_lapses_acknowledged=0,
          scheduler_revision=scheduler_revision+1,last_recall_mutation_id=?
          WHERE id=? AND content_revision=? AND scheduler_revision=? AND status_revision=? AND repair_status=?`)
        .bind(afterScheduler.ease_factor, afterScheduler.difficulty, afterScheduler.stability, afterScheduler.interval_days, afterScheduler.repetitions, afterScheduler.lapses, afterScheduler.learning_steps, afterScheduler.scheduled_days, afterScheduler.fsrs_state, afterScheduler.scheduler_version, afterScheduler.due_at, mutationId, card.id, ...recallCasWhere(expected)),
      recallRepairEvent(DB, card.id, mutationId, 'reset', {
        beforeContent: recallContentSnapshot(card), afterContent: recallContentSnapshot(card), beforeScheduler, afterScheduler,
        reason: body.reason || 'Learner explicitly reset FSRS scheduling.',
      }),
    ])
    if (Number(results[0]?.meta?.changes || 0) !== 1) return recallConflictResponse(c, card.id)
    return c.json({ ok: true, card_id: card.id, next_due: afterScheduler.due_at, scheduling: 'reset', review_history_preserved: true, state: { ...expected, scheduler_revision: expected.scheduler_revision + 1 } })
  } catch (err) {
...
app.post('/srs/create', async (c) => {
  const { DB } = c.env
  try {
    const { recommendation_id, note_id, thread_id, stage_id, lesson_id, question, answer, topic, branch, source_anchor, annotation_id } = await c.req.json<{ recommendation_id?: string; note_id?: string; thread_id?: string; stage_id?: string; lesson_id?: string; question: string; answer: string; topic?: string; branch?: string; source_anchor?: string; annotation_id?: string }>()
    if (!question || !answer) return c.json({ error: 'question and answer required' }, 400)
    const languageError = validateArabicRecall(question, answer)
    if (languageError) return c.json({ error: 'recall_language_required', message: languageError }, 400)
    ...
    if (!scope && annotation?.thread_id) scope = await resolveLearningScope(DB, { kind: 'thread', id: annotation.thread_id })
    ...
    `).bind(id, recommendationId, note_id || null, scope?.kind === 'thread' ? scope.threadId : null, scope?.kind === 'level' ? scope.levelId : null, scope?.lessonId || null, question, answer, topic || 'general', cardBranch, FSRS_SCHEDULER_VERSION, sourceAnchor, annotation?.id || null).run()

    return c.json({ ok: true, card_id: id, recommendation_id: recommendationId, annotation_id: annotation?.id || null, source_anchor: sourceAnchor, thread_id: scope?.kind === 'thread' ? scope.threadId : null, stage_id: scope?.kind === 'level' ? scope.levelId : null, lesson_id: scope?.lessonId || null })
  } catch (err) {
