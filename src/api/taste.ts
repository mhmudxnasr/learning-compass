...
export function tasteEvidence(sampleCount: number, lastConsumed?: string | null) {
  ...
  return {
    sample_count: count,
    minimum_sample: 3,
    evidence_status: count >= 3 ? 'usable' : 'insufficient',
    confidence,
    stale_days: recency.staleDays,
  }
}
  ...
  try {
    ...
    `,
      ...
      for (const row of rawScores.results || []) {
        ...
        if (!current.last_consumed || String(row.last_consumed || '') > current.last_consumed)
          current.last_consumed = row.last_consumed
...
app.post('/rerank', async (c) => {
  ...
  try {
    const active = await DB.prepare(
      "SELECT r.*,m.branch_id FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id WHERE r.status = 'active' ORDER BY r.created_at DESC",
    ).all<any>()
