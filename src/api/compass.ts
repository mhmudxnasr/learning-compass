...
async function learnFromOutcome(DB: D1Database, pick: any, score: number | null, outcome: string, reasonTags: string[]) {
  const recommendationOutcome = pick.recommendation_id
    ? await DB.prepare(`SELECT predicted_components_json FROM recommendation_outcomes WHERE recommendation_id=?`).bind(pick.recommendation_id).first<any>()
    : null
  const winner = !recommendationOutcome ? await DB.prepare(`SELECT features_json FROM compass_candidates WHERE pick_id=? AND is_winner=1`).bind(pick.id).first<any>() : null
  ...
  const reward = score !== null ? Math.max(0, Math.min(1, score / 10)) : outcome === 'completed' ? .75 : outcome === 'started' ? .5 : outcome === 'abandoned' ? .15 : .2
  ...
  const raw = rows.map((row: any) => {
    ...
    const signal = (reward - .5) * (component - .5) * .05 + Number(adjustments[row.dimension] || 0)
...
app.post('/picks', async (c) => {
  try {
    ...
    const scored: ScoredCandidate[] = prepared.map(({ item, index, features, sourceCheck }) => {
      const baseScore = serverScore(features, strategy, context.featureWeights?.get(strategy) || DEFAULT_FEATURE_WEIGHTS[strategy])
    ...
    // Keep a weak but eligible winner visible for curator review. It remains outside
    // the Queue until the curator explicitly adds it.
...
app.post('/pick/:id/start', async (c) => {
  try {
    ...
    await c.env.DB.batch([
      ...
      c.env.DB.prepare(`INSERT INTO learning_sessions (id,recommendation_id,status,intent,started_at) VALUES (?,?, 'active', ?, datetime('now'))`).bind(sessionId, recommendationId, 'Compass Pick'),
    ])
    return c.json({ ok: true, pick_id: pick.id, recommendation_id: recommendationId, session_id: sessionId })
...
app.post('/pick/:id/feedback', async (c) => {
  try {
    ...
    const feedbackJobId = reflection || rating.score !== null ? `job_${crypto.randomUUID()}` : null
    ...
    if (pick.recommendation_id) {
      statements.push(
        ...
        c.env.DB.prepare(`INSERT INTO recommendation_outcomes (id,recommendation_id,actual_score,outcome_status,consumed_at,evaluated_at) VALUES (?,?,?,?,CASE WHEN ? THEN date('now') ELSE NULL END,datetime('now')) ON CONFLICT(recommendation_id) DO UPDATE SET actual_score=COALESCE(excluded.actual_score,recommendation_outcomes.actual_score),outcome_status=excluded.outcome_status,consumed_at=COALESCE(recommendation_outcomes.consumed_at,excluded.consumed_at),evaluated_at=datetime('now')`).bind(`outcome_${pick.recommendation_id}`, pick.recommendation_id, rating.score, completed ? 'consumed' : excluded ? outcome === 'declined' ? 'rejected' : 'abandoned' : 'active', completed ? 1 : 0),
      )
    ...
    return c.json({ ok: true, pick_id: pick.id, status: nextStatus, recommendation_state: completed ? 'completed' : excluded ? 'excluded' : 'queued', feedback_job: feedbackJobId, learning_receipt: learning, source_page: pick.recommendation_id ? `/#/learn/notes?source=${encodeURIComponent(pick.recommendation_id)}` : null })
