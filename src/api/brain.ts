...
import { Bindings, safeError, isNonEmptyStr, isValidLength, VALID_LOG_KINDS } from '../lib'
...
import {
  actOnResurfacing,
  createResurfacingPresentation,
  getDailyResurfacing,
  setResurfacingPreference,
  ...
  try {
    ...
    const childrenParsed = (children.results || []).map((c: any) => {
      let cx = null,
        cy = null
      try {
        const m = JSON.parse(c.meta_json || '{}')
        cx = m.x
        cy = m.y
      } catch {}
    ...
    while (cur && cur.parent_id) {
      const p = await DB.prepare('SELECT id, type, label, status, parent_id, meta_json FROM tree_nodes WHERE id = ?')
        .bind(cur.parent_id)
        .first<any>()
      if (p) {
        let px = null,
  ...
  try {
    const data = await cached('brain.profile.' + recentLimit, 0, async () => {
      const [
        ...
        DB.prepare(
          "SELECT * FROM patterns ORDER BY CASE strength WHEN 'locked' THEN 0 WHEN 'confirmed' THEN 1 ELSE 2 END, confirmed_date DESC",
        ).all(),
        DB.prepare('SELECT * FROM update_log ORDER BY id DESC LIMIT ?').bind(recentLimit).all(),
        DB.prepare(
          'SELECT id, title, feed_url, site_url, last_fetched_at, is_active FROM feed_sources ORDER BY is_active DESC, title ASC',
        )
          .all()
          .catch(() => ({ results: [] })),
        DB.prepare('SELECT COUNT(*) as count FROM srs_cards')
          .first<{ count: number }>()
          .catch(() => ({ count: 0 })),
        DB.prepare("SELECT COUNT(*) as count FROM srs_drafts WHERE status = 'draft'")
          .first<{ count: number }>()
          .catch(() => ({ count: 0 })),
        DB.prepare(
          "SELECT COUNT(*) as total_sessions, SUM(CASE WHEN reflection IS NOT NULL AND reflection != '' THEN 1 ELSE 0 END) as reflections_count FROM learning_sessions",
        )
          .first<{ total_sessions: number; reflections_count: number }>()
          .catch(() => ({ total_sessions: 0, reflections_count: 0 })),
        DB.prepare('SELECT COUNT(*) as count FROM notes')
          .first<{ count: number }>()
          .catch(() => ({ count: 0 })),
        DB.prepare(
          "SELECT creator, COUNT(*) as total, ROUND(AVG(COALESCE(user_score, CASE user_rating WHEN 'love' THEN 10 WHEN 'like' THEN 8 WHEN 'meh' THEN 5 WHEN 'dislike' THEN 2 END)), 2) as average_score, SUM(CASE WHEN user_rating='love' THEN 1 ELSE 0 END) as loves FROM recommendations WHERE creator IS NOT NULL AND creator != '' AND status='consumed' GROUP BY creator ORDER BY average_score DESC, total DESC LIMIT 50",
        )
          .all()
          .catch(() => ({ results: [] })),
        DB.prepare(
          `SELECT tv.topic, COALESCE(NULLIF(n.label, ''), NULLIF(p.label, '')) AS branch_label,
          ...
          ORDER BY tv.affinity_score DESC, tv.consumption_count DESC LIMIT 50`,
        )
          .all()
          .catch(() => ({ results: [] })),
        DB.prepare(
          "SELECT s.id, r.video_title, r.creator, s.reflection, s.completed_at FROM learning_sessions s LEFT JOIN recommendations r ON r.id=s.recommendation_id WHERE s.reflection IS NOT NULL AND s.reflection != '' ORDER BY s.completed_at DESC LIMIT 20",
        )
          .all()
          .catch(() => ({ results: [] })),
        DB.prepare(
          "SELECT id, video_title, creator, user_rating, user_score, user_review, consumed_date FROM recommendations WHERE status='consumed' AND (user_rating IS NOT NULL OR user_score IS NOT NULL OR user_review IS NOT NULL) ORDER BY consumed_date DESC LIMIT 30",
        )
          .all()
          .catch(() => ({ results: [] })),
        DB.prepare(
          "SELECT COUNT(*) as count FROM artifacts WHERE COALESCE(json_extract(metadata_json,'$.publication_state'),'ready')!='staged'",
        )
          .first<{ count: number }>()
          .catch(() => ({ count: 0 })),
        DB.prepare("SELECT COUNT(*) as count FROM feedback_proposals WHERE status = 'pending'")
          .first<{ count: number }>()
          .catch(() => ({ count: 0 })),
        profileIntelligenceSnapshot(DB).catch(() => ({
          assertions: [],
          revisions: [],
      ...
      return {
        ...
        patterns: (patterns.results || []).filter(
  ...
  try {
    const data = await cached('brain.tree.' + limit + '.' + offset, 60000, async () => {
      const result = await DB.prepare(
        'SELECT id, type, label, status, super_category, parent_id, meta_json FROM tree_nodes ORDER BY id LIMIT ? OFFSET ?',
      )
        .bind(limit, offset)
        .all()
...
app.get('/branches', async (c) => {
  ...
  try {
    const data = await cached('brain.branches', 60000, async () => {
      const result = await DB.prepare(
        "SELECT super_category, status, COUNT(*) as c FROM tree_nodes WHERE type IN ('branch','leaf') GROUP BY super_category, status",
      ).all()
...
app.get('/branches/:id/items', async (c) => {
  ...
  try {
    const branch = await DB.prepare(
      'SELECT id,type,label,status,super_category,parent_id,meta_json FROM tree_nodes WHERE id=?',
    )
      .bind(id)
      .first<any>()
    ...
    let meta: any = {}
    try {
      if (branch.meta_json) meta = JSON.parse(branch.meta_json)
    } catch {}
    const [path, recommendations, notes, cards, drafts, artifacts, balance, bridges] = await Promise.all([
      DB.prepare(
        `WITH RECURSIVE chain(id,label,type,parent_id) AS (
        ...
        .all<any>(),
      DB.prepare(
        `SELECT r.id,r.video_title,r.creator,r.content_type,r.status,r.user_score,r.user_rating,r.user_review,r.consumed_date,r.created_at,m.learning_state,m.priority_rank
        ...
        ORDER BY CASE WHEN r.status='consumed' THEN 0 ELSE 1 END, COALESCE(r.consumed_date,r.created_at) DESC`,
      )
        .bind(id)
        .all<any>(),
      DB.prepare(
        `SELECT n.id,n.recommendation_id,n.title,n.kind,n.status,n.revision,n.updated_at,r.video_title source_title
        ...
        .all<any>(),
      DB.prepare(
        `SELECT sc.id,sc.recommendation_id,sc.question,sc.answer,sc.topic,sc.due_at,sc.repetitions,sc.ease_factor,sc.scheduler_version,sc.repair_status,sc.paused_at,sc.retired_at,r.video_title source_title
        ...
        .all<any>(),
      DB.prepare(
        `SELECT sd.id,sd.recommendation_id,sd.question,sd.answer,sd.topic,sd.status,r.video_title source_title
        ...
        .all<any>(),
      DB.prepare(
        `SELECT a.id,a.filename,a.media_type,a.size_bytes,a.metadata_json,a.created_at
    ...
    const priority = await DB.prepare('SELECT rank,label,rationale FROM priorities WHERE branch_id=?')
      .bind(id)
      .first<any>()
    ...
    const recs = enrichedRecommendations.map((row: any) => ({
      ...
      recall_count: undefined,
      due_count: undefined,
      html_count: undefined,
      pdf_count: undefined,
      html_artifact_id: undefined,
      pdf_artifact_id: undefined,
      note_id: undefined,
    ...
    return c.json({
      branch: {
        ...
        description: meta.notes || meta.description || null,
...
app.patch('/resurfacing/:recommendationId/preference', async (c) => {
  ...
  try {
    ...
    return c.json(safeError('Resurfacing preference failed')(err), 500)
  }
})

app.post('/resurfacing/presentations', async (c) => {
  const body = await c.req.json<{ recommendation_id?: string }>().catch(() => null)
  if (!body?.recommendation_id || !isNonEmptyStr(body.recommendation_id, 100))
    return c.json({ error: 'recommendation_id required' }, 400)
  try {
    const result = await createResurfacingPresentation(c.env.DB, body.recommendation_id)
    return result
      ? c.json({ ok: true, presentation: result })
      : c.json({ error: "source is not today's due resurfacing item" }, 409)
  } catch (err) {
    return c.json(safeError('Resurfacing presentation failed')(err), 500)
  }
})

app.post('/resurfacing/:eventId/action', async (c) => {
  const body = await c.req.json<{ action?: string }>().catch(() => null)
  if (!body || !['reviewed', 'snooze', 'dismissed'].includes(String(body.action)))
    return c.json({ error: 'action must be reviewed, snooze, or dismissed' }, 400)
  try {
    const result = await actOnResurfacing(
      c.env.DB,
      c.req.param('eventId'),
      body.action as 'reviewed' | 'snooze' | 'dismissed',
    )
    return result ? c.json({ ok: true, event: result }) : c.json({ error: 'resurfacing event not found' }, 404)
  } catch (err) {
    return c.json(safeError('Resurfacing action failed')(err), 500)
  }
})
...
app.get('/contradictions', async (c) => {
  ...
  try {
    const result = await DB.prepare(
      'SELECT * FROM contradictions WHERE resolved_at IS NULL ORDER BY detected_at DESC',
    ).all()
...
app.post('/log', async (c) => {
  ...
  try {
    ...
    await DB.prepare('INSERT INTO update_log (kind, summary, details_json) VALUES (?, ?, ?)')
      .bind(kind || 'system', summary, details ? JSON.stringify(details) : null)
      .run()
...
app.post('/seed', async (c) => {
  ...
  try {
    ...
    if (body.profile) {
      ...
      if (p.reaction_style && !isValidLength(p.reaction_style, 0, 5000))
        return c.json({ error: 'reaction_style too long' }, 400)
      if (p.quality_rules && !isValidLength(p.quality_rules, 0, 5000))
        return c.json({ error: 'quality_rules too long' }, 400)
      if (p.operational_style && !isValidLength(p.operational_style, 0, 5000))
        return c.json({ error: 'operational_style too long' }, 400)
      if (p.patterns_summary && !isValidLength(p.patterns_summary, 0, 5000))
        return c.json({ error: 'patterns_summary too long' }, 400)
      if (p.recent_signal && !isValidLength(p.recent_signal, 0, 5000))
        return c.json({ error: 'recent_signal too long' }, 400)
      stmts.push(
        DB.prepare(
          `
    ...
    if (Array.isArray(body.priorities)) {
      for (const [rank, branch_id, label, rationale] of body.priorities) {
        ...
        if (rationale && !isValidLength(rationale, 0, 500))
          return c.json({ error: 'rationale too long (max 500 chars)' }, 400)
        stmts.push(
          DB.prepare('INSERT OR REPLACE INTO priorities (rank, branch_id, label, rationale) VALUES (?, ?, ?, ?)').bind(
            rank,
            branch_id,
            label || null,
            rationale || null,
            ...
            n.meta_json ||
              (n.color
                ? JSON.stringify({
                    ...
                    user_rating: n.user_rating,
                    consumed_date: n.consumed_date,
                  })
                : null),
          ),
        )
    ...
    if (Array.isArray(body.blacklist)) {
      for (const b of body.blacklist) {
        ...
        if (b[3] && !isValidLength(b[3], 0, 1000))
          return c.json({ error: `reason too long for blacklist ${b[0]}` }, 400)
        stmts.push(
          DB.prepare(
            `
    ...
    if (Array.isArray(body.patterns_confirmed)) {
      for (const p of body.patterns_confirmed) {
        ...
        if (p[2] && !Array.isArray(p[2]))
          return c.json({ error: `evidence_json must be an array for pattern ${p[0]}` }, 400)
...
app.post('/profile', async (c) => {
  ...
  try {
    ...
    if (body.core_filter !== undefined) {
      if (typeof body.core_filter !== 'string' || !isValidLength(body.core_filter, 0, 5000))
        return c.json({ error: 'core_filter must be a string of 5000 characters or fewer' }, 400)
    ...
    if (body.mega_priority !== undefined) {
      ...
      if (value === null)
        return c.json({ error: 'mega_priority must be 5000 characters or fewer when serialized' }, 400)
    ...
    for (const [column, name, value] of jsonFields) {
      if (value !== undefined) {
        ...
        if (serializedValue === null)
          return c.json({ error: `${name} must be 5000 characters or fewer when serialized` }, 400)
        fields.push(`${column} = ?`)
        bindings.push(serializedValue)
      }
    }
    if (body.recent_signal !== undefined) {
      if (typeof body.recent_signal !== 'string' || !isValidLength(body.recent_signal, 0, 5000))
        return c.json({ error: 'recent_signal must be a string of 5000 characters or fewer' }, 400)
      fields.push('recent_signal = ?')
      bindings.push(body.recent_signal)
    }
    if (fields.length === 0) return c.json({ ok: true, count: 0 })
    fields.push("last_synced_at = datetime('now')")
    await DB.prepare(`UPDATE profile SET ${fields.join(', ')} WHERE id = 1`)
      .bind(...bindings)
      .run()
    ...
    for (const [key, category, value] of typedUpdates)
      if (value !== undefined)
        await applyProfileAssertion(DB, {
          assertionKey: `user.profile.${key}`,
          category,
          value,
          confidence: 1,
          sourceKind: 'user',
          evidence: [{ source: 'brain_profile_edit', field: key }],
          actorType: 'user',
          decisionSource: 'user',
...
app.get('/profile/intelligence', async (c) => {
  try {
    return c.json({ model_version: 'profile_v2', ...(await profileIntelligenceSnapshot(c.env.DB)) })
  } catch (err) {
    return c.json(safeError('Profile intelligence failed')(err), 500)
  }
})
...
app.put('/profile/assertions/:key', async (c) => {
  try {
    ...
    if (body.value === undefined || !String(body.category || '').trim())
      return c.json({ error: 'category and value required' }, 400)
    const result = await applyProfileAssertion(c.env.DB, {
      assertionKey: c.req.param('key'),
      category: String(body.category).slice(0, 80),
      scope: String(body.scope || 'global').slice(0, 120),
      value: body.value,
      weight: body.weight == null ? null : Number(body.weight),
      confidence: 1,
      status: body.status === 'inactive' ? 'inactive' : 'active',
      sourceKind: 'user',
      evidence: [{ source: 'profile_assertion_edit', note: String(body.reason || '').slice(0, 500) }],
      actorType: 'user',
      decisionSource: 'user',
      directUserStatement: true,
      targetVersion: body.target_version == null ? null : Number(body.target_version),
    })
    return result.ok ? c.json(result) : c.json(result, result.error === 'profile_version_conflict' ? 409 : 422)
  } catch (err) {
    return c.json(safeError('Profile assertion update failed')(err), 500)
  }
})
...
app.post('/profile/revisions/:id/revert', async (c) => {
  try {
    ...
    return c.json(safeError('Profile revision revert failed')(err), 500)
  }
})
...
app.get('/branch-deck', async (c) => {
  ...
  try {
    const [
      ...
      mappedByBranch,
      unmappedRows,
      learningUnitsByBranch,
    ] = await Promise.all([
      DB.prepare(
        `SELECT id, type, label, status, super_category, parent_id, meta_json
        FROM tree_nodes
        WHERE type='branch'
          AND (parent_id='root' OR parent_id IN (SELECT id FROM tree_nodes WHERE type='category'))
        ORDER BY CASE WHEN status = 'love' THEN 0 WHEN status = 'active' THEN 1 WHEN status = 'fresh' THEN 2 WHEN status = 'locked' THEN 3 WHEN status = 'held' THEN 4 ELSE 5 END, label COLLATE NOCASE`,
      ).all<any>(),
      DB.prepare(
        "SELECT id,label,status,super_category FROM tree_nodes WHERE type='category' AND status!='pruned' ORDER BY label COLLATE NOCASE",
      ).all<any>(),
      ...
      buildLearningBalance(DB, 90).catch(() => null),
      DB.prepare(
        `SELECT m.branch_id, COUNT(*) mapped_count,
        ...
        WHERE m.branch_id IS NOT NULL AND m.branch_id != '' GROUP BY m.branch_id`,
      )
        .all<any>()
        .catch(() => ({ results: [] })),
      DB.prepare(
        `SELECT r.dedup_key, COUNT(*) c
        FROM recommendations r LEFT JOIN recommendation_meta m ON m.recommendation_id=r.id
        ...
        .catch(() => ({ results: [] })),
      DB.prepare(
        `SELECT m.branch_id, COUNT(*) units
    ...
    const isMastered = (id: string, label: string) =>
      masteredIdsAndLabels.has(id.toLowerCase()) || masteredIdsAndLabels.has(label.toLowerCase())
    ...
    const branchIds = (existingNodes.results || [])
      .map((n: any) => n.id)
    ...
    const mappedById = new Map<string, any>(
      (mappedByBranch.results || []).map((row: any) => [String(row.branch_id), row]),
    )
    const unitsById = new Map<string, number>(
      (learningUnitsByBranch.results || []).map((row: any) => [String(row.branch_id), Number(row.units || 0)]),
    )
    ...
    const existing = (existingNodes.results || [])
      ...
      .map((node: any) => {
        let meta: any = {}
        try {
          if (node.meta_json) meta = JSON.parse(node.meta_json)
        } catch {}
        ...
        return {
          ...
          category_label:
            (categories.results || []).find((category: any) => category.id === node.super_category)?.label ||
            categoryName,
          ...
          is_candidate: ['candidate', 'active', 'fresh'].includes(
            prunedSet.has(node.id) ? 'pruned' : node.status || 'active',
          ),
...
app.post('/branch-explanations', async (c) => {
  ...
  try {
    ...
    if (!items.length || items.length > 200)
      return c.json({ ok: false, error: 'explanations must contain 1..200 items' }, 400)
    let updated = 0
    ...
    for (const item of items) {
      ...
      if (
        !node ||
        !['branch', 'leaf'].includes(
          String(node.type || '')
            .trim()
            .toLowerCase(),
        ) ||
        (nodeStatus && !['candidate', 'active', 'fresh'].includes(nodeStatus))
      ) {
        skipped.push(id)
        continue
      }
      ...
      await DB.prepare("UPDATE tree_nodes SET meta_json = ?, updated_at = datetime('now') WHERE id = ?")
    ...
    return c.json(safeError('Branch explanations failed')(err), 500)
  }
})
...
app.post('/branch-swipe', async (c) => {
  ...
  try {
    ...
    if (!['keep', 'prune', 'priority', 'hold', 'add', 'update', 'undo'].includes(action))
    ...
    const log = (summary: string, details: any) =>
      DB.prepare("INSERT INTO update_log (kind, summary, details_json) VALUES ('tree_change', ?, ?)")
        .bind(summary, JSON.stringify({ id, action, ...details }))
        .run()
        .catch(() => {})
    ...
    const readPriorityOrder = async (): Promise<PriorityRow[]> => {
      const rows = await DB.prepare(
        'SELECT branch_id,label,rationale FROM priorities ORDER BY rank ASC',
      ).all<PriorityRow>()
      return rows.results || []
    }
    const writePriorityOrder = async (rows: PriorityRow[]) => {
      await DB.batch([
        ...
        ...rows.map((row, index) =>
          DB.prepare('INSERT INTO priorities (rank,branch_id,label,rationale) VALUES (?,?,?,?)').bind(
            index + 1,
            row.branch_id,
            row.label,
            row.rationale,
    ...
    if (action === 'undo') {
      if (String(restore_status) === 'candidate') {
        ...
        await DB.prepare('DELETE FROM branch_exploration WHERE id = ?')
          .bind(id)
          .run()
          .catch(() => {})
        await DB.prepare('DELETE FROM priorities WHERE branch_id = ?')
          .bind(id)
          .run()
          .catch(() => {})
        await DB.prepare('DELETE FROM taste_vectors WHERE topic = ?')
          .bind(id)
          .run()
          .catch(() => {})
        await DB.prepare(
          "UPDATE profile_assertions SET status='inactive', updated_at=datetime('now') WHERE assertion_key=? AND status='active'",
        )
          .bind(`user.profile.branch_preference.${id}`)
          .run()
          .catch(() => {})
        ...
        const restoredStatus = [
          ...
          'standard',
          'locked',
        ].includes(String(restore_status))
          ? String(restore_status)
          : 'active'
        await DB.prepare("UPDATE tree_nodes SET status = ?, updated_at = datetime('now') WHERE id = ?")
          .bind(restoredStatus, id)
          .run()
        ...
        if (restoredStatus !== 'pruned') {
          await DB.prepare(
            "UPDATE branch_exploration SET is_pruned = 0, lifecycle_state = 'active', pruning_reason = NULL, updated_at = datetime('now') WHERE id = ?",
          )
            .bind(id)
            .run()
            .catch(() => {})
      ...
      const existing = await DB.prepare(
        "SELECT label,status,super_category,parent_id,meta_json FROM tree_nodes WHERE id=? AND type='branch'",
      )
        .bind(id)
        .first<any>()
      if (!existing) return c.json({ error: 'branch not found' }, 404)
      const nextParent = parent_id || super_category || existing.parent_id || 'root'
      const category = await DB.prepare("SELECT id FROM tree_nodes WHERE id=? AND type='category' AND status!='pruned'")
        .bind(nextParent)
        .first<any>()
      if (!category) return c.json({ error: 'valid category required' }, 400)
      let nextMeta: Record<string, unknown> = {}
      try {
        nextMeta = existing.meta_json ? JSON.parse(existing.meta_json) : {}
      } catch {}
      ...
      await DB.prepare(
        `UPDATE tree_nodes SET label=?,super_category=?,parent_id=?,meta_json=?,updated_at=datetime('now') WHERE id=?`,
      )
        .bind(branchLabel, nextParent, nextParent, JSON.stringify(nextMeta), id)
        .run()
      await DB.prepare('UPDATE priorities SET label=? WHERE branch_id=?')
        .bind(branchLabel, id)
        .run()
        .catch(() => {})
      ...
      await DB.prepare(
        "INSERT INTO tree_nodes (id, type, label, super_category, parent_id, status, updated_at) VALUES (?, 'branch', ?, ?, 'root', 'love', datetime('now')) ON CONFLICT(id) DO UPDATE SET status = 'love', updated_at = datetime('now')",
      )
        .bind(id, branchLabel, cat)
        .run()
      await log(`Branch ${branchLabel} kept and updated to LOVE status`, {})
    } else if (action === 'prune') {
      await DB.prepare(
        "INSERT INTO tree_nodes (id, type, label, super_category, parent_id, status, updated_at) VALUES (?, 'branch', ?, ?, 'root', 'pruned', datetime('now')) ON CONFLICT(id) DO UPDATE SET status = 'pruned', updated_at = datetime('now')",
      )
        .bind(id, branchLabel, cat)
        .run()
      await DB.prepare(
        "INSERT INTO branch_exploration (id, name, path, lifecycle_state, confidence_score, is_pruned, pruning_reason, updated_at) VALUES (?, ?, ?, 'pruned', 0, 1, 'Pruned in Branch Deck', datetime('now')) ON CONFLICT(id) DO UPDATE SET is_pruned = 1, lifecycle_state = 'pruned', pruning_reason = 'Pruned in Branch Deck', updated_at = datetime('now')",
      )
        .bind(id, branchLabel, id)
        .run()
      ...
      await writePriorityOrder([
        {
          branch_id: id,
          label: branchLabel,
          rationale: rationale || existingPriority?.rationale || 'Promoted in Branch Deck',
        },
      ...
      await DB.prepare(
        "INSERT INTO tree_nodes (id, type, label, super_category, parent_id, status, updated_at) VALUES (?, 'branch', ?, ?, 'root', 'held', datetime('now')) ON CONFLICT(id) DO UPDATE SET status = 'held', updated_at = datetime('now')",
      )
        .bind(id, branchLabel, cat)
        .run()
      ...
      if (!category) return c.json({ error: 'valid category required' }, 400)
      await DB.prepare(
        "INSERT INTO tree_nodes (id, type, label, super_category, parent_id, status, meta_json, updated_at) VALUES (?, 'branch', ?, ?, ?, 'active', ?, datetime('now')) ON CONFLICT(id) DO UPDATE SET label = excluded.label, super_category = excluded.super_category, parent_id = excluded.parent_id, status = 'active', meta_json = excluded.meta_json, updated_at = datetime('now')",
      )
        .bind(id, branchLabel, nextParent, nextParent, JSON.stringify(suppliedMeta))
        .run()
    ...
    if (action !== 'update') {
      try {
        const lastSwipes = await DB.prepare(
          "SELECT summary FROM update_log WHERE kind = 'tree_change' ORDER BY ts DESC LIMIT 8",
        ).all<any>()
        const summaries = (lastSwipes.results || []).map((r: any) => r.summary).join(' | ')
        await DB.prepare("UPDATE profile SET recent_signal = ?, last_synced_at = datetime('now') WHERE id = 1")
          .bind(`Branch decisions: ${summaries}`)
          .run()
      } catch {}
    }

    const profileState = await DB.prepare('SELECT last_synced_at, recent_signal FROM profile WHERE id = 1')
      .first<any>()
      .catch(() => null)
    const assertionState = await DB.prepare('SELECT updated_at FROM profile_assertions WHERE assertion_key = ?')
      .bind(`user.profile.branch_preference.${id}`)
      .first<any>()
      .catch(() => null)
    ...
    return c.json({
      ...
      profile_sync: {
        ...
        context_refresh:
          action === 'update'
            ? 'Branch metadata updated without changing recommendation preferences.'
            : 'Compass context will include this branch decision on its next read.',
      },
    })
  } catch (err) {
...
const SUGGEST_MODES: Record<string, string> = {
  surprise:
    "Suggest an unexpected but genuinely promising direction not obviously implied by the current map — a creative leap grounded in this person's learning profile. Avoid anything already on the map.",
  expand:
    'Suggest a natural next expansion of an existing loved or high-priority branch — an adjacent subtopic that would deepen that branch.',
  bridge:
    'Suggest a new hybrid branch that connects two existing branches which rarely appear together, creating a novel synthesis.',
  challenge:
    'Suggest a counterbalancing branch that challenges a dominant conviction or over-weighted direction — a productive contrarian path, not a troll.',
}
...
app.post('/branch-suggest', async (c) => {
  ...
  try {
    const body = await c.req
      .json<{ count?: number; mode?: string }>()
      .catch(() => ({}) as { count?: number; mode?: string })
    const mode = Object.prototype.hasOwnProperty.call(SUGGEST_MODES, String(body?.mode))
      ? String(body!.mode)
      : 'surprise'
    const count = Math.max(1, Math.min(Number(body?.count) || 3, 6))

    const [context, nodes] = await Promise.all([
      loadCompassContext(DB).catch(() => null),
      DB.prepare(
        "SELECT id, label, status, super_category FROM tree_nodes WHERE type='branch' ORDER BY CASE status WHEN 'love' THEN 0 WHEN 'priority' THEN 0 ELSE 1 END, id",
      )
        .all<any>()
        .catch(() => ({ results: [] })),
    ])
    ...
    const loved = branchRows
      .filter((n: any) => n.status === 'love' || n.status === 'priority')
      .map((n: any) => String(n.label || n.id))
    ...
    if (context?.topicAffinities)
      for (const [topic, value] of context.topicAffinities) if (topic) affinities.push([String(topic), Number(value)])
    affinities.sort((a, b) => b[1] - a[1])

    const creators: Array<[string, number]> = []
    if (context?.creatorTrust)
      for (const [key, info] of context.creatorTrust)
        if (key && info.count > 0) creators.push([String(key), Number(info.average)])
    ...
    const grounding = [
      ...
      `STRONGEST TOPIC AFFINITIES: ${
        affinities
          .slice(0, 8)
          .map(([t, v]) => `${t} (${v.toFixed(1)})`)
          .join(', ') || '(no signal yet)'
      }`,
      `HIGHEST-TRUST CREATORS: ${
        creators
          .slice(0, 6)
          .map(([k]) => k)
          .join(', ') || '(no signal yet)'
    ...
    const result = await freeAi(
      c.env,
      'You are the branch curator for a private learning OS. Return ONLY valid JSON as instructed.',
      prompt,
      2048,
    )
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            for (const item of parsed) {
              ...
              if (blocked.some((b: string) => low.includes(b.toLowerCase()) || String(b).toLowerCase().includes(low)))
                continue
              const slug =
                label
                  .toLowerCase()
              ...
              suggestions.push({
                id: `branch-${slug}`,
                label,
                super_category: /^cat-/.test(String(item?.super_category || ''))
                  ? String(item.super_category).slice(0, 60)
                  : `cat-${String(item?.super_category || 'mind')
                      .toLowerCase()
                      .replace(SLUG_RE, '-')
                      .slice(0, 40)}`,
                description: String(item?.description || '')
                  .trim()
                  .slice(0, 1000),
                plain_language: String(item?.plain_language || '')
                  .trim()
                  .slice(0, 500),
                leaves_sample: Array.isArray(item?.leaves_sample)
                  ? item.leaves_sample
                      .map((leaf: any) => String(leaf).trim().slice(0, 120))
                      .filter(Boolean)
                      .slice(0, 12)
                  : [],
                contrast_hook: String(item?.contrast_hook || '')
                  .trim()
                  .slice(0, 500),
                why_now: String(item?.why_now || '')
                  .trim()
                  .slice(0, 500),
                evidence_grounding: String(item?.evidence_grounding || '')
                  .trim()
                  .slice(0, 300),
                evidence_confidence: ['low', 'medium', 'high'].includes(
                  String(item?.evidence_confidence || '').toLowerCase(),
                )
                  ? String(item.evidence_confidence).toLowerCase()
                  : 'low',
                overlap_candidates: Array.isArray(item?.overlap_candidates)
                  ? item.overlap_candidates
                      .map((label: any) => String(label).trim().slice(0, 100))
                      .filter(Boolean)
                      .slice(0, 3)
                  : [],
                suggested_next_move: String(item?.suggested_next_move || '')
                  .trim()
                  .slice(0, 300),
                uncertainty_note: String(item?.uncertainty_note || '')
                  .trim()
                  .slice(0, 300),
                status: 'candidate',
                source: 'suggest',
          ...
          /* malformed LLM output — fall through with what parsed */
        }
      }
    }
    ...
    return c.json({
      ...
      message: result
        ? undefined
        : 'LLM unavailable — suggest is read-only; the grounded Hermes copy-prompt is the fallback.',
      generated_at: new Date().toISOString(),
    })
  } catch (err) {
...
app.post('/profile/sync-swipes', async (c) => {
  ...
  try {
    const nodes = await DB.prepare(
      "SELECT id, label, status, super_category FROM tree_nodes WHERE status IN ('love','pruned','held')",
    ).all<any>()
    let synced = 0
    for (const node of nodes.results || []) {
      ...
      await applyProfileAssertion(DB, {
        ...
        value: JSON.stringify({
          ...
          label: node.label,
          status: node.status,
          super_category: node.super_category,
        }),
        confidence: 1.0,
        sourceKind: 'user',
    ...
    const lastSwipes = await DB.prepare(
      "SELECT summary FROM update_log WHERE kind = 'tree_change' ORDER BY ts DESC LIMIT 12",
    ).all<any>()
    const summaries = (lastSwipes.results || []).map((r: any) => r.summary).join(' | ')
    await DB.prepare("UPDATE profile SET recent_signal = ?, last_synced_at = datetime('now') WHERE id = 1")
      .bind(`Synced Branch Swipes (${synced}): ${summaries}`)
      .run()
...
app.post('/priorities', async (c) => {
  ...
  try {
    ...
    for (const p of body) {
      ...
      if (p.rationale && !isValidLength(p.rationale, 0, 500))
        return c.json({ error: 'rationale too long (max 500 chars)' }, 400)
      stmts.push(
        DB.prepare('INSERT INTO priorities (rank, branch_id, label, rationale) VALUES (?, ?, ?, ?)').bind(
          p.rank,
...
app.post('/node', async (c) => {
  const { DB } = c.env
  try {
    const { id, label, type, super_category, parent_id } = await c.req.json<{
      id?: string
      label: string
      type?: string
      super_category?: string
    ...
    await DB.prepare(
      ...
      .bind(nodeId, nodeType, label.trim(), 'active', cat.startsWith('cat-') ? cat : 'cat-' + cat, parent)
      .run()
    return c.json({
      ok: true,
      node: {
...
app.put('/node/:id', async (c) => {
  ...
  await c.env.DB.prepare(`UPDATE tree_nodes SET ${fields.join(',')} WHERE id=?`)
    .bind(...values, id)
    .run()
  return c.json({ ok: true })
})
