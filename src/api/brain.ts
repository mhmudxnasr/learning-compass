  ...
  try {
    ...
    const [path, recommendations, notes, cards, drafts, artifacts, balance, bridges] = await Promise.all([
      ...
      DB.prepare(
        ...
        .all<any>(),
      DB.prepare(
        `SELECT sc.id,sc.recommendation_id,sc.question,sc.answer,sc.topic,sc.due_at,sc.repetitions,sc.ease_factor,sc.scheduler_version,r.video_title source_title
