...
export async function activateWaitingRun(DB: D1Database, targetRunId?: string) {
  const run = targetRunId
    ? await DB.prepare(`SELECT * FROM discovery_runs WHERE id = ?`).bind(targetRunId).first<any>()
    : await DB.prepare(
        `SELECT * FROM discovery_runs WHERE lifecycle = 'waiting_for_capacity' ORDER BY created_at ASC LIMIT 1`,
      ).first<any>()
  ...
  if (run.lifecycle === 'active' || run.lifecycle === 'interviewing' || run.lifecycle === 'resolved') {
    const candidate = await DB.prepare(`SELECT * FROM discovery_candidates WHERE id = ?`)
      .bind(run.selected_candidate_id)
      .first<any>()
    const rec = candidate
      ? await DB.prepare(
  ...
  if (queueCount >= 5) {
    ...
    await DB.prepare(
      `UPDATE discovery_runs SET lifecycle = 'waiting_for_capacity', updated_at = datetime('now') WHERE id = ?`,
    )
      .bind(run.id)
      .run()
    return null
  ...
  await DB.batch([
    DB.prepare(`UPDATE recommendations SET creator = ?, content_type = ?, why_this = ? WHERE id = ?`).bind(
      candidate.creator || null,
      candidate.format || null,
      receipt?.why_this || 'Activated via Discovery Engine V2',
      capture.id,
    ),
    DB.prepare(
      `INSERT INTO recommendation_meta (recommendation_id, priority_rank, branch_id, learning_state, updated_at) VALUES (?, 1, ?, 'queued', datetime('now'))
    ...
    DB.prepare(`UPDATE recommendations SET status = 'active', updated_at = datetime('now') WHERE id = ?`).bind(
      capture.id,
    ),
    DB.prepare(
      `INSERT INTO learning_sessions (id, recommendation_id, status, intent, started_at) VALUES (?, ?, 'active', ?, datetime('now'))`,
    ).bind(sessionId, capture.id, `Explore discovery frontier: ${candidate.title}`),
    DB.prepare(
      `INSERT INTO recommendation_outcomes (id,recommendation_id,discovery_run_id,source_class,format,creator,branch_id,predicted_score,predicted_confidence,predicted_components_json,outcome_status,outcome_origin,training_eligible,objective_version,format_key,creator_key)
      ...
      capture.id,
      run.id,
      candidate.source_class || null,
      candidate.format || null,
      candidate.creator || null,
      run.selected_branch_id || 'general',
      Number(candidate.total_score || 0),
      receipt?.confidence == null ? null : Number(receipt.confidence),
      JSON.stringify(candidate.score_components || candidate.score_components_json || {}),
      canonicalFormat(candidate.format || candidate.source_class),
      canonicalCreatorKey(candidate.creator),
    ),
  ])
...
async function getGateState(DB: D1Database) {
  ...
  return {
    ...
    active_run: activeRun
      ? {
          ...activeRun,
          decision_receipt: activeRun.decision_receipt_json ? JSON.parse(activeRun.decision_receipt_json) : null,
        }
      : null,
  }
}
...
app.get('/state', async (c) => {
  ...
  try {
    ...
    if (gate.active_run?.selected_candidate_id) {
      selectedCandidate = await DB.prepare(`SELECT * FROM discovery_candidates WHERE id = ?`)
        .bind(gate.active_run.selected_candidate_id)
        .first<any>()
    }

    let activeInterview = null
    if (gate.active_run?.id) {
      activeInterview = await DB.prepare(
        `SELECT * FROM discovery_interviews WHERE run_id = ? ORDER BY created_at DESC LIMIT 1`,
      )
        .bind(gate.active_run.id)
        .first<any>()
    }
...
app.get('/context', async (c) => {
  ...
  try {
    ...
    const weights = await DB.prepare(
      `SELECT dimension, baseline_weight, current_weight, evidence_count FROM engine_weights`,
    ).all<any>()
    const branches = await DB.prepare(
      `SELECT id, name, parent_id, lifecycle_state, confidence_score, probe_count, current_wave, is_pruned, pruning_reason FROM branch_exploration`,
    ).all<any>()
    const recentEvidence = await DB.prepare(
      `SELECT e.branch_id, e.signal_dimension, e.signal_value, e.confidence, e.source_type, e.created_at FROM branch_evidence e ORDER BY e.created_at DESC LIMIT 20`,
    ).all<any>()
    const recentInterviews = await DB.prepare(
      `SELECT i.id, i.run_id, i.learning_receipt_json, i.status, i.updated_at FROM discovery_interviews i WHERE i.status = 'resolved' ORDER BY i.updated_at DESC LIMIT 5`,
    ).all<any>()
...
app.post('/runs', async (c) => {
  ...
  try {
    const body = await c.req
      .json<{ mission?: string; selected_branch_id?: string; model_version?: string }>()
      .catch(() => ({}) as { mission?: string; selected_branch_id?: string; model_version?: string })
      ...
      return c.json(
        {
          ...
          message:
            gate.blocked_reason || 'An active discovery run or queue capacity block prevents starting a new run.',
          active_run: gate.active_run,
        },
        409,
      )
    ...
    const lastWaveRow = await DB.prepare(`SELECT MAX(wave) as max_wave FROM discovery_runs`).first<{
      max_wave: number
    }>()
    ...
    await DB.prepare(
      ...
      .bind(runId, mission, nextWave, body.selected_branch_id || null, modelVersion, skillVersion)
      .run()
    ...
    await DB.prepare(
      ...
      .bind(
        jobId,
        JSON.stringify({ run_id: runId, mission, wave: nextWave, selected_branch_id: body.selected_branch_id }),
      )
      .run()
...
app.post('/runs/:id/candidates', async (c) => {
  ...
  try {
    ...
    const existingCandidatesRows = await DB.prepare(`SELECT * FROM discovery_candidates WHERE run_id = ?`)
      .bind(runId)
      .all<any>()
    ...
    for (const cand of allCandidates) {
      const canonical = String(cand.canonical_url || '')
        .trim()
        .replace(/\/$/, '')
        ...
        return c.json(
          {
            ...
            message: `Candidate "${cand.title || 'untitled'}" has an invalid canonical_url.`,
          },
          400,
        )
      if (!allowedSourceClasses.has(String(cand.source_class || '')))
        return c.json(
          { error: 'quality_rule_violation', message: `Unsupported source class: ${cand.source_class}` },
          400,
        )
    ...
    if (duplicateUrls.length)
      return c.json(
        {
          error: 'quality_rule_violation',
          message: 'Every candidate must have a unique canonical URL.',
    ...
    const weightMap: Record<string, number> = (weightsRows.results || []).reduce(
      (acc: any, w: any) => {
        acc[w.dimension] = w.current_weight
        return acc
      },
      {
        frontier_potential: 0.3,
        info_gain: 0.2,
        personal_pull: 0.15,
    ...
    for (const item of body.candidates) {
      ...
      const dialecticScore =
        sc.dialectic_divergence !== undefined
...
app.post('/runs/:id/select', async (c) => {
  ...
  try {
    const body = await c.req
      .json<{ selected_candidate_id?: string; decision_receipt?: DecisionReceipt }>()
      .catch(() => ({}) as { selected_candidate_id?: string; decision_receipt?: DecisionReceipt })

    const run = await DB.prepare(`SELECT * FROM discovery_runs WHERE id = ?`).bind(runId).first<any>()
    if (!run) return c.json({ error: 'discovery run not found' }, 404)

    const candidatesRows = await DB.prepare(`SELECT * FROM discovery_candidates WHERE run_id = ?`)
      .bind(runId)
      .all<any>()
    ...
    if (candidatesList.length < 20 || sourceClasses.size < 4 || verifiedCount < 8) {
      await DB.prepare(`UPDATE discovery_runs SET lifecycle = 'withheld', updated_at = datetime('now') WHERE id = ?`)
        .bind(runId)
        .run()
      return c.json({
        ok: false,
    ...
    const candidate = await DB.prepare(`SELECT * FROM discovery_candidates WHERE id = ? AND run_id = ?`)
      .bind(candidateId, runId)
      .first<any>()
    if (!candidate) return c.json({ error: 'candidate not found' }, 404)

    if (!candidate.is_verified || candidate.total_score < 0.6) {
      await DB.prepare(`UPDATE discovery_runs SET lifecycle = 'withheld', updated_at = datetime('now') WHERE id = ?`)
        .bind(runId)
        .run()
      return c.json({
        ...
        lifecycle: 'withheld',
        reason:
          'Selected candidate failed verification or minimum score threshold (score < 0.60 or unverified). Result withheld.',
      })
    }
    ...
    await DB.prepare(
      ...
      .bind(candidateId, JSON.stringify(receipt), runId)
      .run()
...
app.post('/runs/:id/cancel', async (c) => {
  ...
  try {
    ...
    await DB.prepare(`UPDATE discovery_runs SET lifecycle = 'cancelled', updated_at = datetime('now') WHERE id = ?`)
      .bind(runId)
      .run()
    await DB.prepare(
      `UPDATE agent_jobs SET status = 'cancelled', error = 'Discovery run cancelled by user', updated_at = datetime('now') WHERE job_type = 'discover_source' AND payload_json LIKE ? AND status IN ('pending', 'running')`,
    )
      .bind(`%${runId}%`)
      .run()
...
app.post('/runs/:id/interview', async (c) => {
  ...
  try {
    ...
    const ambiguities =
      body.unresolved_ambiguities !== undefined
        ? body.unresolved_ambiguities
        : interview?.unresolved_ambiguities_json
          ? JSON.parse(interview.unresolved_ambiguities_json)
          : []
    ...
    await DB.prepare(`UPDATE discovery_runs SET lifecycle = 'interviewing', updated_at = datetime('now') WHERE id = ?`)
      .bind(runId)
      .run()
...
app.post('/runs/:id/resolve', async (c) => {
  ...
  try {
    ...
    const unresolvedAmbiguities = interview.unresolved_ambiguities_json
      ? JSON.parse(interview.unresolved_ambiguities_json)
      : []
      ...
      return c.json(
        {
          ...
          message:
            'Cannot resolve discovery: all questions in the adaptive interview must have matching non-empty answers.',
    ...
    for (const mut of body.branch_mutations || []) {
      ...
      const pastEvidenceRows = await DB.prepare(`SELECT signal_value, run_id FROM branch_evidence WHERE branch_id = ?`)
        .bind(mut.branch_id)
        .all<any>()
      ...
      const negativeRunIds = new Set(
        pastEvidence.filter((e: any) => e.signal_value <= -0.6 && e.run_id).map((e: any) => e.run_id),
      )
      ...
      if (mut.action === 'promote') {
        if (isFrontier || positiveCount >= 2) {
          ...
          validatedMutations.push({
            ...
            action: 'promote',
            status: 'rejected',
            reason: 'Requires 1 frontier-opening signal or 2 positive evidence probes.',
          })
        }
        ...
        if (distinctNegativeProbesCount >= 2 || explicitTopicRejection) {
          ...
          validatedMutations.push({
            ...
            action: 'prune',
            status: 'rejected',
            reason: 'Requires 2 distinct negative probes/runs or explicit topic rejection.',
          })
        }
        ...
        if (isFrontier || isImpact || (body.learning_receipt?.confidence || 0) >= 0.8) {
          ...
          validatedMutations.push({
            ...
            action: 'reopen',
            status: 'rejected',
            reason: 'Requires explicit high-confidence positive evidence.',
          })
        }
      }
    }
    ...
    statements.push(
      DB.prepare(`UPDATE discovery_runs SET lifecycle = 'resolved', updated_at = datetime('now') WHERE id = ?`).bind(
        runId,
      ),
    )
    ...
    if (body.learned_heuristics_patch) {
      ...
      await DB.prepare(
        ...
        .bind(
          ...
          fileHash,
          null,
          JSON.stringify({ patch, owner: 'learning-compass-self-evolution', target: 'taste-rec' }),
          validationStatus,
          interview.id,
        )
        .run()
...
app.get('/revisions/pending', async (c) => {
  ...
  try {
    const rows = await DB.prepare(
      `SELECT * FROM skill_revisions WHERE validation_result = 'staged' ORDER BY created_at ASC`,
    ).all<any>()
    return c.json({
...
app.post('/revisions/:id/confirm', async (c) => {
  ...
  try {
    ...
    await DB.prepare(
      ...
      .bind(body.file_hash, body.backup_path || null, status, revId)
      .run()
...
app.get('/drift-check', async (c) => {
  ...
  try {
    ...
    try {
      ...
      activeSkills = Object.fromEntries(
        ACTIVE_SKILL_PATHS.map((path) => [path, Boolean(fs.existsSync && fs.existsSync(path))]),
      )
    } catch {
      /* Workerd runtime */
    }
    ...
    return c.json({
      ...
      active_hermes_workflows: [...ACTIVE_SKILL_PATHS.map((path) => ({ path, exists: activeSkills[path] ?? false }))],
