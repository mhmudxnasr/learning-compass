  ...
  try {
    const data = await cached('stats.' + allLimit + '.' + allOffset, 60000, async () => {
      const [
        ...
        DB.prepare(
          "SELECT user_rating, COUNT(*) as c FROM recommendations WHERE status='consumed' AND user_rating != 'unset' AND user_rating != '' GROUP BY user_rating ORDER BY c DESC",
        ).all<{ user_rating: string; c: number }>(),
        DB.prepare(
          "SELECT substr(consumed_date,1,7) as m, COUNT(*) as c FROM recommendations WHERE status='consumed' AND consumed_date != 'unset' GROUP BY m ORDER BY m ASC",
        ).all<{ m: string; c: number }>(),
        DB.prepare(
          "SELECT creator, COUNT(*) as c FROM recommendations WHERE creator IS NOT NULL AND creator != '' GROUP BY creator ORDER BY c DESC LIMIT 15",
        ).all<{ creator: string; c: number }>(),
        DB.prepare(
          "SELECT video_title, creator, user_rating, user_review, consumed_date FROM recommendations WHERE status='consumed' ORDER BY consumed_date DESC LIMIT 25",
        ).all(),
        DB.prepare(
          "SELECT video_title, creator, why_this, created_at FROM recommendations WHERE status='active' ORDER BY created_at DESC LIMIT 25",
        ).all(),
        DB.prepare(
          "SELECT synergy_bundle_id, COUNT(*) as c FROM recommendations WHERE synergy_bundle_id != 'unset' GROUP BY synergy_bundle_id ORDER BY c DESC",
        ).all<{ synergy_bundle_id: string; c: number }>(),
        DB.prepare(
          'SELECT video_title, creator, status, user_rating, user_review, why_this, synergy_bundle_id, content_type, created_at FROM recommendations ORDER BY created_at ASC LIMIT ? OFFSET ?',
        )
          .bind(allLimit, allOffset)
          .all(),
        DB.prepare(
          'SELECT id, filename, created_at, length(content) as size FROM html_files ORDER BY created_at DESC',
        ).all(),
      ...
      `,
        ...
        DB.prepare(
          "SELECT MAX(c) as max FROM (SELECT COUNT(*) as c FROM learning_log GROUP BY strftime('%W', date) || '-' || strftime('%Y', date) ORDER BY 1 DESC)",
        ).first<{ max: number }>(),

        DB.prepare(
          "SELECT COUNT(*) as c FROM learning_log WHERE date >= date('now', 'weekday 0', '-7 days') AND date < date('now', 'weekday 0')",
        ).first<{ c: number }>(),
