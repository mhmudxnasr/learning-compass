...
app.get('/', async (c) => {
  ...
  if (recIds.length) {
    ...
    `,
      ...
      .bind(...recIds)
      .all<{ id: string; video_url: string; notebook_url: string }>()
...
app.get('/:id/view', async (c) => {
  const row = await c.env.DB.prepare(
    `SELECT a.id,a.filename,a.media_type,a.r2_key,a.metadata_json,r.id owner_id,r.status owner_status,r.deleted_at owner_deleted_at FROM artifacts a LEFT JOIN recommendations r ON r.id=json_extract(a.metadata_json,'$.recommendation_id') WHERE a.id=?`,
  )
    .bind(c.req.param('id'))
    .first<any>()
  ...
  return new Response(markdownToHtml(markdown, row.filename), {
    headers: {
      ...artifactHeaders({ media_type: 'text/html', filename: row.filename }),
