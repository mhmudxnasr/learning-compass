...
const reasonAliases: Record<string, ReasonCode> = {
  ...
  busy: 'not_now',
  not_for_me: 'bad_fit',
  irrelevant: 'wrong_topic',
  redundant: 'too_familiar',
  familiar: 'too_familiar',
  shallow: 'too_shallow',
  long: 'too_long',
  source: 'poor_source',
  format: 'wrong_format',
  mastered: 'already_mastered',
  advanced: 'too_advanced',
  relevant: 'highly_relevant',
  excellent: 'excellent_source',
}
...
export function classifyRecommendationFeedback(outcome: unknown, reasons: unknown[] = []) {
  const normalizedOutcome = String(outcome || 'declined')
    .trim()
    .toLowerCase()
  ...
  if (neutral)
    return {
      ...
      signalScope: 'none' as SignalScope,
      reasonCodes,
      normalizedOutcome: 'dismissed',
    }
  if (normalizedOutcome === 'started')
    return {
      eventType: 'recommendation_started',
      signalScope: 'none' as SignalScope,
      reasonCodes,
      normalizedOutcome: 'started',
    }
  ...
  if (normalizedOutcome === 'abandoned')
    return {
      eventType: 'recommendation_abandoned',
      signalScope: 'eligibility' as SignalScope,
      reasonCodes,
  ...
  return {
    ...
    signalScope: 'eligibility' as SignalScope,
    reasonCodes: reasonCodes.length ? reasonCodes : ['bad_fit' as ReasonCode],
...
const evidenceResultFactors: Record<string, number> = { pass: 1, recorded: 1, partial: 0.5, fail: 0 }
...
export function computeLearningUtility(input: {
  ...
  const rating =
    input.rating == null || !Number.isFinite(Number(input.rating)) ? null : clamp(Number(input.rating), 0, 10) / 10
  const disposition = (
    String(input.disposition || 'undecided').toLowerCase() in dispositionValues
      ? String(input.disposition || 'undecided').toLowerCase()
      : 'undecided'
  ) as LearningDisposition
  ...
  for (const item of input.evidence || []) {
    ...
    const resultFactor =
      item.score == null ? (evidenceResultFactors[String(item.result || '').toLowerCase()] ?? 0) : clamp(item.score)
  ...
  const normalized = availableWeight
    ? available.reduce((sum, [value, weight]) => sum + value * weight, 0) / availableWeight
    : null
  return {
    tasteValue: rating,
    dispositionValue,
...
export function structuredEvidenceStatus(value: unknown): 'structured' | 'legacy' | 'missing' | 'invalid' {
  ...
  const valid = evidence.every(
    (item) =>
      item &&
      typeof item === 'object' &&
      typeof (item as CandidateEvidence).claim === 'string' &&
      (item as CandidateEvidence).claim!.trim().length >= 12 &&
      typeof (item as CandidateEvidence).source_url === 'string' &&
      /^https?:\/\//i.test(String((item as CandidateEvidence).source_url)),
  )
...
export function profileMutationPolicy(input: {
  ...
  if (input.replacingExplicit) {
    const eligible = Boolean(input.directContradiction) || (confidence >= 0.95 && input.evidenceCount >= 3)
    return {
      eligible,
      threshold: 0.95,
      reason: eligible ? 'explicit_assertion_superseded' : 'explicit_assertion_requires_stronger_evidence',
    }
  }
  const eligible = confidence >= 0.8 && (Boolean(input.directUserStatement) || input.evidenceCount >= 2)
  return { eligible, threshold: 0.8, reason: eligible ? 'evidence_threshold_met' : 'insufficient_profile_evidence' }
}

export function assertionKey(value: unknown) {
  return (
    String(value || 'profile.signal')
      .trim()
      .toLowerCase()
