...
- **Five Root Destinations**: `#/home` (Today), `#/library` (Books, Queue, RSS Feeds, Archive, Files), `#/learn` (Threads by default; Practice contains Notes, Recall, and Contradictions), `#/map` (Atlas, unified Branch Review), `#/settings` (Learning profile, Preferences, Data & recovery, System).
...
- **Hardcover Is Visible, Not Silently Imported**: My Books composes the D1 Hardcover mirror as read-only history, deduplicates unambiguous matches, and sends mirror-only titles to Hardcover. Only the explicit branch-gated import flow creates editable personal records.
...
- **Direct Lesson Progression**: Completing lessons is the sole progression signal for Levels and Threads. No separate Level-start step is required: completing a lesson activates an available Level, starts the next unfinished lesson automatically, and completing a Level activates the next Level and its first lesson. A current lesson can be finished directly from Home without leaving the learning desk; Home confirms the write, refreshes the dashboard, animates the completed turn away, and replaces it with the next ordered lesson. Completion from a typed lesson route opens the next ordered lesson automatically when one remains.
...
- **Reading priority**: A note has one readable column, optional contents and study tools, and provenance details outside the narrative. Source blocks retain their order and direction. Books presents the next chapter before healthy-source diagnostics, with My Books and Canon jumps inside the continuous room. Compact lesson navigation starts collapsed. Home and lessons use visible format labels and touch-sized actions; branch names resolve from canonical branch records. Search offers eight local recent selections with clear and keyboard-reopen controls. Atlas starts with a domain selector and readable domain labels.
