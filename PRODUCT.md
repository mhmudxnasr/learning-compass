...
- Books is one Learn mode with two subordinate lenses. Shelf owns captured books, chapters, companions, and passive dossier/file links; Queue alone owns tracked Start/Resume sessions. Canon atlas is the evergreen browse-first exception to finite Threads: it separates complete **Ready to explore** trios from non-actionable **Coming next** coverage. Every domain carries a verified branch, boundary, curation state, and field-test state; every selection carries a full dossier, sources, and strongest rejected alternative. Surprise discovery selects only complete trios. A Canon selection becomes a Shelf record only through explicit branch-preserving capture, and a domain can create a finite Thread only after its approved trio is complete and at least one selected book is captured.
