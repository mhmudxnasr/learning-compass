...
4. Inspect the remote D1 migration ledger. Migrations through `0076_retire_unrebuilt_riyadh_companions.sql` are applied; require exact repository parity and never replay applied migrations.
...
8. After every prior gate passes, apply only verified pending migrations, check the ledger and readiness, then deploy only from the task worktree with `npx wrangler deploy --config wrangler.toml`.
