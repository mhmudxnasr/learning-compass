...
Large replacement runs create one manifest-, ordered-target-set-, and aggregate-audit-bound corpus through `POST /artifacts/corpora`. The authoritative Riyadh manifest contains 294 ordered targets with target-set SHA-256 `1de52aac5305c071c027650b8494971d7ccbd83a1bfe497b8fbcb067eeae6be4`. Full completion never narrows that set. An early release is valid only after Mahmood explicitly directs it, and only as the exact ordered prefix of the hash-bound full manifest; every included pair must have signed-v6 semantic-completeness evidence and the entire prefix must pass the aggregate anti-template audit. Each pair upload derives its identity from the five receipt hashes, matches one active `visualise_source` job and current-pair supersession precondition, survives R2 head verification, and remains `staged`. Activation through `POST /artifacts/corpora/:id/activate` exposes every expected pair in one guarded D1 transaction, supersedes only their prior pairs, completes the bound jobs, and moves the Thread pointer. Omitted lessons retain their current ready pairs, superseded objects remain available for guarded rollback, and a prefix is never reported as full-corpus completion. If any precondition or readback fails, do not retry blindly or alter the active corpus: reconcile the exact staged corpus and job lineage first.
...
3. Before any corpus mutation, require signed `lite-visual-validation/v6` receipts and accepted semantic-completeness evidence for every target in the release manifest, its exact ordered target-set hash, a passing aggregate `lite-visual-corpus-audit/v1`, and independent acceptance. A full Riyadh release uses all 294 authoritative targets. A user-directed early release may use only the exact ordered prefix bound to that full manifest and must remain labeled as partial. Until the applicable release scope passes, do not register a corpus, stage pairs, upload, activate, or roll back. Application-only deployment remains a separate lane and must prove it performs none of those operations.
4. Inspect the remote D1 migration ledger. Migrations through `0073_source_annotation_revisions.sql` are already applied and must never be replayed; for this release require `0074_lite_visual_corpus_scope_lineage.sql` to be the only pending migration.
...
8. After every prior gate passes, apply only `0074_lite_visual_corpus_scope_lineage.sql`, check the migration ledger and readiness, then deploy only from this repository with `npx wrangler deploy --config wrangler.toml`.
