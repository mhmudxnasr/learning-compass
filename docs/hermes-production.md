...
New native Lite Visual validations also require `meaning_units` in each coverage claim and `editorial_review` (`lite-visual-editorial-review/v1`) inside the existing hash-bound coverage ledger. Its four passes reference actual authored passages and bind the final parsed authored text. Signed receipt stats expose `editorial_review_schema`, `editorial_review_passes`, `meaning_units`, and `authored_words`. This is a local authoring enhancement within the existing v6 API contract: the deployed Worker still verifies the signed receipt boundary and does not independently judge the prose or retroactively require these fields in historical receipts. The active default Hermes profile owns one installed skill tree; do not recreate the retired Compass profile to satisfy a stale parity test. Batch work preserves the same per-source reading and revision effort regardless of target count.
