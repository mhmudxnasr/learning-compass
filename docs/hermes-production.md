...
4. Run `npm run release:status -- --output docs/release-snapshot.json` and inspect its remote D1 ledger. Migrations `0066`, `0067`, and `0068_lite_visual_corpus_activation.sql` are already applied and must never be replayed; require exact repository/remote parity and preserve the machine-readable receipt.
...
6. Confirm the deployment configuration declares D1, R2, Assets, AI, and Vectorize bindings and that the signing key passes its minimum-32-character boundary without printing it. Require current production `/health/live`, `/health/ready`, and `/health/free-tier-budget`; the budget must expose 4,000,000/70,000 internal ceilings with 1,000,000/30,000 headroom below provider limits. Require clean job/data-quality blockers, Hermes configuration/parity, the deterministic manager harness, and focused regressions for every observed defect. Stop on any readiness, headroom, or deterministic blocker.
...
8. After every prior gate passes, deploy only the reviewed application delta from this repository with `npx wrangler deploy --config wrangler.toml`; do not apply or replay `0068`.
