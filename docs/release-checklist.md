...
- `npm run test:e2e`
- `npm run build`
- Rehearse `schema.sql` and every numbered migration through `0023` on a clean D1 database.
...
- Smoke-test `/health`, `/dashboard/briefing`, `/capture`, `/capture/feeds`, `/notes`, `/learning/srs/due`, `/learning/core/integrity/health`, `/learning/core/threads`, `/learning/core/consolidation/open`, `/agent/jobs`, `/agent/capabilities`, and `/agent/system`.
