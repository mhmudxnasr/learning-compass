...
- `POST /capture/:id/visualise` — enqueue-only `visualise_source` Hermes work. The payload requires a custom prompt and QA, sets the HTML quality threshold to 8, and expects the `html` and `pdf` roles. Books additionally receive `book_mode=true`, `visual_mode=book_annotation_companion`, and `chapter_outputs=true`; Hermes uploads one linked HTML/PDF pair per chapter with `chapter_key`, `chapter_title`, and `chapter_number` metadata. Requests are idempotent per source.
