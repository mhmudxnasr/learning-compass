...
- `POST /artifacts`, `GET /artifacts`, `GET /artifacts/:id` — R2-backed files and metadata. Lite Visual HTML/PDF uploads accept ordinary pair, source, role, revision, and optional `recommended_start=original|html|pdf|notebooklm` metadata without a QA score or release gate. Momentum uses this value to make the selected medium its primary start action. NotebookLM cinematic video validation remains unchanged. `GET /artifacts` exposes compact quality state for compatibility and joins each file's `notebook_url` from the owning recommendation.
