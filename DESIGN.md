---
name: Continuum
description: Soft graphite knowledge console for Learning Compass
colors:
  rail-bg: "#07080b"
  shell-bg: "#090a0d"
  ledger-bg: "#0c0d11"
...
components:
  button-primary:
    backgroundColor: "color-mix(in srgb, {colors.iris} 21%, {colors.surface-bg})"
...
Continuum treats Learning Compass as a private knowledge console. The default mode uses low-glare graphite planes, clear type, and one violet working signal. The interface should feel fast and settled: few visible choices, obvious current work, and no ornamental dashboard furniture.
...
* Typed object views, the inspector, Capture, Search, and other dialogs use the same Canvas, Surface, Inspector, Seam, and Iris roles as their owning workspace.
...
Linear Graphite is the default Continuum theme. Preferences offers eight complete art directions rather than palette-only recolors: Continuum, Raycast Command, Superhuman Focus, Reader Study, Notion Minimal, Craft Journal, Arc Space, and Are.na Index. Each is an original Learning Compass interpretation of one premium product reference and binds a distinct semantic palette, loaded font system, reading measure, density, text scale, and corner system; individual Day/Night palettes and expert controls remain available below them.

* Rail `#07080a`, shell `#0c0d10`, ledger `#0e0f13`, canvas `#111217`, surface `#15161b`, and inspector `#131419` form the default structural planes.
* Ink `#f4f4f5`, secondary `#979899`, and muted `#7f8081` define accessible default text levels.
* Iris `#8a7cff` marks focus, current work, selected navigation, and deliberate actions. Iris Soft `#232331` is its quiet selected surface.
* Due `#d8a657`, danger `#ee6b76`, and map `#62b5d4` keep their semantic roles across themes.
* Seam `#2a2b32` separates regions. A 1px seam and a tonal plane change are the default structural treatment.
...
Controls use a 9px radius. Cards and panels use 14px. Pills are reserved for compact status. The default canvas padding scales with the viewport, and every mobile or tablet target is at least 44px.
...
- **Decision controls:** Keep active, Make first priority, Pause branch, and Archive branch remain visible together. Active uses Iris, priority uses Map, pause stays neutral, and archive uses Danger without making destructive treatment dominant.
...
- **Type first:** The global action opens one dialog with Source, Book, Movie, Series, Podcast, Course, Game, Album, and Other choices. Each choice has a short task-oriented hint; the selected choice uses Iris and remains visible without opening another surface.
...
- **Primary:** Iris-tinted control surface, Ink text, and 8px 16px padding.
- **Secondary:** Control Surface background, a 1px Seam border, and Secondary text.
- **Hover / Focus:** Iris focus ring with a Canvas separation ring; subtle tonal background shift.
...
- **Focus:** Iris border with a visible outer ring.
