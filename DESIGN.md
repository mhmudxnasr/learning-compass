...
Lite Visual companions teach through fluent Egyptian-Arabic prose, exact source quotations, and source-specific semantic structure. Definitions, examples, equations, and comparisons belong where they resolve a reader's actual difficulty. Coverage scopes are internal evidence boundaries, not visible section templates. Long companions use compact native contents links and stable section anchors; their tagged A4 PDFs retain heading bookmarks. Short companions omit unnecessary navigation.

The authored explanation precedes a clearly labeled, fully visible complete-source edition inside the canonical article. Preserve the source's punctuation, numeric notation, diacritics, and paragraph breaks. Arabic type, contrast, clipping, actual 200% text enlargement, and print visibility are checked across both parts. Layout never substitutes for the per-source fidelity, teaching, language, and continuity review.
