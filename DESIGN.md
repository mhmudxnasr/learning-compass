...
The five root destinations are Home, Library, Learn, Map, and Settings. Their 12 grouped modes are Today; Triage, Catalog, Assets; Threads, Canon, Practice; Atlas, Review; and Personal, Data & recovery, System. Queue, Inbox, All sources, Files, Books, Reading journal, Collections, Archive, Notes, Recall, Branches, Balance, Learning profile, and Preferences are subordinate focus filters or mode surfaces, not peer destinations.
...
Home is an Evidence Desk: resume the active Thread/source, show one next evidence action, expose only actionable due work, and keep capture close. Library is one source ledger with Triage, Catalog, and Assets modes plus focus filters for Inbox, Queue, All sources, Files, Books, Collections, and Archive. Learn contains Threads, Canon, and Practice. Canon is a learner-first editorial atlas: its default surface separates complete **Ready to explore** trios from unfinished **Coming next** coverage, uses family browsing and search instead of curation-status navigation, and keeps counts and selection rules behind disclosure. Surprise discovery can choose only a complete trio. A ready domain presents Foundation, Representative, and Boundary as one vertical reading path with progressively disclosed selection notes, a compact comparison table, explicit Inbox capture, and Thread creation only after at least one book is captured; an unfinished domain renders one honest preparation state with no false reading or Thread action. It avoids cover walls, scores, and a global completion percentage. A Thread level is a focused workbench: it identifies the current level, separates Study progress from Proof progress, makes one ready lesson the dominant action, presents lessons as a numbered curriculum spine, and states the level finish line before exposing evidence actions. Lesson states must distinguish Ready, In progress, Completed, and Needs material; a lesson without authored content or a verified source cannot present completion as available. The lesson detail starts with its learning contract, then material, contextual note/file/recall tools, and completion. Level and Thread materials use progressive disclosure and remain after active work. On small screens the active work always precedes the level roadmap so the bottom dock cannot hide the primary learning action. Map is the spatial knowledge workspace: Atlas and Review modes use Branches and Balance focus filters. Branches uses an index-and-dossier layout: live area filters and the stable branch list stay on the left, while purpose, topics, activity, status controls, and linked contents stay in one readable dossier. Internal IDs never lead the surface, and archive is explicitly reversible. Settings is a dedicated utility surface with Personal, Data & recovery, and System modes. Personal leads with a readable summary of what shapes learning; internal profile signals and technical model details are progressively disclosed rather than rendered as the default surface.
